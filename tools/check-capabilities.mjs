...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    102: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    101:  (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    84: (c) => c.hasCode(/YS_KV_LEGACY_WRITE\s*=\s*false/),
    ...
    100: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [29, 32, 33, 34, 37, 42, 57, 38];
...
const MATRIX = [
  ...
  { row: 75, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 102, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 90, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  { row: 104, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 83, name: 'עריכת נתונים אופליין',
  ...
  { row: 93, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 107, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 56, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 101, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  { row: 115, name: 'פינוי אוטומטי',
    probe: () => /tier2\s*[:=]\s*\[\s*\{/.test(policyBlock()) },
  { row: 94, name: 'אימות פינוי מול הענן',
    probe: () => /\bverify\s*:/.test(policyBlock()) },
  { row: 78, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 68, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 70, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 82, name: 'מפתח חתימה קבוע בריפו',
    probe: () => hasPath('signing') &&
                 fs.readdirSync('signing').some((f) => f.endsWith('.keystore')) },
  { row: 86, name: 'מקור אמת יחיד לסכימה', probe: () => hasPath(APP.schemaFile) },
  { row: 87, name: 'קובץ התקנה מלא',       probe: () => hasPath(APP.schemaFile) },
  { row: 44, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 44, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 84, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 58, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  { row: 45, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 45, name: 'שחזור מקומי מהענן',
    probe: () => callSites('hwRestoreMount').length > 0 },
  { row: 100, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 123, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 44, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 91, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 46, name: 'מנוע מיזוג עם הגנת ⏳',
    probe: () => /isPend \|\| tsOf\(loc\) > tsOf\(rem\)/.test(fnBody('_mergePick')) },
  { row: 104, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 68, name: 'מעטפת WebView חתומה',
  ...
  { row: 104, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 80, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 123, name: 'מודול ה-service worker',
  ...
  { row: 49, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 52, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 122, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 43, name: 'רענון תקופתי של מונה הממתינים',
    probe: () => hasCode(/setInterval\(\s*pendRender\s*,\s*60000\s*\)/) },
  { row: 60, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  בשקט».                                                        */
  { row: 92, name: 'גריעת tombstones לפי גיל',
    probe: () => hasCode(/TOMBSTONE_TTL_MS/) },
  { row: 121, name: 'אוטו-אפדייט מ-raw.githubusercontent',
  ...
  { row: 73, name: 'שכבת אייקונים',
    probe: () => iconAudit('.').length === 0 },
  { row: 95, name: 'שכבת קלט אחידה',
...
const GATES = {
  ...
  14: { claim: 'CANON_MD' },
  36: { claim: 'תוכן הקבצים הנלווים' },
  72: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  39: { manual: 'המדידה שלפני המחיקה היא התנהגות סשן — ⛔ נאכפת רק בתוצאתה, בשער ההסרות' },
  38: { claim: 'אין פרק פערים נפרד' },
  67: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  116: { manual: 'מחזור העבודה ב-git הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  40: { claim: 'מזהה שנמחק ונשאר לו קורא' },
  74: { claim: 'tile-bg' },
  ...
  29: { claim: 'כיסוי הטבלה' },
  30: { claim: 'ועמודת התקן שלה אוסרת במפורש' },
  32: { claim: 'מספור רציף ובלי כפילויות' },
  33: { claim: 'הערה ריקה' },
  34: { claim: 'COUNT_NOTE' },
  35: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  31: { manual: 'קריאת הטבלה לפני הכתיבה היא התנהגות סשן שאינה בעץ — ⛔ נאכפת רק בתוצאתה' },
  124: { claim: 'CACHE_NAME' },
  26: { claim: 'drift' },
  ...
  37: { claim: 'כל כלל מיוצג בטבלה' },
  41: { manual: 'התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו קריאה חסכונית' },
  42: { claim: 'sha:' },
  53: { claim: 'CANON' },
  57: { claim: 'storage' },
  59: { claim: 'ג · פעולה שדורשת רשת' },
  62: { claim: 'עברית' },
  63: { claim: 'RULE_W' },
  64: { claim: 'BANNER_W' },
  111: { claim: 'CSS מתות' },
  ...
  66: { claim: 'מפנה לקובץ' },
  71: { claim: 'android/app/src/main' },
  76: { claim: 'fgDriftMax' },
  77: { claim: 'CANON_MANIFEST' },
  79: { claim: 'BUILD_SHA' },
  81: { claim: 'versionCode' },
  69: { claim: 'WebView' },
  85: { manual: 'נפילה-חזרה ל-`kv` נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  88: { claim: 'migrations' },
  89: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  96: { claim: 'type=password' },
  97: { claim: 'aria-label' },
  98: { manual: 'שני מנועי תאריך — ⛔ טרם הוכרע איזה, ואין מה לאכוף' },
  99: { manual: '«`catch` ריק סביב כתיבה» טרם נוסח כשער — נסרק ידנית בכל סבב שנוגע' },
  103: { claim: 'pass_salt' },
  105: { manual: 'מצב העמודה במסד אינו נראה מהריפו' },
  106: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  108: { claim: '⏳' },
  109: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  119: { manual: 'קיום טבלה או מפתח במסד אינו נראה מהריפו' },
  113: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  110: { manual: '«קובץ בלי קורא» דורש סריקה על ארבעת הריפו — נעשה ידנית' },
  120: { manual: 'קיום מפתח במסד אינו נראה מהריפו' },
  112: { manual: 'רשימת-היתר הגיבויים יושבת במיגרציה שכבר רצה' },
  114: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  117: { claim: 'CLEANUP_SHA' },
  118: { claim: 'בלוק ה-APP' },
};
...
const RULE_ROWS = {
  '⛔ git': 116,
  '⛔ הסבב מעדכן את הטבלה': 35,
  '⛔ הטבלה נקראת לפני שכותבים': 31,
  '⛔ שינוי בקוד מקדם את המטמון': 124,
  '⛔ מדידה לפני שינוי': 39,
  '⛔ סריקת האזור שנגעו בו': 109,
  '⛔ משמעת קריאה וחיסכון טוקנים': 41,
  '⛔ הכרעה שאינה של הסשן': 41,
  '⛔⛔ רכיב משותף = קוד זהה בית-לבית + מיקום זהה + חיווט חי': 42,
  '⛔ תקציב הקבצים, ותקרה שהיא מספר עגול': 4,
  '⛔ ההערה — מתי נכתבת, צורתה ותוכנה': 65,
  '⛔ דפוסי כתיבת ממשק': 54,
  '⛔ הבדל מכוון מנומק בכתב, במקום שבו רואים אותו': 67,
  '⛔ ערך שקיים בקוד או במסד אינו מוצהר בתיעוד': 25,
  '⛔ קובץ נשפט לפי תפקידו': 3,
  '⛔ קובץ תיעוד אינו מסביר כלל שהטבלה אוכפת': 36,
  '⛔ שרשרת היכולת החדשה': 29,
  '⛔ כל כלל בא לידי ביטוי בטבלה': 37,
  '⛔ מספור רציף ומבני, וסריקת כפילויות': 32,
  '⛔ `probe` מאמת **ערך** — ⛔ ולא קיום': 29,
  '⛔ בדיקה שסופרת מילה אינה בדיקה': 24,
  '⛔ מבחן מצהיר אילו שורות בטבלה הוא אוכף': 29,
  '⛔ מה שאינו ניתן לאכיפה מכנית — נרשם ככזה': 38,
  '⛔ שינוי מכוון ברכיב משותף': 42,
};
   ...
   ──────────────────────────────────────────────────────────────────────── */
{
  ...
  const DOUBLE_LOOP = ['⛔ כל כלל בא לידי ביטוי בטבלה', '⛔ הטבלה נקראת לפני שכותבים'];
