 ...
 *  ⛔ **שני דפוסים נמדדים**: ⚠️ «נמדד ב-<תאריך>» ⛔ ואחוז שאינו נגזר —
 *  ⭐ ואחוז שהוא **ערך חי** (סף, יעד, הסטה) מוחרג בשמו, ⛔ שהוא עובדה
 *  קבועה על הקוד ⚠️ ולא ספירה של מה שקרה. */
{
  let bad = 0, seen = 0;
  ...
  for (const { name, blocks: bs } of SCOPE) for (const b of bs) {
    seen++;
    const line = (re) => b.text.split('\n').find((x) => re.test(x)) || '';
    if (DATED.test(b.text)) {
      fail(`${name}:${b.line}: מספר בהערה — נמדדה ספירה של אירוע שחלף «${line(DATED).trim().slice(0, 60)}» והצפוי עובדה קבועה או מדידה שניתנת לשחזור. מסירים את המספר ואת התאריך, ומשאירים את האיסור ואת הסיבה`);
      bad++;
    } else if (PCT.test(b.text) && !PCT_LIVE.some((x) => b.text.indexOf(x) >= 0)) {
      fail(`${name}:${b.line}: מספר בהערה — נמדד אחוז שאינו ערך חי «${line(PCT).trim().slice(0, 60)}» והצפוי עובדה קבועה או מדידה שניתנת לשחזור. מסירים את האחוז, או מצהירים אותו ב-PCT_LIVE עם נימוק`);
      bad++;
    }
  }
  if (!bad) pass(`מספר בהערה: ${seen} בלוקים, ואפס ספירות של אירוע שחלף`);
}
