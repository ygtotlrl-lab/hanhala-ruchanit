-- ============================================================================
-- 009_sleep_structured_tables.sql — סדרי השינה עוברים לטבלאות מובנות (סבב 39)
-- ============================================================================
-- ⛔⛔ **הקובץ הזה נכתב ולא הורץ (סבב 39) — «נכתב» אינו «רץ».** הרצתו היא
--     החלטת המנהל. הקוד ב-`index.html` בטוח לפריסה גם בלי הרצה:
--     `ysRowsPushSleep` נכשלת סגור ומדלגת בשקט, וה-`kv` נשאר המאסטר.
--
-- ── מה זה פותר ─────────────────────────────────────────────────────────────
-- `ys_sleep_sessions` הוא המפתח האחרון מסוגו שנשאר מחוץ לשכבת השורות: סבב 36
-- העביר את הנוכחות (`ys_attend_sessions`) ואת המצבה, והשינה נשארה בחוץ
-- **מטעמי היקף בלבד** — לא מטעם מבני. נמדד מול המסד החי ב-2026-08-19,
-- ב-`SELECT` בלבד:
--   kv.ys_sleep_sessions   39,368 בתים · 21 סדרים · 988 סימונים
-- כלומר כל סימון שינה בודד דורס את מלוא הערך, בדיוק כמו בנוכחות לפני סבב 36.
--
-- ── ⭐ אותו דפוס בדיוק, ולא דפוס חדש ───────────────────────────────────────
-- המבנה נמדד והושווה לנוכחות לפני שנכתבה כאן שורה (סבב 39): רשומת סדר שינה
...
--
-- ⭐ **ההבדל היחיד שנמדד: לסימון שינה יש שדה `note`.** בנוכחות הסימון הוא
--    `{s, min}` בלבד; בשינה הוא `{s, min, note}` — 25 סימונים נושאים הערה,
--    הארוכה שבהן 248 תווים. ⛔ הוא **חייב** עמודה משלו (סבב 39): השמטתו
...
--
-- ── טיפוסים — נמדדו מהנתונים, לא הוצהרו ───────────────────────────────────
...
--                        uuid מסבב 37, ו-`::smallint` היה מפיל כל סימון חדש
-- ============================================================================
...
-- ⚠️ **ואינו ייחודי — למרות שבנתונים היום אין אף התנגשות** (נמדד: 0 זוגות
--    `(session, date_iso)` כפולים, מול 5 בנוכחות). ⛔ אפס היום הוא **נתון,
--    לא אילוץ** (סבב 39): אותה מציאות תפעולית שיצרה חמש התנגשויות בנוכחות
...
--    התלמיד, ולכן לתלמיד יש לכל היותר סימון אחד בסדר. נמדד: אפס מפתחות
...
-- ────────────────────────────────────────────────────────────────────────────
-- ⚠️ **אינו תחליף ל-`revoke` המפורש** (סבב 29) — הוא משפיע רק על טבלאות
