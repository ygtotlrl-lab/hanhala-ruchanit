...
const APP = {
  ...
  /*  ⛔ קבועי מסך הצפייה — ⚠️ מסך «טבלת התשתית» קיים ביומן
  ...
  /*  ⛔ נקודת המעבר היחידה של סוד אל הדיסק — ⚠️ השם נבדל בין
  ...
  /*  ⛔ אין כאן מסלול השלמת טביעה — ⚠️ הוא הוסר בהחלטת מנהל
  ...
  /*  ⛔⛔ קריאה לשכבת השורות **בלי חלון** — ⚠️ הנימוק המדוד:
  ...
  tableProbe: {
    ...
    /*  ⛔ ערך ולא שם — ⚠️ שלושת הפרמטרים כתובים בקוד ונקראים
     *  ממנו: מספר הסיבובים מהקבוע, ⛔ ו-`SHA-256` ו-256 הביט מהקריאה
     *  ל-`deriveBits` עצמה. ⭐ בעבר השורה נשאה «אין שער שמודד», ⛔ והיא
     *  ניתנת למדידה במלואה. */
    ...
    /*  ⛔ נקודת כניסה חיה ולא קיום פונקציה —
    ...
    // ⛔ שלב א בלבד — ה-`kv` הוא עדיין המאסטר וכל קריאה עוברת בו. הדגל
    //    הזה הוא המתג — כשהוא יכובה (שלב ב), התא חייב להפוך ל-✅.
    /*  ⛔ ערך ולא דגל — ⚠️ הדגל נמחק, ⭐ ומה שנמדד הוא **היעדר
    ...
    /*  ⛔ נקודת כניסה חיה ולא קיום פונקציה — עד הסבב הזה ה-probe
  ...
  /*  ⛔ המפתחות המקומיים שאינם טבלה — ⚠️ **מה נכנס**: מפתח
  ...
  /*  ⛔ דרגה שקיימת באוצר המילים ואיש אינו נושא אותה —
  ...
  rolesUnused: [],
  authUser: 'AUTH.user',
  /*  ⛔ אתר השוואה שאינו בבלוק המשותף — ⚠️ **מה נכנס**: שם
  ...
  probeInput: {
    ...
    '104|פריסה במסכי טלפון וטאבלט': 'mixed',
    '105|ערכת נושא — בהיר וכהה': 'mixed',
...
export const ROWS = [37, 34, 50, 51, 59, 98, 57, 189, 119, 167, 63, 27, 68, 30, 39, 43, 205, 211];
...
/*  ⭐ נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  /*  ⛔ שתי נקודות כניסה חיות ולא שתי פונקציות — פונקציה
...
const GATES = {
  ...
  106: { claims: { test_names: ['[token-pattern]', '[token-app]', '[token-slot]'] } },
