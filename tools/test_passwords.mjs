...
/*  ⚠️ טוקניזציה של ה-JS המוטבע ולא regex על ה-HTML — התיעוד
...
function stripComments(t) {
  ...
  while (i < t.length) {
    ...
    if (c === '"' || c === "'" || c === '`') {
      /*  ⚠️ מרכאה בודדת/כפולה **אינה חוצה שורה** ב-JS. בלי
...
if (!APP.usersTable) {
   ...
   *  ל«הערה»: ⚠️ התיעוד ההיסטורי מלא באזכורי שם העמודה, ⛔ וסריקה
...
if (fs.existsSync(MIG_A)) {
  ...
  /*  ⛔ הקובץ שכבר רץ אינו נערך — ⚠️ צעד ב יושב בקובץ **חדש**,
...
if (fs.existsSync(MIG_B)) {
  ...
  /*  ⛔ הבאנר מצהיר את **מצב ההרצה**, ⛔ ולא איזה מצב — ⚠️ הטענה
...
/*  ⛔ המוטציה מחזירה **כתיבה** ולא אזכור — ⚠️ שורת `strip` או
 ...
 *  «מסלול חי» ל«טקסט בהערה». ⚠️ זה נמדד: שורת הערה
