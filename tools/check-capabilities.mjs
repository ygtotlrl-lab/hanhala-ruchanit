...
const APP = {
  ...
  gapRows: [45, 122, 126, 150, 60, 135, 96],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    170: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    169: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    133: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    166: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '81|מאזין אחד, בהאצלה מ-`document`': 'src',
    '82|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '166|מבנה טופס הכניסה': 'src',
    '135|שם נגזר אינו נשמר ברשומה': 'src',
    '184|מפתח אחסון בלי קורא': 'src',
    '37|שער מריץ את כל טענותיו': 'tools',
    '88|שכבת המראה': 'src',
    '185|הגירה מקומית שהושלמה': 'mixed',
    '168|מראת המשתמשים': 'src',
    '167|מודל ההרשאות': 'src',
    '24|שער סורק קוד מולבן': 'tools',
    '24|ההלבנה שומרת על מספרי השורות': 'mixed',
    '151|הקשר נלכד בכניסה לפונקציה': 'src',
    '91|פריסה במסכי טלפון וטאבלט': 'src',
    '92|ערכת נושא — בהיר וכהה': 'src',
    '93|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '94|אין סגנון מוטבע': 'src',
    '96|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '179|הערה שמתארת מצב שחלף': 'mixed',
    '186|שאילתת `@media` מתה': 'src',
    '109|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '110|ייצוא והנפקה — דרך מוצהרת': 'src',
    '97|המסמך מתפרסר נקי': 'src',
    '98|לוג מסלול — תג ורמה': 'src',
    '76|מיכל המודאל ילד ישיר של `body`': 'src',
    '105|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '153|הגיבוי שלם ומעומד': 'src',
    '72|פעולה מגיבה מיד': 'src',
    '73|משיכה מסוננת בשרת': 'src',
    '121|דגימת-יתר במחולל': 'tools',
    '170|`PBKDF2` — פרמטרים': 'src',
    '139|`migrations/` — תוכן': 'src',
    '140|`migrations/` — כמות': 'src',
    '159|מיזוג מכל — מפה ומערך': 'src',
    '160|ברירת מחדל אינה ממוזגת': 'src',
    '161|רשימה אינה נושאת פריט כפול': 'src',
    '154|`sh_backup` — טבלת הגיבוי': 'src',
    '22|שער אינו מקליד מספר שורה': 'tools',
    '182|פונקציה בלי קוראים': 'mixed',
    '28|שער ממתין לתנאי ולא לשעון': 'tools',
    '174|סודות אינם עוזבים את המכשיר': 'src',
    '148|דפוס `upsert` — onConflict מוכרז': 'src',
    '172|טביעה חסרה מושלמת אוטומטית': 'src',
    '172|כניסה אופליין': 'src',
    '132|עריכת נתונים אופליין': 'src',
    '158|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '173|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '70|אובייקט יקר נבנה פעם אחת': 'src',
    '71|מסך מציג מיד ומרענן ברקע': 'src',
    '125|האפליקציה ניתנת להתקנה מכרום': 'src',
    '177|דגלי מעבר — אין דגל דלוק': 'src',
    '77|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '169|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '45|ארבעת מימדי הסריקה': 'src',
    '87|פינוי אוטומטי': 'src',
    '87|אופק הפינוי נוקה מהסנכרון': 'src',
    '163|אימות פינוי מול הענן': 'src',
    '126|שיתוף קבצים': 'src',
    '111|מעטפת APK (WebView)': 'src',
    '114|אין נכסים מוטבעים': 'src',
    '130|מפתח חתימה קבוע בריפו': 'src',
    '136|מקור אמת יחיד לסכימה': 'src',
    '137|קובץ התקנה מלא': 'src',
    '56|גיבוי יומי אוטומטי': 'src',
    '56|יומן פעולות': 'src',
    '133|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '78|דפוס הודעת שגיאה יחיד': 'src',
    '83|`toast` — חתימה, גוף ומחלקות': 'src',
    '83|סיווג ההודעה נגזר מהמסלול': 'src',
    '192|מיכל באנר העדכון במקור': 'src',
    '192|הרענון מ-controllerchange בלבד': 'src',
    '192|סימן דחיית הבאנר מתמיד': 'src',
    '106|רישום כשלי כתיבה': 'src',
    '57|חלון חם במכשיר': 'src',
    '166|מסך שינוי סיסמה עצמי': 'src',
    '194|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '58|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '172|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '59|מודול מזהי רשומות': 'src',
    '59|מזהה אינו מומר למספר': 'src',
    '61|מזהה מכשיר': 'src',
    '58|ליבת מיזוג משותפת': 'src',
    '111|מעטפת WebView חתומה': 'tools',
    '172|אימות מול טביעה בענן': 'mixed',
    '128|בניית APK אחידה עם שער חתימה': 'src',
    '194|מודול ה-service worker': 'src',
    '62|ניסיון חוזר בתור הסנכרון': 'src',
    '63|מנגנון משיכה אחיד': 'src',
    '66|נעילת חוסר-פעילות': 'src',
    '67|מודל הסשן — בזיכרון בלבד': 'src',
    '193|בדיקת עדכון תקופתית ל-service worker': 'src',
    '89|פסק זמן אחיד לקריאות רשת': 'src',
    '90|מאזיני מצב רשת': 'src',
    '157|גריעת tombstones לפי גיל': 'src',
    '192|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '126|גשר שיתוף': 'src',
    '68|העברת מזהה ל-DOM': 'src',
    '69|מיון אחד לכל התצוגות': 'src',
    '74|ייצוא — שני דפוסים מוצהרים': 'src',
    '118|שכבת אייקונים': 'src',
    '164|שכבת קלט אחידה': 'src',
    '164|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '75|שכבת המודאל': 'src',
    '166|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [30, 44, 46, 48, 53, 86, 51, 162, 102, 142, 55, 23, 60, 26, 35, 39];
...
const CAPS = {
  ...
  backup: {
    ...
    block: { sha: '25d5b61c96ba7eee', lines: 304,
...
const CAT_ORDER = [
  ...
  { g: 'ד', phases: [{ p: 'הכלל', w: ['חתימות sha256', 'כל פונקציה —'] },
...
const MATRIX = [
  ...
  { row: 82, name: 'כפתור שכותב מושבת בזמן הכתיבה',
    probe: () => busyGuardGaps().length === 0 },
  { row: 81, name: 'מאזין אחד, בהאצלה מ-`document`',
  ...
  { row: 80, name: '`Enter` שומר בכל שדה עריכה',
  ...
  { row: 135, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 24, name: 'שער סורק קוד מולבן',
    probe: () => scanKindGaps().length === 0 },
  { row: 24, name: 'ההלבנה שומרת על מספרי השורות',
  ...
  { row: 151, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 91, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 93, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 96, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 92, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 179, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 76, name: 'מיכל המודאל ילד ישיר של `body`',
  ...
  { row: 105, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 153, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 150, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 72, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 73, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 121, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 170, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 139, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 140, name: '`migrations/` — כמות',
  ...
  { row: 159, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 160, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 161, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 154, name: '`sh_backup` — טבלת הגיבוי',
    probe: () => (/BK_TABLE\s*=\s*'([a-z_]+)'/.exec(srcRefs) || [])[1] === 'sh_backup' },
  { row: 22, name: 'שער אינו מקליד מספר שורה',
    probe: () => typedRowSites().length === 0 },
  { row: 182, name: 'פונקציה בלי קוראים',
  ...
  { row: 28, name: 'שער ממתין לתנאי ולא לשעון',
  ...
  { row: 174, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 148, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 172, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 172, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 132, name: 'עריכת נתונים אופליין',
  ...
  { row: 158, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 173, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 70, name: 'אובייקט יקר נבנה פעם אחת',
  ...
  { row: 71, name: 'מסך מציג מיד ומרענן ברקע',
  ...
  { row: 125, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 177, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 77, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 169, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 45, name: 'ארבעת מימדי הסריקה',
    probe: () => {
  ...
  { row: 87, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 163, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 126, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 111, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 114, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 130, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 136, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 137, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 56, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 56, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 133, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 78, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 83, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 83, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 106, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 57, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 166, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 166, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 194, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 56, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 152, name: 'פינוי גיבויים אוטומטי במסד',
   ...
   *  ⛔ ולא `NaN`. */
  { row: 59, name: 'מזהה אינו מומר למספר',
  ...
  { row: 111, name: 'מעטפת WebView חתומה',
  ...
  { row: 172, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 128, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 194, name: 'מודול ה-service worker',
  ...
  { row: 62, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 67, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 193, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 37, name: 'שער מריץ את כל טענותיו',
    probe: () => gateSealGaps().length === 0 },
  { row: 184, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 88, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 185, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 89, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 157, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 192, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 192, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 192, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 192, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 69, name: 'מיון אחד לכל התצוגות',
  ...
  { row: 74, name: 'ייצוא — שני דפוסים מוצהרים',
    probe: () => exportMarkupSites().length === 0 },
  { row: 118, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 164, name: 'שכבת קלט אחידה',
  ...
  { row: 164, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  13: { claim: 'CANON_MD' },
  50: { claim: 'תוכן הקבצים הנלווים' },
  116: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  119: { claim: 'margin' },
  40: { manual: 'השוואת זמנים בין הריפו אינה בהישג ידו של שער שרץ בריפו אחד — ⛔ נאכפת בתוצאתה בלבד' },
  51: { claim: 'אין פרק פערים נפרד' },
  104: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  181: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
                  test_wiring: 'W.missing' } },
  120: { claim: 'tile-bg' },
  17: { claim: 'const SHARED' },
  20: { claims: { test_filesets: 'appGates', 'check-comments': 'מכריז היעדר',
   ...
   *  מוטציה שאין בו. */
  33: { claim: 'לכל אחד שינוי, תווית' },
  34: { claim: 'שם הטענה שתיפול' },
  30: { claim: 'כיסוי הטבלה' },
  43: { claim: 'ועמודת התקן שלה אוסרת במפורש' },
  44: { claim: ['מספור רציף ובלי כפילויות', 'nameSignGaps', 'catOrderGaps'] },
  46: { claim: 'הערה ריקה' },
  48: { claim: 'COUNT_NOTE' },
  49: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  195: { claim: 'CACHE_NAME' },
  42: { manual: 'מספר בדיווח הוא התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו את הדיווח, ⚠️ ונאכף בתוצאתו בלבד' },
  23: { claims: { test_readonly: 'drift', 'check-capabilities': 'writeGateGaps' } },
  ...
  35: { claims: { 'check-comments': 'תווית מוטציה מודפסת',
                  'check-capabilities': ['regexBoundGaps', 'anchorGaps'],
                  'test_anchors': 'מ1' } },
  19: { claim: 'תקן הבאנר' },
  16: { claim: 'דפוס הבודקים' },
  95: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  ...
  26: { claims: { 'check-comments': 'ריצת check-js', 'check-capabilities': 'gateKindGaps' } },
  ...
  39: { claims: { 'check-js': 'תקציב זמן לסט',
                  'check-capabilities': 'gateSizeGaps' } },
  27: { claim: 'ברירת המחדל אינה המהירה' },
  ...
  86: { claim: 'storage' },
  85: { claim: 'ג · פעולה שדורשת רשת' },
  100: { claim: 'עברית' },
  102: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  103: { claim: 'BANNER_W' },
  183: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
                 'אין מחלקה שנוספת ל-DOM בלי כלל CSS'] },
  47: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  101: { claim: 'מפנה לקובץ' },
  115: { claim: 'android/app/src/main' },
  122: { claim: 'fgDriftMax' },
  123: { claim: 'CANON_MANIFEST' },
  124: { claim: 'שדות זהים' },
  127: { claim: 'BUILD_SHA' },
  131: { claim: 'SHARED_SHA' },
  129: { claim: 'versionCode' },
  112: { claim: 'WebView' },
  113: { claim: 'android:theme' },
  117: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  134: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  138: { claim: 'migrations' },
  147: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  149: { claim: 'א. חותמת בכל רשומה' },
  55: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  142: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  143: { claim: 'ו. טבלה מקבילה' },
  144: { claim: 'ב. חתימת סכימה' },
  145: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  165: { claim: 'type=password' },
  79: { claim: 'aria-label' },
  ...
  60: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps'] } },
  171: { claim: 'pass_salt' },
  176: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  178: { claim: '⏳' },
  180: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  188: { claim: 'ט. ערך נגזר שהתיישן' },
  190: { claim: 'ח. הגדרה שהוחלפה' },
  189: { claim: 'ד. רשימת-היתר' },
  191: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  99: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  107: { claim: 'CLEANUP_SHA' },
  108: { claim: 'בלוק ה-APP' },
  162: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
