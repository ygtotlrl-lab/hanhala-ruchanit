#!/usr/bin/env node
/*  סבב 53 — מודל הסשן, מודול משותף.
 *
 *  ⭐ **מה נשמר כאן:** המשתמש המחובר חי **בזיכרון בלבד** ואינו יורד
 *  לדיסק, בשלוש האפליקציות שיש בהן כניסה. כל טעינה מחדש דורשת כניסה.
 *
 *  ⚠️ **מה שנמדד לפני ההכרעה (2026-08-25), ולא הוצהר:** ⛔ תפוגת סשן לא
 *  הייתה קיימת באף אחת מהארבע — ההבדל היה **עצם השמירה**. הנהלה החזיקה
 *  את המשתמש בזיכרון, ואילו schar ו-gius שמרו אותו ב-localStorage **בלי
 *  תפוגה**, יחד עם `role`. ⭐ המנהל הכריע ב-2026-08-26 ליישר לדפוס
 *  ההנהלה (כלל ברזל 9 — החלטת מודל אבטחה היא שלו).
 *
 *  ⚠️ **הרתמה מריצה את הליבה האמיתית** ברתמת `vm` עם localStorage מזויף.
 *  ⛔ המוטציות אינן נכתבות לעץ (הלקח של סבב 42ג).
 *
 *  הקובץ זהה בית-לבית בארבעת הריפו פרט לבלוק `APP` שבראשו.
 *
 *  ⚠️ **באפליקציה שאין בה מסך כניסה** — `present:false` בבלוק `APP` —
 *  שלוש הטענות כאן הן טענות-**חסר** ובלוק המוטציות מדולג (סבב 72).
...
const APP = {
   ...
   *  שאין בה מסך כניסה ואין משתמש מחובר להחזיק. */
  present: true,
  bootFn: 'loadDash',        // פונקציית העלייה — נקודת ההפעלה היחידה
  /*  ⚠️ מפתחות הסשן שהאפליקציה שמרה בהם עד סבב 53. ⛔ ריק כאן, וזה נמדד:
   *  hanhala-ruchanit מעולם לא שמרה סשן בדיסק — ולכן אין מה לנקות. */
  legacy: [],
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
