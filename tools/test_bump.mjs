#!/usr/bin/env node
/*  test_bump.mjs — שער ה-versionCode והבסיס שממנו הוא נמדד.
 *
 ...
 *  ⛔ אין ליישר את התלויות בין הריפו — ליומן לבדה יש
 *  `androidx.webkit`, מפני שרק לה יש גשר שיתוף; חריגה מנומקת ומדודה.
 *
 *  ⛔ `BUMP_GATE_ONLY=1` מריץ את חלק השער בלבד — ⚠️ הרתמה
...
const APP = {
  ...
  applicationId: 'com.hanhala.ruchanit',
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
 ...
 *  זמין. שלוש התבניות נגזרו מכל הקבצים בפועל ולא הוצהרו.            */
...
function lastBumpCommit() {
  ...
  for (const c of commits) {
    ...
    try {
       ...
       *  ⛔ ואז הקומיט הזה הוא הבסיס — הוא הקידום
...
try {
  ...
  if (baseRef) {
    ...
    /* ⛔ תיעוד אינו מעטפת — קובץ `.md` תחת `android/` יוצא
     ...
     *  ⚠️ נמדד: יישור `android/README.md` בכל הריפו הפיל
     *  את השער הזה בכולם, ⛔ בלי ששורה אחת של קוד מעטפת נגעה. */
 ...
 *  ⛔ אינו קישוט — `android/README.md` הוא הקובץ שסשן חדש
 *  ומנהל קוראים כדי לדעת באיזו גרסה המעטפת נמצאת, והוא **הצהיר מספר
 *  שגוי בכל הריפו במשך ארבעה סבבים**. הפורמט זהה בכולם, ולכן
...
/*  ⛔ בריצה בתוך הרתמה — עוצרים כאן — ⚠️ הרתמה מריצה את הקובץ
...
/*  ⚠️ נמדד על **חלק השער בלבד** — ⛔ הרתמה שמתחתיו מציבה
...
if (/'origin\/main'/.test(gateSrc)) {
  fail("חלק השער עדיין מריץ git מול 'origin/main' — זו הנקודה העיוורת");
} else pass('חלק השער אינו נשען על origin/main בקוד');

/*  ⛔ `-S` סופר מופעים ולא ערכים — נמדד: הוא פספס את קומיט
 *  הקידום. שער שיחזור אליו חוזר לנקודה העיוורת.               */
if (/log[^\n]*'-S/.test(gateSrc)) {
...
const WORK = mkdtempSync(join(tmpdir(), FACTS.slug + '-bump-'));
const ENV = {
  ...process.env,
  BUMP_GATE_ONLY: '1',
...
/*  ⛔ המוטציה רצה על **עותק** ולא על העץ — ⚠️ מדידה שנפלה
 *  באמצע הייתה משאירה את העץ שגוי לתמיד. */
{
  ...
  const mut = (label, file, edit, expectFail) => {
    /*  ⛔ עותק לכל מוטציה, ⛔ ובכוונה — ⚠️ נמדדו **שלושה**
