...
const APP = {
  ...
  kvMeta: { table: 'ys_settings_meta', why: '' },
   ...
   *  הפונקציה ⟵ הסיבה שהסגנון שם אינו יכול לעבור למחלקה; ⛔ **ומה מפיל**:
   *  אזור שאין בו סגנון מוטבע, ⛔ וסגנון מוטבע מחוץ לכל אזור. ⭐ **וההיעדר
  ...
  appTokens: {
    ...
    '--navy2': 'דרגת ההבהרה של הכחול העמוק לכותרות — מוצר ולא תשתית',
    '--navy3': 'דרגת ההכהיה של הכחול העמוק למסגרות — מוצר ולא תשתית',
  ...
  cloudPutWhy: '',
  cloudWrites: ['savePerms', 'atSaveCfg', 'atSaveTreats', '_slCleanCfg', 'slSaveCfg', 'slSaveTreats',
                'saveUser', 'toggleUserActive', 'changeMyPassword'],
  cloudWriteAllow: {
  ...
  gapRows: [48, 131, 135, 159, 66, 144, 103],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    180: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    179: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    142: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    176: (c) => !!c.fnBody('changeMyPassword')
  ...
  flatKeys: {
    ys_sw_dismissed: 'סימן דחיית באנר העדכון — נושא את שם המטמון שנדחה, ואינו בענן כלל',
    ...
    ys_user_order:      'סדר תצוגה מקומי — אינו בענן כלל',
    ys_perms_cache:     'מטמון ההרשאות — מסלול הכניסה האופליין ולא מטמון מהירות',
    ys_offline_queue:   'תור יוצא מקומי — אינו בענן, והוא מחסום הפינוי',
    ys_login_log_queue: 'תור יומן הכניסות — אינו בענן, והוא מחסום הפינוי',
  },
  ...
  keydownExempt: [
    ...
    { site: 'atConfirmLate(', why: 'אישור איחור — הכפתור מופיע רק אחרי הקלדה, והמקש הוא המסלול שלפניו' },
    { site: 'slConfirmLate(', why: 'אישור איחור — הכפתור מופיע רק אחרי הקלדה, והמקש הוא המסלול שלפניו' },
  ],
  probeInput: {
    '95|חותמת רעננות נכשלת סגור': 'src',
    '86|`Enter` שומר בכל שדה עריכה': 'src',
    '87|מאזין אחד, בהאצלה מ-`document`': 'src',
    '88|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '176|מבנה טופס הכניסה': 'src',
    '144|שם נגזר אינו נשמר ברשומה': 'src',
    '195|קבוע בלי קורא': 'mixed',
    '197|מפתח אחסון בלי קורא': 'src',
    '39|שער מריץ את כל טענותיו': 'tools',
    '94|שכבת המראה': 'src',
    '198|הגירה מקומית שהושלמה': 'mixed',
    '178|מראת המשתמשים': 'src',
    '177|מודל ההרשאות': 'src',
    '26|שער סורק קוד מולבן': 'tools',
    '26|ההלבנה שומרת על מספרי השורות': 'mixed',
    '160|הקשר נלכד בכניסה לפונקציה': 'src',
    '98|פריסה במסכי טלפון וטאבלט': 'src',
    '99|ערכת נושא — בהיר וכהה': 'src',
    '100|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '101|אין סגנון מוטבע': 'src',
    '103|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '191|הערה שמתארת מצב שחלף': 'mixed',
    '199|שאילתת `@media` מתה': 'src',
    '118|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '119|ייצוא והנפקה — דרך מוצהרת': 'src',
    '104|המסמך מתפרסר נקי': 'src',
    '105|לוג מסלול — תג ורמה': 'src',
    '82|מיכל המודאל ילד ישיר של `body`': 'src',
    '114|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '162|הגיבוי שלם ומעומד': 'src',
    '78|פעולה מגיבה מיד': 'src',
    '79|משיכה מסוננת בשרת': 'src',
    '130|דגימת-יתר במחולל': 'tools',
    '180|`PBKDF2` — פרמטרים': 'src',
    '148|`migrations/` — תוכן': 'src',
    '149|`migrations/` — כמות': 'src',
    '169|מיזוג מכל — מפה ומערך': 'src',
    '170|ברירת מחדל אינה ממוזגת': 'src',
    '171|רשימה אינה נושאת פריט כפול': 'src',
    '163|`sh_backup` — טבלת הגיבוי': 'src',
    '24|שער אינו מקליד מספר שורה': 'tools',
    '194|פונקציה בלי קוראים': 'mixed',
    '30|שער ממתין לתנאי ולא לשעון': 'tools',
    '184|סודות אינם עוזבים את המכשיר': 'src',
    '157|דפוס `upsert` — onConflict מוכרז': 'src',
    '182|טביעה חסרה מושלמת אוטומטית': 'src',
    '182|כניסה אופליין': 'src',
    '141|עריכת נתונים אופליין': 'src',
    '167|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '183|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '76|אובייקט יקר נבנה פעם אחת': 'src',
    '77|מסך מציג מיד ומרענן ברקע': 'src',
    '134|האפליקציה ניתנת להתקנה מכרום': 'src',
    '188|דגלי מעבר — אין דגל דלוק': 'src',
    '83|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '179|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '48|ארבעת מימדי הסריקה': 'src',
    '93|פינוי אוטומטי': 'src',
    '93|אופק הפינוי נוקה מהסנכרון': 'src',
    '173|אימות פינוי מול הענן': 'src',
    '135|שיתוף קבצים': 'src',
    '120|מעטפת APK (WebView)': 'src',
    '123|אין נכסים מוטבעים': 'src',
    '139|מפתח חתימה קבוע': 'src',
    '145|מקור אמת יחיד לסכימה': 'src',
    '146|קובץ התקנה מלא': 'src',
    '62|גיבוי יומי אוטומטי': 'src',
    '62|יומן פעולות': 'src',
    '142|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '84|דפוס הודעת שגיאה יחיד': 'src',
    '89|`toast` — חתימה, גוף ומחלקות': 'src',
    '89|סיווג ההודעה נגזר מהמסלול': 'src',
    '206|מיכל באנר העדכון במקור': 'src',
    '206|הרענון מ-controllerchange בלבד': 'src',
    '206|סימן דחיית הבאנר מתמיד': 'src',
    '115|רישום כשלי כתיבה': 'src',
    '63|חלון חם במכשיר': 'src',
    '176|מסך שינוי סיסמה עצמי': 'src',
    '208|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '64|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '182|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '65|מודול מזהי רשומות': 'src',
    '65|מזהה אינו מומר למספר': 'src',
    '67|מזהה מכשיר': 'src',
    '64|ליבת מיזוג משותפת': 'src',
    '120|מעטפת WebView חתומה': 'tools',
    '182|אימות מול טביעה בענן': 'mixed',
    '137|בניית APK אחידה עם שער חתימה': 'src',
    '208|מודול ה-service worker': 'src',
    '68|ניסיון חוזר בתור הסנכרון': 'src',
    '69|מנגנון משיכה אחיד': 'src',
    '72|נעילת חוסר-פעילות': 'src',
    '73|מודל הסשן — בזיכרון בלבד': 'src',
    '207|בדיקת עדכון תקופתית ל-service worker': 'src',
    '96|פסק זמן אחיד לקריאות רשת': 'src',
    '97|מאזיני מצב רשת': 'src',
    '166|גריעת tombstones לפי גיל': 'src',
    '206|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '135|גשר שיתוף': 'src',
    '74|העברת מזהה ל-DOM': 'src',
    '75|מיון אחד לכל התצוגות': 'src',
    '80|ייצוא — שני דפוסים מוצהרים': 'src',
    '127|שכבת אייקונים': 'src',
    '174|שכבת קלט אחידה': 'src',
    '174|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '81|שכבת המודאל': 'src',
    '176|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [32, 47, 49, 51, 58, 92, 56, 172, 110, 151, 61, 25, 66, 28, 37, 41];
...
const CAPS = {
  ...
  pending: {
    ...
    block: { sha: '4285297c6458b39e', lines: 285,
  ...
  retry: {
    ...
    block: { sha: '98dbc4ab41ed8da7', lines: 68,
  ...
  pull: {
    ...
    block: { sha: '6727ad50f63ef139', lines: 82,
  ...
  lock: {
    ...
    block: { sha: '590d6a4ba6d7043f', lines: 109,
...
const FLOOR = { shared: 86, app: 9, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי' };
/* ⚠️ סוף פר-אפליקציה */
...
/*  ⛔ `public.kv` בלבד — ⚠️ **ולא** `tb_kv_rishon`/`tb_kv_ramataviv`: ⭐ שם עדיין
...
function inlineStyleGaps() {
  const out = [];
  const zones = styleZones(APP.inlineStyleAllow || {}, out);
  const seen = new Set();
 ...
 *  `--text3` שבשלוש, ⛔ ואיש לא נתפס — ⚠️ שני שמות לדרגת הדיו השלישית.
...
const VOCAB = ['--bg', '--card', '--card-2', '--text', '--text2', '--text3',
...
const MATRIX = [
  ...
  { row: 95, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 88, name: 'כפתור שכותב מושבת בזמן הכתיבה',
    probe: () => busyGuardGaps().length === 0 },
  { row: 87, name: 'מאזין אחד, בהאצלה מ-`document`',
  ...
  { row: 86, name: '`Enter` שומר בכל שדה עריכה',
  ...
  { row: 144, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 160, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 98, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 100, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 103, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 99, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 191, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 105, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 114, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 162, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 159, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 130, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 180, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 148, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 149, name: '`migrations/` — כמות',
  ...
  { row: 169, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 170, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 171, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 184, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 157, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 182, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 182, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 141, name: 'עריכת נתונים אופליין',
  ...
  { row: 167, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 183, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 134, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 188, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 83, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 179, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 48, name: 'ארבעת מימדי הסריקה',
    probe: () => {
      const m = /YS_INF_DIMS\s*=\s*\[([\s\S]*?)\n\];/.exec(code);
      if (!m) return false;
      const rows = m[1].split('\n').filter((l) => /\[\s*$|\['|\[\s*'/.test(l) || /^\s*\[/.test(l));
      return rows.length === 4 &&
             (code.match(/YS_INF_DIMS\.forEach/g) || []).length === 1;
    } },
  { row: 93, name: 'פינוי אוטומטי',
  ...
  { row: 93, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 173, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 135, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 120, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 123, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 139, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 145, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 146, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 62, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 62, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 142, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 84, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 89, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 89, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 115, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 63, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 176, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 176, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 208, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 161, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 182, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 120, name: 'מעטפת WebView חתומה',
  ...
  { row: 182, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 137, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 208, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 207, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 195, name: 'קבוע בלי קורא',
    probe: () => constNoReaderGaps().length === 0 },
  { row: 197, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 94, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 198, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 96, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 166, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 206, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 206, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 206, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 206, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 127, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 174, name: 'שכבת קלט אחידה',
  ...
  { row: 174, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  190: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  185: { claim: 'אין `service_role` בקובץ' },
  34: { claim: 'מסלול «' },
  ...
  187: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  71: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  150: { claim: 'בעלות הסכימה המשותפת' },
  164: { claim: '`tb_kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  165: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  200: { claim: 'ז. עמודה בלי קורא' },
  90: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  125: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  128: { claim: 'margin' },
  ...
  113: { claims: { test_codescan: ['[fn-product]'] },
  ...
  193: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  102: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  ...
  92: { claim: 'storage' },
  91: { claim: 'ג · פעולה שדורשת רשת' },
  107: { claim: 'עברית' },
  110: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  111: { claim: 'BANNER_W' },
  196: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
                 'אין מחלקה שנוספת ל-DOM בלי כלל CSS'] },
  50: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  108: { claim: 'מפנה לקובץ' },
  124: { claim: 'android/app/src/main' },
  131: { claim: 'fgDriftMax' },
  132: { claim: 'CANON_MANIFEST' },
  133: { claim: 'שדות זהים' },
  136: { claim: 'BUILD_SHA' },
  140: { claim: 'SHARED_SHA' },
  138: { claim: 'versionCode' },
  121: { claim: 'WebView' },
  122: { claim: 'android:theme' },
  126: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  143: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  147: { claim: 'migrations' },
  156: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  158: { claim: 'א. חותמת בכל רשומה' },
  61: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  151: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  152: { claim: 'ו. טבלה מקבילה' },
  153: { claim: 'ב. חתימת סכימה' },
  154: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  175: { claim: 'type=password' },
  85: { claim: 'aria-label' },
  ...
  66: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps'] } },
  181: { claim: 'pass_salt' },
  /*  ⭐ סבב 148 — ⛔ סריקה הפוכה: ⚠️ כל אתר חשוד מפיל אלא אם הוא
   *  מוכרז עם נימוקו, ⭐ ורשימת דפוסים הייתה מוצאת את מה שכבר תוקן. */
  186: { claim: '[sec-keyfile]' },
  189: { claim: '⏳' },
  192: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  201: { claim: 'ט. ערך נגזר שהתיישן' },
  203: { claim: 'ח. הגדרה שהוחלפה' },
  202: { claim: 'ד. רשימת-היתר' },
  204: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  106: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  116: { claim: 'CLEANUP_SHA' },
  117: { claim: 'בלוק ה-APP' },
  172: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
 ...
 *  משניהם מפילה כאן, ⛔ ולא מתגלה סבבים אחר כך. */
{
  ...
  const MODULE_FILES = ['appsrc.mjs', 'db_schema.mjs', 'peers.mjs', 'scope.mjs', 'whiten.mjs'];
