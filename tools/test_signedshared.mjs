/* ───────────────────────────────────────────────────────────────────────────
   ...
   **מה יישבר בלעדיו:** ⛔ שורה בטבלה שמצהירה «זהה בית-לבית» על קוד שאין
   ...
   הבא בונה עליה, ⛔ ומגלה מימוש שונה בכל אפליקציה לאותה יכולת.
...
const APP = {
   ...
   *  נרשם כאן. */
  unsignedAllow: {
    migratePrefixValue: 'עוזר להגירת התחילית שמתקן שם ישן בתוך ערך — ⚠️ והגירה היא קוד לסבב אחד: ⭐ הכנסתה לבלוק חתום הייתה מחייבת את ארבע האחיות לשאת אותה, ⛔ ולשלוש מהן אין תחילית שירדה (סבב 148)',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
for (const L of SHARED_LISTS) {
  ...
  t(n++, diff.length === 0,
    ...
    `. מיישרים את הרשימה בכל הריפו באותו סבב`);
}
