...
const APP = {
  ...
  gateNoRows: {
    ...
    'test_attend.mjs':           'מפתח הנוכחות הנטוש והמנגנון החי ששרד — ⚠️ שער ניקוי שאריות, ⛔ והשורה ב-`MATRIX`',
    ...
    'test_dupsession.mjs':       'מניעת כפילות סדרים — ⚠️ יכולת שקיימת בהנהלה בלבד, ⛔ והשורה ב-`MATRIX`',
    ...
    'test_import.mjs':           'מסלול ייבוא התלמידים — ⚠️ יכולת שקיימת בהנהלה בלבד, ⛔ והשורה ב-`MATRIX`',
    'test_kvmeta.mjs':           'חותמת השורה, המתג והנכשל-סגור — ⚠️ המסלול עצמו, ⛔ והשורה עצמה ב-`MATRIX`',
    ...
    'test_mirror_split.mjs':     'שכבת המראה מפורקת לשורות וארבעת מקרי המיזוג — ⚠️ המסלול עצמו, ⛔ והשורה ב-`MATRIX`',
    'test_offline_login.mjs':    'כניסה אופליין מרובת-משתמשים — ⚠️ המסלול עצמו, ⛔ והשורה עצמה ב-`MATRIX`',
    ...
    'test_roles.mjs':            'מודל ההרשאות — ⚠️ המסלול עצמו, ⛔ והשורה עצמה ב-`MATRIX`',
    ...
    'test_sleep_rows.mjs':       'סדרי השינה בשכבת השורות — ⚠️ יכולת שקיימת בהנהלה בלבד, ⛔ והשורה ב-`MATRIX`',
    ...
    'test_tables.mjs':           'שכבת השורות של הנוכחות — ⚠️ יכולת שקיימת בהנהלה בלבד, ⛔ והשורה ב-`MATRIX`',
    'test_users_patch.mjs':      'נתיב העדכון החלקי למראת המשתמשים — ⚠️ המסלול עצמו, ⛔ והשורה עצמה ב-`MATRIX`',
    'test_window.mjs':           'חלון החודש העברי והפעלת החלון החם — ⚠️ יכולת שקיימת בהנהלה בלבד, ⛔ והשורה ב-`MATRIX`',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
 ...
 *  רשימה שנייה בבודק. */
export const ROWS = [58, 51, 53, 52, 55];
