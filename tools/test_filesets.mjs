...
const APP = {
  ...
  testsOnly: {
      'attend': 'מחיקת `ys_attend` — המפתח היחיד מסוגו בארגון (לוגיקה עסקית)',
      'date': 'התאריך העברי אינו נופל-חזרה ל«היום», ושני האדרים אינם קורסים לחודש אחד — ⛔ נמדד: המנוע כאן הוא `ysHebDate` מעל `Intl`; ⚠️ בשכר ובגיוס אפס צרכני תאריך עברי, ⛔ ולכן השער חי ביומן ובהנהלה בלבד',
      'import': 'מסלול הייבוא — היחיד בארגון (לוגיקה עסקית)',
      'mirror_split': 'פיצול המראה לשורות וארבעת מקרי המיזוג — ⛔ רשומת הסדר שנושאת מפה של סימונים קיימת כאן בלבד, ⚠️ ובשלוש האחרות אין מה למדוד',
      'kvmeta': 'חותמת השורה שנכשלת-סגור ומחזירה 0 — ⛔ «אין ראיה שהענן חדש יותר» ולא «הענן חדש»',
      'roles': 'מודל ההרשאות — ⛔ ביומן אין כניסה ואין `role` שיוכרע',
      'offline_login': 'כניסה אופליין — ⛔ ביומן אין כניסה כלל (מטריצה — שורת הכניסה האופליין)',
      'read': 'נועל את **היעדר** הנפילה-חזרה ל-`kv` — ⛔ נתיב שיוחזר «ליתר ביטחון» הוא מקור אמת שני',
      ...
      'tables': 'מבנה שכבת השורות של הנוכחות והשינה — ⛔ טבלאות שקיימות בהנהלה בלבד',
      'window': 'חלון הזמן של הסדרים — לוגיקה עסקית של ריפו זה',
  },
  only: {
...
export const ROWS = [17, 20, 107, 165];
