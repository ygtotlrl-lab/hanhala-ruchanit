 ...
 *
 *  **הנימוק המדוד:** אותו תפקיד נכתב בצורות נבדלות — ⛔ באחת דחיפת שורה
...
const APP = {
   ...
   *  ⭐ ובאפליקציה עם משתמש יחיד אין למי לנהל. */
   ...
   *  נקרא «נמדד ואין». */
  syncLogUser: 'AUTH.user.full_name',
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */

/*  ⛔ שורת טבלת התשתית שהקובץ הזה אוכף — ⚠️ המיפוי נגזר מכאן, ⛔ ואינו
 *  רשימה שנייה בבודק. */
export const ROWS = [72, 73, 63, 206];
...
const START = '/* ═══ שכבת הדחיפה — מודול משותף';
const BLOCK = Object.assign({
  start: START,
  end: '/* ═══════════════ סוף מודול שכבת הדחיפה',
}, capsBlock(START));

/*  ⛔ המנה המוצהרת — ⚠️ הערך אחד בכולן, ⭐ והוא נמדד מול האחיות למטה:
...
/* ── 10ג. מסלול חסום נושא הודעה משלו ───────────────────────────────────── */
/*  ⛔ ההודעה הייעודית ⛔ ולא `MSG_OFFLINE` הכללית — ⚠️ הכללית אינה אומרת
 *  **מה** נחסם, ⭐ וכתיבת משתמש היא המסלול היחיד שאין לו «יסונכרן כשתחזור
 *  הרשת»: ⛔ והנוסח זהה בשלוש שיש בהן כניסה, ⚠️ ששני ניסוחים לאותה חסימה
 *  הם שני מסלולים בעיני הקורא.
 *  ⛔ **והמדידה משני צדדיה** — ⚠️ הקבוע נושא את הנוסח, ⭐ ונקודת המעבר
 *  מחזירה אותו: ⛔ קבוע שאיש אינו מחזיר הוא הודעה שאינה מגיעה לאיש. */
const OFF_UW = '📴 אין חיבור — ניהול משתמשים דורש חיבור לאינטרנט';
if (!APP.userTables.length) {
  pass('10ג. הודעת החסימה — אין כאן טבלת משתמשים, ואין מה לאכוף');
} else {
  const m = /var\s+MSG_OFF_USER_WRITE\s*=\s*'([^']*)'/.exec(src);
  const wired = /offMsg:\s*function\s*\(\s*\)\s*\{\s*return\s+MSG_OFF_USER_WRITE\s*;/
                .test(codeOnly(src));
  if (!m) fail('10ג. הודעת החסימה — `MSG_OFF_USER_WRITE` אינו מוצהר, נמדדו 0 הצהרות ' +
               'והצפוי אחת. מוסיפים אותו ליד הודעות הכניסה');
  else if (m[1] !== OFF_UW)
    fail('10ג. הודעת החסימה — הנוסח «' + m[1] + '» והצפוי «' + OFF_UW + '». ' +
         'מיישרים לנוסח המשותף');
  else if (!wired)
    fail('10ג. הודעת החסימה — `USER_CFG.offMsg` אינו מחזיר את ההודעה הייעודית, ' +
         'נמדדה החזרה אחרת והצפוי `MSG_OFF_USER_WRITE`. מחליפים אותו');
  else pass('10ג. הודעת החסימה — נוסח משותף אחד, ונקודת המעבר מחזירה אותו');
}

/* ── 10ד. אתר אימות שש הספרות מפנה לקבוע המוצהר ────────────────────────── */
/*  ⛔ מחרוזת שנכתבת באתר האימות — ⚠️ שני אתרים נשאו שני ניסוחים, ⭐ והמשתמש
 *  קרא שתי תקלות שונות לאותה חסימה: ⛔ ולכן הנוסח יושב בקבוע אחד.
 *  ⛔ **ואפס אתרים אינם דורשים קבוע** — ⚠️ אפליקציה שאין בה מסך שיוצר או
 *  משנה סיסמה אין לה מה לאמת, ⭐ וקבוע בלי אתר הוא הצהרה שאיש אינו רואה. */
{
  const c = codeOnly(src);
  const PASS_SIX = '⚠️ הסיסמה חייבת להיות שש ספרות';
  /*  ⛔ גוף התנאי נחתך בהתאמת סוגריים ⛔ ולא בחלון תווים — ⚠️ תנאי שנשבר
   *  לשתי שורות היה נחתך באמצע, ⭐ והאתר היה נספר «בלי הקבוע». */
  const braceAt = (text, open) => {
    let d = 0;
  ...
  const sites = [];
  for (const s of c.matchAll(/PASS_SIX_RE\s*\.\s*test|length\s*!==\s*6(?![0-9])/g)) {
    const b = c.indexOf('{', s.index);
    sites.push({ line: c.slice(0, s.index).split('\n').length,
                 body: b < 0 ? '' : braceAt(c, b) });
  }
  const mm = /var\s+MSG_PASS_SIX\s*=\s*'([^']*)'/.exec(src);
  const bare = sites.filter((s) => s.body.indexOf('MSG_PASS_SIX') < 0);
  if (sites.length && !mm)
    fail('10ד. הודעת שש הספרות — נמדדו ' + sites.length + ' אתרי אימות ואפס הצהרות, ' +
         'והצפוי הצהרה אחת. מוסיפים את `MSG_PASS_SIX`');
  /*  ⛔ הקבוע חי בבלוק החתום — ⚠️ ולכן אפליקציה בלי מסך סיסמה נושאת אותו
   *  בכל זאת: ⭐ המשפחה תשתית ולא מפקד, ⛔ ורשימה שנגזרת ממי שמציג אותה
   *  היום משתנה עם המוצר. */
  else if (mm && mm[1] !== PASS_SIX)
    fail('10ד. הודעת שש הספרות — הנוסח «' + mm[1] + '» והצפוי «' + PASS_SIX + '». ' +
         'מיישרים לנוסח המשותף');
  else if (bare.length)
    fail('10ד. הודעת שש הספרות — נמדדו ' + bare.length + ' אתרי אימות בלי הקבוע ' +
         '(שורות ' + bare.map((s) => s.line).join(', ') + '), והצפוי אפס. ' +
         'מחליפים למחרוזת המוצהרת');
  else if (!sites.length)
    pass('10ד. הודעת שש הספרות — אין כאן מסך שיוצר או משנה סיסמה, ואין מה לאכוף');
  else pass('10ד. הודעת שש הספרות — ' + sites.length + ' אתרי אימות, וכולם מפנים לקבוע');
}

/* ── 10ה. שכבת הטביעה — ליבה אחת, מסך שינוי, וכל עזר נוסף מוצהר ────────── */
/*  ⛔ **מה נכנס**: שלוש פונקציות הליבה שקיימות בשלוש שיש בהן כניסה,
 *  ⛔ **ומה מפיל**: אחת שחסרה · מסך שינוי סיסמה שאינו קיים · ועזר שקיים
 *  כאן בלבד ואין לו נימוק — ⭐ **משתמש שאינו יכול לשנות סיסמה בעצמו תלוי
 *  במסד בכל שינוי**, ⚠️ ושלוש שכבות טביעה שנבדלות הן שלוש התנהגויות
 *  באותו מקש. ⚠️ **ומספר המשתמשים אינו הנימוק** — ⛔ מסך **ניהול**
 *  המשתמשים הוא שנגזר ממנו, ⭐ ולכן הוא מוצהר בנפרד. */
const PASS_CORE = ['fp', 'make', 'fields'];
if (!APP.userTables.length) {
  pass('10ה1. שכבת הטביעה — אין כאן טבלת משתמשים, ואין מה לאכוף');
  pass('10ה2. מסך שינוי הסיסמה — אין כאן כניסה, ואין מה לאכוף');
  pass('10ה3. מסך ניהול המשתמשים — אין כאן כניסה, ואין מה לאכוף');
  pass('10ה4. עזר שקיים כאן בלבד — אין כאן כניסה, ואין מה לאכוף');
  pass('10ה5. נפילת דיאלוג — אין כאן מסך סיסמה, ואין מה לאכוף');
} else {
  const c = codeOnly(src);
  const fns = APP.passFns || {};
  const has = (n) => !!n && fnBody(c, n) !== '';
  const missing = PASS_CORE.filter((k) => !fns[k] || !has(fns[k].name || fns[k]));
  if (missing.length)
    fail('10ה1. שכבת הטביעה — נמדדו ' + missing.length + ' פונקציות ליבה חסרות (' +
         missing.join(', ') + ') והצפוי אפס. מוסיפות ל-`APP.passFns` ולקוד');
  else pass('10ה1. שכבת הטביעה — שלוש פונקציות הליבה קיימות ומוצהרות');

  /*  ⛔ המסך נמדד בשני צדדיו — ⚠️ הפותח **והפעולה שבמפה**: ⭐ מסך שנפתח
   *  ואין לו שמירה הוא חצי יכולת, ⛔ ופעולה בלי מסך היא מטפל שאיש אינו
   *  מגיע אליו.
   *  ⛔ **ושתי פעולות ולא אחת** — ⚠️ `my-pass` פותח
   *  ו-`my-pass-save` שומר, ⭐ **ושמותיהן אחידים בשלוש שיש בהן כניסה**:
   *  ⛔ שם שנבדל בין השלוש הוא אותה יכולת בשלושה חיפושים. */
  const scr = APP.passScreen || {};
  const actIn = (a) => !!a && new RegExp("'" + a + "'\\s*:").test(c);
  if (!scr.open || !scr.act || !scr.open_act)
    fail('10ה2. מסך שינוי הסיסמה — נמדדו 0 הצהרות שלמות והצפוי `open` · ' +
         '`open_act` · `act`. משלימים את `APP.passScreen`');
  else if (scr.open_act !== 'my-pass' || scr.act !== 'my-pass-save')
    fail('10ה2. מסך שינוי הסיסמה — נמדדו «' + scr.open_act + '»/«' + scr.act +
         '» והצפוי «my-pass»/«my-pass-save». מיישרים את שמות הפעולות');
  else if (!has(scr.open))
    fail('10ה2. מסך שינוי הסיסמה — הפותח «' + scr.open + '» מוצהר ואינו בקוד, ' +
         'והצפוי פונקציה חיה. כותבים אותה או מוחקים את ההצהרה');
  else if (!actIn(scr.open_act) || !actIn(scr.act))
    fail('10ה2. מסך שינוי הסיסמה — נמדדה פעולה מוצהרת שאינה במפת הפעולות ' +
         'והצפוי שתיהן. מחווטים את «' + scr.open_act + '» ואת «' + scr.act + '»');
  else pass('10ה2. מסך שינוי הסיסמה — «' + scr.open + '», «' + scr.open_act +
            '» ו-«' + scr.act + '» חיים');

  /*  ⛔ וכל דיאלוג נופל ל-`uiNoDialog` כשהמיכל אינו ב-DOM — ⚠️ ארבעת
   *  קוראי שכבת המודאל ועוד אחד ממסך הסיסמה: ⭐ שומר שקורא ערך משדה
   *  שאינו קיים זורק, ⛔ והמסלול נגמר בלי שהמשתמש יודע למה. */
  const noDlg = (c.match(/uiNoDialog\(/g) || []).length - 1;
  if (noDlg !== 5)
    fail('10ה5. נפילת דיאלוג — נמדדו ' + noDlg + ' קוראי `uiNoDialog` והצפוי 5: ' +
         'ארבעת קוראי שכבת המודאל ושומר מסך הסיסמה. מוסיפים את החסר');
  else pass('10ה5. נפילת דיאלוג — 5 קוראי `uiNoDialog`, ובהם שומר מסך הסיסמה');

  if (!Object.prototype.hasOwnProperty.call(APP, 'usersScreen'))
    fail('10ה3. מסך ניהול המשתמשים — נמדדו 0 הצהרות והצפוי אחת (ריקה כשאין). ' +
         'מוסיפים `APP.usersScreen`');
  else if (APP.usersScreen && !has(APP.usersScreen))
    fail('10ה3. מסך ניהול המשתמשים — «' + APP.usersScreen + '» מוצהר ואינו בקוד, ' +
         'והצפוי פונקציה חיה. כותבים אותה או מרוקנים את ההצהרה');
  else pass('10ה3. מסך ניהול המשתמשים — ' +
            (APP.usersScreen ? '«' + APP.usersScreen + '» מוצהר וחי' : 'מוצהר ריק — נמדד ואין'));

  const extra = Object.keys(fns).filter((k) => PASS_CORE.indexOf(k) < 0);
  const bareWhy = extra.filter((k) => !String((fns[k] && fns[k].why) || '').trim());
  const ghost = Object.keys(fns).filter((k) => !has(fns[k].name || fns[k]));
  if (ghost.length)
    fail('10ה4. שכבת הטביעה — נמדדו ' + ghost.length + ' הצהרות בלי פונקציה בקוד (' +
         ghost.join(', ') + ') והצפוי אפס. מוחקים את ההצהרה');
  else if (bareWhy.length)
    fail('10ה4. עזר שקיים כאן בלבד — נמדדו ' + bareWhy.length + ' בלי נימוק (' +
         bareWhy.join(', ') + ') והצפוי אפס. מוסיפים נימוק ל-`APP.passFns`');
  else pass('10ה4. עזר שקיים כאן בלבד — ' + extra.length + ' מוצהרים, וכולם נושאים נימוק');
}

/* ── 11. המנה זהה בכל הריפו ────────────────────────────────────────────── */
{
  const seen = [];
...
/* ══════════════════════════════════════════════════════════════════════════
   ⛔ שומר ההקשר — החלפה באמצע מחזור
   ══════════════════════════════════════════════════════════════════════════
   ⛔ מה נאכף: הדפוס שכל האפליקציות מחווטות בו — ⚠️ `dirty` לוכד את
...
/*  ⛔ מ9 — פעולה מוצהרת שאיבדה את אתר הקריאה. ⚠️ זה בדיוק
...
/*  ⛔ מ10 — אתר קריאה שאין לו הצהרה. ⚠️ הצד השני של אותו
