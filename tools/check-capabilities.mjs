...
const APP = {
  ...
  actMap: 'DOM_ACTIONS',
  gapRows: [104, 108, 126, 127, 131, 141],
...
export const ROWS = [37, 41, 42, 44, 40, 52, 74, 47, 136, 84];
...
const tier1Evicts = () => {
  ...
  if (/get\s+tier1\s*\(\s*\)\s*\{/.test(pb))
    return /get\s+tier1\s*\(\s*\)\s*\{[\s\S]{0,800}?_\w+PushedThrough\s*\(/.test(pb);
  return /tier1\s*:\s*\[/.test(pb);
};
...
const MATRIX = [
  ...
  { row: 72, name: 'מיכל המודאל ילד ישיר של `body`',
  ...
  { row: 68, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 69, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 155, name: 'פונקציה בלי קוראים',
  ...
  { row: 66, name: 'אובייקט יקר נבנה פעם אחת',
  ...
  { row: 67, name: 'מסך מציג מיד ומרענן ברקע',
  ...
  { row: 73, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 75, name: 'פינוי אוטומטי',
    probe: () => (/tier2\s*[:=]\s*\[\s*\{/.test(policyBlock()) || tier1Evicts()) &&
  ...
  { row: 54, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 54, name: 'יומן פעולות',          probe: () => present.log === true },
  ...
  { row: 76, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 77, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 56, name: 'מנוע מיזוג עם הגנת ⏳',
  ...
  { row: 162, name: 'מודול ה-service worker',
  ...
  { row: 59, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 62, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 161, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 53, name: 'רענון תקופתי של מונה הממתינים',
    probe: () => hasCode(/setInterval\(\s*pendRender\s*,\s*60000\s*\)/) },
  { row: 79, name: 'פסק זמן אחיד לקריאות רשת',
  ...
  { row: 160, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 65, name: 'מיון אחד לכל התצוגות',
...
const GATES = {
  ...
  48: { manual: 'המדידה שלפני המחיקה היא התנהגות סשן — ⛔ נאכפת רק בתוצאתה, בשער ההסרות' },
  47: { claim: 'אין פרק פערים נפרד' },
  87: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  51: { manual: 'מחזור העבודה ב-git הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  ...
  49: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  50: { manual: 'התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו קריאה חסכונית' },
  52: { claims: { 'check-capabilities': ['sha:', 'orderGaps'],
                  test_sharedsync: ['block-drift', 'canon-drift'],
                  test_signedshared: 'unsignedTwins' } },
  63: { claim: 'CANON' },
  74: { claim: 'storage' },
  78: { claim: 'ג · פעולה שדורשת רשת' },
  83: { claim: 'עברית' },
  84: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  85: { claim: 'BANNER_W' },
  156: { claim: 'CSS מתות' },
  43: { manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ והמונה נמדד ידנית' },
  86: { claim: 'מפנה לקובץ' },
  ...
  158: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  154: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  157: { claim: 'ד. רשימת-היתר' },
  159: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
   ...
   ──────────────────────────────────────────────────────────────────────── */
{
  ...
  /*  ⛔ וסדר הסעיפים בתוך כל בלוק נגזר מהזיווג (סבב 92) — ⚠️ לפי מספר
   *  השורה שהסעיף מזווג אליה, ⭐ וזו גם סדר הקטגוריות: המספור רץ א ⟵ יב.
   *  ⛔ והסדר **נגזר ⛔ ואינו מוקלד** — ⚠️ רשימה מוקלדת נסחפת בכל מספור
   *  מחדש, ⭐ והזיווג אינו. ⚠️ הנימוק המדוד: הסעיפים ישבו בסדר שאיש לא
   *  קבע, ⛔ ומי שחיפש כלל לפי השורה שלו סרק את כל הבלוק. */
  const order = {};
  {
    let cur = null;
    for (const l of ls) {
      const m = /^<!--\s*SHARED:start\s+id="(rules-[a-z]+)"/.exec(l);
      if (m) { cur = m[1]; order[cur] = []; continue; }
      if (/^<!--\s*SHARED:end/.test(l)) { cur = null; continue; }
      if (cur && l.startsWith('### ')) {
        const raw = l.slice(4).trim().replace(/\s+[◆◇⧉]$/, '');
        order[cur].push([raw, RULE_ROWS[raw]]);
      }
    }
  }
  const outOfOrder = [];
  for (const bid of Object.keys(order)) {
    const seq = order[bid];
    for (let i = 1; i < seq.length; i++)
      if (seq[i][1] < seq[i - 1][1])
        outOfOrder.push(`${bid}: «${seq[i][0]}» (${seq[i][1]}) אחרי «${seq[i - 1][0]}» (${seq[i - 1][1]})`);
  }
  if (outOfOrder.length)
    fail(`סדר סעיפי הכללים אינו נגזר מהזיווג: ${outOfOrder.join(' · ')} — ` +
         `נמדדו ${outOfOrder.length} היפוכים והצפוי אפס. מיישרים את הסעיפים ` +
         'לפי מספר השורה שהם מזווגים אליה — הסדר נגזר, ואינו נכתב ביד');
  else
    pass(`סדר סעיפי הכללים נגזר מהזיווג — ${Object.keys(order).length} בלוקים, ` +
         `${heads.length} סעיפים, וכולם בסדר עולה לפי שורתם`);
