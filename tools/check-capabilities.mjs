...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    162: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    161: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    127: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    158: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '158|מבנה טופס הכניסה': 'src',
    '129|שם נגזר אינו נשמר ברשומה': 'src',
    '174|מפתח אחסון בלי קורא': 'src',
    '33|שער מריץ את כל טענותיו': 'tools',
    '86|שכבת המראה': 'src',
    '175|הגירה מקומית שהושלמה': 'mixed',
    '160|מראת המשתמשים': 'src',
    '159|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'tools',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '143|הקשר נלכד בכניסה לפונקציה': 'src',
    '89|פריסה במסכי טלפון וטאבלט': 'src',
    '90|ערכת נושא — בהיר וכהה': 'src',
    '170|הערה שמתארת מצב שחלף': 'mixed',
    '176|שאילתת `@media` מתה': 'src',
    '103|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '104|ייצוא והנפקה — דרך מוצהרת': 'src',
    '91|המסמך מתפרסר נקי': 'src',
    '92|לוג מסלול — תג ורמה': 'src',
    '75|מיכל המודאל ילד ישיר של `body`': 'src',
    '99|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '145|הגיבוי שלם ומעומד': 'src',
    '71|פעולה מגיבה מיד': 'src',
    '72|משיכה מסוננת בשרת': 'src',
    '115|דגימת-יתר במחולל': 'tools',
    '162|`PBKDF2` — פרמטרים': 'src',
    '133|`migrations/` — תוכן': 'src',
    '134|`migrations/` — כמות': 'src',
    '151|מיזוג מכל — מפה ומערך': 'src',
    '152|ברירת מחדל אינה ממוזגת': 'src',
    '153|רשימה אינה נושאת פריט כפול': 'src',
    '146|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'tools',
    '172|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'tools',
    '166|סודות אינם עוזבים את המכשיר': 'src',
    '140|דפוס `upsert` — onConflict מוכרז': 'src',
    '164|טביעה חסרה מושלמת אוטומטית': 'src',
    '164|כניסה אופליין': 'src',
    '126|עריכת נתונים אופליין': 'src',
    '150|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '165|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '69|אובייקט יקר נבנה פעם אחת': 'src',
    '70|מסך מציג מיד ומרענן ברקע': 'src',
    '119|האפליקציה ניתנת להתקנה מכרום': 'src',
    '168|דגלי מעבר — אין דגל דלוק': 'src',
    '76|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '161|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '41|ארבעת מימדי הסריקה': 'src',
    '85|פינוי אוטומטי': 'src',
    '85|אופק הפינוי נוקה מהסנכרון': 'src',
    '155|אימות פינוי מול הענן': 'src',
    '120|שיתוף קבצים': 'src',
    '105|מעטפת APK (WebView)': 'src',
    '108|אין נכסים מוטבעים': 'src',
    '124|מפתח חתימה קבוע בריפו': 'src',
    '130|מקור אמת יחיד לסכימה': 'src',
    '131|קובץ התקנה מלא': 'src',
    '55|גיבוי יומי אוטומטי': 'src',
    '55|יומן פעולות': 'src',
    '127|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    ...
    '158|מסך שינוי סיסמה עצמי': 'src',
    '184|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '57|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '164|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '58|מודול מזהי רשומות': 'src',
    '58|מזהה אינו מומר למספר': 'src',
    '60|מזהה מכשיר': 'src',
    '57|ליבת מיזוג משותפת': 'src',
    '105|מעטפת WebView חתומה': 'tools',
    '164|אימות מול טביעה בענן': 'mixed',
    '122|בניית APK אחידה עם שער חתימה': 'src',
    '184|מודול ה-service worker': 'src',
    '61|ניסיון חוזר בתור הסנכרון': 'src',
    '62|מנגנון משיכה אחיד': 'src',
    '65|נעילת חוסר-פעילות': 'src',
    '66|מודל הסשן — בזיכרון בלבד': 'src',
    '183|בדיקת עדכון תקופתית ל-service worker': 'src',
    '87|פסק זמן אחיד לקריאות רשת': 'src',
    '88|מאזיני מצב רשת': 'src',
    '149|גריעת tombstones לפי גיל': 'src',
    '182|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '120|גשר שיתוף': 'src',
    '67|העברת מזהה ל-DOM': 'src',
    '68|מיון אחד לכל התצוגות': 'src',
    '73|ייצוא — שני דפוסים מוצהרים': 'src',
    '112|שכבת אייקונים': 'src',
    '156|שכבת קלט אחידה': 'src',
    '156|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '74|שכבת המודאל': 'src',
    '158|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
const CAPS = {
  ...
  storage: {
    ...
    block: { sha: '5928d918bb1d4ed6', lines: 617,
...
const BLOCK_ORDER = ['toastcss', 'bp', 'neterr', 'rowswin', 'guardonline', 'storage', 'schemastale', 'busyguard', 'savepipe', 'swreg', 'backup',
...
const CAT_ORDER = [
  ...
  { g: 'ו', phases: [{ p: 'נתונים', w: ['העברת מזהה', 'מיון אחד', 'אובייקט יקר'] },
                     ...
                     { p: 'עיצוב', w: ['פריסה במסכי', 'ערכת נושא'] }] },
...
export function run(over, changed) {
_OVER = over || null;
...
const FLOOR = { shared: 164, app: 10, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי ושער מצב הרשת' };
/* ⚠️ סוף פר-אפליקציה */
 ...
 *  שקיים במקום הלא-נכון מפיל. */
{
  ...
  if (brg.length)
    fail(`בלוקים שאינם מוצהרים בטבלה — נמדדו ${brg.length} והצפוי אפס (${brg.join(' · ')}). ` +
         `מוסיפים שורה בטבלה ומצהירים את שמה ב-docRows, או מיישרים את השם הקיים`);
  else
    pass(`הצהרת הבלוקים בטבלה — ${Object.keys(CAPS).length} בלוקים, וכל אחד מוצהר בשורה`);
...
const MATRIX = [
  ...
  { row: 129, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 143, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 170, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 104, name: 'ייצוא והנפקה — דרך מוצהרת',
  ...
  { row: 99, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 145, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 142, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 115, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 162, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 133, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 134, name: '`migrations/` — כמות',
  ...
  { row: 151, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 152, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 153, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 166, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 140, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 164, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 164, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 126, name: 'עריכת נתונים אופליין',
  ...
  { row: 150, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 165, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 119, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 168, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 161, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 155, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 120, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 105, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 108, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 124, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 130, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 131, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 55, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 55, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 127, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 100, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 56, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 56, name: 'שחזור מקומי מהענן',
    probe: () => horizonReleaseWired() },
  { row: 158, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 158, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 184, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 144, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 164, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 105, name: 'מעטפת WebView חתומה',
  ...
  { row: 164, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 122, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 184, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 183, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 149, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 112, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 156, name: 'שכבת קלט אחידה',
  ...
  { row: 156, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  148: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  177: { claim: 'ז. עמודה בלי קורא' },
  ...
  110: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  113: { claim: 'margin' },
  ...
  98: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  94: { claim: 'עברית' },
  96: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  97: { claim: 'BANNER_W' },
  173: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  95: { claim: 'מפנה לקובץ' },
  109: { claim: 'android/app/src/main' },
  116: { claim: 'fgDriftMax' },
  117: { claim: 'CANON_MANIFEST' },
  118: { claim: 'שדות זהים' },
  121: { claim: 'BUILD_SHA' },
  125: { claim: 'SHARED_SHA' },
  123: { claim: 'versionCode' },
  106: { claim: 'WebView' },
  107: { claim: 'android:theme' },
  111: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  128: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  132: { claim: 'migrations' },
  139: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  141: { claim: 'א. חותמת בכל רשומה' },
  54: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  136: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  137: { claim: 'ו. טבלה מקבילה' },
  138: { claim: 'ב. חתימת סכימה' },
  157: { claim: 'type=password' },
  78: { claim: 'aria-label' },
  ...
  163: { claim: 'pass_salt' },
  167: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  169: { claim: '⏳' },
  49: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  171: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  178: { claim: 'ט. ערך נגזר שהתיישן' },
  180: { claim: 'ח. הגדרה שהוחלפה' },
  179: { claim: 'ד. רשימת-היתר' },
  181: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  93: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  101: { claim: 'CLEANUP_SHA' },
  102: { claim: 'בלוק ה-APP' },
  154: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
...
if (!changed) {
  ...
  const keys = MATRIX.filter(probeFn).map(probeKey);
  const g = [];
  for (const k of keys) if (!declMap[k]) g.push('בלי הצהרה: ' + k);
  for (const k of Object.keys(declMap)) if (keys.indexOf(k) < 0) g.push('הצהרה בלי בדיקה: ' + k);
  for (const k of keys) {
