...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    99: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    98:  (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    81: (c) => c.hasCode(/YS_KV_LEGACY_WRITE\s*=\s*false/),
    ...
    97: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [31, 33, 34, 36, 37, 41, 56, 119];
...
const MATRIX = [
  { row: 99, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 87, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  { row: 101, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 80, name: 'עריכת נתונים אופליין',
  ...
  { row: 90, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 104, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 55, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 98, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  { row: 111, name: 'פינוי אוטומטי',
    probe: () => /tier2\s*[:=]\s*\[\s*\{/.test(policyBlock()) },
  { row: 91, name: 'אימות פינוי מול הענן',
    probe: () => /\bverify\s*:/.test(policyBlock()) },
  { row: 75, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 67, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 69, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 79, name: 'מפתח חתימה קבוע בריפו',
    probe: () => hasPath('signing') &&
                 fs.readdirSync('signing').some((f) => f.endsWith('.keystore')) },
  { row: 83, name: 'מקור אמת יחיד לסכימה', probe: () => hasPath(APP.schemaFile) },
  { row: 84, name: 'קובץ התקנה מלא',       probe: () => hasPath(APP.schemaFile) },
  { row: 43, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 43, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 81, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 57, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  { row: 44, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 44, name: 'שחזור מקומי מהענן',
    probe: () => callSites('hwRestoreMount').length > 0 },
  { row: 97, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 118, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 43, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 88, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 45, name: 'מנוע מיזוג עם הגנת ⏳',
    probe: () => /isPend \|\| tsOf\(loc\) > tsOf\(rem\)/.test(fnBody('_mergePick')) },
  { row: 101, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 67, name: 'מעטפת WebView חתומה',
  ...
  { row: 101, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 77, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 118, name: 'מודול ה-service worker',
  ...
  { row: 48, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 51, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 117, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 42, name: 'רענון תקופתי של מונה הממתינים',
    probe: () => hasCode(/setInterval\(\s*pendRender\s*,\s*60000\s*\)/) },
  { row: 59, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  בשקט».                                                        */
  { row: 89, name: 'גריעת tombstones לפי גיל',
    probe: () => hasCode(/TOMBSTONE_TTL_MS/) },
  { row: 116, name: 'אוטו-אפדייט מ-raw.githubusercontent',
  ...
  { row: 71, name: 'שכבת אייקונים',
    probe: () => iconAudit('.').length === 0 },
  { row: 92, name: 'שכבת קלט אחידה',
...
const GATES = {
  ...
  14: { claim: 'CANON_MD' },
  35: { claim: 'תוכן הקבצים הנלווים' },
  26: { claim: 'gen-icons' },
  38: { manual: 'המדידה שלפני המחיקה היא התנהגות סשן — ⛔ נאכפת רק בתוצאתה, בשער ההסרות' },
  119: { claim: 'אין פרק פערים נפרד' },
  120: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  121: { manual: 'מחזור העבודה ב-git הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  39: { claim: 'מזהה שנמחק ונשאר לו קורא' },
  72: { claim: 'tile-bg' },
  ...
  31: { claim: 'כיסוי הטבלה' },
  32: { claim: 'עמודת «מה זה» היא הוראה מחייבת' },
  33: { claim: 'מספור רציף ובלי כפילויות' },
  34: { claim: 'הערה ריקה' },
  25: { claim: 'drift' },
  28: { claim: 'measure-gap' },
  29: { manual: 'חפיפה בין שערים נסרקת ידנית — ⛔ «אותה טענה» היא קריאת משמעות' },
  24: { claim: 'תווית מוטציה מודפסת' },
  21: { claim: 'תקן הבאנר' },
  17: { claim: 'דפוס הבודקים' },
  30: { claim: 'מספרי הבאנר' },
  27: { claim: 'ריצת check-js' },
  36: { claim: 'כל כלל מיוצג בטבלה' },
  37: { claim: '🔲 אינו קבוע' },
  40: { manual: 'התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו קריאה חסכונית' },
  41: { claim: 'sha:' },
  52: { claim: 'CANON' },
  56: { claim: 'storage' },
  58: { claim: 'ג · פעולה שדורשת רשת' },
  61: { claim: 'עברית' },
  62: { claim: 'RULE_W' },
  63: { claim: 'BANNER_W' },
  64: { claim: 'CSS מתות' },
  65: { manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ והמונה נמדד ידנית' },
  66: { claim: 'מפנה לקובץ' },
  70: { claim: 'android/app/src/main' },
  73: { claim: 'fgDriftMax' },
  74: { claim: 'CANON_MANIFEST' },
  76: { claim: 'BUILD_SHA' },
  78: { claim: 'versionCode' },
  68: { claim: 'WebView' },
  82: { manual: 'נפילה-חזרה ל-`kv` נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  85: { claim: 'migrations' },
  86: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  93: { claim: 'type=password' },
  94: { claim: 'aria-label' },
  95: { manual: 'שני מנועי תאריך — ⛔ טרם הוכרע איזה, ואין מה לאכוף' },
  96: { manual: '«`catch` ריק סביב כתיבה» טרם נוסח כשער — נסרק ידנית בכל סבב שנוגע' },
  100: { claim: 'pass_salt' },
  102: { manual: 'מצב העמודה במסד אינו נראה מהריפו' },
  103: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  105: { claim: '⏳' },
  106: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  114: { manual: 'קיום טבלה או מפתח במסד אינו נראה מהריפו' },
  109: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  107: { manual: '«קובץ בלי קורא» דורש סריקה על ארבעת הריפו — נעשה ידנית' },
  115: { manual: 'קיום מפתח במסד אינו נראה מהריפו' },
  108: { manual: 'רשימת-היתר הגיבויים יושבת במיגרציה שכבר רצה' },
  110: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  112: { claim: 'CLEANUP_SHA' },
  113: { claim: 'בלוק ה-APP' },
};
...
const RULE_ROWS = {
  '⛔ git': 121,
  '⛔ הסבב מעדכן את הטבלה': 34,
  '⛔ מדידה לפני שינוי': 38,
  '⛔ סריקת האזור שנגעו בו': 106,
  '⛔ משמעת קריאה וחיסכון טוקנים': 40,
  '⛔ הכרעה שאינה של הסשן': 40,
  '⛔⛔ רכיב משותף = קוד זהה בית-לבית + מיקום זהה + חיווט חי': 41,
  '⛔ תקציב הקבצים': 4,
  '⛔ מתי כותבים הערה': 65,
  '⛔ הערה אינה מפנה לקובץ': 66,
  '⛔ צורת ההערה': 61,
  '⛔ דפוסי כתיבת ממשק': 53,
  '⛔ מבחן שקודו מנוטרל נושא נימוק בבאנר': 19,
  '⛔ הבדל מכוון מנומק בכתב, במקום שבו רואים אותו': 120,
  '⛔ ערך שקיים בקוד או במסד אינו מוצהר בתיעוד': 30,
  '⛔ קובץ נשפט לפי תפקידו': 3,
  '⛔ קובץ תיעוד אינו מסביר כלל שהטבלה אוכפת': 35,
  '⛔ תקרה היא מספר עגול שנקבע בהחלטה': 4,
  '⛔ שרשרת היכולת החדשה': 31,
  '⛔⛔ עמודת «מה זה» היא הוראה מחייבת': 32,
  '⛔ כל כלל בא לידי ביטוי בטבלה': 36,
  '⛔ מספור רציף ומבני, וסריקת כפילויות': 33,
  '⛔ תקן עמודת ההערות': 34,
  '⛔ `probe` מאמת **ערך** — ⛔ ולא קיום': 31,
  '⛔ תקן תוכן המבחן': 20,
  '⛔ תקן תוכן המוטציה': 23,
  '⛔ שער אינו מריץ את `check-js` במלואו': 27,
  '⛔ שני שערים על אותה טענה — אחד מהם מיותר': 29,
  '⛔ בדיקה שסופרת מילה אינה בדיקה': 24,
  '⛔ מבחן מצהיר אילו שורות בטבלה הוא אוכף': 31,
  '⛔ מוטציה נוקבת בשם הטענה שאמורה ליפול': 23,
  '⛔ תקן הבאנר — ארבע פסקאות בסדר קבוע': 21,
  '⛔ דפוס הבודקים והודעת הכשל': 17,
  '⛔ מספר בבאנר נגזר מהקבוע שבקוד': 30,
  '⛔ מה שאינו ניתן לאכיפה מכנית — נרשם ככזה': 119,
  '⛔ שער קורא בלבד': 25,
  '⛔ שער חדש נכנס ל-`APP.gates`': 31,
  '⛔ שינוי מכוון ברכיב משותף': 41,
};
