 ...
 *  ביטויים רגולריים על הטקסט. ⛔ המוטציות אינן נכתבות לעץ.
 *
...
const APP = {
  ...
  noteExtra: {
    _hrQueueAdd: 'תור ההגדרות האופליין — כתיבה מקומית שאינה עוברת במתזמן הדחיפה, ⛔ ופריט שנכנס אחרי כשל לא היה מקבל ניסיון חוזר',
  },
  bootFn: 'loadDash',
...
console.log('\n🔎 אישור ה-⏳ בריקון האוטומטי — ' + FACTS.slug + '\n');
...
function scenarios(moduleSrc) {
  ...
  /* ג. תור שהתרוקן ⇒ המונה אינו מציג דבר */
  {
    const h = harness(moduleSrc);
    h.ctx.pendMark('entry:1');
    h.st.now += 3000;                      // מעבר להשהיית הציור
    h.ctx.pendDrawFlush();
    h.ctx.pendRender();
  ...
  /* ה. סימון שנוצר **אחרי** t0 שורד — ר\' כלל ה-t0 */
  {
    const h = harness(moduleSrc);
    const t0 = h.st.now;
    h.st.now += 50;
 ...
 *  ⚠️ הטענות מודדות את מסלול האישור ואת המונה שנגזר ממנו, ⛔ ולא את שמות
 *  המפות שמחזיקות אותו: ⭐ שער שהיה נופל על שינוי שם היה חוסם כל ניקוי,
 *  ⛔ ומי שנתקל בו היה מחזיר את השם הישן בתום לב. */
{
  const renamed = block.replace(/_pendDrawHold\b/g, '_pendHoldUntil');
  if (renamed === block) fail('5. מוטציית-הנגד לא מצאה את שם המפה בליבה');
  else {
    let res;
    try { res = scenarios(renamed); } catch (e) { res = [{ name: 'שגיאה: ' + e.message, ok: false }]; }
 ...
 *  גופו בלבד — בשלוש מהאפליקציות האישור יושב פונקציה או שתיים
...
/* ══ מודול הניסיון החוזר ══════════════════════════════════════════════════ */
/*  ⛔ בלוק משלו — ⚠️ שני השערים חתכו בלוקים שונים מאותו קובץ
 *  ותחת אותם שמות, ⛔ והמיזוג בלי הפרדה היה מחליף ביניהם בשקט. */
{
  const START = '/* ═══ ניסיון חוזר בסנכרון — מודול משותף';
  ...
  /*  ⛔ **וכל משפך דורך גם את המתזמן** — ⚠️ משפך שדורך את הניסיון החוזר
   *  ולא את המתזמן מייצר שני זמני תגובה לאותה פעולה: ⭐ אחת נדחפת בתוך
   *  ההשהיה, ⛔ והשנייה ממתינה לפולינג הבא. ⚠️ והמתזמן מושהה ומאחד,
   *  ⭐ ולכן קריאה נוספת ממי שדרך גם הוא אינה מחזור סנכרון נוסף. */
  /*  ⛔ הנמדד הוא **המשפכים המוכרזים** ⛔ ולא המתזמן עצמו — ⚠️ הוא
   *  `APP.noteFn`, ⭐ והוא זה שדורך את הניסיון החוזר: ⛔ דרישה שיקרא
   *  לעצמו אינה טענה. ⚠️ **והרשימה שלמה** — ⭐ טענה 6 מקבעת שמספר קריאות
   *  `rtyNote()` הוא בדיוק המתזמן ועוד המוכרזים, ⛔ ולכן משפך שלא הוכרז
   *  אינו יכול להתקיים. */
  {
    const mute = extra.filter((f) => !/\bschedulePush\s*\(/.test(fnBody(codeOutside, f)));
    if (mute.length)
      fail(`7ב. משפך שאינו דורך את המתזמן: ${mute.join(', ')} — נמדדו ${mute.length} ` +
           'והצפוי אפס. מוסיפים `schedulePush()` במשפך, ' +
           'או מצהירים את המסלול המיידי בתקן «`push` — שכבת הדחיפה» עם נימוקו');
    else if (extra.length)
      pass(`7ב. ${extra.length} משפכים מוכרזים, וכולם דורכים את \`schedulePush()\``);
    else pass('7ב. אין כאן משפך מוכרז נוסף — ⚠️ והמתזמן הוא המשפך היחיד');
  }
  ...
  /* ══════════════════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════════════════
     ⚠️ המוטציה רצה על **עותק בזיכרון** ולא על הקובץ שבעץ —
