...
const APP = {
  ...
  dragHandles: [],
  dragWhy: 'אין כאן רשימה שסדרה נקבע ביד — ⚠️ המצבה ממוינת במיון האחד, ⭐ והנוכחות נקבעת בסימון: ⛔ ואין מסלול גרירה למדוד',
  visualAllow: {},
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
 ...
 *  רשימה שנייה בבודק. */
export const ROWS = [90, 108, 109, 83, 105, 212, 119, 89];
 ...
 *  תמיד, ⛔ גם כשאין ריפו לצידנו: ⚠️ והמוטציה עורכת את המיכל בזיכרון. */
{
  ...
  /*  ⛔ מ37–מ40 — מיכל הדיאלוג: ⚠️ שם מחלקה · מיקום ה-`role` · קישור
   *  הכותרת · ומיכל שאינו במרשם. ⭐ והאחות סינתטית, ⛔ ולכן הן רצות תמיד. */
  const DM = [
    { m: 'מ37', lbl: 'שם מחלקה במיכל נבדל', a: '<div class="sheet ksave" role="dialog"',
      b: '<div class="modal-box ksave" role="dialog"' },
    { m: 'מ38', lbl: '`role` עבר למיכל החיצוני', a: '<div id="modal" class="veil">\n  <div class="sheet ksave" role="dialog" aria-modal="true" ',
      b: '<div id="modal" class="veil" role="dialog" aria-modal="true">\n  <div class="sheet ksave" ' },
    { m: 'מ39', lbl: 'קישור הכותרת הוסר', a: ' aria-labelledby="modal-title"', b: '' },
  ];
  for (const r of DM) {
    const mutated = IDX.replace(r.a, () => r.b);
  ...
  }
  {
    ...
    t(dlg.indexOf('zz-dlg') >= 0 && !('zz-dlg' in (APP.sharedSurfaces || {})),
      'מ40 · מיכל דיאלוג שאינו במרשם — [dialog-surface] הייתה נכשלת');
  }
  ...
  t(surfGaps(injCss('#modal .sheet{z-index:var(--z-5)}'), { 'zz-sister': IDX }).off.length === 0,
    'נ12 · ⭐ דרגת שכבה שנבדלת ⛔ **אינה** מפילה את [shared-surface]');
  t(surfGaps(injCss('#ask .sheet{max-width:340px}'), { 'zz-sister': IDX }).off.length === 0,
    'נ13 · ⭐ רוחב מרבי שנבדל ⛔ **אינו** מפיל את [shared-surface]');
