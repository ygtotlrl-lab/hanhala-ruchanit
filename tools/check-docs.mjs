 ...
 *
 *  ⭐ משימת הבודק: סורק את כל קובצי התיעוד ואת בלוקי ה-SHARED שבהם
 *  — ⛔ ומפיל על סחיפה בתוכן המשותף, על שלד חסר ועל חציית תקציב שורות.
 *
 *  ⭐ **פרוטוקול תיעוד קבוע.** סבב 17 הוכיח שרכיב
 *  שמוכרז «משותף» נסחף בלי אכיפה — וסבב 18 מדד שאותו דבר בדיוק קרה
 *  לתיעוד עצמו: פרק «בדיקה» של סבב 14 הועתק כלשונו לכל הריפו, כולל
 ...
 *
 *  סימונים שבתוך גדר קוד (```) אינם נספרים, אחרת הדוגמה שבתוך פרק הכללים
 *  עצמו הייתה נקראת כבלוק אמיתי.
 *
 *  החתימות זהות בכל הריפו. שינוי מכוון בפרק משותף = עדכון הפרק
...
export const ROWS = [2, 3, 4, 6, 7, 9, 10, 11, 12, 13, 138, 220];

/* הרשימה הקנונית — מזהה ← חתימת sha256 (16 תווים) של תוכן הבלוק, מקוצץ. */
const CANON = [
  ['table', '7fbd1ab28d356f8c'],
];
...
for (const [id, sha] of CANON) {
  ...
  if (got !== sha) {
    fail(`בלוק "${id}" (שורות ${b.from}–${b.to}): אינו זהה לחתימה הקנונית — ${got} ` +
         `במקום ${sha}. שינוי בפרק משותף חייב להיעשות בכל האפליקציות ` +
         `ובכל עותקי הבדיקה, באותו סבב. מיישרים את הבלוק, או מעדכנים את החתימה`);
  } else {
 ...
 *  מול `origin/main`, ואם התוכן שונה אבל השורה זהה — זו הפרה.
    ...
    if (base.status !== 0) {
      ...
      if (there === null) {
        ...
        fail('CLAUDE.md שונה ושורת העדכון לא קודמה — ' +
             `«${there}» זהה לזו שב-origin/main. כל סבב ` +
 ...
 *  נסחפו בדיוק כך: ל-CONTEXT של יומן חסרה הייתה אזהרת
 ...
 *  מעצם טבעו, וכפייה של נוסח אחיד היא בדיוק מה שאסור.
...
const MD_SKELETONS = [
   ...
   *  נכסים מוטבעים» ו«המפתח הקבוע»: ⚠️ כולם הסבירו כלל שהטבלה אוכפת,
...
for (const spec of MD_SKELETONS) {
  if (!fs.existsSync(spec.file)) { fail(`${spec.file} חסר — נמדדו 0 עותקים והצפוי אחד. מוסיפים אותו ` +
      `מריפו אחות: שלושת קובצי ה-md הנלווים חובה בכל הריפו`); continue; }
  ...
  for (const [re, label] of spec.need) {
    ...
    if (found < 0) {
      ...
      fail(`${spec.file}: הפרק «${label}» ` +
        ...
        'שלד הקובץ נקבע בסבב 39 והוא זהה בכל הריפו');
    } else at = found + 1;
  }
 ...
 *  ⛔ שלד שעובר על פרק חסר-תוכן נותן בדיוק את הביטחון השווא
 *  שהשער הזה בא למנוע.
 *
 ...
 *  הסימון לפסקה שאינה זהה בכולן בפועל — זו בדיוק אותה סחיפה,
 *  בציר אחר.                                                             */
...
const CANON_MD = [
  ...
  ['android/README.md',  'android-shell-split',   '7eb52b8d08315056'],
  ['android/README.md',  'android-smali-scope',   '44eed6fab6198c4e'],
...

{
  ...
  for (const [file, want] of byFile) {
    ...
    if (extra.length) fail(`${file}: בלוקי SHARED שאינם ברשימה הקנונית — ${extra.join(', ')} ` +
        ...
        'לרשימה בכל עותקי הבדיקה, או מסירים אותן');
    ...
    for (const [id, sha] of want) {
      ...
      if (got !== sha) {
        ...
        fail(`${file} — פסקה "${id}" (שורות ${b.from}–${b.to}): אינה זהה לחתימה ` +
             `הקנונית — ${got} במקום ${sha}. שינוי בפסקה משותפת נעשה בכל ` +
             'הריפו ובכל עותקי הבדיקה, באותו סבב. מיישרים את הפסקה, ' +
 ...
 *  ⚠️ ערך יחסי כמו `./` נפתר לכתובת מוחלטת, ⭐ ובכולן זה אותו origin
 *  בדיוק: הדפדפן ראה בהן אפליקציה **אחת**, והתקנה של אחת החליפה את
 *  האחרת במסך הבית. ⛔ והרישום מחזיק את כולן ⛔ ולא את המקומית בלבד —
 *  ⚠️ שער שרואה ערך אחד אינו יכול למדוד ייחודיות. */
const CANON_APP_ID = APP_SCOPES;
{
  ...
  if (!fs.existsSync(file)) {
    ...
    if (mf) {
      ...
      for (const [key, want] of CANON_MANIFEST) {
        if (!(key in mf)) {
          ...
          fail(`manifest.json: "${key}" הוא «${mf[key]}» במקום «${want}». ` +
               'מיישרים את הערך — ערך משותף משתנה בכל הריפו ובכל ' +
       ...
       *  `icon-maskable-512.png` — שקיים בכולן מסבב 46 — לא הופיע
...
/* ────── ט. תקציב התיעוד — חלון של שני סבבים ותקרה של 700 שורות (סבב 71) ────
   ...
   סבב אינו מוחק אחד. שני תנאים: **חלון של שני פרקי
...
/*  ⛔ תקרת החלק המשותף (סבב 69) — ⚠️ בלעדיה בלוקי הכללים גדלים
 *  בלי גבול, והתקרה הכוללת נבלעת בהם.
 *  ⛔ **הסף בשער 403** — ⚠️ שלוש שורות מרווח למוטציית-הנגד: ⭐ התקרה
 *  לכללים היא **400**, ⛔ ומוטציית-הנגד שמוכיחה שהשער מודד סף ואינו
 *  נופל על כל תוספת מוסיפה שלוש שורות משותפות ⛔ ואסור לה להפיל.
 *  ⚠️ סף 400 היה הופך את 400 עצמו לבלתי-ניתן לכתיבה, ⭐ ושני מספרים
 *  מוצהרים עדיפים על תקרה שנגזמת בשקט ל-397. */
const DOC_MAX_SHARED = 403;
 ...
 *  הוא בדיוק מה שיש (12 · 13 · 85 שורות, זהות בית-לבית בכולן), והחלק
 *  הפרטי הוא הגבוה שנמדד בכולן ועוד מרווח קטן.
...
const DOC_MAX_PRIVATE = 300;
{
  ...
  if (priv > DOC_MAX_PRIVATE) {
    fail(`${APP.file}: החלק הפרטי-הקבוע נמדד ${priv} שורות — ${priv - DOC_MAX_PRIVATE} ` +
         `מעל התקרה של ${DOC_MAX_PRIVATE}. גוזמים באותו קומיט, ` +
         'ולקח שראוי להישמר עולה לטבלה או להערה בקוד לפני המחיקה.');
  } else {
   ...
   הקובץ בפועל. */
{
  ...
  /*  ⛔ בחלק המשותף אין כותרת `##` כלל (סבב 71) — ⚠️ הבלוקים נפתחים
...
/* ────── יב. סדר הבלוקים (סבב 71; ⛔ פרקי הכללים ירדו בסבב 96) ───────────────
   ⛔ **סדר הבלוקים נאכף מול הרשימה הקנונית** — ⚠️ הרשימה זהה בכל
   עותקי השער, ⛔ ולכן סדר שנשמר מולה הוא סדר אחיד בכל הריפו: זו
   הדרך היחידה לאכוף אחידות בין ריפו מתוך ריפו אחד.
   ⛔ **ואין כאן יותר «מבנה פרק הכלל»** — ⚠️ אין פרקי כללים: ⭐ הטבלה היא
   ההוראה, ⛔ וניסוחה נמדד ב-`sha256` של הבלוק היחיד. */
{
  ...
  /*  ⛔ **ואין `###` בתוך הבלוק המשותף** (סבב 96) — ⚠️ הבלוק מחזיק את
   *  הטבלה בלבד: ⭐ פרק שיוחזר לתוכו הוא מקור אמת שני להוראה, ⛔ ופרוזה
   *  משותפת שאיש אינו מודד חוזרת תוך סבבים ספורים. */
  let stray = 0, open = null;
  for (let i = 0; i < lines.length; i++) {
    if (inFence[i]) continue;
    const st = START_RE.exec(lines[i]);
    if (st) { open = st[1]; continue; }
    if (END_RE.test(lines[i])) { open = null; continue; }
    if (open && /^###\s+\S/.test(lines[i])) stray++;
  }
  if (stray) fail(`${APP.file}: נמדדו ${stray} פרקי `+'`###`'+` בתוך הבלוק המשותף והצפוי אפס — ` +
    `הבלוק מחזיק את הטבלה בלבד. מעבירים את הפרק לחלק הפרטי, או לשורה בטבלה`);
  else pass(`הבלוק המשותף — אין בו פרק `+'`###`'+`, והטבלה היא ההוראה היחידה שבו`);
}
