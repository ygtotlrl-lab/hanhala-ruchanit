...
const APP = {
  ...
  gapRows: [46, 127, 131, 155, 63, 140, 100],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    176: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    175: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    138: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    172: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '92|חותמת רעננות נכשלת סגור': 'src',
    '83|`Enter` שומר בכל שדה עריכה': 'src',
    '84|מאזין אחד, בהאצלה מ-`document`': 'src',
    '85|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '172|מבנה טופס הכניסה': 'src',
    '140|שם נגזר אינו נשמר ברשומה': 'src',
    '190|קבוע בלי קורא': 'mixed',
    '192|מפתח אחסון בלי קורא': 'src',
    '38|שער מריץ את כל טענותיו': 'tools',
    '91|שכבת המראה': 'src',
    '193|הגירה מקומית שהושלמה': 'mixed',
    '174|מראת המשתמשים': 'src',
    '173|מודל ההרשאות': 'src',
    '25|שער סורק קוד מולבן': 'tools',
    '25|ההלבנה שומרת על מספרי השורות': 'mixed',
    '156|הקשר נלכד בכניסה לפונקציה': 'src',
    '95|פריסה במסכי טלפון וטאבלט': 'src',
    '96|ערכת נושא — בהיר וכהה': 'src',
    '97|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '98|אין סגנון מוטבע': 'src',
    '100|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '186|הערה שמתארת מצב שחלף': 'mixed',
    '194|שאילתת `@media` מתה': 'src',
    '114|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '115|ייצוא והנפקה — דרך מוצהרת': 'src',
    '101|המסמך מתפרסר נקי': 'src',
    '102|לוג מסלול — תג ורמה': 'src',
    '79|מיכל המודאל ילד ישיר של `body`': 'src',
    '110|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '158|הגיבוי שלם ומעומד': 'src',
    '75|פעולה מגיבה מיד': 'src',
    '76|משיכה מסוננת בשרת': 'src',
    '126|דגימת-יתר במחולל': 'tools',
    '176|`PBKDF2` — פרמטרים': 'src',
    '144|`migrations/` — תוכן': 'src',
    '145|`migrations/` — כמות': 'src',
    '165|מיזוג מכל — מפה ומערך': 'src',
    '166|ברירת מחדל אינה ממוזגת': 'src',
    '167|רשימה אינה נושאת פריט כפול': 'src',
    '159|`sh_backup` — טבלת הגיבוי': 'src',
    '23|שער אינו מקליד מספר שורה': 'tools',
    '189|פונקציה בלי קוראים': 'mixed',
    '29|שער ממתין לתנאי ולא לשעון': 'tools',
    '180|סודות אינם עוזבים את המכשיר': 'src',
    '153|דפוס `upsert` — onConflict מוכרז': 'src',
    '178|טביעה חסרה מושלמת אוטומטית': 'src',
    '178|כניסה אופליין': 'src',
    '137|עריכת נתונים אופליין': 'src',
    '163|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '179|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '73|אובייקט יקר נבנה פעם אחת': 'src',
    '74|מסך מציג מיד ומרענן ברקע': 'src',
    '130|האפליקציה ניתנת להתקנה מכרום': 'src',
    '184|דגלי מעבר — אין דגל דלוק': 'src',
    '80|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '175|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '46|ארבעת מימדי הסריקה': 'src',
    '90|פינוי אוטומטי': 'src',
    '90|אופק הפינוי נוקה מהסנכרון': 'src',
    '169|אימות פינוי מול הענן': 'src',
    '131|שיתוף קבצים': 'src',
    ...
    '59|יומן פעולות': 'src',
    '138|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '81|דפוס הודעת שגיאה יחיד': 'src',
    '86|`toast` — חתימה, גוף ומחלקות': 'src',
    '86|סיווג ההודעה נגזר מהמסלול': 'src',
    '201|מיכל באנר העדכון במקור': 'src',
    '201|הרענון מ-controllerchange בלבד': 'src',
    '201|סימן דחיית הבאנר מתמיד': 'src',
    '111|רישום כשלי כתיבה': 'src',
    '60|חלון חם במכשיר': 'src',
    '172|מסך שינוי סיסמה עצמי': 'src',
    '203|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '61|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '178|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '62|מודול מזהי רשומות': 'src',
    '62|מזהה אינו מומר למספר': 'src',
    '64|מזהה מכשיר': 'src',
    '61|ליבת מיזוג משותפת': 'src',
    '116|מעטפת WebView חתומה': 'tools',
    '178|אימות מול טביעה בענן': 'mixed',
    '133|בניית APK אחידה עם שער חתימה': 'src',
    '203|מודול ה-service worker': 'src',
    '65|ניסיון חוזר בתור הסנכרון': 'src',
    '66|מנגנון משיכה אחיד': 'src',
    '69|נעילת חוסר-פעילות': 'src',
    '70|מודל הסשן — בזיכרון בלבד': 'src',
    '202|בדיקת עדכון תקופתית ל-service worker': 'src',
    '93|פסק זמן אחיד לקריאות רשת': 'src',
    '94|מאזיני מצב רשת': 'src',
    '162|גריעת tombstones לפי גיל': 'src',
    '201|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '131|גשר שיתוף': 'src',
    '71|העברת מזהה ל-DOM': 'src',
    '72|מיון אחד לכל התצוגות': 'src',
    '77|ייצוא — שני דפוסים מוצהרים': 'src',
    '123|שכבת אייקונים': 'src',
    '170|שכבת קלט אחידה': 'src',
    '170|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '78|שכבת המודאל': 'src',
    '172|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [31, 45, 47, 49, 55, 89, 53, 168, 107, 147, 58, 24, 63, 27, 36, 40];
...
const MATRIX = [
  ...
  { row: 92, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 85, name: 'כפתור שכותב מושבת בזמן הכתיבה',
    probe: () => busyGuardGaps().length === 0 },
  { row: 84, name: 'מאזין אחד, בהאצלה מ-`document`',
  ...
  { row: 83, name: '`Enter` שומר בכל שדה עריכה',
  ...
  { row: 140, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 156, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 95, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 97, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 100, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 96, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 186, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 102, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 79, name: 'מיכל המודאל ילד ישיר של `body`',
  ...
  { row: 110, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 158, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 155, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 75, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 76, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 126, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 176, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 144, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 145, name: '`migrations/` — כמות',
  ...
  { row: 165, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 166, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 167, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 180, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 153, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 178, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 178, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 137, name: 'עריכת נתונים אופליין',
  ...
  { row: 163, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 179, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 73, name: 'אובייקט יקר נבנה פעם אחת',
  ...
  { row: 74, name: 'מסך מציג מיד ומרענן ברקע',
  ...
  { row: 130, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 184, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 80, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 175, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 90, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 169, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 131, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 116, name: 'מעטפת APK (WebView)',
  ...
  { row: 59, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 59, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 138, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 81, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 86, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 86, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 111, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 60, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 172, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 172, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 203, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 59, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 157, name: 'פינוי גיבויים אוטומטי במסד',
   ...
   *  ⛔ ולא `NaN`. */
  { row: 62, name: 'מזהה אינו מומר למספר',
  ...
  { row: 116, name: 'מעטפת WebView חתומה',
  ...
  { row: 178, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 133, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 203, name: 'מודול ה-service worker',
  ...
  { row: 65, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 70, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 202, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 190, name: 'קבוע בלי קורא',
    probe: () => constNoReaderGaps().length === 0 },
  { row: 192, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 91, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 193, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 93, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 162, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 201, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 201, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 201, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 201, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 72, name: 'מיון אחד לכל התצוגות',
  ...
  { row: 77, name: 'ייצוא — שני דפוסים מוצהרים',
    probe: () => exportMarkupSites().length === 0 },
  { row: 123, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 170, name: 'שכבת קלט אחידה',
  ...
  { row: 170, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  56: { claims: { test_toolsid: ['[tool-uncovered]', '[gate-run]', '[gate-count]'] } },
  ...
  181: { claim: 'אין `service_role` בקובץ' },
  33: { claim: 'מסלול «' },
  ...
  67:  { claim: 'שכבת הדחיפה' },
  /*  ⛔ שלוש טענות באותו שער (סבב 131) — ⚠️ המשפך, הודעת החסימה שהוא
   *  מחזיר, והקבוע שאתרי האימות מפנים אליו: ⭐ שלושתן על אותו מסלול. */
  183: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  68: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  146: { claim: 'בעלות הסכימה המשותפת' },
  160: { claim: '`tb_kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  161: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  195: { claim: 'ז. עמודה בלי קורא' },
  87: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  121: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  124: { claim: 'margin' },
  ...
  188: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  99: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  ...
  89: { claim: 'storage' },
  88: { claim: 'ג · פעולה שדורשת רשת' },
  104: { claim: 'עברית' },
  107: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  108: { claim: 'BANNER_W' },
  191: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  105: { claim: 'מפנה לקובץ' },
  120: { claim: 'android/app/src/main' },
  127: { claim: 'fgDriftMax' },
  128: { claim: 'CANON_MANIFEST' },
  129: { claim: 'שדות זהים' },
  132: { claim: 'BUILD_SHA' },
  136: { claim: 'SHARED_SHA' },
  134: { claim: 'versionCode' },
  117: { claim: 'WebView' },
  118: { claim: 'android:theme' },
  122: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  139: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  143: { claim: 'migrations' },
  152: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  154: { claim: 'א. חותמת בכל רשומה' },
  58: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  147: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  148: { claim: 'ו. טבלה מקבילה' },
  149: { claim: 'ב. חתימת סכימה' },
  150: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  171: { claim: 'type=password' },
  82: { claim: 'aria-label' },
  ...
  198: { claim: 'ח. הגדרה שהוחלפה' },
  197: { claim: 'ד. רשימת-היתר' },
  199: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  103: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  112: { claim: 'CLEANUP_SHA' },
  113: { claim: 'בלוק ה-APP' },
  168: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
