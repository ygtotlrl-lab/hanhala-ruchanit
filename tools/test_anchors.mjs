#!/usr/bin/env node
/* ───────────────────────────────────────────────────────────────────────────
   test_anchors.mjs — כל טענה נמדדת בתוך ההיקף שלה
...
const MUT = [
  ...
  /*  ⛔ מ18 · מ19 — מסך שמתאר תהליך שמחוץ לקוד: ⚠️ **מה נכנס**:
 ...
 *  **טקסט קבוע**, ⛔ ולא הצורה הדו-ממדית: ⭐ מה שנגזר נמדד מול מקורו. */
{
  ...
  /*  ⛔⛔ מ81 · מ82 · מ83 · נ52 — התווית מתארת את מה שקורה:
   ...
   *  שהועתקה לטופס אחר כדי לאחד — ⚠️ **והאיחוד הפך אותה לשקר**. */
  {
    ...
    if (!num) {
      ...
      t(runGateOn({ 'index.html': swap('תאריך') },
                  'test_caps_ui.mjs'),
        'מ81 · תווית «תאריך» על שדה מספרי **מפילה** את «הערה מכילה נימוק, ולא מצביע»');
    ...
    if (!dt) t(true, 'מ82 · ⭕ אין כאן שדה `type="date"` — ⛔ ואין מה למוטט');
      ...
      t(runGateOn({ 'index.html':
                  ...
                  'test_caps_ui.mjs'),
        'מ82 · `type="date"` שהוחלף בטקסט **מפיל** את «הערה מכילה נימוק, ולא מצביע»');
    ...
    if (!bt) t(true, 'מ83 · ⭕ אין כאן כפתור מחיקה במקור — ⛔ ואין מה למוטט');
      ...
      t(runGateOn({ 'index.html': idx.replace(bt[0], bt[1] + 'שמור' + bt[3]) },
                  'test_caps_ui.mjs'),
        'מ83 · תווית «שמור» על כפתור מחיקה **מפילה** את «הערה מכילה נימוק, ולא מצביע»');
  }
