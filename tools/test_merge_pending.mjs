...
const APP = {
  ...
  dataEra: { base: 1, era: 2,
    why: 'שם הטבלה ומפתח האחסון הוסבו לתחילית שנגזרת משם הריפו — ⚠️ והמראה ממופתחת בשם: ⭐ ההגירה המקומית מכסה מכשיר שעלה, ⛔ והעידן מכסה מכשיר שההגירה נפלה בו באמצע' },
  ...
  // ⭐ כלל ההכרעה עבר לליבה המשותפת, ולכן גם המוטציה מכוונת
  ...
  /*  ⭐ שכבת ליבת המיזוג — ⚠️ השמות, המעטפת והרשומה נבדלים
  ...
  core: {
    ...
    //    לפי הסדר. ⛔ אין ליישר את זה ל-yoman «לשם אחידות».
...
function harness(src, pending) {
  ...
  // ⚠️ פונקציית הכניסה האופליין נחתכת גם היא — הצהרת פונקציה אינה
...
console.log('· ' + FACTS.slug + ' — הגנת ⏳ במנוע המיזוג');
...
/* ══ ליבת המיזוג המשותפת ═════════════════════════════════════════════════ */
/*  ⛔ סביבה נפרדת משל הגנת ה-⏳ — ⚠️ רשימת השמות והמעטפת
...
function coreBuild(src) {
  ...
  /*  ⛔ ההצהרות קודמות לפונקציות — ⚠️ מודול גריעת ה-tombstones
...
/* ── 4 · הבלוק המשותף ──────────────────────────────────────────────────── */
assert(SRC.indexOf('/* ═══ מיזוג רשומות — מודול משותף') !== -1,
...
console.log(failed ? `\n✗ מנוע המיזוג — ${failed} טענות נכשלו`
                   : '\n✓ מנוע המיזוג — ליבה והגנת ⏳ — כל הטענות עברו');
process.exit(failed ? 1 : 0);
