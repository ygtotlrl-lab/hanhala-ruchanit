...
const APP = {
  ...
  gapRows: [214, 48, 135, 139, 163, 68, 148, 106],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    185: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    184: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    146: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    181: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '181|מבנה טופס הכניסה': 'src',
    '148|שם נגזר אינו נשמר ברשומה': 'src',
    '202|קבוע בלי קורא': 'mixed',
    '204|מפתח אחסון בלי קורא': 'src',
    '39|שער מריץ את כל טענותיו': 'tools',
    '97|שכבת המראה': 'src',
    '205|הגירה מקומית שהושלמה': 'mixed',
    '183|מראת המשתמשים': 'src',
    '182|מודל ההרשאות': 'src',
    '26|שער סורק קוד מולבן': 'tools',
    '26|ההלבנה שומרת על מספרי השורות': 'mixed',
    '164|הקשר נלכד בכניסה לפונקציה': 'src',
    ...
    '197|הערה שמתארת מצב שחלף': 'mixed',
    '206|שאילתת `@media` מתה': 'src',
    '121|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '122|ייצוא והנפקה — דרך מוצהרת': 'src',
    '107|המסמך מתפרסר נקי': 'src',
    '108|לוג מסלול — תג ורמה': 'src',
    '84|מיכל המודאל ילד ישיר של `body`': 'src',
    '117|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '166|הגיבוי שלם ומעומד': 'src',
    '80|פעולה מגיבה מיד': 'src',
    '81|משיכה מסוננת בשרת': 'src',
    '134|דגימת-יתר במחולל': 'tools',
    '185|`PBKDF2` — פרמטרים': 'src',
    '152|`migrations/` — תוכן': 'src',
    '153|`migrations/` — כמות': 'src',
    '174|מיזוג מכל — מפה ומערך': 'src',
    '175|ברירת מחדל אינה ממוזגת': 'src',
    '176|רשימה אינה נושאת פריט כפול': 'src',
    '167|`sh_backup` — טבלת הגיבוי': 'src',
    '24|שער אינו מקליד מספר שורה': 'tools',
    '201|פונקציה בלי קוראים': 'mixed',
    '30|שער ממתין לתנאי ולא לשעון': 'tools',
    '189|סודות אינם עוזבים את המכשיר': 'src',
    '161|דפוס `upsert` — onConflict מוכרז': 'src',
    '187|טביעה חסרה מושלמת אוטומטית': 'src',
    '187|כניסה אופליין': 'src',
    '145|עריכת נתונים אופליין': 'src',
    '171|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '188|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '78|אובייקט יקר נבנה פעם אחת': 'src',
    '79|מסך מציג מיד ומרענן ברקע': 'src',
    '138|האפליקציה ניתנת להתקנה מכרום': 'src',
    '194|דגלי מעבר — אין דגל דלוק': 'src',
    '85|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '184|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '48|ארבעת מימדי הסריקה': 'src',
    '96|פינוי אוטומטי': 'src',
    '96|אופק הפינוי נוקה מהסנכרון': 'src',
    '178|אימות פינוי מול הענן': 'src',
    '139|שיתוף קבצים': 'src',
    '123|מעטפת APK (WebView)': 'src',
    '126|אין נכסים מוטבעים': 'src',
    '143|מפתח חתימה קבוע': 'src',
    '149|מקור אמת יחיד לסכימה': 'src',
    '150|קובץ התקנה מלא': 'src',
    '64|גיבוי יומי אוטומטי': 'src',
    '64|יומן פעולות': 'src',
    '146|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '86|דפוס הודעת שגיאה יחיד': 'src',
    '92|`toast` — חתימה, גוף ומחלקות': 'src',
    '92|סיווג ההודעה נגזר מהמסלול': 'src',
    '213|מיכל באנר העדכון במקור': 'src',
    '213|הרענון מ-controllerchange בלבד': 'src',
    '213|סימן דחיית הבאנר מתמיד': 'src',
    '118|רישום כשלי כתיבה': 'src',
    '65|חלון חם במכשיר': 'src',
    '181|מסך שינוי סיסמה עצמי': 'src',
    '215|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '66|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '187|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '67|מודול מזהי רשומות': 'src',
    '67|מזהה אינו מומר למספר': 'src',
    '69|מזהה מכשיר': 'src',
    '66|ליבת מיזוג משותפת': 'src',
    '123|מעטפת WebView חתומה': 'tools',
    '187|אימות מול טביעה בענן': 'mixed',
    '141|בניית APK אחידה עם שער חתימה': 'src',
    '215|מודול ה-service worker': 'src',
    '70|ניסיון חוזר בתור הסנכרון': 'src',
    '71|מנגנון משיכה אחיד': 'src',
    '74|נעילת חוסר-פעילות': 'src',
    '75|מודל הסשן — בזיכרון בלבד': 'src',
    '214|בדיקת עדכון תקופתית ל-service worker': 'src',
    '99|פסק זמן אחיד לקריאות רשת': 'src',
    '100|מאזיני מצב רשת': 'src',
    '170|גריעת tombstones לפי גיל': 'src',
    '213|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '139|גשר שיתוף': 'src',
    '76|העברת מזהה ל-DOM': 'src',
    '77|מיון אחד לכל התצוגות': 'src',
    '82|ייצוא — שני דפוסים מוצהרים': 'src',
    '131|שכבת אייקונים': 'src',
    '179|שכבת קלט אחידה': 'src',
    '179|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '83|שכבת המודאל': 'src',
    '181|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [32, 47, 49, 51, 60, 95, 58, 177, 113, 155, 63, 25, 68, 28, 37, 41];
...
const MATRIX = [
  ...
  { row: 148, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 164, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 197, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 117, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 166, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 163, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 134, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 185, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 152, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 153, name: '`migrations/` — כמות',
  ...
  { row: 174, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 175, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 176, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 189, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 161, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 187, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 187, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 145, name: 'עריכת נתונים אופליין',
  ...
  { row: 171, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 188, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 138, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 194, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 184, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 178, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 139, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 123, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 126, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 143, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 149, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 150, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 64, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 64, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 146, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 181, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 181, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 215, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 165, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 187, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 123, name: 'מעטפת WebView חתומה',
  ...
  { row: 187, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 141, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 215, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 214, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 170, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 213, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 213, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 213, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 213, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 131, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 179, name: 'שכבת קלט אחידה',
  ...
  { row: 179, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  196: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  190: { claim: 'אין `service_role` בקובץ' },
  34: { claim: 'מסלול «' },
  ...
  193: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  73: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  154: { claim: 'בעלות הסכימה המשותפת' },
  168: { claim: '`tb_kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  169: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  207: { claim: 'ז. עמודה בלי קורא' },
  ...
  129: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  132: { claim: 'margin' },
  ...
  116: { claims: { test_codescan: ['[fn-product]'] },
  ...
  200: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  114: { claim: 'BANNER_W' },
  203: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  128: { claim: 'android/app/src/main' },
  135: { claim: 'fgDriftMax' },
  136: { claim: 'CANON_MANIFEST' },
  137: { claim: 'שדות זהים' },
  140: { claim: 'BUILD_SHA' },
  144: { claim: 'SHARED_SHA' },
  142: { claim: 'versionCode' },
  124: { claim: 'WebView' },
  125: { claim: 'android:theme' },
  130: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  147: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  151: { claim: 'migrations' },
  160: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  162: { claim: 'א. חותמת בכל רשומה' },
  63: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  155: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  156: { claim: 'ו. טבלה מקבילה' },
  157: { claim: 'ב. חתימת סכימה' },
  158: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  180: { claim: 'type=password' },
  87: { claim: 'aria-label' },
  ...
  191: { claim: '[sec-keyfile]' },
  195: { claim: '⏳' },
  199: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  208: { claim: 'ט. ערך נגזר שהתיישן' },
  210: { claim: 'ח. הגדרה שהוחלפה' },
  209: { claim: 'ד. רשימת-היתר' },
  211: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  177: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
