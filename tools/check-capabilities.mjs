...
const APP = {
  ...
  gapRows: [47, 129, 133, 157, 64, 142, 101],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    178: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    177: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    140: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    174: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '93|חותמת רעננות נכשלת סגור': 'src',
    '84|`Enter` שומר בכל שדה עריכה': 'src',
    '85|מאזין אחד, בהאצלה מ-`document`': 'src',
    '86|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '174|מבנה טופס הכניסה': 'src',
    '142|שם נגזר אינו נשמר ברשומה': 'src',
    '192|קבוע בלי קורא': 'mixed',
    '194|מפתח אחסון בלי קורא': 'src',
    '38|שער מריץ את כל טענותיו': 'tools',
    '92|שכבת המראה': 'src',
    '195|הגירה מקומית שהושלמה': 'mixed',
    '176|מראת המשתמשים': 'src',
    '175|מודל ההרשאות': 'src',
    '25|שער סורק קוד מולבן': 'tools',
    '25|ההלבנה שומרת על מספרי השורות': 'mixed',
    '158|הקשר נלכד בכניסה לפונקציה': 'src',
    '96|פריסה במסכי טלפון וטאבלט': 'src',
    '97|ערכת נושא — בהיר וכהה': 'src',
    '98|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '99|אין סגנון מוטבע': 'src',
    '101|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '188|הערה שמתארת מצב שחלף': 'mixed',
    '196|שאילתת `@media` מתה': 'src',
    '116|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '117|ייצוא והנפקה — דרך מוצהרת': 'src',
    '102|המסמך מתפרסר נקי': 'src',
    '103|לוג מסלול — תג ורמה': 'src',
    '80|מיכל המודאל ילד ישיר של `body`': 'src',
    '112|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '160|הגיבוי שלם ומעומד': 'src',
    '76|פעולה מגיבה מיד': 'src',
    '77|משיכה מסוננת בשרת': 'src',
    '128|דגימת-יתר במחולל': 'tools',
    '178|`PBKDF2` — פרמטרים': 'src',
    '146|`migrations/` — תוכן': 'src',
    '147|`migrations/` — כמות': 'src',
    '167|מיזוג מכל — מפה ומערך': 'src',
    '168|ברירת מחדל אינה ממוזגת': 'src',
    '169|רשימה אינה נושאת פריט כפול': 'src',
    '161|`sh_backup` — טבלת הגיבוי': 'src',
    '23|שער אינו מקליד מספר שורה': 'tools',
    '191|פונקציה בלי קוראים': 'mixed',
    '29|שער ממתין לתנאי ולא לשעון': 'tools',
    '182|סודות אינם עוזבים את המכשיר': 'src',
    '155|דפוס `upsert` — onConflict מוכרז': 'src',
    '180|טביעה חסרה מושלמת אוטומטית': 'src',
    '180|כניסה אופליין': 'src',
    '139|עריכת נתונים אופליין': 'src',
    '165|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '181|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '74|אובייקט יקר נבנה פעם אחת': 'src',
    '75|מסך מציג מיד ומרענן ברקע': 'src',
    '132|האפליקציה ניתנת להתקנה מכרום': 'src',
    '186|דגלי מעבר — אין דגל דלוק': 'src',
    '81|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '177|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '47|ארבעת מימדי הסריקה': 'src',
    '91|פינוי אוטומטי': 'src',
    '91|אופק הפינוי נוקה מהסנכרון': 'src',
    '171|אימות פינוי מול הענן': 'src',
    '133|שיתוף קבצים': 'src',
    '118|מעטפת APK (WebView)': 'src',
    '121|אין נכסים מוטבעים': 'src',
    '137|מפתח חתימה קבוע': 'src',
    '143|מקור אמת יחיד לסכימה': 'src',
    '144|קובץ התקנה מלא': 'src',
    '60|גיבוי יומי אוטומטי': 'src',
    '60|יומן פעולות': 'src',
    '140|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '82|דפוס הודעת שגיאה יחיד': 'src',
    '87|`toast` — חתימה, גוף ומחלקות': 'src',
    '87|סיווג ההודעה נגזר מהמסלול': 'src',
    '203|מיכל באנר העדכון במקור': 'src',
    '203|הרענון מ-controllerchange בלבד': 'src',
    '203|סימן דחיית הבאנר מתמיד': 'src',
    '113|רישום כשלי כתיבה': 'src',
    '61|חלון חם במכשיר': 'src',
    '174|מסך שינוי סיסמה עצמי': 'src',
    '205|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '62|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '180|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '63|מודול מזהי רשומות': 'src',
    '63|מזהה אינו מומר למספר': 'src',
    '65|מזהה מכשיר': 'src',
    '62|ליבת מיזוג משותפת': 'src',
    '118|מעטפת WebView חתומה': 'tools',
    '180|אימות מול טביעה בענן': 'mixed',
    '135|בניית APK אחידה עם שער חתימה': 'src',
    '205|מודול ה-service worker': 'src',
    '66|ניסיון חוזר בתור הסנכרון': 'src',
    '67|מנגנון משיכה אחיד': 'src',
    '70|נעילת חוסר-פעילות': 'src',
    '71|מודל הסשן — בזיכרון בלבד': 'src',
    '204|בדיקת עדכון תקופתית ל-service worker': 'src',
    '94|פסק זמן אחיד לקריאות רשת': 'src',
    '95|מאזיני מצב רשת': 'src',
    '164|גריעת tombstones לפי גיל': 'src',
    '203|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '133|גשר שיתוף': 'src',
    '72|העברת מזהה ל-DOM': 'src',
    '73|מיון אחד לכל התצוגות': 'src',
    '78|ייצוא — שני דפוסים מוצהרים': 'src',
    '125|שכבת אייקונים': 'src',
    '172|שכבת קלט אחידה': 'src',
    '172|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '79|שכבת המודאל': 'src',
    '174|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [31, 46, 48, 50, 56, 90, 54, 170, 108, 149, 59, 24, 64, 27, 36, 40];
...
const CAPS = {
  ...
  mergecore: {
    ...
    block: { sha: '182161a5d0eab10b', lines: 76,
  ...
  tomb: {
    ...
    docRows: ['`tombstones`'],
    block: { sha: 'a190e13c0fbe948e', lines: 65,
  ...
  ids: {
    ...
    block: { sha: '4b9b00d46e4cf207', lines: 47,
  ...
  ctxguard: {
    ...
    block: { sha: '82022151c3b0cc20', lines: 24,
  ...
  hebdate: {
    ...
    block: { sha: '8657e838aad2c408', lines: 86,
  ...
  readnum: {
    ...
    docRows: ['שדה מספרי'],
    block: { sha: 'f8b233ccafc4d007', lines: 21,
  ...
  uniq: {
    ...
    block: { sha: '1315fa4b4cf7d0ce', lines: 28,
...
const SRC_READ = /readFileSync\([^)]*index\.html|rd\('index\.html'\)|readOnce\(APP\.file\)|readFileSync\([^)]*APP\.file/;
...
const MATRIX = [
  ...
  { row: 93, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 86, name: 'כפתור שכותב מושבת בזמן הכתיבה',
    probe: () => busyGuardGaps().length === 0 },
  { row: 85, name: 'מאזין אחד, בהאצלה מ-`document`',
  ...
  { row: 84, name: '`Enter` שומר בכל שדה עריכה',
  ...
  { row: 142, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 25, name: 'שער סורק קוד מולבן',
    probe: () => scanKindGaps().length === 0 },
  { row: 25, name: 'ההלבנה שומרת על מספרי השורות',
  ...
  { row: 158, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 96, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 98, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 101, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 97, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 188, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 103, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 80, name: 'מיכל המודאל ילד ישיר של `body`',
  ...
  { row: 112, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 160, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 157, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 76, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 77, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 128, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 178, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 146, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 147, name: '`migrations/` — כמות',
  ...
  { row: 167, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 168, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 169, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 161, name: '`sh_backup` — טבלת הגיבוי',
    probe: () => (/BK_TABLE\s*=\s*'([a-z_]+)'/.exec(srcRefs) || [])[1] === 'sh_backup' },
  { row: 23, name: 'שער אינו מקליד מספר שורה',
    probe: () => typedRowSites().length === 0 },
  { row: 191, name: 'פונקציה בלי קוראים',
  ...
  { row: 29, name: 'שער ממתין לתנאי ולא לשעון',
  ...
  { row: 182, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 155, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 180, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 180, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 139, name: 'עריכת נתונים אופליין',
  ...
  { row: 165, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 181, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 74, name: 'אובייקט יקר נבנה פעם אחת',
  ...
  { row: 75, name: 'מסך מציג מיד ומרענן ברקע',
  ...
  { row: 132, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 186, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 81, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 177, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 47, name: 'ארבעת מימדי הסריקה',
    probe: () => {
  ...
  { row: 91, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 171, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 133, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 118, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 121, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 137, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 143, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 144, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 60, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 60, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 140, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 82, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 87, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 87, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 113, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 61, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 174, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 174, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 205, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 60, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 159, name: 'פינוי גיבויים אוטומטי במסד',
   ...
   *  ⛔ ולא `NaN`. */
  { row: 63, name: 'מזהה אינו מומר למספר',
  ...
  { row: 118, name: 'מעטפת WebView חתומה',
  ...
  { row: 180, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 135, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 205, name: 'מודול ה-service worker',
  ...
  { row: 66, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 71, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 204, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 38, name: 'שער מריץ את כל טענותיו',
    probe: () => gateSealGaps().length === 0 },
  { row: 192, name: 'קבוע בלי קורא',
    probe: () => constNoReaderGaps().length === 0 },
  { row: 194, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 92, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 195, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 94, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 164, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 203, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 203, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 203, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 203, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 73, name: 'מיון אחד לכל התצוגות',
  ...
  { row: 78, name: 'ייצוא — שני דפוסים מוצהרים',
    probe: () => exportMarkupSites().length === 0 },
  { row: 125, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 172, name: 'שכבת קלט אחידה',
  ...
  { row: 172, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  190: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  18: { claim: '[no-deps]' },
  21: { claims: { test_filesets: 'appGates', 'check-comments': 'מכריז היעדר',
                  test_toolsid: 'tools-drift' } },
  19: { claim: 'טענות על התנהגות' },
   ...
   *  מוטציה שאין בו. */
  34: { claim: 'לכל אחד שינוי, תווית' },
  35: { claim: 'שם הטענה שתיפול' },
  31: { claim: 'כיסוי הטבלה' },
  45: { claim: 'ועמודת התקן שלה אוסרת במפורש' },
  46: { claim: ['מספור רציף ובלי כפילויות', 'nameSignGaps', 'catOrderGaps'] },
  48: { claim: 'הערה ריקה' },
  50: { claim: 'COUNT_NOTE' },
  52: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  206: { claim: 'CACHE_NAME' },
  43: { manual: 'מספר בדיווח הוא התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו את הדיווח, ⚠️ ונאכף בתוצאתו בלבד' },
  24: { claims: { test_readonly: 'drift', 'check-capabilities': 'writeGateGaps' } },
  ...
  36: { claims: { 'check-comments': 'תווית מוטציה מודפסת',
                  'check-capabilities': ['regexBoundGaps', 'anchorGaps'],
                  'test_anchors': 'מ1' } },
  20: { claim: 'תקן הבאנר' },
  16: { claim: 'דפוס הבודקים' },
  100: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  ...
  27: { claims: { 'check-comments': 'ריצת check-js', 'check-capabilities': 'gateKindGaps' } },
  ...
  40: { claims: { 'check-js': 'תקציב זמן לסט',
                  'check-capabilities': 'gateSizeGaps' } },
  28: { claim: 'ברירת המחדל אינה המהירה' },
  56: { claims: { 'check-capabilities': ['sha:', 'orderGaps', 'blockRowGaps'],
                  test_sharedsync: ['block-drift', 'canon-drift'],
                  test_signedshared: 'unsignedTwins',
                  test_toolsid: ['pure-tools', 'pure-undeclared'] } },
  90: { claim: 'storage' },
  89: { claim: 'ג · פעולה שדורשת רשת' },
  105: { claim: 'עברית' },
  108: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  109: { claim: 'BANNER_W' },
  193: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  106: { claim: 'מפנה לקובץ' },
  122: { claim: 'android/app/src/main' },
  129: { claim: 'fgDriftMax' },
  130: { claim: 'CANON_MANIFEST' },
  131: { claim: 'שדות זהים' },
  134: { claim: 'BUILD_SHA' },
  138: { claim: 'SHARED_SHA' },
  136: { claim: 'versionCode' },
  119: { claim: 'WebView' },
  120: { claim: 'android:theme' },
  124: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  141: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  145: { claim: 'migrations' },
  154: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  156: { claim: 'א. חותמת בכל רשומה' },
  59: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  149: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  150: { claim: 'ו. טבלה מקבילה' },
  151: { claim: 'ב. חתימת סכימה' },
  152: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  173: { claim: 'type=password' },
  83: { claim: 'aria-label' },
  ...
  64: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps'] } },
  179: { claim: 'pass_salt' },
  /*  ⭐ סבב 148 — ⛔ סריקה הפוכה: ⚠️ כל אתר חשוד מפיל אלא אם הוא
   *  מוכרז עם נימוקו, ⭐ ורשימת דפוסים הייתה מוצאת את מה שכבר תוקן. */
  184: { claim: '[sec-keyfile]' },
  187: { claim: '⏳' },
  189: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  198: { claim: 'ט. ערך נגזר שהתיישן' },
  200: { claim: 'ח. הגדרה שהוחלפה' },
  199: { claim: 'ד. רשימת-היתר' },
  201: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  104: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  114: { claim: 'CLEANUP_SHA' },
  115: { claim: 'בלוק ה-APP' },
  170: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
 ...
 *  משניהם מפילה כאן, ⛔ ולא מתגלה סבבים אחר כך. */
{
  ...
  const MODULE_FILES = ['db_schema.mjs', 'peers.mjs', 'scope.mjs', 'whiten.mjs'];
