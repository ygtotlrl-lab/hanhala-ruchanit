   ...
   ──────────────────────────────────────────────────────────────────────── */
{
  ...
  /*  ⛔ מוטציית-נגד לכל מבחן — **מפילה** (סבב 79) — ⚠️ עד כאן היא נמדדה
  ...
  if (noAnti) {
    bad += noAnti;
    fail(`tools/: ${noAnti} מ-${seen} מבחנים בלי מוטציית-נגד, והצפוי אפס. ` +
  ...
  if (noExpect) {
    bad += noExpect;
    fail(`tools/: ${noExpect} מ-${seen} מבחנים בלי שם הטענה שתיפול, והצפוי אפס. ` +
      'מוסיפים את שם הטענה — במרשם המוטציה (`hits:`), או בתווית שנכתבת ' +
      'לפלט לצד הערך שנמדד («… — טענה 2ד נופלת»)');
  }

  if (!bad) pass(`מוטציות: ${seen} מבחנים (${excused} מנומקים), לכל אחד שינוי, תווית,` +
