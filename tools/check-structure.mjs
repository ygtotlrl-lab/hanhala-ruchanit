#!/usr/bin/env node
/*  בדיקת המבנה הקנוני של הריפו.
 *
 *  ⭐ משימת הבודק: סורק את סט התיקיות, הקבצים והשערים שבשורש — ⛔ ומפיל על קובץ
 *  שקיים באחת ואינו בשלוש, בלי חריגה מוכרזת.
 *
 *  «יכולת, שער או מודול שקיים באפליקציה אחת — או שהוא קיים בכולן, או
 ...
 *       חייב להיות קובץ שער (test_<נושא>.mjs) או חריגה מנומקת.
 *
 *  ⚠️ הרקע: שער check-js חי ב-gius לבדה עשרה סבבים, קבצים
 *  שרידיים ישבו בשורש של כל ריפו, ואיש לא החליט על אף אחד מהמצבים האלה.
 *  מהסבב הזה אחידות מבנית נאכפת בשער, לא במשמעת.
 *
 *  זהה בית-לבית בכל הריפו פרט לבלוק APP. שינוי במבנה הקנוני = עדכון
 *  בכל האפליקציות ובכל עותקי הבדיקה, באותו סבב.
 */
...
const APP = {
  ...
  toolsExtra: {
    /* ⭐ נימוקי מטריצת היכולות — עברו לכאן מ-CLAUDE.md (תקציב
       התיעוד). ⛔ מעבר ולא גיזום: אף שורה לא נמחקה, ואף שער אינו
       קורא אותם — הטבלה עצמה נשארה ב-CLAUDE.md. */
    '_capability-notes.md': 'נימוקי המטריצה שנמחקה — טקסט היסטורי בלבד; ⛔ אינו נקרא בשום שער והנימוקים החיים עברו לעמודת ההערות שבטבלה',
    /* חילוץ לקחי הסבבים שנגזמו — לא בודק ולא רתמה; נשמר בריפו
       מפני שהפרקים שממנו נשלף נמחקו, וג'יט לבדו אינו מקום שקוראים בו. */
    '_prune-lessons.md': 'לקחי פרקי הסבבים שנגזמו בהפרקים עצמם נמחקו',
  },
  keystore: 'hanhala.keystore',
  /*  ⛔ חריגות ב-`android/` ובתת-תיקיות `tools/` — ⚠️ **מה נכנס**: שם ⟵
   *  נימוק לקובץ או לתיקייה שקיימים כאן ולא באחיות. ⛔ **ומה מפיל**: שם
...
const MODULES = SHARED_MODULES.concat(['roles-harness.mjs']);
/* ⚠️ סוף פר-אפליקציה */
/*  ⛔ שם השער נגזר מהנושא ולא ממספר הסבב — 53 שערים
...
/*  ⛔ והתבנית הישנה נפסלת במפורש — `test_round99_legacy.mjs`
...
if (extraD.length)   fail(`תיקיות עודפות בשורש: ${extraD.join(', ')} — נמדדו ${extraD.length} ` +
                          ...
                          `בכל עותקי הבדיקה, או מסירים אותן`);
...
if (missingM.length) fail(`מודולים משותפים חסרים ב-tools/: ${missingM.join(', ')} — נמדדו ` +
                      ...
                      `מוסיפים את החסרים — מודול הוא יכולת משותפת ונכנס בכולם`);
...
if (badT.length) fail(`קבצים לא-רשומים ב-tools/: ${badT.join(', ')} — נמדדו ${badT.length} ` +
                      ...
                      `בכולם; לחריגה פרטית מוסיפים שורה מנומקת`);
...
/* ── ד. `icons/` ו-`design/` — היפוך ברירת המחדל ────────────────────────────
   ...
   ראה אותו (בדיוק כמו ערכי `manifest.json` בעבר, שם נאכף
   **קיום** הקובץ ולא **תוכנו**).
   ...
   ⛔ **ו-`design/` אינו נטען בדף** — קובץ המאסטר הוא נכס עיצוב
...
/* ── ה. תוכן ארבע התיקיות שלא נבדקו ─────────────────────────────────────────
   ...
   בשקט לא נראה לאף שער — ⛔ וזו בדיוק צורת הכשל שהשער הזה תופס.
