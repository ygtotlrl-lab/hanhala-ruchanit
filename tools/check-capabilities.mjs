...
const APP = {
  ...
  gapRows: [223, 141, 145, 169, 66, 154, 108],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    192: (c) => c.hasCode(/HR_PASS_ITER\s*=\s*100000\b/)
    ...
    191: (c) => !!c.fnBody('hrUsersCacheSave')
    ...
    152: (c) => !/hrCfgSet\('(?:hr_students|hr_sessions|hr_sleep_sessions)'/.test(c.src)
    ...
    188: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '90|המשוב בלחיצה נקבע ולא נירש': 'src',
    ...
    '188|מבנה טופס הכניסה': 'src',
    '154|שם נגזר אינו נשמר ברשומה': 'src',
    '40|שער מריץ את כל טענותיו': 'tools',
    '98|שכבת המראה': 'src',
    '210|הגירה מקומית שהושלמה': 'mixed',
    '190|מראת המשתמשים': 'src',
    '189|מודל ההרשאות': 'src',
    '27|שער סורק קוד מולבן': 'tools',
    '27|ההלבנה שומרת על מספרי השורות': 'mixed',
    '170|הקשר נלכד בכניסה לפונקציה': 'src',
    ...
    '204|הערה שמתארת מצב שחלף': 'mixed',
    '211|שאילתת `@media` מתה': 'src',
    '127|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '128|ייצוא והנפקה — דרך מוצהרת': 'src',
    '109|המסמך מתפרסר נקי': 'src',
    '110|לוג מסלול — תג ורמה': 'src',
    '82|מיכל המודאל ילד ישיר של `body`': 'src',
    '123|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '172|הגיבוי שלם ומעומד': 'src',
    '78|פעולה מגיבה מיד': 'src',
    '79|משיכה מסוננת בשרת': 'src',
    '140|דגימת-יתר במחולל': 'tools',
    '192|`PBKDF2` — פרמטרים': 'src',
    '158|`migrations/` — תוכן': 'src',
    '159|`migrations/` — כמות': 'src',
    '180|מיזוג מכל — מפה ומערך': 'src',
    '181|ברירת מחדל אינה ממוזגת': 'src',
    '182|רשימה אינה נושאת פריט כפול': 'src',
    '173|`sh_backup` — טבלת הגיבוי': 'src',
    '25|שער אינו מקליד מספר שורה': 'tools',
    '31|שער ממתין לתנאי ולא לשעון': 'tools',
    '196|סודות אינם עוזבים את המכשיר': 'src',
    '167|דפוס `upsert` — onConflict מוכרז': 'src',
    '194|טביעה חסרה מושלמת אוטומטית': 'src',
    '194|כניסה אופליין': 'src',
    '151|עריכת נתונים אופליין': 'src',
    '177|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '195|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '76|אובייקט יקר נבנה פעם אחת': 'src',
    '77|מסך מציג מיד ומרענן ברקע': 'src',
    '144|האפליקציה ניתנת להתקנה מכרום': 'src',
    '201|דגלי מעבר — אין דגל דלוק': 'src',
    '84|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '191|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '97|פינוי אוטומטי': 'src',
    '97|אופק הפינוי נוקה מהסנכרון': 'src',
    '184|אימות פינוי מול הענן': 'src',
    '145|שיתוף קבצים': 'src',
    '129|מעטפת APK (WebView)': 'src',
    '132|אין נכסים מוטבעים': 'src',
    '149|מפתח חתימה קבוע': 'src',
    '155|מקור אמת יחיד לסכימה': 'src',
    '156|קובץ התקנה מלא': 'src',
    '62|גיבוי יומי אוטומטי': 'src',
    '62|יומן פעולות': 'src',
    '152|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '85|דפוס הודעת שגיאה יחיד': 'src',
    '93|`toast` — חתימה, גוף ומחלקות': 'src',
    '93|סיווג ההודעה נגזר מהמסלול': 'src',
    '222|מיכל באנר העדכון במקור': 'src',
    '222|הרענון מ-controllerchange בלבד': 'src',
    '222|סימן דחיית הבאנר מתמיד': 'src',
    '124|רישום כשלי כתיבה': 'src',
    '63|חלון חם במכשיר': 'src',
    '188|מסך שינוי סיסמה עצמי': 'src',
    '224|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '64|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '194|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '65|מודול מזהי רשומות': 'src',
    '65|מזהה אינו מומר למספר': 'src',
    '67|מזהה מכשיר': 'src',
    '64|ליבת מיזוג משותפת': 'src',
    '129|מעטפת WebView חתומה': 'tools',
    '194|אימות מול טביעה בענן': 'mixed',
    '147|בניית APK אחידה עם שער חתימה': 'src',
    '224|מודול ה-service worker': 'src',
    '68|ניסיון חוזר בתור הסנכרון': 'src',
    '69|מנגנון משיכה אחיד': 'src',
    '72|נעילת חוסר-פעילות': 'src',
    '73|מודל הסשן — בזיכרון בלבד': 'src',
    '223|בדיקת עדכון תקופתית ל-service worker': 'src',
    '100|פסק זמן אחיד לקריאות רשת': 'src',
    '101|מאזיני מצב רשת': 'src',
    '176|גריעת tombstones לפי גיל': 'src',
    '222|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '145|גשר שיתוף': 'src',
    '74|העברת מזהה ל-DOM': 'src',
    '75|מיון אחד לכל התצוגות': 'src',
    '80|ייצוא — שני דפוסים מוצהרים': 'src',
    '137|שכבת אייקונים': 'src',
    '185|שכבת קלט אחידה': 'src',
    '185|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '81|שכבת המודאל': 'src',
    '188|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [33, 48, 49, 58, 96, 56, 183, 115, 161, 61, 26, 66, 29, 38, 42, 199, 206];
...
const MATRIX = [
  { row: 90, name: 'המשוב בלחיצה נקבע ולא נירש',
  ...
  { row: 154, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 170, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 204, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 123, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 172, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 169, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 140, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 192, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 158, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 159, name: '`migrations/` — כמות',
  ...
  { row: 180, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 181, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 182, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 173, name: '`sh_backup` — טבלת הגיבוי',
  ...
  { row: 196, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 167, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 194, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 194, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 151, name: 'עריכת נתונים אופליין',
  ...
  { row: 177, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 195, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 144, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 201, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 191, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 184, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 145, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 129, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 132, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 149, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 155, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 156, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 62, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 62, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 152, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 188, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 188, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 224, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 171, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 194, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 129, name: 'מעטפת WebView חתומה',
  ...
  { row: 194, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 147, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 224, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 223, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 210, name: 'הגירה מקומית שהושלמה',
   ...
   *  ולא רק מוגדרים. */
  { row: 176, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 222, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 222, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 222, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 222, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 137, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 185, name: 'שכבת קלט אחידה',
  ...
  { row: 185, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  209: { claims: { test_visual: ['[class-token]'] } },
  ...
  186: { claims: { test_codescan: ['[zero-guard]', '[zero-guard-registry]'] } },
  ...
  119: { claims: { test_failsurface: ['[fail-surface]', '[fail-literal]', '[queued-msg]', '[msg-shared]'] } },
  ...
  120: { claims: { test_names: ['[name-policy]', '[name-policy-why]', '[fn-sister-prefix]',
  ...
  218: { claims: { test_mignames: ['[mig-pattern]', '[mig-seq]', '[mig-file-no-entry]',
  ...
  199: { claims: { test_docfacts: ['[doc-name]', '[doc-round]'],
  ...
  203: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  197: { claim: 'אין `service_role` בקובץ' },
  35: { claim: 'מסלול «' },
  ...
  200: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  71: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  160: { claim: 'בעלות הסכימה המשותפת' },
  174: { claim: '`ya_settings_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  175: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  212: { claim: 'ז. עמודה בלי קורא' },
  ...
  135: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  138: { claim: 'margin' },
  ...
  122: { claims: { test_codescan: ['[fn-product]'] },
  ...
  207: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  89: { claim: 'מחלקה משותפת שמצביה נבדלים' },
  ...
  134: { claim: 'android/app/src/main' },
  141: { claim: 'fgDriftMax' },
  142: { claim: 'CANON_MANIFEST' },
  143: { claim: 'שדות זהים' },
  146: { claim: 'BUILD_SHA' },
  150: { claim: 'SHARED_SHA' },
  148: { claim: 'versionCode' },
  130: { claim: 'WebView' },
  131: { claim: 'android:theme' },
  136: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  153: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  157: { claim: 'migrations' },
  166: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  168: { claim: 'א. חותמת בכל רשומה' },
  61: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  161: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  162: { claim: 'ו. טבלה מקבילה' },
  163: { claim: 'ב. חתימת סכימה' },
  164: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  187: { claim: 'type=password' },
  86: { claim: 'aria-label' },
  ...
  66: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps', 'hebFormGaps'] } },
  193: { claim: 'pass_salt' },
  /*  ⭐ סבב 148 — ⛔ סריקה הפוכה: ⚠️ כל אתר חשוד מפיל אלא אם הוא
   *  מוכרז עם נימוקו, ⭐ ורשימת דפוסים הייתה מוצאת את מה שכבר תוקן. */
  198: { claim: '[sec-keyfile]' },
  202: { claims: { test_filesets: ['[oneoff-gate]', '[oneoff-round]', '[oneoff-stale]'] } },
  206: { claims: { test_orphans: 'כל קובץ בעץ מוזכר במקום אחר',
  ...
  215: { claim: 'ט. ערך נגזר שהתיישן' },
  217: { claim: 'ח. הגדרה שהוחלפה' },
  216: { claim: 'ד. רשימת-היתר' },
  219: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  125: { claim: 'CLEANUP_SHA' },
  126: { claim: 'בלוק ה-APP' },
  183: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
