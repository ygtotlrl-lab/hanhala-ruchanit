...
const APP = {
  ...
  /*  ⛔ קובץ תשתית שקיים בחלק מהריפו — ⚠️ כל שם נושא את הסיבה,
   *  ⭐ והוא נמדד בית-לבית בין מי שיש לו: ⛔ קובץ שנופל בין הכיסאות
   *  אינו נמדד כלל. */
  ...
  notGates: {
    ...
    'roles-harness.mjs':
      'רתמת מודל ההרשאות — שער ההרשאות מריץ אותה בתהליך משלו, ⛔ והיא אינה נקודת כניסה בסט',
  },
  subsetTools: {
    'roles-harness.mjs': 'רתמת מודל ההרשאות — שלוש האפליקציות שיש בהן כניסה, '
      + 'וביומן אין כניסה ואין תפקיד שיוכרע',
    'test_date.mjs':
  ...
  perAppTools: {
    ...
    'test_offline_login.mjs':
      'שער הכניסה האופליין — שלוש האפליקציות שיש בהן כניסה, ומסלול השלמת הטביעה נבדל ביניהן, ולכן גופו אינו זהה',
    'test_roles.mjs':
      'שער מודל ההרשאות — שלוש האפליקציות שיש בהן כניסה, ואוצר הדרגות והחסימה נבדלים ביניהן, ולכן גופו אינו זהה',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
const CASE = declCases(import.meta.url, APP);

/*  ⛔ השורות בטבלת התשתית שהקובץ הזה אוכף — ⚠️ המיפוי נגזר מכאן
 *  ⛔ ואינו רשימה שנייה בבודק. */
export const ROWS = [23, 59, 61, 225, 226];
...
/*  ⛔ רשימת הסריקה נגזרת מרשימת הריצה — ⚠️ **מה נכנס**: קובצי
...
/*  ⛔ כלי שאוכף שורה נסרק כשער — ⚠️ **מה מפיל**: קובץ שמצהיר
...
if (!away.length) {
  ...
  const subDrift = names.filter((f) => {
    /*  ⛔ ההשוואה על השלד ⛔ ולא על הבייטים — ⚠️ קובץ תת-קבוצה
...
/*  ⛔ מ3 — קובץ בלי `APP` שחי באחת בלבד ואינו מוכרז. ⚠️ זה
...
/*  ⛔ מ4 — שער שהוסר מרשימת הריצה ונשאר בתיקייה. */
{
...
/*  ⛔ מ6 — כלי מחוץ לרשימת הריצה שמייצא `ROWS` ריק: ⚠️ זה
...
/*  ⛔ מ7 — קבוצת סריקה שהמריץ אינו בה: ⚠️ המריץ אוכף שורות,
...
console.log(fail ? `\n✗ זהות קובצי tools — ${fail} טענות נכשלו`
                 : `\n✓ זהות קובצי tools — ${pass} טענות עברו`);
