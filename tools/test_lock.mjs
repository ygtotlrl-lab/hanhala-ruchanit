#!/usr/bin/env node
/*  test_lock.mjs — נעילת חוסר-פעילות, מודול משותף.
 *
 *  **מה נאכף:** משתמש שהתחבר ואינו נוגע במסך מקבל אחרי ארבע דקות חלון
 *  אזהרה ואחרי חמש מוצא החוצה — ⛔ **אותם ערכים ואותה התנהגות** בשלוש
 *  האפליקציות שיש בהן כניסה. ⚠️ הרתמה מריצה את הליבה האמיתית ב-`vm` עם
 *  DOM ושעון מזויפים.
 *
 *  **הנימוק המדוד:** לאחת היה המנגנון המלא, ⛔ לשנייה אותם ערכים **בלי
 *  חלון אזהרה**, ⛔ ולשלישית לא היה כלום — ⚠️ שלושה מימושים לאותה יכולת,
 *  בלי ששום שער חיפש אותה ובלי שאיש החליט.
 *
 *  **מה יישבר בלעדיו:** ⛔ מכשיר משותף נשאר פתוח בזהות של מי שהתחבר בו,
 *  ⚠️ והפער נראה רק אחרי שמישהו כבר השתמש בו.
 *
 *  **מה אינו נאכף כאן:** ⛔ ערכי הזמן עצמם אינם הכרעת השער — ⚠️ הם הכרעת
 *  המנהל, ⭐ והשער נועל את מה שהוכרע.
 *
 *  ⛔ המוטציות אינן נכתבות לעץ.
 *  הקובץ זהה בית-לבית בכל הריפו פרט לבלוק `APP` שבראשו.
 *  ⚠️ **באפליקציה שאין בה מסך כניסה** — `present:false` בבלוק `APP` — ארבע
...
const APP = {
  ...
  present: true,
  bootFn:  'loadDash',      // פונקציית העלייה — נקודת ההפעלה היחידה
  resetFn: '_doLoginInner', // מסלול הכניסה המוצלחת — נקודת הדריכה מחדש
  stopFn:  'doLogout',      // מסלול היציאה
  /*  ⚠️ מפתח הסשן הקבוע — `null` פירושו שהסשן אינו נשמר במכשיר.
   *  ⭐ הוא `null` **בכולן** (מטריצה — שורת מודול הנעילה), והאכיפה
...
const START = '/* ═══ נעילת חוסר-פעילות — מודול משותף';
...
const FLOOR = { shared: 3, app: 16, appWhy: 'שכבת הכניסה — הנעילה מחווטת לסשן בשלוש, וביומן נמדד המודול בלבד' };
/* ⚠️ סוף פר-אפליקציה */
...
/* ── 9. סשן שנשמר במכשיר — נמדד מול ההצהרה שבמטריצה ────────────────────── */
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
function fakeEl(id) {
  /*  ⛔ `dataset` הוא חלק מהאלמנט המזויף — ⚠️ הכפתור מסומן
 ...
 *  ⚠️ הטענות מודדות את מסלול הטיימרים ואת הסדר שבו הוא מנוקה, ⛔ ולא את
 *  שמות המשתנים שמחזיקים אותו: ⭐ שער שהיה נופל על שינוי שם היה חוסם כל
 *  ניקוי, ⛔ ומי שנתקל בו היה מחזיר את השם הישן בתום לב. */
{
  const renamed = block.replace(/_lkWarnT\b/g, '_lkWarnTimer');
  if (renamed === block) fail('11. מוטציית-הנגד לא מצאה את שם הטיימר בליבה');
  else {
    let res;
