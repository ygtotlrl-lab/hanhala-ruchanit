...
const APP = {
  ...
  gapRows: [48, 134, 138, 162, 68, 147, 105],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    184: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    183: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    145: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    180: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '97|חותמת רעננות נכשלת סגור': 'src',
    '88|`Enter` שומר בכל שדה עריכה': 'src',
    '89|מאזין אחד, בהאצלה מ-`document`': 'src',
    '90|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '180|מבנה טופס הכניסה': 'src',
    '147|שם נגזר אינו נשמר ברשומה': 'src',
    '201|קבוע בלי קורא': 'mixed',
    '203|מפתח אחסון בלי קורא': 'src',
    '39|שער מריץ את כל טענותיו': 'tools',
    '96|שכבת המראה': 'src',
    '204|הגירה מקומית שהושלמה': 'mixed',
    '182|מראת המשתמשים': 'src',
    '181|מודל ההרשאות': 'src',
    '26|שער סורק קוד מולבן': 'tools',
    '26|ההלבנה שומרת על מספרי השורות': 'mixed',
    '163|הקשר נלכד בכניסה לפונקציה': 'src',
    '100|פריסה במסכי טלפון וטאבלט': 'src',
    '101|ערכת נושא — בהיר וכהה': 'src',
    '102|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '103|אין סגנון מוטבע': 'src',
    '105|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '196|הערה שמתארת מצב שחלף': 'mixed',
    '205|שאילתת `@media` מתה': 'src',
    '120|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '121|ייצוא והנפקה — דרך מוצהרת': 'src',
    '106|המסמך מתפרסר נקי': 'src',
    '107|לוג מסלול — תג ורמה': 'src',
    '84|מיכל המודאל ילד ישיר של `body`': 'src',
    '116|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '165|הגיבוי שלם ומעומד': 'src',
    '80|פעולה מגיבה מיד': 'src',
    '81|משיכה מסוננת בשרת': 'src',
    '133|דגימת-יתר במחולל': 'tools',
    '184|`PBKDF2` — פרמטרים': 'src',
    '151|`migrations/` — תוכן': 'src',
    '152|`migrations/` — כמות': 'src',
    '173|מיזוג מכל — מפה ומערך': 'src',
    '174|ברירת מחדל אינה ממוזגת': 'src',
    '175|רשימה אינה נושאת פריט כפול': 'src',
    '166|`sh_backup` — טבלת הגיבוי': 'src',
    '24|שער אינו מקליד מספר שורה': 'tools',
    '200|פונקציה בלי קוראים': 'mixed',
    '30|שער ממתין לתנאי ולא לשעון': 'tools',
    '188|סודות אינם עוזבים את המכשיר': 'src',
    '160|דפוס `upsert` — onConflict מוכרז': 'src',
    '186|טביעה חסרה מושלמת אוטומטית': 'src',
    '186|כניסה אופליין': 'src',
    '144|עריכת נתונים אופליין': 'src',
    '170|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '187|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '78|אובייקט יקר נבנה פעם אחת': 'src',
    '79|מסך מציג מיד ומרענן ברקע': 'src',
    '137|האפליקציה ניתנת להתקנה מכרום': 'src',
    '193|דגלי מעבר — אין דגל דלוק': 'src',
    '85|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '183|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '48|ארבעת מימדי הסריקה': 'src',
    '95|פינוי אוטומטי': 'src',
    '95|אופק הפינוי נוקה מהסנכרון': 'src',
    '177|אימות פינוי מול הענן': 'src',
    '138|שיתוף קבצים': 'src',
    '122|מעטפת APK (WebView)': 'src',
    '125|אין נכסים מוטבעים': 'src',
    '142|מפתח חתימה קבוע': 'src',
    '148|מקור אמת יחיד לסכימה': 'src',
    '149|קובץ התקנה מלא': 'src',
    '64|גיבוי יומי אוטומטי': 'src',
    '64|יומן פעולות': 'src',
    '145|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '86|דפוס הודעת שגיאה יחיד': 'src',
    '91|`toast` — חתימה, גוף ומחלקות': 'src',
    '91|סיווג ההודעה נגזר מהמסלול': 'src',
    '212|מיכל באנר העדכון במקור': 'src',
    '212|הרענון מ-controllerchange בלבד': 'src',
    '212|סימן דחיית הבאנר מתמיד': 'src',
    '117|רישום כשלי כתיבה': 'src',
    '65|חלון חם במכשיר': 'src',
    '180|מסך שינוי סיסמה עצמי': 'src',
    '214|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '66|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '186|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '67|מודול מזהי רשומות': 'src',
    '67|מזהה אינו מומר למספר': 'src',
    '69|מזהה מכשיר': 'src',
    '66|ליבת מיזוג משותפת': 'src',
    '122|מעטפת WebView חתומה': 'tools',
    '186|אימות מול טביעה בענן': 'mixed',
    '140|בניית APK אחידה עם שער חתימה': 'src',
    '214|מודול ה-service worker': 'src',
    '70|ניסיון חוזר בתור הסנכרון': 'src',
    '71|מנגנון משיכה אחיד': 'src',
    '74|נעילת חוסר-פעילות': 'src',
    '75|מודל הסשן — בזיכרון בלבד': 'src',
    '213|בדיקת עדכון תקופתית ל-service worker': 'src',
    '98|פסק זמן אחיד לקריאות רשת': 'src',
    '99|מאזיני מצב רשת': 'src',
    '169|גריעת tombstones לפי גיל': 'src',
    '212|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '138|גשר שיתוף': 'src',
    '76|העברת מזהה ל-DOM': 'src',
    '77|מיון אחד לכל התצוגות': 'src',
    '82|ייצוא — שני דפוסים מוצהרים': 'src',
    '130|שכבת אייקונים': 'src',
    '178|שכבת קלט אחידה': 'src',
    '178|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '83|שכבת המודאל': 'src',
    '180|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [32, 47, 49, 51, 60, 94, 58, 176, 112, 154, 63, 25, 68, 28, 37, 41];
...
const MATRIX = [
  ...
  { row: 97, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 90, name: 'כפתור שכותב מושבת בזמן הכתיבה',
  ...
  { row: 147, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 163, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 100, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 102, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 105, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 101, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 196, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 107, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 116, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 165, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 162, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 133, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 184, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 151, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 152, name: '`migrations/` — כמות',
  ...
  { row: 173, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 174, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 175, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 188, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 160, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 186, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 186, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 144, name: 'עריכת נתונים אופליין',
  ...
  { row: 170, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 187, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 137, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 193, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 183, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 95, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 177, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 138, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 122, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 125, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 142, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 148, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 149, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 64, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 64, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 145, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 91, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 91, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 117, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 65, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 180, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 180, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 214, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 164, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 186, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 122, name: 'מעטפת WebView חתומה',
  ...
  { row: 186, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 140, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 214, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 213, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 201, name: 'קבוע בלי קורא',
    probe: () => constNoReaderGaps().length === 0 },
  { row: 203, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 96, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 204, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 98, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 169, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 212, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 212, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 212, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 212, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 130, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 178, name: 'שכבת קלט אחידה',
  ...
  { row: 178, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  195: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  189: { claim: 'אין `service_role` בקובץ' },
  34: { claim: 'מסלול «' },
  ...
  192: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  73: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  153: { claim: 'בעלות הסכימה המשותפת' },
  167: { claim: '`tb_kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  168: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  206: { claim: 'ז. עמודה בלי קורא' },
  92: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  128: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  131: { claim: 'margin' },
  ...
  115: { claims: { test_codescan: ['[fn-product]'] },
  ...
  199: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  104: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  ...
  94: { claim: 'storage' },
  93: { claim: 'ג · פעולה שדורשת רשת' },
  109: { claim: 'עברית' },
  112: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  113: { claim: 'BANNER_W' },
  202: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  110: { claim: 'מפנה לקובץ' },
  127: { claim: 'android/app/src/main' },
  134: { claim: 'fgDriftMax' },
  135: { claim: 'CANON_MANIFEST' },
  136: { claim: 'שדות זהים' },
  139: { claim: 'BUILD_SHA' },
  143: { claim: 'SHARED_SHA' },
  141: { claim: 'versionCode' },
  123: { claim: 'WebView' },
  124: { claim: 'android:theme' },
  129: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  146: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  150: { claim: 'migrations' },
  159: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  161: { claim: 'א. חותמת בכל רשומה' },
  63: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  154: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  155: { claim: 'ו. טבלה מקבילה' },
  156: { claim: 'ב. חתימת סכימה' },
  157: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  179: { claim: 'type=password' },
  87: { claim: 'aria-label' },
  ...
  190: { claim: '[sec-keyfile]' },
  194: { claim: '⏳' },
  198: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  207: { claim: 'ט. ערך נגזר שהתיישן' },
  209: { claim: 'ח. הגדרה שהוחלפה' },
  208: { claim: 'ד. רשימת-היתר' },
  210: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  108: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  118: { claim: 'CLEANUP_SHA' },
  119: { claim: 'בלוק ה-APP' },
  176: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
