...
const APP = {
  ...
  commitsSince: '5c7f8ecca4492e6f62265188180afe6811bd2e75',
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */

export const ROWS = [212, 223];
...
/*  ⛔ המזהים נשלפים מהתוכן ⛔ ולא משורות הדיף — ⚠️ שורה שהשתנתה
    מופיעה גם כמחיקה וגם כהוספה, ⭐ והשוואת **הגדרות** היא מה שמבדיל בין
    «נמחק» ל«נערך». */
/*  ⛔ והשמה למאפיין `window` היא צורת הגדרה אף היא — ⚠️ הנימוק
    נמדד: שער ההסרות **אישר** מחיקה של `window.X = function` שנשאר לה
    קורא, ⭐ מפני שזיהה `function X(` ו-`const X =` בלבד. */
/*  ⛔ ו-`const X =` הוא הגדרה בכל צורת ערך — ⚠️ פונקציה שנעטפה
    במטמון (`const X = memo(XScan)`) נשארת אותו שם שנקרא באותה צורה,
    ⭐ ותבנית שדרשה `function` או חץ אחרי ה-`=` דיווחה אותה כמחוקה. */
const DEF = [/\b(?:async\s+)?function\s+([A-Za-z_$][\w$]{3,})\s*\(/g,
             /\b(?:const|let|var)\s+([A-Za-z_$][\w$]{3,})\s*=(?!=)/g,
             /^[ \t]*(?:async\s+)?([A-Za-z_$][\w$]{3,})\s*\([^()]*\)\s*\{/gm,
             /\bwindow\s*\.\s*([A-Za-z_$][\w$]{3,})\s*=\s*(?:async\s+)?(?:function|\([^)]*\)\s*=>)/g];
/*  ⛔ מילת שפה אינה הגדרה — ⚠️ תבנית המתודה תופסת גם `while (x) {`,
...
/*  ⛔ צורת ההגדרה קובעת את צורת הקריאה — ⚠️ **מתודה** נקראת
...
/*  ⛔ השערים והאפליקציה הם שתי תוכניות — ⚠️ המקור אינו מייבא
 ...
 *  שהוא משנה, ⭐ והוא הרשומה היחידה של שינוי שאינו בטבלה. */
{
  ...
  if (subs === null && !fs.existsSync(join(ROOT, '.git'))) {
    ...
    /*  ⛔ ואין דפוס פטור — ⚠️ המיזוג ל-`main` הוא הרצה-קדימה, ⭐ ולכן אין
     *  הודעת מיזוג: ⛔ דפוס שאין לו אף קומיט הוא הצהרה בלי מקרה. */
    const off = subs.filter((s) => !/^סבב \d+/.test(s));
    t(off.length === 0,
      ...
      ' — מה עושים: פותחים את ההודעה ב-«סבב N — »');
  }
}
...
/*  ⛔ המוטציה על **מודל** ולא על הריפו — ⚠️ מחיקת פונקציה אמיתית
    כדי לבדוק שער היא בדיוק מה שהשער בא למנוע. */
{
  ...
  /*  ⛔ מוטציית-נגד על **צורה** — ⚠️ זו בדיוק המוטציה שהפילה את
  ...
  /*  ⛔ הצורה שהשער היה עיוור לה — ⚠️ היא הצורה שבה כתובה כמעט כל
  ...
  /*  ⛔ עטיפה במטמון אינה מחיקה — ⚠️ השם נשאר ונקרא באותה צורה. */
  t(orphans(plain, asDefs('const callersOf = memo1(callersOfScan);'), fake).length === 0,
    '⭐ מוטציית-נגד: עטיפת `const f` במטמון ⛔ אינה מפילה');
  t(orphans(asDefs('const callersOf = memo1(callersOfScan);'), new Map(), fake).length === 1,
    '⛔ מוטציה: מחיקת `const f = memo(…)` שיש לה קורא — מפילה');
  t(orphans(plain, new Map(), fake).length === 1,
    '⛔ מוטציה: מחיקה אמיתית של אותה הגדרה, ולה קורא — מפילה');
}

/*  ⛔ האימות על העץ החי — ⚠️ העיוורון לא נראה במודל אלא בעץ:
...
/*  ⭐ מוטציית-נגד: הודעה שנפתחת בסבב אינה מפילה — ⚠️ גם כשהיא ארוכה,
 *  ⭐ ומסתיימת בשורות ייחוס. */
t(/^סבב \d+/.test('סבב 172 — ההצהרה נמדדת\n\nCo-Authored-By: x'),
  'נק1 · ⭐ מוטציית-נגד: «ההצהרה נמדדת» ⛔ אינה מפילה');
