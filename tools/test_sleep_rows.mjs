#!/usr/bin/env node
/*  בדיקת סבב 39 — סדרי השינה עוברים לשכבת השורות.
 *
 *  ⚠️ פרטי ל-hanhala-ruchanit. ⛔ אין ליישר אותו מריפו אחר — שכבת השורות
 *     של ההנהלה אינה קיימת באחיות.
 *
 *  ⭐ הנקודה שהבדיקה הזו נועדה לנעול: השינה נוספה כ**תצורה** ולא כהעתקה
 *     של מסלול הנוכחות. שני עותקים של מסלול הכתיבה הם שתי הזדמנויות
 *     שאחד מהם ייסחף (הלקח של איחוד מנועי המיזוג, סבב 38), ולכן חלק
 *     מהטענות כאן בודקות דווקא ש**אין** עותק שני.
 *
 *  ⚠️ הבדיקה מריצה את הקוד האמיתי: שכבת השורות נחתכת מ-`index.html`
 *     ורצה ב-`vm` מעל `SB` מזויף שרושם כל `upsert`.
 */
...
ok('1י · ⛔ שתי המיגרציות מסומנות «נכתב ולא הורץ»',
  /נכתב ולא הורץ/.test(M009) && /נכתב ולא הורץ/.test(M010));
