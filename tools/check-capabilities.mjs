...
const APP = {
  ...
  gapRows: [221, 138, 142, 166, 67, 151, 106],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    188: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    187: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    149: (c) => !/ysCfgSet\('(?:hr_students|hr_sessions|hr_sleep_sessions)'/.test(c.src)
    ...
    184: (c) => !!c.fnBody('changeMyPassword')
  ...
  migrateKeep: {
    migratePrefixKeys:
      'הגירת תחילית האחסון (סבב 148) — ⚠️ היא המקום היחיד בקוד החי שנוקב בתחילית הישנה, ⛔ ומכשיר שלא נטען מאז מחזיק תחתיה את התור שלא נדחף: ⭐ הסרתה מאבדת אותו לנצח, ⛔ והמדידה שהמכשירים עברו טרם נעשתה',
  },
  ...
  probeInput: {
    '98|חותמת רעננות נכשלת סגור': 'src',
    '87|`Enter` שומר בכל שדה עריכה': 'src',
    '88|מאזין אחד, בהאצלה מ-`document`': 'src',
    '91|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '184|מבנה טופס הכניסה': 'src',
    '151|שם נגזר אינו נשמר ברשומה': 'src',
    '205|קבוע בלי קורא': 'mixed',
    '207|מפתח אחסון בלי קורא': 'src',
    '39|שער מריץ את כל טענותיו': 'tools',
    '97|שכבת המראה': 'src',
    '208|הגירה מקומית שהושלמה': 'mixed',
    '186|מראת המשתמשים': 'src',
    '185|מודל ההרשאות': 'src',
    '26|שער סורק קוד מולבן': 'tools',
    '26|ההלבנה שומרת על מספרי השורות': 'mixed',
    '167|הקשר נלכד בכניסה לפונקציה': 'src',
    '101|פריסה במסכי טלפון וטאבלט': 'src',
    '102|ערכת נושא — בהיר וכהה': 'src',
    '103|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '104|אין סגנון מוטבע': 'src',
    '106|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '200|הערה שמתארת מצב שחלף': 'mixed',
    '209|שאילתת `@media` מתה': 'src',
    '124|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '125|ייצוא והנפקה — דרך מוצהרת': 'src',
    '107|המסמך מתפרסר נקי': 'src',
    '108|לוג מסלול — תג ורמה': 'src',
    '83|מיכל המודאל ילד ישיר של `body`': 'src',
    '120|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '169|הגיבוי שלם ומעומד': 'src',
    '79|פעולה מגיבה מיד': 'src',
    '80|משיכה מסוננת בשרת': 'src',
    '137|דגימת-יתר במחולל': 'tools',
    '188|`PBKDF2` — פרמטרים': 'src',
    '155|`migrations/` — תוכן': 'src',
    '156|`migrations/` — כמות': 'src',
    '177|מיזוג מכל — מפה ומערך': 'src',
    '178|ברירת מחדל אינה ממוזגת': 'src',
    '179|רשימה אינה נושאת פריט כפול': 'src',
    '170|`sh_backup` — טבלת הגיבוי': 'src',
    '24|שער אינו מקליד מספר שורה': 'tools',
    '204|פונקציה בלי קוראים': 'mixed',
    '30|שער ממתין לתנאי ולא לשעון': 'tools',
    '192|סודות אינם עוזבים את המכשיר': 'src',
    '164|דפוס `upsert` — onConflict מוכרז': 'src',
    '190|טביעה חסרה מושלמת אוטומטית': 'src',
    '190|כניסה אופליין': 'src',
    '148|עריכת נתונים אופליין': 'src',
    '174|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '191|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '77|אובייקט יקר נבנה פעם אחת': 'src',
    '78|מסך מציג מיד ומרענן ברקע': 'src',
    '141|האפליקציה ניתנת להתקנה מכרום': 'src',
    '197|דגלי מעבר — אין דגל דלוק': 'src',
    '84|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '187|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '96|פינוי אוטומטי': 'src',
    '96|אופק הפינוי נוקה מהסנכרון': 'src',
    '181|אימות פינוי מול הענן': 'src',
    '142|שיתוף קבצים': 'src',
    '126|מעטפת APK (WebView)': 'src',
    '129|אין נכסים מוטבעים': 'src',
    '146|מפתח חתימה קבוע': 'src',
    '152|מקור אמת יחיד לסכימה': 'src',
    '153|קובץ התקנה מלא': 'src',
    '63|גיבוי יומי אוטומטי': 'src',
    '63|יומן פעולות': 'src',
    '149|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '85|דפוס הודעת שגיאה יחיד': 'src',
    '92|`toast` — חתימה, גוף ומחלקות': 'src',
    '92|סיווג ההודעה נגזר מהמסלול': 'src',
    '220|מיכל באנר העדכון במקור': 'src',
    '220|הרענון מ-controllerchange בלבד': 'src',
    '220|סימן דחיית הבאנר מתמיד': 'src',
    '121|רישום כשלי כתיבה': 'src',
    '64|חלון חם במכשיר': 'src',
    '184|מסך שינוי סיסמה עצמי': 'src',
    '222|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '65|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '190|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '66|מודול מזהי רשומות': 'src',
    '66|מזהה אינו מומר למספר': 'src',
    '68|מזהה מכשיר': 'src',
    '65|ליבת מיזוג משותפת': 'src',
    '126|מעטפת WebView חתומה': 'tools',
    '190|אימות מול טביעה בענן': 'mixed',
    '144|בניית APK אחידה עם שער חתימה': 'src',
    '222|מודול ה-service worker': 'src',
    '69|ניסיון חוזר בתור הסנכרון': 'src',
    '70|מנגנון משיכה אחיד': 'src',
    '73|נעילת חוסר-פעילות': 'src',
    '74|מודל הסשן — בזיכרון בלבד': 'src',
    '221|בדיקת עדכון תקופתית ל-service worker': 'src',
    '99|פסק זמן אחיד לקריאות רשת': 'src',
    '100|מאזיני מצב רשת': 'src',
    '173|גריעת tombstones לפי גיל': 'src',
    '220|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '142|גשר שיתוף': 'src',
    '75|העברת מזהה ל-DOM': 'src',
    '76|מיון אחד לכל התצוגות': 'src',
    '81|ייצוא — שני דפוסים מוצהרים': 'src',
    '134|שכבת אייקונים': 'src',
    '182|שכבת קלט אחידה': 'src',
    '182|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '82|שכבת המודאל': 'src',
    '184|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [32, 47, 48, 50, 59, 95, 57, 180, 113, 158, 62, 25, 67, 28, 37, 41, 195];
...
const CAPS = {
  ...
  msgs: {
    ...
    block: { sha: 'a0a4a7ec9d717adb', lines: 47,
  ...
  swreg: {
    ...
    block: { sha: '4d72c6b9579a3fe0', lines: 97,
...
const MATRIX = [
  ...
  { row: 98, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 91, name: 'כפתור שכותב מושבת בזמן הכתיבה',
  ...
  { row: 151, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 167, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 101, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 103, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 106, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 102, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 200, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 108, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 120, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 169, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 166, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 137, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 188, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 155, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 156, name: '`migrations/` — כמות',
  ...
  { row: 177, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 178, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 179, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 192, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 164, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 190, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 190, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 148, name: 'עריכת נתונים אופליין',
  ...
  { row: 174, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 191, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 141, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 197, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 187, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 96, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 181, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 142, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 126, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 129, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 146, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 152, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 153, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 63, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 63, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 149, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 92, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 92, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 121, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 64, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 184, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 184, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 222, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 168, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 190, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 126, name: 'מעטפת WebView חתומה',
  ...
  { row: 190, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 144, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 222, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 221, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 205, name: 'קבוע בלי קורא',
    probe: () => constNoReaderGaps().length === 0 },
  { row: 207, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 97, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 208, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 99, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 173, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 220, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 220, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 220, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 220, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 134, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 182, name: 'שכבת קלט אחידה',
  ...
  { row: 182, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  117: { claims: { test_names: ['[name-policy]', '[name-policy-why]', '[fn-sister-prefix]',
  ...
  216: { claims: { test_mignames: ['[mig-pattern]', '[mig-seq]', '[mig-file-no-entry]',
  ...
  195: { claims: { test_docfacts: ['[doc-name]', '[doc-round]'],
  ...
  199: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  193: { claim: 'אין `service_role` בקובץ' },
  34: { claim: 'מסלול «' },
  ...
  196: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  72: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  157: { claim: 'בעלות הסכימה המשותפת' },
  171: { claim: '`ya_settings_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  172: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  210: { claim: 'ז. עמודה בלי קורא' },
  93: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  132: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  135: { claim: 'margin' },
  ...
  119: { claims: { test_codescan: ['[fn-product]'] },
  ...
  203: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  105: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  ...
  95: { claim: 'storage' },
  94: { claim: 'ג · פעולה שדורשת רשת' },
  110: { claim: 'עברית' },
  113: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  114: { claim: 'BANNER_W' },
  206: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  111: { claim: 'מפנה לקובץ' },
  131: { claim: 'android/app/src/main' },
  138: { claim: 'fgDriftMax' },
  139: { claim: 'CANON_MANIFEST' },
  140: { claim: 'שדות זהים' },
  143: { claim: 'BUILD_SHA' },
  147: { claim: 'SHARED_SHA' },
  145: { claim: 'versionCode' },
  127: { claim: 'WebView' },
  128: { claim: 'android:theme' },
  133: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  150: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  154: { claim: 'migrations' },
  163: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  165: { claim: 'א. חותמת בכל רשומה' },
  62: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  158: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  159: { claim: 'ו. טבלה מקבילה' },
  160: { claim: 'ב. חתימת סכימה' },
  161: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  183: { claim: 'type=password' },
  86: { claim: 'aria-label' },
  ...
  194: { claim: '[sec-keyfile]' },
  198: { claim: '⏳' },
  202: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  213: { claim: 'ט. ערך נגזר שהתיישן' },
  215: { claim: 'ח. הגדרה שהוחלפה' },
  214: { claim: 'ד. רשימת-היתר' },
  217: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  109: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  122: { claim: 'CLEANUP_SHA' },
  123: { claim: 'בלוק ה-APP' },
  180: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
