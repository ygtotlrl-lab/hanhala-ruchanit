...
--    נעשה בקובץ הבא בתור.
--
-- ⭐ **למה במסד ולא בקוד**: מדיניות השמירה נכתבה למודול הגיבוי
-- ב והיא **דרוכה אך אינה יכולה לרוץ** — ל-`kv_backup` יש
...
--
-- ⚠️ **שתי השכבות מכוונות, ואינן כפילות**: המסד מפנה, והלקוח
...
-- את הגיבוי. ⛔ אין להסיר אותה מהקוד — היא מה שיפעל אם ביום
...
-- ───────────────────────────────────────────────────────────────────────────
-- ⛔⛔ **רשימת-היתר מפורשת ולא קידומת** — זו הנקודה המסוכנת
...
--, בהחלטת המנהל): `rishon_tb_entries` · `rishon_tb_archive` ·
--    `ramataviv_tb_entries` · `ramataviv_tb_archive`. אלה שמות **גיבוי
-- השגרה** שרצו, לפני שהיומן והארכיון עברו לגיבוי-טבלאות
-- (`*_tb_entries_rows`). ⚠️ נבדק מול המסד: 23 שורות שגרה שהרשימה
--    הראשונה החמיצה, ולכן לא היו מתפנות לעולם — היסט **שמות**, ולא
--    היסטוריה שיש להגן עליה. ⛔ וההוספה היא ארבעה שמות מפורשים ולא
-- קידומת — `like 'rishon\_%'` היה תופס גם את
...
-- ───────────────────────────────────────────────────────────────────────────
-- ⛔ **שתי בדיקות שפיות שמסרבות לרוץ** — פונקציה שמוחקת נתוני
...
-- ───────────────────────────────────────────────────────────────────────────
-- ⚠️ 03:00 UTC: הדגל היומי של הגיבוי הוא תאריך **UTC**
--    (`toISOString().slice(0,10)`), ולכן חצות UTC היא הרגע שבו כל המכשירים
--    מתחילים לגבות. שעה שלוש לפנות בוקר רחוקה מהגל הזה, ומכל מקום הגריעה
--    נוגעת רק בשורות בנות 30+ יום ולעולם לא בעותק שנכתב היום.
-- ⚠️ **תיקון עובדה:** הקובץ נשא עד כה `'17 3 * * *'` ותיאר 03:17,
-- בעוד שהמשימה שנרשמה בפועל במסד היא `'0 3 * * *'`. הקובץ
--    יושר למציאות. ⛔ הקובץ הוא מקור האמת, ואי-התאמה בינו לבין המסד היא
--    בדיוק מה שמייצר גרסה שלישית שאיש אינו יודע עליה.
-- ⛔ `unschedule` לפני `schedule` — `cron.schedule` על שם קיים
...
-- ───────────────────────────────────────────────────────────────────────────
-- ⛔ **בלי הסעיף הזה המיגרציה פותחת בדיוק את מה שהיא באה לסגור**:
