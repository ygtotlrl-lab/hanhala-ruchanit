 ...
 *
 *  **מה אינו נאכף כאן:** ⛔ חמישה ממצאים שאינם ניתנים לאכיפה מכנית —
...
const APP = {
  ...
  dynamicClasses: ['role-admin', 'role-junior', 'role-manager',
                   'u-ty-approved', 'u-ty-suspended', 'u-ty-left',
                   'u-atv-p', 'u-atv-l', 'u-atv-e', 'u-atv-x',
                   'u-atv-ap', 'u-atv-ak', 'u-atv-a'],
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [5, 8, 49, 211, 117];
...
/*  ⛔ שורש נדרס בסביבת מוטציה — ⚠️ המוטציות רצות על עותק
 ...
 *  ⛔ אין שני דגלים לאותו מצב — שניים היו נופלים לרקורסיה. */
...
const FLOOR = { shared: 29, app: 22, appWhy: 'מספר השערים והבודקים שהריפו נושא — כל שער פרטי מוסיף טענת תוכן' };
/* ⚠️ סוף פר-אפליקציה */
...
/*  ⛔ מספר סבב בהערה ובפרוזה — ⚠️ **מה נכנס**: הערות הקוד שבמקור ובשערים,
 *  ⭐ ופרוזת ארבעת קובצי התיעוד; ⛔ **ומה מפיל**: «סבב N» בכל אחד מהם.
 *  ⭐ **ולמה**: מספר סבב הוא זמן ⛔ ולא נימוק — ⚠️ הקורא אינו יכול לאמת
 *  אותו ממקום עמידתו, ⭐ ומה שנשאר נכון בלעדיו הוא האיסור והסיבה.
 *  ⚠️ **ומה נשאר בחוץ, בשמו**: פרקי הסבב וכותרותיהם, ⛔ שהם הקישור בין
 *  קומיט לתקן · שורת העדכון · שורות הטבלה, ⭐ שעמודותיהן נמדדות למעלה ·
 *  ⚠️ ומחרוזת בקוד, ⛔ שהיא נתון ולא הסבר. */
const ROUND_NUM = /סבב(?:ים)?\s*\d/;
const DOCS_MD = ['CLAUDE.md', 'README.md', 'CONTEXT.md', 'android/README.md'];
export function roundNums(files) {
  const out = [];
  for (const [rel, text] of Object.entries(files)) {
    const kind = rel.endsWith('.html') ? 'html' : rel.endsWith('.css') ? 'css'
      : rel.endsWith('.md') ? 'md' : 'js';
    if (kind === 'md') {
      let chapter = false;
      text.split('\n').forEach((l, i) => {
        if (/^##\s/.test(l)) chapter = /^##\s+(?:⭐\s+)?סבב\s+\d/.test(l);
        if (chapter || l.startsWith('עודכן לאחרונה:')) return;
        if (rel === 'CLAUDE.md' && l.startsWith('|')) return;
        if (ROUND_NUM.test(l)) out.push(rel + ':' + (i + 1));
      });
      continue;
    }
    for (const [a, b] of commentRanges(text, kind)) {
      const c = text.slice(a, b);
      const m = ROUND_NUM.exec(c);
      if (m) out.push(rel + ':' + (text.slice(0, a + m.index).split('\n').length));
    }
  }
  return out;
}
/*  ⛔ זנבות עמודת התקן — ⚠️ **מה נכנס**: ארבע פסוקיות שחוזרות בשורות
 *  רבות, ⭐ וכל אחת נכתבת בסוף העמודה ובסדר קבוע: מקור ⟵ נגזר ⟵ נמדד
 *  ⟵ מספר; ⛔ **ומה מפיל**: זנב שאחריו פסוקית שאינה זנב, ⚠️ או שני זנבות
 *  בסדר הפוך. ⭐ **ולמה**: זנב שנדד לאמצע העמודה נקרא כחלק מהפסוקית
 *  שלפניו, ⛔ ומי שסורק את סוף השורות אינו מוצא אותו. */
const TAILS = ['ומקור אמת אחד', 'ונגזר ואינו מוקלד', 'ונמדד מול מקורו',
               'ואין מספר סבב', 'ומספר סבב נדרש'];
export function tableTails(doc) {
  const bad = [];
  const RE = /^\| (\d+) \| [^|]* \| (.*?) \| (?:✅|⭕|❌|🔲) \|/;
  for (const l of doc.split('\n')) {
    const m = RE.exec(l);
    if (!m) continue;
    const ks = m[2].split(' · ').map((c) => {
      const h = /^⛔ \*\*([^*]+)\*\*/.exec(c.trim());
      return h ? TAILS.indexOf(h[1]) : -1;
    });
    const first = ks.findIndex((k) => k >= 0);
    if (first < 0) continue;
    const suf = ks.slice(first);
    if (suf.some((k, i) => k < 0 || (i > 0 && k < suf[i - 1]))) bad.push(m[1]);
  }
  return bad;
}
/*  ⚠️ הקבצים שהמדידה קוראת — ⭐ נגזרים מהעץ ⛔ ואינם רשימה: `core/` ו-`tools/`
 *  נקראים כפי שהם, ⚠️ וקובץ שנוסף נסרק בלי שאיש יוסיף אותו כאן. */
function roundFiles(root) {
  const out = {};
  const add = (rel) => { const p = path.join(root, rel); if (fs.existsSync(p)) out[rel] = fs.readFileSync(p, 'utf8'); };
  for (const f of ['index.html', 'sw.js', 'app.css', ...DOCS_MD]) add(f);
  for (const d of ['core', 'tools'])
    for (const f of fs.readdirSync(path.join(root, d)).sort())
      if (/\.(?:js|mjs)$/.test(f)) add(d + '/' + f);
  return out;
}
...
/* ── ב. תוכן הקבצים לפי תפקידם ─────────────────────────────────────────── */
{
  ...
  /*  ⛔ ושלושת הקבצים הנלווים אינם מחזיקים עותק של הטבלה —
   *  ⚠️ טבלה שמופיעה בשניהם היא מקור אמת שני, וזה בדיוק מה שנסחף.
   *  ⛔ **והחצי שמדד «פרק כללים» ירד** — ⚠️ בלוקי הכללים
   *  ירדו, ⭐ ומאז אין כלל שאפשר להבריח לכאן. */
...
/* ── ג. הערה מכילה נימוק, ולא מצביע ────────────────────────────────────── */
/*  ⛔ אין לתפוס אזכור סתם — התבנית היא **הפניה**: «ר'/ראה …
...
/*  ⛔ ומספר שורה בטבלה הוא הפניה אף הוא — ⚠️ נמדדו 113 הפניות
...
/*  ⛔ אין לסרוק את הקובץ הזה עצמו — ⚠️ הוא מחזיק את התבנית
...
/*  ⛔ והפניה למספר כלל היא הפניה אף היא — ⚠️ «כלל ברזל N»
...
{
  /*  ⛔ מספר גרסה בתיעוד נסחף תמיד — נמדד: חמישה מקומות הצהירו
...
{
  /*  ⛔ אחד-עשר המודולים המשותפים מתועדים באינדקס אחד — פרק
  ...
  /*  ⛔ טבלת ידיות פר-אפליקציה — 49 שורות שאף שער לא קרא,
...
/* ── כלל 23 — קובץ נשפט לפי תפקידו ─────────────────────────────────────── */
{
   ...
   *   — ⚠️ ב-`CLAUDE.md` נשארות ההכרעות בלבד. */
   ...
   *  ⛔ ופרק «מצב נוכחי» ירד — ⚠️ הוא צילום מצב, כלומר היסטוריה,
   *  ⛔ וכלל «ערך שקיים בקוד אינו מוצהר בתיעוד» אוסר אותו במפורש. */
  /*  ⛔ ו-`README.md` מחזיק התקנה · הפעלה · פיתוח — ⚠️ ארבע
   *  הכותרות זהות בכל הריפו, ⛔ וכותרת חמישית היא פרק שנדד לכאן. */
...
/* ── כלל 24 — הבדל מכוון מנומק במקום שבו הוא נראה ──────────────────────── */
{
  ...
  /*  ⛔ תחולת תקן ההערות — `sw.js` ו-`tools/` אינם מוחרגים. */
...
/* ── סלקטור בלי קורא (ממצא 15) ─────────────────────────────────────────── */
{
  ...
  /*  ⛔ והכיוון השני נמדד אף הוא — ⚠️ מחלקה שנוספת ל-DOM ואין
   *  לה כלל CSS ואין קורא שבוחר אותה **אינה עושה דבר**: ⭐ הנימוק המדוד —
   *  `.bkStale` נוסף לבאנר «הגיבוי לא רץ N ימים» בכל האפליקציות,
  ...
  /*  ⛔ ומזהה `id` נמדד באותה מידה — ⚠️ המחלקות נמדדו והמזהים לא,
...
/* ── כלל 21 (המשך) — אין הצהרת «זהה בכולן» ─────────────────────────────── */
{
  /*  ⛔ הצהרת זהות היא ערך שנקבע במקום אחר — `check-docs` מודדת
  ...
  const DECL = [/זהה (מילה במילה|בית-לבית) בכל קבצי/,
...
/* ── מספר סבב — ⛔ זמן ולא נימוק ────────────────────────────────────────── */
const ROUND_FILES = roundFiles(ROOT);
{
  const hits = roundNums(ROUND_FILES);
  t(hits.length === 0,
    `[round-num] · אפס מספר סבב בהערות ובתיעוד — נמדדו ${hits.length} ב-` +
    `${Object.keys(ROUND_FILES).length} קבצים והצפוי אפס` +
    (hits.length ? ' (' + hits.slice(0, 8).join(' · ') + ')' : '') +
    '. מסירים את המספר ומשאירים את הנימוק, ⛔ ומשפט שכולו זמן יורד כולו');
  const tails = tableTails(DOC);
  t(tails.length === 0,
    `[round-tail] · זנבות עמודת התקן בסופה ובסדרם — נמדדו ${tails.length} שורות והצפוי אפס` +
    (tails.length ? ' (' + tails.join(' · ') + ')' : '') +
    '. מזיזים את הזנב לסוף העמודה, בסדר מקור ⟵ נגזר ⟵ נמדד ⟵ מספר');
}
...
/* ── מוטציות מספר הסבב — ⛔ בזיכרון, על העותק שכבר נקרא ─────────────────── */
{
  /*  ⛔ המוטציה סורקת את הקובץ שנגעה בו בלבד — ⚠️ השער רץ שוב כריצה
   *  פנימית בכל מוטציה של הרתמה, ⭐ וסריקת העץ כולו בכל מוטציה הכפילה
   *  את זמנו: ⛔ מה שנמדד כאן הוא שהמספר שהוזרק נתפס, ⚠️ והעץ כולו כבר
   *  נסרק בטענה שמעל. */
  const withF = (rel, fn) => ({ [rel]: fn(ROUND_FILES[rel]) });
  /*  ⛔ מוטציה: «(סבב N)» בהערה — ⚠️ המקום שבו הוא נולד. */
  t(roundNums(withF('index.html', (s) => s.replace('<script type="module">',
      '<script type="module">\n// ⛔ אין לשנות (סבב 12) — הסדר נמדד'))).length > 0,
    'מ[round-num] · «(סבב N)» בהערת קוד **מפיל** את [round-num]');
  /*  ⛔ מוטציה: «(סבב N)» בבאנר של בלוק חתום — ⚠️ התבנית הקודמת בדיוק. */
  t(roundNums(withF('core/sync.js', (s) => s.replace('— מודול משותף ═', '— מודול משותף (סבב 12) ═'))).length > 0,
    'מ[round-num] · «(סבב N)» בבאנר של בלוק חתום **מפיל** את [round-num]');
  /*  ⛔ מוטציה: «(סבב N)» בפרוזה של README — ⚠️ התיעוד נסרק כמו הקוד. */
  t(roundNums(withF('README.md', (s) => s + '\nהמטמון מקודם בכל שינוי (סבב 12).\n')).length > 0,
    'מ[round-num] · «(סבב N)» ב-README **מפיל** את [round-num]');
  /*  ⛔ מוטציה: זנב שעבר לראש העמודה — ⚠️ הוא נקרא כחלק מהפסוקית שאחריו. */
  const NO = ' · ⛔ **ואין מספר סבב** — ⚠️ הוא זמן, ולא נימוק';
  const r2 = DOC.split('\n').find((l) => l.startsWith('| 2 | ') && l.includes(NO));
  const moved = r2 ? r2.replace(NO, '').replace(/^(\| 2 \| [^|]* \| )/, '$1' + NO.slice(3) + ' · ') : '';
  t(!!r2 && tableTails(DOC.replace(r2, moved)).length > 0,
    'מ[round-tail] · זנב שהוזז מסוף עמודת התקן **מפיל** את [round-tail]');
  /*  ⭐ מוטציית-נגד: הודעת קומיט במחרוזת ⛔ אינה מפילה — ⚠️ היא נתון,
   *  ⭐ והסבב בה הוא הקישור בין קומיט לתקן. */
  t(roundNums(withF('tools/test_removals.mjs', (s) => s +
      "\nconst _commitMsg = 'סבב 172 — ההצהרה נמדדת';\nvoid _commitMsg;\n")).length === 0,
    'נ[round-num] · ⭐ הודעת קומיט במחרוזת ⛔ **אינה** מפילה');
  /*  ⭐ מוטציית-נגד: כותרת פרק סבב ⛔ אינה מפילה — ⚠️ היא מותרת בהגדרה. */
  t(roundNums(withF('CLAUDE.md', (s) => s + '\n## סבב 999 (2099-01-01) — פרק נוסף\n- גוף.\n')).length === 0,
    'נ[round-num] · ⭐ כותרת פרק סבב ⛔ **אינה** מפילה');
}
...
/*  ⛔ הרצת `check-capabilities` על העותק — ⚠️ הטענה על עמודת
...
/*  ⛔ מוטציה שמכוונת לשער אחר — ⚠️ «פרק שחופף לשורה בטבלה»
...
const okRow = (doc) => new RegExp('^\\| (\\d+) \\|(?:[^\\n|]*\\|){2}(?: ✅ \\|){' +
  PEERS.length + '}\\s*\\|$', 'm').exec(doc);
t(!!okRow(DOC), 'מ18 · נמצאה שורה ✅✅✅✅ עם הערה ריקה למוטציה');

if (!INNER) {

  /*  ⛔ עשר קריאות רתמת המוטציה יושבות מתחת לסוגר — ⚠️ כל
...
/*  ⛔ מוטציה: הערת נימוק על שורה ✅✅✅✅ — ⚠️ היא מתארת מצב
...
/*  ⛔ מוטציה: קריאת רתמה שהוזזה מעל סוגר הריצה הפנימית —
...
/*  ⭐ מוטציית-נגד: אותה קריאה בדיוק **מתחת** לסוגר ⛔ אינה מפילה —
...
/*  ⛔ מוטציה: `every` בלי שומר גודל — ⚠️ `[].every()` הוא `true`,
...
/*  ⛔ מוטציה על שדה שאינו קיים — ⚠️ בדיוק הכשל שנמדד: הטענה
...
/*  ⛔⛔ ספירה שנמדדה בכלי חיצוני — ⚠️ «נמדד ב-<כלי>» ורשימת
...
/*  ⛔ קריאת רתמה אחת לדפוס ⛔ ולא שתיים — ⚠️ `hit` היא
...
/*  ⛔ השורה נגזרת ⛔ ואינה מוקלדת — ⚠️ מספר קשיח נסחף בכל
...
/*  ⛔⛔ פתיחת ההערה — ⚠️ שלוש פתיחות בלבד: «הבדל מכוון» ·
...
const openRow = DOC.split('\n').find((l) => /^\|\s*\d+\s*\|/.test(l) &&
  /^[\s*⛔⚠️⭐️\uFE0F]*נמדד/.test((l.split('|')[COL_NOTE] || '').trim()));
...
/*  ⛔⛔ הפתיחה מודגשת — ⚠️ שלוש הפתיחות חיו בשתי צורות,
 ...
 *  ⚠️ מפקד נגזר מהמוצר וישתנה איתו, ⛔ ומי שיוסיף שדה או
...
t(!commentsFails({ 'CLAUDE.md': DOC.replace(noteRow,
   ...
   *  ⚠️ זה ההיקף שנוסף, ⛔ ומוטציה על `sw.js` לבדו לא הייתה
   *  מודדת אותו. */
  ...
  /*  ⭐ מוטציית-נגד: אותו משפט בלי מספר ⛔ אינו מפיל — ⚠️ המצביע הוא
   *  מה שנמדד, ⛔ ולא עצם ההערה. */
  t(!fails({ [GATE]: rd(GATE).replace(' *\n', ' *  ⚠️ הרכיב נאכף כאן (בשער הזה).\n *\n') }),
    'נ13 · ⭐ «נאכף בשער הזה» באותו מקום ⛔ **אינו** מפיל');

  /*  ⛔ מ23 — הערה שנוקבת בתוכן המבנה שמתחתיה מפילה את ב3:
   *  ⚠️ «ריקה בכל הריפו» נכונה עד הפריט הראשון שייכנס, ⛔ ואיש אינו
   *  חוזר לתקן אותה. */
  t(commentsFails({ 'tools/test_bump.mjs': rd('tools/test_bump.mjs')
      + '\n/*  ⚠️ נמדד: ריקה בכל הריפו. */\nconst _r97c = [];\nvoid _r97c;\n' }),
...
/*  ⛔ המוטציה כותבת הפניה למספר שורה — ⚠️ בדיוק הצורה שנמחקה
...
t(!fails({ 'sw.js': rd('sw.js') + '\nconst _r72 = (n) => `שורה ${n} נמדדה`;\n' }),
  ...
  /*  ⛔ ד — קובץ תיעוד אינו מסביר כלל שהטבלה אוכפת. */
  ...
  t(fails({ 'index.html': rd('index.html').replace(
      '— מודול משותף', "— מודול משותף. ר' CLAUDE.md") }),
...
/* ── מה אינו נאכף — ⛔ ונרשם כאן במפורש ──────────────────────────────────────
   ⚠️ שער שמובן לא נכון גרוע משער שאינו קיים, ולכן חמשת אלה נרשמים:
     · **ממצא 16 — מספר במקום שם קבוע.** ⛔ אינו ניתן לאכיפה: «12 רשומות»
       בהערה יכול להיות קבוע שקיים ויכול להיות מדידה חד-פעמית, וההבחנה
       היא קריאת משמעות. סריקה גורפת הייתה מפילה כל תאריך ומספר.
     · **תאריך או גרסה בהערה.** ⛔ `[round-num]` נוקב במילה «סבב» בלבד —
       ⚠️ «ב-2026-09-15» ו-«v2» הם אותו זמן בלבוש אחר, ⭐ ואינם נמדדים:
       ⛔ תאריך הוא גם עובדה קבועה (תאריך החלפת מפתח), ⚠️ וההבחנה היא קריאת
       משמעות. ⚠️ ומחוץ לסריקה בשמם: תיקיית המיגרציות, ⛔ שמיגרציה שרצה אינה
       נערכת · ⭐ ושני קובצי הארכיון שב-`tools/`, ⛔ שהם לקחי העבר בכוונה.
...
console.log(fail ? `\n✗ כללים ותוכן הקבצים — ${fail} נכשלו, ${pass} עברו`
                 : `\n✓ כללים, תוכן הקבצים וכללי הברזל 21–24 — ${pass} טענות עברו`);
