#!/usr/bin/env node
/*  test_stage_a.mjs — הגיבוי היומי ויומן הפעולות (מודול משותף).
 *
 *  **מה נאכף:** הליבה המשותפת נחתכת מ-`index.html` בין סמני המודול ורצה
 *  ב-`vm` מעל localStorage, DOM ולקוח Supabase מדומים — ⛔ ומוטציה בקוד
 *  האמיתי מפילה טענה, ⛔ ולא בעותק שנכתב לבדיקה.
 *
 *  **הנימוק המדוד:** הגיבוי של אחת האפליקציות הפסיק לרוץ **25 יום** ואיש
 *  לא ידע — ⚠️ הקוד היה זהה, ⛔ ונקודת ההפעלה נבדלה.
 *
 *  **מה יישבר בלעדיו:** ⛔ `supabase-js` **אינו זורק** ומחזיר
 *  `{data, error}` — ⚠️ ובלי רתמה שמשקפת את החוזה הזה כל טענת «נכשל בשקט»
 *  נבדקת מול חוזה שאינו קיים, ⛔ וה-`error` המפורש אינו נבדק כלל.
 *
 *  **מה אינו נאכף כאן:** ⛔ קיומה של טבלת הגיבוי במסד — ⚠️ היא עובדת מסד
 *  שאינה נראית מהריפו, ⭐ ומוחרגת במפורש ברשימת עובדות המסד.
 *
 *  ⚠️ ההמתנה היא ל**אירוע** ולא לשעון: כל מסלול נאחז ב-`await` על ה-Promise
 *  שהמודול מחזיר, ⛔ ו-`waitFor` נכשלת ברעש בתום תקרה במקום לעבור בשקט.
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════
   ⭐ הכשל שנמדד: מקור-טבלה שאינו קיים החזיר `error` ⇒ `ok=false` ⇒
...
mutStage();
if (!RUN_MUT) {
  console.log('\n⏭ test_stage_a: המוטציות רצות ברמה המלאה (--full) — ⛔ ואינן נמדדות כאן');
