-- ═══════════════════════════════════════════════════════════════════════════
-- 008 — `student_id` מ-`smallint` ל-`text` בשתי טבלאות השורות
-- ═══════════════════════════════════════════════════════════════════════════
-- ✅ **הורצה ואומתה ב-2026-08-18** ע"י המנהל בפרויקט המשותף, בשם
--    `hanhala_008_student_id_to_text`.
--
-- ⭐ **מה שהוליד אותה:** `005` הגדירה `student_id smallint` לפי מדידה שהייתה
--    נכונה באותו יום — מזהי התלמידים היו ≤ 71, כי הם הוקצו ב-`maxId++`.
--    באותו יום עצמו סבב 37 החליף את הקצאת המזהים ל-**uuid שנוצר במכשיר**
...
--
-- ⛔ **ההכרעה: המרת העמודה הקיימת, ולא הוספת `student_key` לצידה** (סבב 37א) —
...
-- ---------- אימות (נמדד ב-2026-08-18, `SELECT` בלבד) ----------
