-- ============================================================================
-- 013_kv_updated_at.sql — `updated_at` לשלוש טבלאות ה-kv
-- ============================================================================
--
...
--
-- ── מה זה פותר ─────────────────────────────────────────────────────────────
-- נבדק מול המסד, ולא הונח: `sl_settings` ו-`g_config` — שתי
...
--
-- ⚠️ **ומה שמראה עד כמה הפיצוי שביר:** `kv.ys_settings_meta` מחזיק
-- **`{}` — שני בתים** (נבדק 26.8). כלומר `rts` הוא תמיד 0, התנאי
...
-- 760 ברמת אביב — נבדק), ⛔ ואין להסירו.
--
...
-- ---------- 2. הטריגר ----------
-- ⛔ פונקציה אחת לשלוש הטבלאות — שלוש הגדרות לאותה לוגיקה הן שלוש
-- הזדמנויות שאחת מהן תיסחף.
...
-- ---------- 3. הרשאות ----------
-- ⛔ הפונקציה נולדת עם `execute` ל-`public` ואינה יורשת דבר — ⚠️ ולכן
--    הגריעה מפורשת ואינה נסמכת על ההרשאות של הטבלה.
