...
const APP = {
  ...
  file: 'index.html',
  /*  ⚠️ אין כאן בלוק משותף שנעדר (סבב 52). */
  skipBlocks: [],
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
/* ───────────────────────────────────────────────────────────────────────────
   ו · ⛔ כל מבחן נכנס עם מוטציה — או עם נימוק כתוב (סבב 67)
   ───────────────────────────────────────────────────────────────────────────
   ⛔ מה נאכף: `tools/test_*.mjs` מכיל «מוטציה» **וגם** «מוטציית-נגד», או
   נימוק מפורש מדוע אינו ניתן למוטציה. ⛔ ולמה זה יכול להישבר: מבחן
   שעובר על עץ תקין ואינו מסוגל ליפול נראה כרשת ביטחון ופועל כאישור.
   ──────────────────────────────────────────────────────────────────────── */
{
  let bad = 0, seen = 0, noAnti = 0;
  let names = [];
  ...
  for (const f of names) {
    ...
    seen++;
    const hasMut  = t.indexOf('מוטציה') >= 0;
    const hasAnti = t.indexOf('מוטציית-נגד') >= 0;
    /*  ⚠️ נימוק מפורש הוא מסלול חוקי — ⛔ אך הוא נקרא בעין ואינו
     *  מדלג בשתיקה (סבב 67) — «אין כאן מה למוטט» חייב להיאמר. */
    const excused = /אינו ניתן למוטציה/.test(t);
    if (excused) continue;
    if (!hasMut) {
      bad++;
      fail(`tools/${f}: אין מוטציה — מבחן נכנס עם מוטציה, או עם נימוק ` +
           'כתוב «אינו ניתן למוטציה» (כלל ברזל 11, סבב 67)');
    }
    if (!hasAnti) noAnti++;
  }
  /*  ⛔ מוטציית-נגד נמדדת ומדווחת ⛔ ואינה מפילה עדיין (סבב 67) — ⚠️ 85
   *  מ-164 המבחנים נכתבו לפניה, והפלה רטרואקטיבית הייתה חוסמת כל דחיפה
   *  עד שהם ייכתבו מחדש. ⛔ מבחן **חדש** נכתב עם שתיהן; הסגירה של
   *  הקיימים היא סבב ייעודי, ⛔ והמספר כאן הוא מה שיאמר מתי הוא נדרש. */
  if (!bad) pass(`מוטציות: ${seen} מבחנים, לכל אחד מוטציה` +
                 (noAnti ? ` (⚠️ ${noAnti} עדיין בלי מוטציית-נגד)` : ''));
}
