...
const APP = {
  ...
  gateNoRows: {
    'check-structure.mjs':     'המבנה הקנוני של הריפו — ⚠️ יכולת שכל האפליקציות נושאות: ⭐ קיום הקבצים ומיקומם, ⛔ והטבלה מודדת תוכן ⛔ ולא נוכחות',
    'test_backup_policy.mjs':  'מדיניות הגיבויים במסד — ⚠️ יכולת שחיה בכל אפליקציה שיש לה מסד: ⭐ שמירה, גריעה, ופינוי לילי',
    'test_budget.mjs':         'תקציב התיעוד — ⚠️ יכולת משותפת לכולן: ⭐ תקרת שורות לכל אחד מקובצי הלוויין',
    'test_caps_build.mjs':     'מעטפת האנדרואיד, הנכסים והמסד — ⚠️ יכולת משותפת לכולן, ⭐ וחלק נושאי בפיצול שנדרש מתקרת הגודל',
    'test_caps_guard.mjs':     'קלט, אבטחה וניקוי שאריות — ⚠️ יכולת משותפת לכולן, ⭐ וחלק נושאי בפיצול שנדרש מתקרת הגודל',
    'test_caps_ui.mjs':        'מסך, עיצוב ודפוסי מימוש — ⚠️ יכולת משותפת לכולן, ⭐ וחלק נושאי בפיצול שנדרש מתקרת הגודל',
    'test_date.mjs':           'מנוע התאריך העברי ואי-הנפילה ל«היום» — ⚠️ יכולת שחיה במי שיש בה צרכן תאריך',
    'test_devid.mjs':          'מזהה המכשיר היציב לרישום ביומן הפעולות — ⚠️ יכולת משותפת לכולן, ⭐ וגוף המודול אות-באות',
    'test_hotwin.mjs':         'החלון החם — ⚠️ יכולת משותפת לכולן: ⭐ צמצום העותק המקומי, אופק הפינוי, והשחזור מהענן',
    'test_icons.mjs':          'נכסי האייקונים והמאסטר — ⚠️ יכולת משותפת לכולן: ⭐ מחולל אחד שמייצר את כל הנכסים מאפס',
    'test_idarg.mjs':          'העברת מזהה ל-DOM — ⚠️ יכולת משותפת לכולן: ⭐ `data-id` ודלגציה, וההשוואה ב-`idEq`',
    'test_ids.mjs':            'מזהי רשומות שנוצרים במכשיר — ⚠️ יכולת משותפת לכולן, ⭐ וגוף המודול אות-באות',
    'test_kvmeta.mjs':         'חותמת רעננות פר-מפתח שנכשלת סגור — ⚠️ יכולת שחיה במי שנושאת שכבת חותמת פר-מפתח',
    'test_lists.mjs':          'שקילות שתי רשימות המודולים המשותפים — ⚠️ יכולת משותפת לכולן: ⭐ צד שני של אותה מדידה',
    'test_lock.mjs':           'נעילת חוסר-הפעילות — ⚠️ יכולת שחיה בכל אפליקציה שיש בה כניסה, ⭐ וגוף המודול אות-באות',
    'test_md.mjs':             'שלד קובצי ה-md הנלווים — ⚠️ יכולת משותפת לכולן: ⭐ הפסקאות עצמן ומקומן הקנוני',
    'test_offline_login.mjs':  'כניסה אופליין מרובת-משתמשים — ⚠️ יכולת שחיה בכל אפליקציה שיש בה כניסה',
    'test_merge_pending.mjs':  'ליבת המיזוג, הגנת ה-⏳ ועידן הנתונים — ⚠️ יכולת משותפת לכולן, ⭐ וגוף המודולים בארגז חול',
    'test_pendflush.mjs':      'הניסיון החוזר ואישור ה-⏳ בריקון התור — ⚠️ יכולת משותפת לכולן, ⭐ וגוף המודול אות-באות',
    'test_pull.mjs':           'מנגנון המשיכה המאוחד — ⚠️ יכולת משותפת לכולן: ⭐ משיכה ⟵ מיזוג ⟵ דחיפה, פר-קטגוריה',
    'test_read.mjs':           'מקור הקריאה — טבלאות בלבד: ⚠️ יכולת שחיה במי שקוראת בעימוד, ⭐ וכשל שמחזיר «אין ראיה»',
    'test_roles.mjs':          'מודל ההרשאות — ⚠️ יכולת שחיה בכל אפליקציה שיש בה כניסה: ⭐ הדרגה נגזרת מטבלת המשתמשים',
    'test_session.mjs':        'מודל הסשן — ⚠️ יכולת שחיה בכל אפליקציה שיש בה כניסה, ⭐ וגוף המודול אות-באות',
    'test_shell.mjs':          'מעטפת ה-WebView — ⚠️ יכולת משותפת לכולן: ⭐ הפעילות המשגרת, הירושה, ואפס סימן TWA',
    'test_stage_a.mjs':        'הגיבוי היומי ויומן הפעולות — ⚠️ יכולת משותפת לכולן, ⭐ וגוף המודול אות-באות',
    'test_swcore.mjs':         'ליבת ה-service worker וקו הבסיס ההתנהגותי — ⚠️ יכולת משותפת לכולן, ⭐ וגוף הקובץ אות-באות',
    'test_users_patch.mjs':    'נתיב העדכון החלקי למראת המשתמשים — ⚠️ יכולת שחיה בכל אפליקציה שיש בה כניסה',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
 ...
 *  רשימה שנייה בבודק. */
export const ROWS = [57, 50, 52, 51, 54];
...
if (RUN_MUT) {
  ...
  const MUT = [
    ...
    /*  ⛔ מ4ב — נימוק שהוא מיקום. ⚠️ המוטציה מחליפה את היכולת בניסוח
    ...
    { m: 'מ4ב', lbl: 'נימוק שהוא מיקום', claim: 'ה2',
      run: () => {
        ...
        const bad = silentGates(C0).located.length > 0;
        APP.gateNoRows[g] = saved;
        return bad;
      } },
  ];
