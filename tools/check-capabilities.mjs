...
const APP = {
   ...
   *  חברות על המקור כולו; ⛔ **ומה מפיל**: רשומה שאינה כאן, הכרזה שאין לה
   *  probe, ונימוק קצר משש מילים. ⭐ **ולמה המבנה קיים**: «המחרוזת קיימת»
   *  מאשר גם גוף שבו היא במקום הלא נכון, ⚠️ והנימוק אומר **מה המיקום
   *  הנכון ולמה נוכחות מספיקה שם**. */
  presenceOnly: {
    '79|`Enter` שומר בכל שדה עריכה':
      'המיקום הוא `ksFire` והמאזין הגלובלי, ושניהם נמדדים בעוגן משני הצדדים — ' +
      'ומה שנשאר נוכחות הוא `<form>` במסמך, שהדפדפן מפעיל בכל מיקום',
    '163|מראת המשתמשים':
      'המיקום הוא קבוע מפתח המראה, והוא ליטרל יחיד בקובץ — ' +
      'מיקומה של הגדרת קבוע אינה משנה את השם שהוא נושא',
    '162|מודל ההרשאות':
      'המיקום הוא `isAdmin` שבבלוק החתום, והוא נמדד בשורה שמעליו — ' +
      'וכאן נמדד אוצר המילים, ששם דרגה בכל מיקום הוא דרגה שהאפליקציה מכריעה לפיה',
    '106|ספרייה חיצונית — גרסה מוצהרת':
      'הסמל הגלובלי נקרא מאתרי קריאה רבים ואין לו מיקום אחד נכון — ' +
      'והטענה היא בדיוק «נקרא לפחות פעם אחת», שספרייה בלי קורא היא טעינה מבוזבזת',
    '107|ייצוא והנפקה — דרך מוצהרת':
      'המיקום הוא הפונקציות שמוצהרות ב-`APP.exportFns`, והמדידה עברה אליהן — ' +
      'והדרך היא תכונה של האפליקציה כולה, שדרך שנייה בכל מיקום היא ההפרה',
    '148|הגיבוי שלם ומעומד':
      'המיקום הוא לולאת הקריאה של הגיבוי, ובקשת עימוד בכל מקום אחר — ' +
      'היא מימוש עימוד שני, שהשורות «מנגנון אחד» כבר אוסרות',
    '155|ברירת מחדל אינה ממוזגת':
      'המיקום הוא אתר קריאת חותמת האיפוס, וקורא בכל מיקום הוא קורא — ' +
      'והנמדד הוא שמפתח מוצהר אינו נשאר בלי אף קורא',
    '169|סודות אינם עוזבים את המכשיר':
      'המדידה כאן היא דו-צדדית וזה מיקום בתחפושת — ' +
      'הדפוס אינו בגוף פונקציית הסינון והוא כן במקור, ולכן הסוד עובר דרכה',
    '153|מחיקה רכה בלבד — אין `DELETE` פיזי':
      'לאיסור אין מיקום נכון, שהוא טענת היעדר — ' +
      'וכל מופע של מחיקה פיזית בכל מיקום הוא ההפרה עצמה',
    '168|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי':
      'לאיסור אין מיקום נכון, שהוא טענת היעדר — ' +
      'ולצידו נמדד ששם פונקציית הכתיבה נושא קורא חי, והיא נקודת המעבר האחת',
    '70|מסך מציג מיד ומרענן ברקע':
      'המיקום הוא מסלול הרינדור, והוא נמדד בסדר — הציור לפני ה-`await` הראשון — ' +
      'ומה שנשאר נוכחות הוא עצם קיום המסלול, שמסך בלי משיכה אינו נמדד כאן',
    '122|האפליקציה ניתנת להתקנה מכרום':
      'המיקום הוא ה-`head` לקישור המניפסט והבלוק החתום לרישום ה-sw — ' +
      'והתקנה היא תכונה של המסמך, שכרום עצמו מכריע בה לפי נוכחות ולא לפי מיקום',
    '76|טיפול באירועים — דלגציה ממאזין אחד':
      'המיקום הוא `document`, והדפוס נושא את ההורה בשמו — ' +
      'ולצידו נמדדת השוואת המונים, דלגציה מול מטפלים מוטבעים',
    '41|ארבעת מימדי הסריקה':
      'המיקום הוא מסך הטבלה שצורך את ההצהרה, והוא הקורא היחיד — ' +
      'ולצידו נמדד שמספר המימדים הוא ארבעה, והמספר הוא הטענה',
    '161|מבנה טופס הכניסה':
      'לטענה אין מיקום, שהיא היעדר `<form>` במסמך כולו — ' +
      'ו-`<form>` בכל מיקום משנה את התנהגות המקש, ולכן הסריקה גורפת',
    '167|אימות מול טביעה בענן':
      'המיקום הוא מסלול הכניסה המקוונת, והוא נמדד בשורה שמעליו — ' +
      'וכאן נמדד שהפונקציה שהמאמת נוקב בשמה נקראת בפועל, שקריאה בכל מיקום היא קורא חי',
    '185|מיכל באנר העדכון במקור':
      'המדידה היא דו-צדדית וזה מיקום בתחפושת — ' +
      'המיכל נמצא בסימון המסמך ואינו נבנה בקוד, והסריקה עברה למקור המולבן',
    '185|עדכון אוטומטי — בדיקה מחזורית':
      'המיקום הוא הבלוק החתום של ההרשמה, וגופו מושווה בית-לבית — ' +
      'והטענה «אין מנגנון זיהוי שני» היא היעדר, שהמקור כולו הוא השׂחת הנכון לה',
    '67|העברת מזהה ל-DOM':
      'המיקום הוא מטפל הדלגציה, והשורה נמדדת בספירת ההטבעות ב-`onclick` — ' +
      'והנוכחות מאשרת רק שהמנגנון קיים, בעוד האפס הוא הטענה',
    '68|מיון אחד לכל התצוגות':
      'המיקום הוא הגדרת הפונקציה שמוצהרת ב-`APP.sortFn`, והדפוס נבנה מהשם המוצהר — ' +
      'ו«מיון אחד» נמדד בספירת המשווים המקומיים בשאר המסכים',
    '74|שכבת המודאל':
      'המיקום הוא הגדרת `openModal`, ולהגדרת פונקציה יש אתר אחד — ' +
      'והחתימה עצמה היא הנמדד, ששם כפול נתפס בשורת הפונקציות בלי קוראים',
  },
  ...
  scanKind: {
    ...
    'test_dbfacts': 'raw — משווה שמות עמודות וטבלאות למסד החי, וכולם מחרוזות',
    ...
    'test_schema_source': 'raw — מודד `create table` — טקסט SQL בתוך מחרוזות',
  ...
  gapRows: [41, 119, 123, 145, 59, 132, 93],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    165: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    164: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    130: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    161: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '79|`Enter` שומר בכל שדה עריכה': 'src',
    '80|מאזין אחד, בהאצלה מ-`document`': 'src',
    '81|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '161|מבנה טופס הכניסה': 'src',
    '132|שם נגזר אינו נשמר ברשומה': 'src',
    '177|מפתח אחסון בלי קורא': 'src',
    '33|שער מריץ את כל טענותיו': 'tools',
    '86|שכבת המראה': 'src',
    '178|הגירה מקומית שהושלמה': 'mixed',
    '163|מראת המשתמשים': 'src',
    '162|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'tools',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '146|הקשר נלכד בכניסה לפונקציה': 'src',
    '89|פריסה במסכי טלפון וטאבלט': 'src',
    '90|ערכת נושא — בהיר וכהה': 'src',
    '91|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '92|אין סגנון מוטבע': 'src',
    '93|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '173|הערה שמתארת מצב שחלף': 'mixed',
    '179|שאילתת `@media` מתה': 'src',
    '106|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '107|ייצוא והנפקה — דרך מוצהרת': 'src',
    '94|המסמך מתפרסר נקי': 'src',
    '95|לוג מסלול — תג ורמה': 'src',
    '75|מיכל המודאל ילד ישיר של `body`': 'src',
    '102|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '148|הגיבוי שלם ומעומד': 'src',
    '71|פעולה מגיבה מיד': 'src',
    '72|משיכה מסוננת בשרת': 'src',
    '118|דגימת-יתר במחולל': 'tools',
    '165|`PBKDF2` — פרמטרים': 'src',
    '136|`migrations/` — תוכן': 'src',
    '137|`migrations/` — כמות': 'src',
    '154|מיזוג מכל — מפה ומערך': 'src',
    '155|ברירת מחדל אינה ממוזגת': 'src',
    '156|רשימה אינה נושאת פריט כפול': 'src',
    '149|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'tools',
    '175|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'tools',
    '169|סודות אינם עוזבים את המכשיר': 'src',
    '143|דפוס `upsert` — onConflict מוכרז': 'src',
    '167|טביעה חסרה מושלמת אוטומטית': 'src',
    '167|כניסה אופליין': 'src',
    '129|עריכת נתונים אופליין': 'src',
    '153|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '168|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '69|אובייקט יקר נבנה פעם אחת': 'src',
    '70|מסך מציג מיד ומרענן ברקע': 'src',
    '122|האפליקציה ניתנת להתקנה מכרום': 'src',
    '171|דגלי מעבר — אין דגל דלוק': 'src',
    '76|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '164|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '41|ארבעת מימדי הסריקה': 'src',
    '85|פינוי אוטומטי': 'src',
    '85|אופק הפינוי נוקה מהסנכרון': 'src',
    '158|אימות פינוי מול הענן': 'src',
    '123|שיתוף קבצים': 'src',
    '108|מעטפת APK (WebView)': 'src',
    '111|אין נכסים מוטבעים': 'src',
    '127|מפתח חתימה קבוע בריפו': 'src',
    '133|מקור אמת יחיד לסכימה': 'src',
    '134|קובץ התקנה מלא': 'src',
    '55|גיבוי יומי אוטומטי': 'src',
    '55|יומן פעולות': 'src',
    '130|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '77|דפוס הודעת שגיאה יחיד': 'src',
    '82|`toast` — חתימה, גוף ומחלקות': 'src',
    '82|סיווג ההודעה נגזר מהמסלול': 'src',
    '185|מיכל באנר העדכון במקור': 'src',
    '185|הרענון מ-controllerchange בלבד': 'src',
    '185|סימן דחיית הבאנר מתמיד': 'src',
    '103|רישום כשלי כתיבה': 'src',
    '56|חלון חם במכשיר': 'src',
    '56|שחזור מקומי מהענן': 'src',
    '161|מסך שינוי סיסמה עצמי': 'src',
    '187|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '57|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '167|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '58|מודול מזהי רשומות': 'src',
    '58|מזהה אינו מומר למספר': 'src',
    '60|מזהה מכשיר': 'src',
    '57|ליבת מיזוג משותפת': 'src',
    '108|מעטפת WebView חתומה': 'tools',
    '167|אימות מול טביעה בענן': 'mixed',
    '125|בניית APK אחידה עם שער חתימה': 'src',
    '187|מודול ה-service worker': 'src',
    '61|ניסיון חוזר בתור הסנכרון': 'src',
    '62|מנגנון משיכה אחיד': 'src',
    '65|נעילת חוסר-פעילות': 'src',
    '66|מודל הסשן — בזיכרון בלבד': 'src',
    '186|בדיקת עדכון תקופתית ל-service worker': 'src',
    '87|פסק זמן אחיד לקריאות רשת': 'src',
    '88|מאזיני מצב רשת': 'src',
    '152|גריעת tombstones לפי גיל': 'src',
    '185|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '123|גשר שיתוף': 'src',
    '67|העברת מזהה ל-DOM': 'src',
    '68|מיון אחד לכל התצוגות': 'src',
    '73|ייצוא — שני דפוסים מוצהרים': 'src',
    '115|שכבת אייקונים': 'src',
    '159|שכבת קלט אחידה': 'src',
    '159|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '74|שכבת המודאל': 'src',
    '161|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [29, 40, 42, 44, 53, 84, 47, 157, 99, 139, 54, 22, 59, 25, 32, 35];
...
const CAPS = {
  ...
  toastcss: {
    ...
    block: { sha: '03bdbb7cfeb951c4', lines: 14,
             css: true,
  ...
  bchartcss: {
    ...
    block: { sha: '34229c243b21f678', lines: 18,
             css: true,
  ...
  bchart: {
    ...
    docRows: ['גרף נבנה מ-CSS ולא מספרייה'],
    block: { sha: 'dcfc58fa4a128aba', lines: 26,
             start: '/* ═══ גרף העמודות — מודול משותף (סבב 137)',
             end:   '/* ═══════════════ סוף מודול גרף העמודות' },
  },
  /*  ⭐ סבב 138 — סולמות הגודל, הריווח והרדיוס. ⚠️ **מה נכנס**: שלוש
   *  קבוצות האסימונים והמחלקות שחיות בארבעתן; ⛔ **ומה מפיל**: סטייה
   *  בטקסט. ⭐ **ולמה הוא חתום**: דרגה שנבדלת באחת מהן היא סולם שני,
   *  ⛔ ומחלקה שנבדלת היא אותו שם עם שתי משמעויות. */
  scales: {
    name: 'CSS סולמות הגודל, הריווח והרדיוס',
    docRows: ['סולם אחד לגודל, לריווח ולרדיוס'],
    block: { sha: '286234fe2cbc979a', lines: 32,
             css: true,
             start: '/* ═══ סולמות הגודל, הריווח והרדיוס — מודול משותף (סבב 138)',
             end:   '/* ═══ סוף CSS הסולמות' },
  },
  bp: {
  ...
  storage: {
    ...
    block: { sha: 'b2d221a093c63855', lines: 609,
  ...
  busyguard: {
    ...
    block: { sha: '074d09aa4aa42d49', lines: 44,
  ...
  savepipe: {
    ...
    block: { sha: '9ac0bcb7dbbba571', lines: 43,
  ...
  pending: {
    ...
    block: { sha: '00defda3c8f43f28', lines: 285,
  ...
  backup: {
    ...
    block: { sha: '43cf21ae7ac7fc26', lines: 304,
  ...
  hotwin: {
    ...
    docRows: ['`hw` — חלון חם'],
    block: { sha: 'e988a4d682d10880', lines: 152,
  ...
  mergecore: {
    ...
    block: { sha: 'ee09592457ee72f4', lines: 76,
  ...
  ids: {
    ...
    block: { sha: 'f4f72b9724b9b0cb', lines: 47,
  ...
  pull: {
    ...
    block: { sha: '53fe9d3d090f3fe4', lines: 82,
   ...
   *  ⭐ מה שנאכף כאן הוא הליבה עצמה, שנכתבה בארבע צורות בלי שאיש מדד. */
  push: {
    name: 'מודול שכבת הדחיפה',
    docRows: ['`push` — שכבת הדחיפה'],
    block: { sha: '806f45e0a69d0ccf', lines: 114,
   ...
   *  אינה מוסרת שם מחלקה — ⚠️ אותן חמש קריאות בארבעתן, ⭐ ושתיים מהן
  ...
  uihelp: {
    ...
    block: { sha: '5d39a3b287ab4686', lines: 123,
  ...
  readnum: {
    ...
    docRows: ['שדה מספרי'],
    block: { sha: '25c3d73651f69250', lines: 21,
  ...
  keysave: {
    ...
    block: { sha: 'c6e3790e32d72a12', lines: 34,
  ...
  swreg: {
    ...
    block: { sha: 'bd833a81add22ec8', lines: 97,
  ...
  swcore: {
    ...
    docRows: ['ליבת `sw.js`'],
    block: { file: 'sw.js', sha: 'e4acf3de63aac685', lines: 250,
 ...
 *  זהה בארבעתן, ⛔ ואינה מוכיחה שיש מי שיודע שהוא קיים: ⭐ בלוק שאף שורה
...
const CAT_ORDER = [
  ...
  { g: 'ג', phases: [{ p: 'מבנה', w: ['עמודת התקן', 'מספור הטבלה'] },
                     { p: 'סריקה', w: ['הסריקה'] },
                     { p: 'הערות', w: ['ההערות', 'בהערות'] },
                     { p: 'סימון', w: ['הסימון', 'נרשם ככזה'] }] },
  { g: 'ד', phases: [{ p: 'לפני', w: ['מדידה לפני'] },
                     { p: 'תוך כדי', w: ['סריקת האזור'] },
                     { p: 'אחרי', w: ['הסרה אינה', 'git —'] }] },
  { g: 'ה', phases: [{ p: 'הכלל', w: ['חתימות sha256'] },
                     { p: 'ליבת הסנכרון', w: ['`pend`', '`bk`', '`hw`', '`merge`', '`ids`',
                                              'מנוע תאריך עברי', '`devid`', '`rty`', '`pull`',
                                              '`push`', 'כתיבת משתמש'] },
                     { p: 'סשן', w: ['`lock`', '`sess`'] }] },
  { g: 'ו', phases: [{ p: 'נתונים', w: ['העברת מזהה', 'מיון אחד', 'אובייקט יקר'] },
  ...
  { g: 'ח', phases: [{ p: 'מעטפת', w: ['מעטפת'] },
                     ...
                     { p: 'בנייה', w: ['`workflows`', '`sign-apk.sh`', '`versionCode`',
                                        'מפתח חתימה'] }] },
  { g: 'ט', phases: [{ p: 'מקור', w: ['מקור הקריאה', 'נתונים בטבלאות מובנות'] },
  ...
  { g: 'י', phases: [{ p: 'קלט', w: ['שדה מספרי', 'שדות סיסמה'] },
                     { p: 'כניסה', w: ['שכבת כניסה', 'מודל ההרשאות', 'מראת המשתמשים',
                                        '`PBKDF2`', 'מלח אקראי', 'כניסה אופליין'] },
                     { p: 'סודות', w: ['סיסמאות בענן', 'סוד'] }] },
  { g: 'יא', phases: [{ p: 'תהליך', w: ['דגלי מעבר', 'שערי מעבר'] },
                      { p: 'קוד', w: ['הערה שמתארת', 'קבצים בלי קוראים', 'פונקציה בלי קוראים',
                                       'מחלקות CSS', '`@media`'] },
                      { p: 'תשתית', w: ['עמודה בלי קורא', 'רשימת-היתר', 'מיגרציה מאוחרת',
                                         'ענפים שלא מוזגו'] }] },
  { g: 'יב', phases: [{ p: 'בדיקה', w: ['בדיקה מחזורית'] },
                      { p: 'באנר', w: ['באנר עדכון'] },
                      { p: 'ליבה', w: ['ליבת `sw.js`'] },
...
function catOrderGaps() {
  ...
  for (let i = 0; i < seg.length; i++) {
    ...
    if (h) {
      ...
      const want = '| | ↳ ' + cur.phases.map((x) => x.p).join(' ⟵ ') + ' | | | | | | |';
...
const PART_CATS = {
  'check-capabilities': ['א', 'ב', 'ג', 'ד', 'ה'],
  'test_caps_ui':       ['ו', 'ז'],
  'test_caps_build':    ['ח', 'ט'],
  'test_caps_guard':    ['י', 'יא', 'יב'],
};
 ...
 *  טענה בארבעה שערים היא אותה מדידה בארבעה מקומות. */
 ...
 *  ⛔ **ומה אינו נאכף כאן**: שהפיצול זהה בארבעת הריפו — ⚠️ המרשם יושב
...
function presenceGaps() {
  ...
  /*  ⛔ הגוף נחתך בהתאמת סוגריים ⛔ ולא בחלון תווים — ⚠️ עוזר ארוך מהחלון
   *  היה נחתך באמצע, ⭐ והשורה שמכריעה בו יוצאת מהמדידה. */
  const bodyOf = (name) => {
    const i = self.indexOf('function ' + name + '(');
    if (i < 0) return '';
    let d = 0;
    const j = self.indexOf('{', i);
    if (j < 0) return '';
    for (let k = j; k < self.length; k++) {
      if (self[k] === '{') d++;
      else if (self[k] === '}') { d--; if (!d) return self.slice(j, k + 1); }
    }
    return '';
  };
  ...
  while ((e = ent.exec(mx[1]))) {
    ...
    if (!why)
      out.push(`probe שבודק נוכחות ואינו מוכרז: ${key} ⛔ — מעגנים אותו ` +
               `למיקום, או מכריזים ב-APP.presenceOnly עם המיקום הנכון ונימוק`);
    else if (String(why).trim().split(/\s+/).length < PRESENCE_MIN_WORDS)
      out.push(`הכרזת נוכחות בלי נימוק: ${key} — נמדדו ` +
               `${String(why).trim().split(/\s+/).length} מילים מול ` +
               `${PRESENCE_MIN_WORDS} ⛔ — כותבים מה המיקום הנכון ולמה נוכחות מספיקה`);
  }
 ...
 *  גלובלי שמקורו ב-CDN באחת מארבע האפליקציות, ⛔ **ומה מפיל**:
...
function cdnGaps() {
  ...
  for (const d of decl) {
    ...
    if (!new RegExp('\\b' + d.sym + '\\s*[.(]').test(code) && !why)
  ...
  for (const s of LIB_SYMS)
    if (new RegExp('\\b' + s + '\\s*[.(]').test(code) && !decl.some((d) => d.sym === s))
...
function exportWayGaps() {
  const has = {
    ...
    'share-image':   /navigator\.share\s*\(/.test(code) || /\bhtml2canvas\s*\(/.test(code),
    'download-file': /\.download\s*=/.test(code) || /createPdf\s*\(/.test(code),
  };
 ...
 *  שערכה זהה בארבעת הריפו, ⭐ כטקסט כפי שהוא ב-`sw.js`; ⛔ **ומה מפיל**:
 *  ידית שנבדלת ואינה מוצהרת בשמה, ⛔ הצהרה לידית שערכה שווה לבסיס,
 *  ⛔ וידית שבבסיס ואינה בקובץ. ⭐ **ולמה הוא קיים**: ארבע האפליקציות הן
...
const VOCAB = ['--bg', '--card', '--text', '--border', '--text2', '--radius'];
/*  ⛔ שם שירד, והשם שבא במקומו — ⚠️ **מה נכנס**: שם שנמדד כמת בארבעתן,
 *  ⛔ **ומה מפיל**: הופעתו בגוף ה-CSS. ⭐ **ומשפחת הרקע היא שלוש רמות** —
 *  ⚠️ `--bg` הדף · `--card` משטח הכרטיס · `--card-2` המשטח המורם שמעליו:
 *  ⛔ **והרמה השלישית קיימת בשתיים בלבד** ⛔ ולכן אינה ב-`VOCAB` — ⭐ נמדד
 *  שערכיה במצב הכהה זהים בית-לבית בשתיהן, ⚠️ ולכן זה מושג אחד ⛔ ולא שניים. */
const VOCAB_RETIRED = { '--ink': '--text', '--line': '--border', '--textmid': '--text2', '--muted': '--text2',
                        '--r': '--radius', '--bgcard': '--card', '--bg2': '--card', '--bgsec': '--card-2' };
...
function bgFamilyGaps() {
  ...
  const li = src.indexOf(':root');
  const light = src.slice(li, src.indexOf('}', li));
  for (const t of ['--bg', '--card', '--card-2'])
    if (light.indexOf(t + ':') < 0)
      out.push(`רמה חסרה במשפחת הרקע: ${t} ⛔ — מוסיפים אותה ל-\`:root\` הבהירה`);
...
function roleModelGaps() {
  ...
  if (au && src.indexOf(au + '.role') < 0 && src.indexOf('get user()') < 0)
    out.push('מקור המשתמש מוצהר ואינו בקוד: ' + au);
...
function userMirrorGaps() {
  ...
  if (src.indexOf("'" + key + "'") < 0 &&
      !(APP.mirrorTables || []).some((t) => mirrorKeyOf(t) === key))
    out.push('המפתח מוצהר ואינו בקוד ואינו נגזר משם טבלה: ' + key);
...
function keySaveGaps() {
  ...
  if (!/function ksFire\([\s\S]{0,1400}?closest\('\.ksave'\)[\s\S]{0,400}?data-ksave[\s\S]{0,200}?data-kesc/.test(src))
    out.push('מודול המקש אינו קורא את ההיקף, את שני הסימונים ואת מפת הפעולות');
  /*  ⛔ והמודול אינו רושם מאזין משלו — ⚠️ הוא נקרא מהמאזין האחד:
   *  ⭐ שני מאזינים לאותו אירוע הם שני מקומות שבהם מקש נתפס. */
  if (!/document\.addEventListener\('keydown'[\s\S]{0,300}?ksKey\(e\)/.test(src))
   ...
   *  קיים באפליקציה, ⚠️ והמקש זרק בשקט. */
  if (!new RegExp('(?<![\\w$.])' + APP.actMap + '\\[act\\]').test(src))
  ...
  for (const s of sites) {
    ...
    if (src.indexOf(s.site) < 0) out.push('מאזין מוכרז שאין לו אתר: ' + s.site);
  }
  return out;
}
const MATRIX = [
  ...
  { row: 81, name: 'כפתור שכותב מושבת בזמן הכתיבה',
    probe: () => busyGuardGaps().length === 0 },
  { row: 80, name: 'מאזין אחד, בהאצלה מ-`document`',
  ...
  { row: 79, name: '`Enter` שומר בכל שדה עריכה',
  ...
  { row: 132, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 146, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 89, name: 'פריסה במסכי טלפון וטאבלט',
   ...
   *  משלו — ⚠️ והדרגות עצמן משותפות לארבעתן. */
  { row: 91, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 93, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 90, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 173, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 75, name: 'מיכל המודאל ילד ישיר של `body`',
  ...
  { row: 102, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 148, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 145, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 71, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 72, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 118, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 165, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 136, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 137, name: '`migrations/` — כמות',
  ...
  { row: 154, name: 'מיזוג מכל — מפה ומערך',
    probe: () => mergePointGaps().length === 0 && itemStampGaps().length === 0 },
  /*  ⛔ ברירת מחדל אינה ממוזגת (סבב 121) — ⚠️ הנימוק המדוד: שבע קטגוריות
   *  ו-17 מפתחות תת-משימה של מוסד אחד נכנסו למיזוג בכל טעינה, ⭐ והמוסד
   *  השני קיבל אותם: ⛔ ניקוי נתוני האתר במכשיר לא עזר, ⚠️ שהמיזוג דחף
   *  אותם חזרה לענן תוך דקות. */
  { row: 155, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 156, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 27, name: 'שער ממתין לתנאי ולא לשעון',
  ...
  { row: 169, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 143, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 167, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 167, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 129, name: 'עריכת נתונים אופליין',
  ...
  { row: 153, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 168, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 69, name: 'אובייקט יקר נבנה פעם אחת',
  ...
  { row: 70, name: 'מסך מציג מיד ומרענן ברקע',
  ...
  { row: 122, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 171, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 76, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 164, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 41, name: 'ארבעת מימדי הסריקה',
    probe: () => {
  ...
  { row: 85, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 158, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 123, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 108, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 111, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 127, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 133, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 134, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 55, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 55, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 130, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 77, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 103, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 56, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 56, name: 'שחזור מקומי מהענן',
    probe: () => horizonReleaseWired() },
  { row: 161, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 161, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 187, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 147, name: 'פינוי גיבויים אוטומטי במסד',
   ...
   *  ⛔ ולא `NaN`. */
  { row: 58, name: 'מזהה אינו מומר למספר',
  ...
  { row: 108, name: 'מעטפת WebView חתומה',
  ...
  { row: 167, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 125, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 187, name: 'מודול ה-service worker',
  ...
  { row: 61, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 66, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 186, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 33, name: 'שער מריץ את כל טענותיו',
    probe: () => gateSealGaps().length === 0 },
  { row: 177, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 86, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 178, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 87, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 152, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 185, name: 'מיכל באנר העדכון במקור',
    probe: () => /<div id="updater">/.test(markupOnly()) &&
                 !/\.id\s*=\s*['"]updater['"]/.test(code) &&
                 !/createElement\([^)]*\)[^;]{0,80}updater/.test(code) },
  { row: 185, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 185, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 185, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 68, name: 'מיון אחד לכל התצוגות',
  ...
  { row: 73, name: 'ייצוא — שני דפוסים מוצהרים',
    probe: () => exportMarkupSites().length === 0 },
  { row: 115, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 159, name: 'שכבת קלט אחידה',
  ...
  { row: 159, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  138: { claim: 'בעלות הסכימה המשותפת' },
  150: { claim: '`kv_rishon`' },
  ...
  113: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  116: { claim: 'margin' },
  36: { manual: 'השוואת זמנים בין הריפו אינה בהישג ידו של שער שרץ בריפו אחד — ⛔ נאכפת בתוצאתה בלבד' },
  48: { manual: 'המדידה שלפני המחיקה היא התנהגות סשן — ⛔ נאכפת רק בתוצאתה, בשער ההסרות' },
  47: { claim: 'אין פרק פערים נפרד' },
  101: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  52: { manual: 'מחזור העבודה ב-git הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  ...
  50: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
   ...
   *  מוטציה שאין בו. */
  30: { claim: 'לכל אחד שינוי, תווית' },
  31: { claim: 'שם הטענה שתיפול' },
  29: { claim: 'כיסוי הטבלה' },
  39: { claim: 'ועמודת התקן שלה אוסרת במפורש' },
  40: { claim: ['מספור רציף ובלי כפילויות', 'nameSignGaps', 'catOrderGaps'] },
  42: { claim: 'הערה ריקה' },
  44: { claim: 'COUNT_NOTE' },
  45: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  188: { claim: 'CACHE_NAME' },
  ...
  25: { claims: { 'check-comments': 'ריצת check-js', 'check-capabilities': 'gateKindGaps' } },
  ...
  35: { claims: { 'check-js': 'תקציב זמן לסט',
                  'check-capabilities': 'gateSizeGaps' } },
  26: { claim: 'ברירת המחדל אינה המהירה' },
  51: { manual: 'התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו קריאה חסכונית' },
  53: { claims: { 'check-capabilities': ['sha:', 'orderGaps', 'blockRowGaps'],
                  test_sharedsync: ['block-drift', 'canon-drift'],
                  test_signedshared: 'unsignedTwins',
                  test_toolsid: ['pure-tools', 'pure-undeclared'] } },
  84: { claim: 'storage' },
  83: { claim: 'ג · פעולה שדורשת רשת' },
  97: { claim: 'עברית' },
  99: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  100: { claim: 'BANNER_W' },
  176: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
                 'אין מחלקה שנוספת ל-DOM בלי כלל CSS'] },
  43: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  98: { claim: 'מפנה לקובץ' },
  112: { claim: 'android/app/src/main' },
  119: { claim: 'fgDriftMax' },
  120: { claim: 'CANON_MANIFEST' },
  121: { claim: 'שדות זהים' },
  124: { claim: 'BUILD_SHA' },
  128: { claim: 'SHARED_SHA' },
  126: { claim: 'versionCode' },
  109: { claim: 'WebView' },
  110: { claim: 'android:theme' },
  114: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  131: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  135: { claim: 'migrations' },
  142: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  144: { claim: 'א. חותמת בכל רשומה' },
  54: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  139: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  140: { claim: 'ו. טבלה מקבילה' },
  141: { claim: 'ב. חתימת סכימה' },
  160: { claim: 'type=password' },
  78: { claim: 'aria-label' },
  ...
  59: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps'] } },
  166: { claim: 'pass_salt' },
  170: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  172: { claim: '⏳' },
  49: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  174: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  181: { claim: 'ט. ערך נגזר שהתיישן' },
  183: { claim: 'ח. הגדרה שהוחלפה' },
  182: { claim: 'ד. רשימת-היתר' },
  184: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  96: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  104: { claim: 'CLEANUP_SHA' },
  105: { claim: 'בלוק ה-APP' },
  157: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
 ...
 *  חלק היא אותה מדידה בארבעה מקומות, ⚠️ וגם ארבע פעמים את המחיר. */
if (CORE) {
   ...
   ──────────────────────────────────────────────────────────────────────── */
{
  ...
  if (prg.length)
    ...
    pass(`presenceGaps — ${Object.keys(APP.presenceOnly || {}).length} בדיקות נוכחות מוכרזות, ` +
         `וכל אחת נושאת את המיקום הנכון ואת נימוקה`);
