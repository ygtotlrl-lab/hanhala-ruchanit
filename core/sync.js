...
// השוואת זהות — תמיד כמחרוזת. עובדת על מזהה מספרי ועל uuid באותה מידה, ⛔ וגם על התאמה חוצת-סוגים שנוצרת כשמזהה עובר דרך מאפיין
// HTML וחוזר כמחרוזת.
function idEq(a, b) {
...
// הוא נכתב. ⛔ אין לשכפל אותה — ⚠️ שני עותקים של כלל ההכרעה נסחפים זה
//    מזה, ⭐ ושני מכשירים מכריעים אחרת על אותה רשומה.
...
//    בארגון, ⭐ ופונקציה שמכירה אחת מהן אינה מודול משותף: ⛔ היא מחזירה
//    «בלי חותמת» לכל רשומה בצורה השנייה, ומשאירה כל tombstone.
...
/* ═══ משיכה מסוננת בשרת — מודול משותף ═════════════════════════════════════
   ⛔ מסך מושך **רק את החלון שהוא מציג** — ⚠️ מסך שמציג חודש אחד ומושך
      את כל הטבלה משלם על כל השורות בכל כניסה. ⛔ סינון שרץ אחרי ההורדה
      אינו חוסך דבר — ⚠️ הרשת כבר שילמה, ⭐ והמסך ממתין לה.
 ...
 *  ⚠️ ניסיון חוזר עליהן נחווה אצל המשתמש כ**איטיות בלבד** — הלולאה מנסה
 ...
 *  ⛔ **ומי שלא הצליח מנסה שוב ב-`load`** — ⚠️ השגיאה יכולה להגיע בזמן
 *  העלייה, ⭐ לפני שהמפרסם רץ: ⛔ ובלי ניסיון נוסף הבאנר נבלע ונשאר
 *  טוסט בלבד. */
...
function sbWatch(c) {
  ...
  c.from = function (t) {
    var qb = from0(t);
    /*  ⛔ הרשימה יושבת כאן ⛔ ולא כמשתנה מודול — ⚠️ לקוח שנוצר **לפני**
     *  טעינת המודול פוגש משתנה מודול `undefined` ברגע העטיפה: ⛔ תלות
     *  בסדר הטעינה שנשברת בשקט. */
...
/* ═══ צינור השמירה — מודול משותף ═════════════════════════════════════════
   ...
   ⚠️ רצף שנכתב בכל אתר שמירה בנפרד משמיט בכל אתר שלב אחר — ⭐ מודאל
      שאינו נסגר, מקוון ואופליין שאינם מובחנים, ⚠️ והודעת «נשמר» בלי רשת.
...
/* ---------------------------------------------- השהיית ציור ----
   ...
   ה-⏳ לא מופיע כלל; אם לא — הוא מופיע כסימון רגיל.
   ⚠️ ההשהיה היא על ה**ציור בלבד**: הסימון הלוגי נכתב ל-localStorage מיידית
   (מנגנוני הבטיחות — פינוי, דחיפה, עידן — נשענים עליו), מסך ההגדרות
...
// כשל סמכותי — הכתיבה **לא** נכנסה, ולכן הסימון יורד: ⚠️ סימון שנשאר
// על כשל סמכותי מקבע את המונה ואת הניסיון החוזר לנצח.
 ...
 *  חולף, ⭐ ולולאה שמנסה שוב ושוב מולה נראית למשתמש כ«איטיות». */
 ...
 *  אפליקציה מספר משלה: ⭐ צורת השורה נבדלת ביניהן, ⛔ ומספר משותף זורק
 *  את העותק המקומי על שינוי שלא נעשה כאן.
