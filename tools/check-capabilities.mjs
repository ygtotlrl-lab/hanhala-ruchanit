...
const APP = {
  ...
  gapRows: [224, 142, 146, 170, 67, 155, 109],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    193: (c) => c.hasCode(/HR_PASS_ITER\s*=\s*100000\b/)
    ...
    192: (c) => !!c.fnBody('hrUsersCacheSave')
    ...
    153: (c) => !/hrCfgSet\('(?:hr_students|hr_sessions|hr_sleep_sessions)'/.test(c.src)
    ...
    189: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '189|מבנה טופס הכניסה': 'src',
    '155|שם נגזר אינו נשמר ברשומה': 'src',
    '40|שער מריץ את כל טענותיו': 'tools',
    '99|שכבת המראה': 'src',
    '211|הגירה מקומית שהושלמה': 'mixed',
    '191|מראת המשתמשים': 'src',
    '190|מודל ההרשאות': 'src',
    '27|שער סורק קוד מולבן': 'tools',
    '27|ההלבנה שומרת על מספרי השורות': 'mixed',
    '171|הקשר נלכד בכניסה לפונקציה': 'src',
    ...
    '205|הערה שמתארת מצב שחלף': 'mixed',
    '212|שאילתת `@media` מתה': 'src',
    ...
    '173|הגיבוי שלם ומעומד': 'src',
    '79|פעולה מגיבה מיד': 'src',
    '80|משיכה מסוננת בשרת': 'src',
    '141|דגימת-יתר במחולל': 'tools',
    '193|`PBKDF2` — פרמטרים': 'src',
    '159|`migrations/` — תוכן': 'src',
    '160|`migrations/` — כמות': 'src',
    '181|מיזוג מכל — מפה ומערך': 'src',
    '182|ברירת מחדל אינה ממוזגת': 'src',
    '183|רשימה אינה נושאת פריט כפול': 'src',
    '174|`sh_backup` — טבלת הגיבוי': 'src',
    '25|שער אינו מקליד מספר שורה': 'tools',
    '31|שער ממתין לתנאי ולא לשעון': 'tools',
    '197|סודות אינם עוזבים את המכשיר': 'src',
    '168|דפוס `upsert` — onConflict מוכרז': 'src',
    '195|טביעה חסרה מושלמת אוטומטית': 'src',
    '195|כניסה אופליין': 'src',
    '152|עריכת נתונים אופליין': 'src',
    '178|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '196|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '77|אובייקט יקר נבנה פעם אחת': 'src',
    '78|מסך מציג מיד ומרענן ברקע': 'src',
    '145|האפליקציה ניתנת להתקנה מכרום': 'src',
    '202|דגלי מעבר — אין דגל דלוק': 'src',
    '85|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '192|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '98|פינוי אוטומטי': 'src',
    '98|אופק הפינוי נוקה מהסנכרון': 'src',
    '185|אימות פינוי מול הענן': 'src',
    '146|שיתוף קבצים': 'src',
    '130|מעטפת APK (WebView)': 'src',
    '133|אין נכסים מוטבעים': 'src',
    '150|מפתח חתימה קבוע': 'src',
    '156|מקור אמת יחיד לסכימה': 'src',
    '157|קובץ התקנה מלא': 'src',
    '63|גיבוי יומי אוטומטי': 'src',
    '63|יומן פעולות': 'src',
    '153|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '86|דפוס הודעת שגיאה יחיד': 'src',
    '94|`toast` — חתימה, גוף ומחלקות': 'src',
    '94|סיווג ההודעה נגזר מהמסלול': 'src',
    '223|מיכל באנר העדכון במקור': 'src',
    '223|הרענון מ-controllerchange בלבד': 'src',
    '223|סימן דחיית הבאנר מתמיד': 'src',
    '125|רישום כשלי כתיבה': 'src',
    '64|חלון חם במכשיר': 'src',
    '189|מסך שינוי סיסמה עצמי': 'src',
    '225|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '65|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '195|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '66|מודול מזהי רשומות': 'src',
    '66|מזהה אינו מומר למספר': 'src',
    '68|מזהה מכשיר': 'src',
    '65|ליבת מיזוג משותפת': 'src',
    '130|מעטפת WebView חתומה': 'tools',
    '195|אימות מול טביעה בענן': 'mixed',
    '148|בניית APK אחידה עם שער חתימה': 'src',
    '225|מודול ה-service worker': 'src',
    '69|ניסיון חוזר בתור הסנכרון': 'src',
    '70|מנגנון משיכה אחיד': 'src',
    '73|נעילת חוסר-פעילות': 'src',
    '74|מודל הסשן — בזיכרון בלבד': 'src',
    '224|בדיקת עדכון תקופתית ל-service worker': 'src',
    '101|פסק זמן אחיד לקריאות רשת': 'src',
    '102|מאזיני מצב רשת': 'src',
    '177|גריעת tombstones לפי גיל': 'src',
    '223|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '146|גשר שיתוף': 'src',
    '75|העברת מזהה ל-DOM': 'src',
    '76|מיון אחד לכל התצוגות': 'src',
    '81|ייצוא — שני דפוסים מוצהרים': 'src',
    '138|שכבת אייקונים': 'src',
    '186|שכבת קלט אחידה': 'src',
    '186|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '82|שכבת המודאל': 'src',
    '189|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [33, 48, 49, 58, 97, 56, 184, 116, 162, 62, 26, 67, 29, 38, 42, 200, 207];
...
const MATRIX = [
  ...
  { row: 155, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 171, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 205, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 173, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 170, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 141, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 193, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 159, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 160, name: '`migrations/` — כמות',
  ...
  { row: 181, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 182, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 183, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 174, name: '`sh_backup` — טבלת הגיבוי',
  ...
  { row: 197, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 168, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 195, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 195, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 152, name: 'עריכת נתונים אופליין',
  ...
  { row: 178, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 196, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 145, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 202, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 192, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 185, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 146, name: 'שיתוף קבצים',
  ...
  { row: 150, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 156, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 157, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 63, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 63, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 153, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 189, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 189, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 225, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 172, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 195, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 195, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 148, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 225, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 224, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 211, name: 'הגירה מקומית שהושלמה',
   ...
   *  ולא רק מוגדרים. */
  { row: 177, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 223, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 223, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 223, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 223, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 138, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 186, name: 'שכבת קלט אחידה',
  ...
  { row: 186, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  219: { claims: { test_mignames: ['[mig-pattern]', '[mig-seq]', '[mig-file-no-entry]',
  ...
  200: { claims: { test_docfacts: ['[doc-name]', '[doc-round]'],
  ...
  204: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  198: { claim: 'אין `service_role` בקובץ' },
  35: { claim: 'מסלול «' },
  ...
  201: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  72: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  161: { claim: 'בעלות הסכימה המשותפת' },
  175: { claim: '`ya_settings_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  176: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  213: { claim: 'ז. עמודה בלי קורא' },
  ...
  136: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  139: { claim: 'margin' },
  ...
  208: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  142: { claim: 'fgDriftMax' },
  143: { claim: 'CANON_MANIFEST' },
  144: { claim: 'שדות זהים' },
  147: { claim: 'BUILD_SHA' },
  151: { claim: 'SHARED_SHA' },
  149: { claim: 'versionCode' },
  131: { claim: 'WebView' },
  132: { claim: 'android:theme' },
  137: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  154: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  158: { claim: 'migrations' },
  167: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  169: { claim: 'א. חותמת בכל רשומה' },
  62: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  162: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  163: { claim: 'ו. טבלה מקבילה' },
  164: { claim: 'ב. חתימת סכימה' },
  165: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  188: { claim: 'type=password' },
  87: { claim: 'aria-label' },
  ...
  199: { claim: '[sec-keyfile]' },
  203: { claims: { test_filesets: ['[oneoff-gate]', '[oneoff-round]', '[oneoff-stale]'] } },
  207: { claims: { test_orphans: 'כל קובץ בעץ מוזכר במקום אחר',
  ...
  216: { claim: 'ט. ערך נגזר שהתיישן' },
  218: { claim: 'ח. הגדרה שהוחלפה' },
  217: { claim: 'ד. רשימת-היתר' },
  220: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  184: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
