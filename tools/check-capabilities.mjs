...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    108: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    107:  (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    90: (c) => c.hasCode(/YS_KV_LEGACY_WRITE\s*=\s*false/),
    ...
    106: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [31, 35, 36, 37, 34, 47, 62, 40];
...
const MATRIX = [
  ...
  { row: 81, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 108, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 96, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  { row: 110, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 89, name: 'עריכת נתונים אופליין',
  ...
  { row: 99, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 113, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 107, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 88, name: 'מפתח חתימה קבוע בריפו',
    probe: () => hasPath('signing') &&
                 fs.readdirSync('signing').some((f) => f.endsWith('.keystore')) },
  { row: 92, name: 'מקור אמת יחיד לסכימה', probe: () => hasPath(APP.schemaFile) },
  { row: 93, name: 'קובץ התקנה מלא',       probe: () => hasPath(APP.schemaFile) },
  { row: 49, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 49, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 90, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 64, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 106, name: 'מסך שינוי סיסמה עצמי', app: true },
  ...
  { row: 97, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 110, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 110, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 86, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 66, name: 'פסק זמן אחיד לקריאות רשת',
  ...
  { row: 98, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 79, name: 'שכבת אייקונים',
    probe: () => iconAudit('.').length === 0 },
  { row: 101, name: 'שכבת קלט אחידה',
...
const GATES = {
  ...
  14: { claim: 'CANON_MD' },
  39: { claim: 'תוכן הקבצים הנלווים' },
  78: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  41: { manual: 'המדידה שלפני המחיקה היא התנהגות סשן — ⛔ נאכפת רק בתוצאתה, בשער ההסרות' },
  40: { claim: 'אין פרק פערים נפרד' },
  73: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  44: { manual: 'מחזור העבודה ב-git הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  ...
  35: { claim: 'מספור רציף ובלי כפילויות' },
  36: { claim: 'הערה ריקה' },
  37: { claim: 'COUNT_NOTE' },
  38: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  ...
  34: { claim: 'כל כלל מיוצג בטבלה' },
  43: { manual: 'התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו קריאה חסכונית' },
  47: { claim: 'sha:' },
  58: { claim: 'CANON' },
  62: { claim: 'storage' },
  65: { claim: 'ג · פעולה שדורשת רשת' },
  68: { claim: 'עברית' },
  69: { claim: 'RULE_W' },
  70: { claim: 'BANNER_W' },
  117: { claim: 'CSS מתות' },
  71: { manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ והמונה נמדד ידנית' },
  72: { claim: 'מפנה לקובץ' },
  77: { claim: 'android/app/src/main' },
  82: { claim: 'fgDriftMax' },
  83: { claim: 'CANON_MANIFEST' },
  85: { claim: 'BUILD_SHA' },
  87: { claim: 'versionCode' },
  75: { claim: 'WebView' },
  91: { manual: 'נפילה-חזרה ל-`kv` נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  94: { claim: 'migrations' },
  95: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  102: { claim: 'type=password' },
  103: { claim: 'aria-label' },
  104: { manual: 'שני מנועי תאריך — ⛔ טרם הוכרע איזה, ואין מה לאכוף' },
  105: { manual: '«`catch` ריק סביב כתיבה» טרם נוסח כשער — נסרק ידנית בכל סבב שנוגע' },
  109: { claim: 'pass_salt' },
  111: { manual: 'מצב העמודה במסד אינו נראה מהריפו' },
  112: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  114: { claim: '⏳' },
  115: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  121: { manual: 'קיום טבלה או מפתח במסד אינו נראה מהריפו' },
  119: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  116: { manual: '«קובץ בלי קורא» דורש סריקה על ארבעת הריפו — נעשה ידנית' },
  122: { manual: 'קיום מפתח במסד אינו נראה מהריפו' },
  118: { manual: 'רשימת-היתר הגיבויים יושבת במיגרציה שכבר רצה' },
  120: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  45: { claim: 'CLEANUP_SHA' },
  46: { claim: 'בלוק ה-APP' },
};
