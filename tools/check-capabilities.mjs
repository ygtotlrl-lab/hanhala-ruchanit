...
const APP = {
  ...
  actMap: 'DOM_ACTIONS',
  gapRows: [38, 102, 106, 124, 125, 129, 139],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    143: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    142: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    113: (c) => !/ysCfgSet\('(?:ys_students|ys_attend_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    141: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [27, 37, 39, 41, 49, 71, 44, 134, 82];
...
const GATES = {
  120: { claim: 'בעלות הסכימה המשותפת' },
  ...
  123: { claim: 'א. חותמת בכל רשומה' },
  133: { claim: 'ב. חתימת סכימה' },
  ...
  80: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  87: { claim: 'CLEANUP_SHA' },
  88: { claim: 'בלוק ה-APP' },
  134: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
