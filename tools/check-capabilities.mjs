...
const APP = {
  ...
  /*  ⛔ אין כאן מסלול השלמת טביעה (סבב 76) — ⚠️ הוא הוסר בהחלטת מנהל
   *  מפני שהיה הקורא האחרון של הסיסמה הגלויה, ⛔ ואין להחזירו: ⭐ מה
  ...
  cdnLibs: { 'supabase-js': '2.111.0', 'pdfmake': '0.3.11', 'xlsx': '0.18.5' },
  /*  ⛔ דרך ההנפקה — ⚠️ קובץ להורדה: ⭐ הדוח מונפק ⛔ ואינו משותף. */
  exportWay: 'download-file',
  exportFallback: '',
  exportFns: ['printStudents'],
  /*  ⛔ שם מפת מטפלי ה-`data-act` (סבב 89) — ⚠️ השם נבדל בין הארבע
   *  והמנגנון אחד: ⭐ מאזין יחיד ב-`document` שמנתב לפי המפה. */
  actMap: 'DOM_ACTIONS',
  gapRows: [38, 105, 109, 127, 128, 132, 144],
  /*  ⛔⛔ קריאה לשכבת השורות **בלי חלון** (סבב 89) — ⚠️ הנימוק המדוד:
   *  פתיחת סדר משכה 18,688 שורות **פעמיים**, ⭐ ובדיקת הכפילות צריכה יום
   *  אחד — 248 שורות: ⛔ ה-probe של השורה מדד את **צורת הקריאה** ⚠️ ולא
   *  את היקפה, ⭐ ולכן לא תפס אותה.
  ...
  fullPullAllow: {
    ...
    slSaveData:       'מיזוג לפני כתיבה — ⛔ לא דורסים רשומה של מכשיר אחר',
    atLoadData:       'מטמון הזיכרון עם נפילה-חזרה לענן — ⚠️ הוא נטען בעלייה, ⭐ והחלון יושב באתרי המשיכה',
    slLoadData:       'מטמון הזיכרון עם נפילה-חזרה לענן — ⚠️ הוא נטען בעלייה, ⭐ והחלון יושב באתרי המשיכה',
   ...
   *  חריגה. */
  listAddExempt: {
    saveStudent: 'שני תלמידים יכולים לשאת אותו שם באותו שיעור — ⛔ הזהות היא ה-`id` שנוצר במכשיר, ⚠️ וחסימה לפי שם הייתה נועלת בחוץ תלמיד אמיתי',
  },
   ...
   *  «נמדד ואין». */
  itemStamp: [{ target: 's.absences', stamp: 'updatedAt' }],
  /*  ⛔ מפתחות ההגדרה שהקוד מבקש — ⚠️ כל מפתח כאן נדרש ב-`ysCfgGet`,
   *  ⛔ וכל מפתח שנדרש שם חייב להיות כאן: ⭐ והמדידה מול המסד עצמו היא
   *  פעולת מנהל. */
  cfgKeys: ['ys_absence_reasons', 'ys_approvals', 'ys_attend_cfg', 'ys_attend_treats',
            'ys_perms', 'ys_reasons', 'ys_sleep_cfg', 'ys_sleep_treats'],
  ...
  inlineErrAllow: {
    'auth-err': 'הודעת הכניסה — ⛔ נשארת על המסך בזמן שהמשתמש מקליד מחדש: ⚠️ שש ההודעות נבדלות זו מזו, וטוסט שנעלם היה מאחד אותן בפועל',
    'pw-err': 'הודעת שינוי הסיסמה העצמי — ⛔ צמודה לשלושת השדות, ⚠️ והיא מה שאומר איזה מהם נדחה',
    'switch-err': 'הודעת החלפת המשתמש — ⛔ צמודה לטופס שנשאר פתוח',
    'um-err': 'הודעת מסך ניהול המשתמשים — ⛔ צמודה לשורה שנדחתה',
  },
  orphanAllow: {
    canEdit: 'שכבת ההרשאות — ⛔ `canAccess` ו-`canEdit` הן זוג, ⚠️ ומחיקת אחת מהן מותירה מודל הרשאות בלי דרגת עריכה',
    hebrewDateShort: 'מנוע התאריך העברי — ⛔ כל צרכן תאריך עובר דרכו, ⚠️ והוא נמדד כגבול חיתוך בשער התאריך',
  },
  /*  ⛔ אסימון שנשאר בשני התפקידים — ⚠️ כל שם נושא את סיבתו, ⛔ ושם
   *  שאינו בשני התפקידים בפועל מפיל אף הוא. ⛔ **ואסימון שנפצל
   *  ל-`-ink` ול-`-fill` אינו נכנס לכאן** — ⚠️ אין לו עוד תפקיד שני. */
  dualRoleAllow: {},
  /*  ⛔ צמד מוצהר שאינו מגיע ל-4.5 — ⚠️ כל צמד נושא את סיבתו, ⛔ וצמד
   *  שכן מגיע מפיל אף הוא. ⛔ **וצמד שחרג בגלל אסימון דו-תפקידי אינו
   *  נכנס לכאן** — ⚠️ הפיצול הוא התיקון שלו, ⭐ ולא ההחרגה. */
  contrastAllow: {},
  tableProbe: {
    ...
    const _r66 = (() => {
      ...
      return _r66 && (callArity(c.code, '_ysRowsPaged').every((a) => a === 3)
          ...
          && callArity(c.code, '_slPullSessions').every((a) => a === 1)
    ...
    147: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    116: (c) => !/ysCfgSet\('(?:ys_students|ys_attend_sessions|ys_sleep_sessions)'/.test(c.src)
          && c.hasCode(/await ysRowsPushSessions\(data\)/)
          && c.hasCode(/await ysRowsPushStudents\(mergedSt\)/),
     ...
     *  מת הוא בדיוק «מטריצה שאינה תואמת לקוד», והיא אסורה. */
    146: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [27, 37, 39, 41, 50, 72, 44, 139, 85];
...
const CAPS = {
  ...
  storage: {
    ...
    block: { sha: '8260246f4630d610', lines: 688,
  ...
  /*  ⭐ סבב 93 — מודול גריעת ה-tombstones. ⛔ הוא יושב אחרי מודול המיזוג
  ...
  tomb: {
    ...
    block: { sha: '96fe1f9f8e65cf92', lines: 65,
             start: '/* ═══ גריעת tombstones — מודול משותף (סבב 93)',
  ...
  /*  ⭐ סבב 101 — כיווץ רשימת ערכים. ⚠️ אין לו `hooks`: הוא נקרא מכל אתר
   *  כתיבה לרשימה ומכל מיזוג שמכווץ — ⛔ ומה שנאכף כאן הוא שהזהות אחת:
   *  ⭐ בדיקת הקיום בכתיבה והכיווץ במיזוג הכריעו בארבעה מימושים נפרדים,
   *  ⚠️ ושניים מהם נבדלו. */
  uniq: {
    name: 'מודול כיווץ רשימת ערכים',
    block: { sha: '683dff568ea039c0', lines: 28,
             start: '/* ═══ כיווץ רשימת ערכים — מודול משותף (סבב 101)',
 ...
 *  ⛔ **וכאן נכנס באנר שאינו בתבנית** — ⚠️ ובאנר שכן בתבנית מפיל אף
 *  הוא: ⭐ מספר הסבב נגזר מהבאנר עצמו ⛔ ואינו מוקלד.
...
const BLOCK_ORDER = ['bp', 'neterr', 'rowswin', 'guardonline', 'storage', 'stale', 'techinfo', 'status', 'backup',
                     ...
                     'devid', 'mergecore', 'tomb', 'uihelp', 'uniq'];
...
/*  ⛔ **מבנה ה-`tier` נמדד, ולא רק שהוא מפנה** (סבב 96ד) — ⚠️ `tier` הוא
...
function bpGaps() {
  ...
  return out.concat(bpOutOfScale());
}
...
/*  ⛔ עוטף שדוחף פרמטר של עצמו לרשימה (סבב 101) — ⚠️ הנימוק המדוד: בשכר
...
/*  ⛔ ופריט אינו מופיע פעמיים באותה רשימה (סבב 101) — ⚠️ **לא בכתיבה
...
/*  ⛔ פריט במערך נושא חותמת משלו (סבב 101) — ⚠️ פריט בלי חותמת מפסיד לכל
...
/*  ⛔ ערכת נושא — בהיר וכהה (סבב 97) — ⚠️ המבנה אחד בארבעתן: בלוק
...
function staleNoteSites() {
  ...
  try { for (const f of fs.readdirSync('tools').filter((x) => x.endsWith('.mjs'))) files.push('tools/' + f); }
...
const MATRIX = [
  ...
  { row: 89, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 130, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 127, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
   ...
   *  אחד. */
  { row: 128, name: 'הקשר נלכד בכניסה לפונקציה', app: true },
  ...
  { row: 66, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 67, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 104, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 148, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 121, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 122, name: '`migrations/` — כמות',
  ...
  { row: 136, name: 'רשימה אינה נושאת פריט כפול',
    probe: () => listDupGaps().length === 0 },
  { row: 137, name: 'דפוס עמודות אחיד',
  ...
  { row: 131, name: '`kv_backup` — טבלת הגיבוי',
  ...
  { row: 132, name: '`kv_rishon` / `kv_ramataviv` — הגדרות היומן', app: true },
  { row: 21, name: 'שער אינו מקליד מספר שורה',
    probe: () => typedRowSites().length === 0 },
  { row: 158, name: 'פונקציה בלי קוראים',
  ...
  { row: 25, name: 'שער ממתין לתנאי ולא לשעון',
  ...
  { row: 152, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 125, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 150, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 150, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 115, name: 'עריכת נתונים אופליין',
  ...
  { row: 134, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 151, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 108, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 154, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 147, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  /*  ⛔ **שני השלבים נמדדים, ⛔ ולא שלב ב לבדו** (סבב 96) — ⚠️ התקן אומר
  ...
  /*  ⛔ ארבעת מימדי הסריקה (סבב 96) — ⚠️ ה-probe מאמת **ערך**: ⭐ ארבע
   *  רשומות במרשם, ⛔ וכל אחת נושאת את שם המימד ואת מה שנבדק בו;
   *  ⚠️ ושהמרשם **מצויר** — ⛔ מרשם שאיש אינו קורא הוא קוד מת שהשער
   *  מאשר. ⭐ ובשלוש האחרות אין מסך תשתית, ⛔ והשורה ⭕ מנומקת. */
  { row: 38, name: 'ארבעת מימדי הסריקה',
    probe: () => {
      const m = /YS_INF_DIMS\s*=\s*\[([\s\S]*?)\n\];/.exec(code);
      if (!m) return false;
      const rows = m[1].split('\n').filter((l) => /\[\s*$|\['|\[\s*'/.test(l) || /^\s*\[/.test(l));
      return rows.length === 4 && /YS_INF_DIMS\.forEach/.test(code);
    } },
  { row: 73, name: 'פינוי אוטומטי',
    probe: () => { const t = tierShape();
                   return t.ok && t.items > 0 &&
                          Number((/LS_SWEEP_PCT\s*=\s*([\d.]+)/.exec(src) || [])[1]) === LS_SWEEP_PCT; } },
  /*  ⛔ **העֵד ולא ה-`verify`** (סבב 96) — ⚠️ התקן אומר «העֵד הוא ראיה
  ...
  { row: 140, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 109, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 94, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 97, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 113, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 118, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 119, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 52, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 52, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 116, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 74, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 75, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 145, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  ...
  { row: 146, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 165, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 129, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 150, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 94, name: 'מעטפת WebView חתומה',
  ...
  { row: 150, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 111, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 165, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 164, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 77, name: 'פסק זמן אחיד לקריאות רשת',
  ...
  { row: 78, name: 'מאזיני מצב רשת',
    probe: () => hasSrc(/window\.addEventListener\('online'/) &&
                 hasSrc(/window\.addEventListener\('offline'/) },
  /*  ⛔⛔ המנגנון עבר לבלוק חתום וחווט בארבעתן (סבב 93) — ⚠️ עד כאן הוא חי
  ...
  { row: 163, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  /*  ⛔ מסבב 67 — לא די בכך שהמזהה **עטוף**: הוא חייב
  ...
  { row: 101, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 141, name: 'שכבת קלט אחידה',
    probe: () => inputAudit('.').length === 0 },
  /*  ⛔ שתי נקודות כניסה חיות ולא שתי פונקציות (סבב 67) — פונקציה
   *  שקיימת בקובץ ואין לה קורא היא קוד מת, ו-✅ עליה נקרא כעדות. */
  ...
  { row: 69, name: 'שכבת המודאל',
    probe: () => modalGaps().length === 0 },
  { row: 146, name: 'שכבת כניסה מלאה',
...
const GATES = {
  123: { claim: 'בעלות הסכימה המשותפת' },
  4: { claim: ['canonIds', 'CANON', 'הסדר הקנוני'] },
  1: { claim: 'שם האפליקציה' },
  3: { claim: ['DOC_MAX_LINES', 'DOC_MAX_SHARED'] },
  2: { claim: 'תוכן החלק' },
  5: { claim: 'RM_OK' },
  6: { claim: ['MD_MAX', 'MD_SPLIT'] },
  7: { claim: 'CANON_MD' },
  8: { claim: 'CTX_OK' },
  9: { claim: ['MD_MAX', 'MD_SPLIT'] },
  10: { claim: 'CANON_MD' },
  11: { claim: 'פרק מתחום של קובץ אחר' },
  12: { claim: ['MD_MAX', 'MD_SPLIT'] },
  13: { claim: 'CANON_MD' },
  43: { claim: 'תוכן הקבצים הנלווים' },
  99: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  102: { claim: 'margin' },
  33: { manual: 'השוואת זמנים בין הריפו אינה בהישג ידו של שער שרץ בריפו אחד — ⛔ נאכפת בתוצאתה בלבד' },
  45: { manual: 'המדידה שלפני המחיקה היא התנהגות סשן — ⛔ נאכפת רק בתוצאתה, בשער ההסרות' },
  44: { claim: 'אין פרק פערים נפרד' },
  88: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  24: { claim: 'ברירת המחדל אינה המהירה' },
  ...
  84: { claim: 'עברית' },
  85: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  86: { claim: 'BANNER_W' },
  159: { claim: 'CSS מתות' },
  40: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  87: { claim: 'מפנה לקובץ' },
  98: { claim: 'android/app/src/main' },
  105: { claim: 'fgDriftMax' },
  106: { claim: 'CANON_MANIFEST' },
  107: { claim: 'שדות זהים' },
  110: { claim: 'BUILD_SHA' },
  114: { claim: 'SHARED_SHA' },
  112: { claim: 'versionCode' },
  95: { claim: 'WebView' },
  96: { claim: 'android:theme' },
  100: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  117: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  120: { claim: 'migrations' },
  124: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  126: { claim: 'א. חותמת בכל רשומה' },
  138: { claim: 'ב. חתימת סכימה' },
  142: { claim: 'type=password' },
  143: { claim: 'aria-label' },
  144: { manual: 'מנוע התאריך — ⛔ נאכף ב-`test_date` ביומן ובהנהלה, ⚠️ ובשכר ובגיוס אין צרכן תאריך עברי ואין שער' },
  149: { claim: 'pass_salt' },
  153: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  155: { claim: '⏳' },
  46: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  157: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  161: { claim: 'ד. רשימת-היתר' },
  162: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  83: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  90: { claim: 'CLEANUP_SHA' },
  91: { claim: 'בלוק ה-APP' },
  139: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
...
/* ────── ⛔ מבנה הטבלה עצמה — מספור ותקן ההערות (סבב 72) ─────────────────────
   ...
   ב-`table` מסבב 70, ⚠️ ואיש לא מדד אותם — ⛔ מספור נשבר בכל
   ...
   ──────────────────────────────────────────────────────────────────────── */
{
   ...
   *  ⛔⛔ **והמדידה נמדדת בפתיחה ⛔ ולא בספרה** (סבב 96) — ⚠️ «נמדד: אפס
  ...
  const noisy = rows.filter((r) => r.cells.length >= 9 &&
                  ...
                  r.cells[8].trim() !== '' && !COUNT_NOTE.test(r.cells[8].trim()));
  if (noisy.length)
