...
const APP = {
  ...
  gapRows: [45, 124, 128, 152, 61, 137, 98],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    173: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    172: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    135: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    169: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '90|חותמת רעננות נכשלת סגור': 'src',
    '81|`Enter` שומר בכל שדה עריכה': 'src',
    '82|מאזין אחד, בהאצלה מ-`document`': 'src',
    '83|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '169|מבנה טופס הכניסה': 'src',
    '137|שם נגזר אינו נשמר ברשומה': 'src',
    '188|מפתח אחסון בלי קורא': 'src',
    '37|שער מריץ את כל טענותיו': 'tools',
    '89|שכבת המראה': 'src',
    '189|הגירה מקומית שהושלמה': 'mixed',
    '171|מראת המשתמשים': 'src',
    '170|מודל ההרשאות': 'src',
    '24|שער סורק קוד מולבן': 'tools',
    '24|ההלבנה שומרת על מספרי השורות': 'mixed',
    '153|הקשר נלכד בכניסה לפונקציה': 'src',
    '93|פריסה במסכי טלפון וטאבלט': 'src',
    '94|ערכת נושא — בהיר וכהה': 'src',
    '95|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '96|אין סגנון מוטבע': 'src',
    '98|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '183|הערה שמתארת מצב שחלף': 'mixed',
    '190|שאילתת `@media` מתה': 'src',
    '111|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '112|ייצוא והנפקה — דרך מוצהרת': 'src',
    '99|המסמך מתפרסר נקי': 'src',
    '100|לוג מסלול — תג ורמה': 'src',
    '77|מיכל המודאל ילד ישיר של `body`': 'src',
    '107|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '155|הגיבוי שלם ומעומד': 'src',
    '73|פעולה מגיבה מיד': 'src',
    '74|משיכה מסוננת בשרת': 'src',
    '123|דגימת-יתר במחולל': 'tools',
    '173|`PBKDF2` — פרמטרים': 'src',
    '141|`migrations/` — תוכן': 'src',
    '142|`migrations/` — כמות': 'src',
    '162|מיזוג מכל — מפה ומערך': 'src',
    '163|ברירת מחדל אינה ממוזגת': 'src',
    '164|רשימה אינה נושאת פריט כפול': 'src',
    '156|`sh_backup` — טבלת הגיבוי': 'src',
    '22|שער אינו מקליד מספר שורה': 'tools',
    '186|פונקציה בלי קוראים': 'mixed',
    '28|שער ממתין לתנאי ולא לשעון': 'tools',
    '177|סודות אינם עוזבים את המכשיר': 'src',
    '150|דפוס `upsert` — onConflict מוכרז': 'src',
    '175|טביעה חסרה מושלמת אוטומטית': 'src',
    '175|כניסה אופליין': 'src',
    '134|עריכת נתונים אופליין': 'src',
    '160|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '176|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '71|אובייקט יקר נבנה פעם אחת': 'src',
    '72|מסך מציג מיד ומרענן ברקע': 'src',
    '127|האפליקציה ניתנת להתקנה מכרום': 'src',
    '181|דגלי מעבר — אין דגל דלוק': 'src',
    '78|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '172|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '45|ארבעת מימדי הסריקה': 'src',
    '88|פינוי אוטומטי': 'src',
    '88|אופק הפינוי נוקה מהסנכרון': 'src',
    '166|אימות פינוי מול הענן': 'src',
    '128|שיתוף קבצים': 'src',
    '113|מעטפת APK (WebView)': 'src',
    '116|אין נכסים מוטבעים': 'src',
    '132|מפתח חתימה קבוע בריפו': 'src',
    '138|מקור אמת יחיד לסכימה': 'src',
    '139|קובץ התקנה מלא': 'src',
    '57|גיבוי יומי אוטומטי': 'src',
    '57|יומן פעולות': 'src',
    '135|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '79|דפוס הודעת שגיאה יחיד': 'src',
    '84|`toast` — חתימה, גוף ומחלקות': 'src',
    '84|סיווג ההודעה נגזר מהמסלול': 'src',
    '197|מיכל באנר העדכון במקור': 'src',
    '197|הרענון מ-controllerchange בלבד': 'src',
    '197|סימן דחיית הבאנר מתמיד': 'src',
    '108|רישום כשלי כתיבה': 'src',
    '58|חלון חם במכשיר': 'src',
    '169|מסך שינוי סיסמה עצמי': 'src',
    '199|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '59|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '175|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '60|מודול מזהי רשומות': 'src',
    '60|מזהה אינו מומר למספר': 'src',
    '62|מזהה מכשיר': 'src',
    '59|ליבת מיזוג משותפת': 'src',
    '113|מעטפת WebView חתומה': 'tools',
    '175|אימות מול טביעה בענן': 'mixed',
    '130|בניית APK אחידה עם שער חתימה': 'src',
    '199|מודול ה-service worker': 'src',
    '63|ניסיון חוזר בתור הסנכרון': 'src',
    '64|מנגנון משיכה אחיד': 'src',
    '67|נעילת חוסר-פעילות': 'src',
    '68|מודל הסשן — בזיכרון בלבד': 'src',
    '198|בדיקת עדכון תקופתית ל-service worker': 'src',
    '91|פסק זמן אחיד לקריאות רשת': 'src',
    '92|מאזיני מצב רשת': 'src',
    '159|גריעת tombstones לפי גיל': 'src',
    '197|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '128|גשר שיתוף': 'src',
    '69|העברת מזהה ל-DOM': 'src',
    '70|מיון אחד לכל התצוגות': 'src',
    '75|ייצוא — שני דפוסים מוצהרים': 'src',
    '120|שכבת אייקונים': 'src',
    '167|שכבת קלט אחידה': 'src',
    '167|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '76|שכבת המודאל': 'src',
    '169|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [30, 44, 46, 48, 54, 87, 52, 165, 104, 144, 56, 23, 61, 26, 35, 39];
...
const CAPS = {
  ...
  msgs: {
    ...
    block: { sha: '0bff21d948a043ea', lines: 35,
  ...
  pending: {
    ...
    block: { sha: '8e657ce5d286b942', lines: 285,
  ...
  backup: {
    ...
    block: { sha: 'edfe62ad4706f745', lines: 304,
  ...
  neterr: {
    ...
    block: { sha: 'ab8fd8922fd8d92d', lines: 31,
  ...
  swcore: {
    ...
    docRows: ['ליבת `sw.js`'],
    block: { file: 'sw.js', sha: '253ac487f92df550', lines: 250,
...
const MATRIX = [
  ...
  { row: 90, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 83, name: 'כפתור שכותב מושבת בזמן הכתיבה',
    probe: () => busyGuardGaps().length === 0 },
  { row: 82, name: 'מאזין אחד, בהאצלה מ-`document`',
  ...
  { row: 81, name: '`Enter` שומר בכל שדה עריכה',
  ...
  { row: 137, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 153, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 93, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 95, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 98, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 94, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 183, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 100, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 77, name: 'מיכל המודאל ילד ישיר של `body`',
  ...
  { row: 107, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 155, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 152, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 73, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 74, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 123, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 173, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 141, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 142, name: '`migrations/` — כמות',
  ...
  { row: 162, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 163, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 164, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 177, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 150, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 175, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 175, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 134, name: 'עריכת נתונים אופליין',
  ...
  { row: 160, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 176, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 71, name: 'אובייקט יקר נבנה פעם אחת',
  ...
  { row: 72, name: 'מסך מציג מיד ומרענן ברקע',
  ...
  { row: 127, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 181, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 78, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 172, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 88, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 166, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 128, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 113, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 116, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 132, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 138, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 139, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 57, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 57, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 135, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 79, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 84, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 84, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 108, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 58, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 169, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 169, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 199, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 57, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 154, name: 'פינוי גיבויים אוטומטי במסד',
   ...
   *  ⛔ ולא `NaN`. */
  { row: 60, name: 'מזהה אינו מומר למספר',
  ...
  { row: 113, name: 'מעטפת WebView חתומה',
  ...
  { row: 175, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 130, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 199, name: 'מודול ה-service worker',
  ...
  { row: 63, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 68, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 198, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 188, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 89, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 189, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 91, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 159, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 197, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 197, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 197, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 197, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 70, name: 'מיון אחד לכל התצוגות',
  ...
  { row: 75, name: 'ייצוא — שני דפוסים מוצהרים',
    probe: () => exportMarkupSites().length === 0 },
  { row: 120, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 167, name: 'שכבת קלט אחידה',
  ...
  { row: 167, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  178: { claim: 'אין `service_role` בקובץ' },
  32: { claim: 'מסלול «' },
  53: { claim: 'א · אין שתי שורות על אותו probe' },
  ...
  65:  { claim: 'שכבת הדחיפה' },
  /*  ⛔ שלוש טענות באותו שער (סבב 131) — ⚠️ המשפך, הודעת החסימה שהוא
   *  מחזיר, והקבוע שאתרי האימות מפנים אליו: ⭐ שלושתן על אותו מסלול. */
  180: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  66: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  143: { claim: 'בעלות הסכימה המשותפת' },
  157: { claim: '`tb_kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  158: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  191: { claim: 'ז. עמודה בלי קורא' },
  85: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  13: { claim: 'CANON_MD' },
  51: { claim: 'תוכן הקבצים הנלווים' },
  118: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  121: { claim: 'margin' },
  40: { manual: 'השוואת זמנים בין הריפו אינה בהישג ידו של שער שרץ בריפו אחד — ⛔ נאכפת בתוצאתה בלבד' },
  52: { claim: 'אין פרק פערים נפרד' },
  106: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  185: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  46: { claim: 'הערה ריקה' },
  48: { claim: 'COUNT_NOTE' },
  50: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  200: { claim: 'CACHE_NAME' },
  ...
  97: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  ...
  54: { claims: { 'check-capabilities': ['sha:', 'orderGaps', 'blockRowGaps'],
                  test_sharedsync: ['block-drift', 'canon-drift'],
                  test_signedshared: 'unsignedTwins',
                  test_toolsid: ['pure-tools', 'pure-undeclared'] } },
  87: { claim: 'storage' },
  86: { claim: 'ג · פעולה שדורשת רשת' },
  102: { claim: 'עברית' },
  104: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  105: { claim: 'BANNER_W' },
  187: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  103: { claim: 'מפנה לקובץ' },
  117: { claim: 'android/app/src/main' },
  124: { claim: 'fgDriftMax' },
  125: { claim: 'CANON_MANIFEST' },
  126: { claim: 'שדות זהים' },
  129: { claim: 'BUILD_SHA' },
  133: { claim: 'SHARED_SHA' },
  131: { claim: 'versionCode' },
  114: { claim: 'WebView' },
  115: { claim: 'android:theme' },
  119: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  136: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  140: { claim: 'migrations' },
  149: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  151: { claim: 'א. חותמת בכל רשומה' },
  56: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  144: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  145: { claim: 'ו. טבלה מקבילה' },
  146: { claim: 'ב. חתימת סכימה' },
  147: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  168: { claim: 'type=password' },
  80: { claim: 'aria-label' },
  ...
  61: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps'] } },
  174: { claim: 'pass_salt' },
  179: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  182: { claim: '⏳' },
  184: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  192: { claim: 'ט. ערך נגזר שהתיישן' },
  194: { claim: 'ח. הגדרה שהוחלפה' },
  193: { claim: 'ד. רשימת-היתר' },
  195: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  101: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  109: { claim: 'CLEANUP_SHA' },
  110: { claim: 'בלוק ה-APP' },
  165: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
