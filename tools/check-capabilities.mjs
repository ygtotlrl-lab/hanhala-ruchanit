...
const APP = {
  ...
  gapRows: [48, 130, 134, 158, 65, 143, 102],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    179: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    178: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    141: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    175: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '94|חותמת רעננות נכשלת סגור': 'src',
    '85|`Enter` שומר בכל שדה עריכה': 'src',
    '86|מאזין אחד, בהאצלה מ-`document`': 'src',
    '87|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '175|מבנה טופס הכניסה': 'src',
    '143|שם נגזר אינו נשמר ברשומה': 'src',
    '194|קבוע בלי קורא': 'mixed',
    '196|מפתח אחסון בלי קורא': 'src',
    '39|שער מריץ את כל טענותיו': 'tools',
    '93|שכבת המראה': 'src',
    '197|הגירה מקומית שהושלמה': 'mixed',
    '177|מראת המשתמשים': 'src',
    '176|מודל ההרשאות': 'src',
    '26|שער סורק קוד מולבן': 'tools',
    '26|ההלבנה שומרת על מספרי השורות': 'mixed',
    '159|הקשר נלכד בכניסה לפונקציה': 'src',
    '97|פריסה במסכי טלפון וטאבלט': 'src',
    '98|ערכת נושא — בהיר וכהה': 'src',
    '99|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '100|אין סגנון מוטבע': 'src',
    '102|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '190|הערה שמתארת מצב שחלף': 'mixed',
    '198|שאילתת `@media` מתה': 'src',
    '117|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '118|ייצוא והנפקה — דרך מוצהרת': 'src',
    '103|המסמך מתפרסר נקי': 'src',
    '104|לוג מסלול — תג ורמה': 'src',
    '81|מיכל המודאל ילד ישיר של `body`': 'src',
    '113|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '161|הגיבוי שלם ומעומד': 'src',
    '77|פעולה מגיבה מיד': 'src',
    '78|משיכה מסוננת בשרת': 'src',
    '129|דגימת-יתר במחולל': 'tools',
    '179|`PBKDF2` — פרמטרים': 'src',
    '147|`migrations/` — תוכן': 'src',
    '148|`migrations/` — כמות': 'src',
    '168|מיזוג מכל — מפה ומערך': 'src',
    '169|ברירת מחדל אינה ממוזגת': 'src',
    '170|רשימה אינה נושאת פריט כפול': 'src',
    '162|`sh_backup` — טבלת הגיבוי': 'src',
    '24|שער אינו מקליד מספר שורה': 'tools',
    '193|פונקציה בלי קוראים': 'mixed',
    '30|שער ממתין לתנאי ולא לשעון': 'tools',
    '183|סודות אינם עוזבים את המכשיר': 'src',
    '156|דפוס `upsert` — onConflict מוכרז': 'src',
    '181|טביעה חסרה מושלמת אוטומטית': 'src',
    '181|כניסה אופליין': 'src',
    '140|עריכת נתונים אופליין': 'src',
    '166|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '182|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '75|אובייקט יקר נבנה פעם אחת': 'src',
    '76|מסך מציג מיד ומרענן ברקע': 'src',
    '133|האפליקציה ניתנת להתקנה מכרום': 'src',
    '187|דגלי מעבר — אין דגל דלוק': 'src',
    '82|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '178|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '48|ארבעת מימדי הסריקה': 'src',
    '92|פינוי אוטומטי': 'src',
    '92|אופק הפינוי נוקה מהסנכרון': 'src',
    '172|אימות פינוי מול הענן': 'src',
    '134|שיתוף קבצים': 'src',
    '119|מעטפת APK (WebView)': 'src',
    '122|אין נכסים מוטבעים': 'src',
    '138|מפתח חתימה קבוע': 'src',
    '144|מקור אמת יחיד לסכימה': 'src',
    '145|קובץ התקנה מלא': 'src',
    '61|גיבוי יומי אוטומטי': 'src',
    '61|יומן פעולות': 'src',
    '141|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '83|דפוס הודעת שגיאה יחיד': 'src',
    '88|`toast` — חתימה, גוף ומחלקות': 'src',
    '88|סיווג ההודעה נגזר מהמסלול': 'src',
    '205|מיכל באנר העדכון במקור': 'src',
    '205|הרענון מ-controllerchange בלבד': 'src',
    '205|סימן דחיית הבאנר מתמיד': 'src',
    '114|רישום כשלי כתיבה': 'src',
    '62|חלון חם במכשיר': 'src',
    '175|מסך שינוי סיסמה עצמי': 'src',
    '207|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '63|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '181|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '64|מודול מזהי רשומות': 'src',
    '64|מזהה אינו מומר למספר': 'src',
    '66|מזהה מכשיר': 'src',
    '63|ליבת מיזוג משותפת': 'src',
    '119|מעטפת WebView חתומה': 'tools',
    '181|אימות מול טביעה בענן': 'mixed',
    '136|בניית APK אחידה עם שער חתימה': 'src',
    '207|מודול ה-service worker': 'src',
    '67|ניסיון חוזר בתור הסנכרון': 'src',
    '68|מנגנון משיכה אחיד': 'src',
    '71|נעילת חוסר-פעילות': 'src',
    '72|מודל הסשן — בזיכרון בלבד': 'src',
    '206|בדיקת עדכון תקופתית ל-service worker': 'src',
    '95|פסק זמן אחיד לקריאות רשת': 'src',
    '96|מאזיני מצב רשת': 'src',
    '165|גריעת tombstones לפי גיל': 'src',
    '205|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '134|גשר שיתוף': 'src',
    '73|העברת מזהה ל-DOM': 'src',
    '74|מיון אחד לכל התצוגות': 'src',
    '79|ייצוא — שני דפוסים מוצהרים': 'src',
    '126|שכבת אייקונים': 'src',
    '173|שכבת קלט אחידה': 'src',
    '173|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '80|שכבת המודאל': 'src',
    '175|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [32, 47, 49, 51, 57, 91, 55, 171, 109, 150, 60, 25, 65, 28, 37, 41];
...
const BLOCK_ORDER = ['toastcss', 'bchartcss', 'scales', 'msgs', 'bp', 'neterr', 'rowswin', 'storage', 'schemastale', 'busyguard', 'savepipe', 'swreg', 'backup',
                     'pending', 'retry', 'lock', 'sess', 'isAdmin', 'pull', 'push', 'hotwin', 'mirror',
...
const FLOOR = { shared: 84, app: 9, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי' };
/* ⚠️ סוף פר-אפליקציה */
...
const MATRIX = [
  ...
  { row: 94, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 87, name: 'כפתור שכותב מושבת בזמן הכתיבה',
    probe: () => busyGuardGaps().length === 0 },
  { row: 86, name: 'מאזין אחד, בהאצלה מ-`document`',
  ...
  { row: 85, name: '`Enter` שומר בכל שדה עריכה',
  ...
  { row: 143, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 159, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 97, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 99, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 102, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 98, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 190, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 104, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 81, name: 'מיכל המודאל ילד ישיר של `body`',
  ...
  { row: 113, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 161, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 158, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 77, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 78, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 129, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 179, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 147, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 148, name: '`migrations/` — כמות',
  ...
  { row: 168, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 169, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 170, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 183, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 156, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 181, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 181, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 140, name: 'עריכת נתונים אופליין',
  ...
  { row: 166, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 182, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 75, name: 'אובייקט יקר נבנה פעם אחת',
  ...
  { row: 76, name: 'מסך מציג מיד ומרענן ברקע',
  ...
  { row: 133, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 187, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 82, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 178, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 92, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 172, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 134, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 119, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 122, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 138, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 144, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 145, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 61, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 61, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 141, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 83, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 88, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 88, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 114, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 62, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 175, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 175, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 207, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 61, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 160, name: 'פינוי גיבויים אוטומטי במסד',
   ...
   *  ⛔ ולא `NaN`. */
  { row: 64, name: 'מזהה אינו מומר למספר',
  ...
  { row: 119, name: 'מעטפת WebView חתומה',
  ...
  { row: 181, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 136, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 207, name: 'מודול ה-service worker',
  ...
  { row: 67, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 72, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 206, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 194, name: 'קבוע בלי קורא',
    probe: () => constNoReaderGaps().length === 0 },
  { row: 196, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 93, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 197, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 95, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 165, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 205, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 205, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 205, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 205, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 74, name: 'מיון אחד לכל התצוגות',
  ...
  { row: 79, name: 'ייצוא — שני דפוסים מוצהרים',
    probe: () => exportMarkupSites().length === 0 },
  { row: 126, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 173, name: 'שכבת קלט אחידה',
  ...
  { row: 173, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  189: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  184: { claim: 'אין `service_role` בקובץ' },
  34: { claim: 'מסלול «' },
  ...
  69:  { claim: 'שכבת הדחיפה' },
  /*  ⛔ שלוש טענות באותו שער (סבב 131) — ⚠️ המשפך, הודעת החסימה שהוא
   *  מחזיר, והקבוע שאתרי האימות מפנים אליו: ⭐ שלושתן על אותו מסלול. */
  186: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  70: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  149: { claim: 'בעלות הסכימה המשותפת' },
  163: { claim: '`tb_kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  164: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  199: { claim: 'ז. עמודה בלי קורא' },
  89: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  192: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  101: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  ...
  91: { claim: 'storage' },
  90: { claim: 'ג · פעולה שדורשת רשת' },
  106: { claim: 'עברית' },
  109: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  110: { claim: 'BANNER_W' },
  195: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  107: { claim: 'מפנה לקובץ' },
  123: { claim: 'android/app/src/main' },
  130: { claim: 'fgDriftMax' },
  131: { claim: 'CANON_MANIFEST' },
  132: { claim: 'שדות זהים' },
  135: { claim: 'BUILD_SHA' },
  139: { claim: 'SHARED_SHA' },
  137: { claim: 'versionCode' },
  120: { claim: 'WebView' },
  121: { claim: 'android:theme' },
  125: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  142: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  146: { claim: 'migrations' },
  155: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  157: { claim: 'א. חותמת בכל רשומה' },
  60: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  150: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  151: { claim: 'ו. טבלה מקבילה' },
  152: { claim: 'ב. חתימת סכימה' },
  153: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  174: { claim: 'type=password' },
  84: { claim: 'aria-label' },
  ...
  65: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps'] } },
  180: { claim: 'pass_salt' },
  /*  ⭐ סבב 148 — ⛔ סריקה הפוכה: ⚠️ כל אתר חשוד מפיל אלא אם הוא
   *  מוכרז עם נימוקו, ⭐ ורשימת דפוסים הייתה מוצאת את מה שכבר תוקן. */
  185: { claim: '[sec-keyfile]' },
  188: { claim: '⏳' },
  191: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  200: { claim: 'ט. ערך נגזר שהתיישן' },
  202: { claim: 'ח. הגדרה שהוחלפה' },
  201: { claim: 'ד. רשימת-היתר' },
  203: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  105: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  115: { claim: 'CLEANUP_SHA' },
  116: { claim: 'בלוק ה-APP' },
  171: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
