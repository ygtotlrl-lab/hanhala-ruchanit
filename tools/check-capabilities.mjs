...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    158: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    157: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    154: (c) => !!c.fnBody('changeMyPassword')
   ...
   *  להיכשל. */
  probeInput: {
    '170|מפתח אחסון בלי קורא': 'src',
    '33|שער מריץ את כל טענותיו': 'tools',
    '84|שכבת המראה': 'src',
    '171|הגירה מקומית שהושלמה': 'mixed',
    '156|מראת המשתמשים': 'src',
    '155|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'tools',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '140|הקשר נלכד בכניסה לפונקציה': 'src',
    '87|פריסה במסכי טלפון וטאבלט': 'src',
    '88|ערכת נושא — בהיר וכהה': 'src',
    '166|הערה שמתארת מצב שחלף': 'mixed',
    '172|שאילתת `@media` מתה': 'src',
    ...
    '142|הגיבוי שלם ומעומד': 'src',
    '72|פעולה מגיבה מיד': 'src',
    '73|משיכה מסוננת בשרת': 'src',
    '113|דגימת-יתר במחולל': 'tools',
    '158|`PBKDF2` — פרמטרים': 'src',
    '130|`migrations/` — תוכן': 'src',
    '131|`migrations/` — כמות': 'src',
    '147|מיזוג מכל — מפה ומערך': 'src',
    '148|ברירת מחדל אינה ממוזגת': 'src',
    '149|רשימה אינה נושאת פריט כפול': 'src',
    '143|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'tools',
    '168|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'tools',
    '162|סודות אינם עוזבים את המכשיר': 'src',
    '137|דפוס `upsert` — onConflict מוכרז': 'src',
    '160|טביעה חסרה מושלמת אוטומטית': 'src',
    '160|כניסה אופליין': 'src',
    '124|עריכת נתונים אופליין': 'src',
    '146|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '161|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '70|אובייקט יקר נבנה פעם אחת': 'src',
    '71|מסך מציג מיד ומרענן ברקע': 'src',
    '117|האפליקציה ניתנת להתקנה מכרום': 'src',
    '164|דגלי מעבר — אין דגל דלוק': 'src',
    '77|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '157|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '41|ארבעת מימדי הסריקה': 'src',
    '83|פינוי אוטומטי': 'src',
    '151|אימות פינוי מול הענן': 'src',
    '118|שיתוף קבצים': 'src',
    '103|מעטפת APK (WebView)': 'src',
    '106|אין נכסים מוטבעים': 'src',
    '122|מפתח חתימה קבוע בריפו': 'src',
    '127|מקור אמת יחיד לסכימה': 'src',
    '128|קובץ התקנה מלא': 'src',
...
export const ROWS = [29, 40, 42, 44, 53, 82, 47, 150, 94, 133, 54, 22, 59, 25];
...
const CAPS = {
  ...
  storage: {
    ...
    block: { sha: 'd93ff78ff9cdfeaf', lines: 693,
...
const FLOOR = { shared: 152, app: 10, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי ושער מצב הרשת' };
/* ⚠️ סוף פר-אפליקציה */
...
const MATRIX = [
  ...
  { row: 140, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 166, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 142, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 139, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 158, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 130, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 131, name: '`migrations/` — כמות',
  ...
  { row: 147, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 149, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 162, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 137, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 160, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 146, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 161, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 164, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 157, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 151, name: 'אימות פינוי מול הענן',
  ...
  { row: 122, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 127, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 128, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  ...
  { row: 154, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 179, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 141, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 160, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 160, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 179, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 178, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 145, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 152, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  95: { claim: 'BANNER_W' },
  169: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  129: { claim: 'migrations' },
  136: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  138: { claim: 'א. חותמת בכל רשומה' },
  54: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  133: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  134: { claim: 'ו. טבלה מקבילה' },
  135: { claim: 'ב. חתימת סכימה' },
  153: { claim: 'type=password' },
  79: { claim: 'aria-label' },
  ...
  159: { claim: 'pass_salt' },
  163: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  165: { claim: '⏳' },
  49: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  167: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  175: { claim: 'ח. הגדרה שהוחלפה' },
  174: { claim: 'ד. רשימת-היתר' },
  176: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  150: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
