...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    105: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    102:  (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    87: (c) => c.hasCode(/YS_KV_LEGACY_WRITE\s*=\s*false/),
    ...
    101: (c) => !!c.fnBody('changeMyPassword')
...
const MATRIX = [
  ...
  { row: 63, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
