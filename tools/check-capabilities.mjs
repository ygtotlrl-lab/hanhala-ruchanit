...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    159: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    158: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    155: (c) => !!c.fnBody('changeMyPassword')
   ...
   *  להיכשל. */
  probeInput: {
    '127|שם נגזר אינו נשמר ברשומה': 'src',
    '171|מפתח אחסון בלי קורא': 'src',
    '33|שער מריץ את כל טענותיו': 'tools',
    '84|שכבת המראה': 'src',
    '172|הגירה מקומית שהושלמה': 'mixed',
    '157|מראת המשתמשים': 'src',
    '156|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'tools',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '141|הקשר נלכד בכניסה לפונקציה': 'src',
    '87|פריסה במסכי טלפון וטאבלט': 'src',
    '88|ערכת נושא — בהיר וכהה': 'src',
    '167|הערה שמתארת מצב שחלף': 'mixed',
    '173|שאילתת `@media` מתה': 'src',
    '101|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '102|ייצוא והנפקה — דרך מוצהרת': 'src',
    '89|המסמך מתפרסר נקי': 'src',
    '90|לוג מסלול — תג ורמה': 'src',
    '76|מיכל המודאל ילד ישיר של `body`': 'src',
    '97|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '143|הגיבוי שלם ומעומד': 'src',
    '72|פעולה מגיבה מיד': 'src',
    '73|משיכה מסוננת בשרת': 'src',
    '113|דגימת-יתר במחולל': 'tools',
    '159|`PBKDF2` — פרמטרים': 'src',
    '131|`migrations/` — תוכן': 'src',
    '132|`migrations/` — כמות': 'src',
    '148|מיזוג מכל — מפה ומערך': 'src',
    '149|ברירת מחדל אינה ממוזגת': 'src',
    '150|רשימה אינה נושאת פריט כפול': 'src',
    '144|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'tools',
    '169|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'tools',
    '163|סודות אינם עוזבים את המכשיר': 'src',
    '138|דפוס `upsert` — onConflict מוכרז': 'src',
    '161|טביעה חסרה מושלמת אוטומטית': 'src',
    '161|כניסה אופליין': 'src',
    '124|עריכת נתונים אופליין': 'src',
    '147|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '162|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '70|אובייקט יקר נבנה פעם אחת': 'src',
    '71|מסך מציג מיד ומרענן ברקע': 'src',
    '117|האפליקציה ניתנת להתקנה מכרום': 'src',
    '165|דגלי מעבר — אין דגל דלוק': 'src',
    '77|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '158|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '41|ארבעת מימדי הסריקה': 'src',
    '83|פינוי אוטומטי': 'src',
    '152|אימות פינוי מול הענן': 'src',
    '118|שיתוף קבצים': 'src',
    '103|מעטפת APK (WebView)': 'src',
    '106|אין נכסים מוטבעים': 'src',
    '122|מפתח חתימה קבוע בריפו': 'src',
    '128|מקור אמת יחיד לסכימה': 'src',
    '129|קובץ התקנה מלא': 'src',
    '55|גיבוי יומי אוטומטי': 'src',
    '55|יומן פעולות': 'src',
    '125|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '78|דפוס הודעת שגיאה יחיד': 'src',
    '80|`toast` — חתימה, גוף ומחלקות': 'src',
    '98|רישום כשלי כתיבה': 'src',
    '56|חלון חם במכשיר': 'src',
    '56|שחזור מקומי מהענן': 'src',
    '155|מסך שינוי סיסמה עצמי': 'src',
    '181|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '57|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '161|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '58|מודול מזהי רשומות': 'src',
    '58|מזהה אינו מומר למספר': 'src',
    '60|מזהה מכשיר': 'src',
    '57|ליבת מיזוג משותפת': 'src',
    '103|מעטפת WebView חתומה': 'tools',
    '161|אימות מול טביעה בענן': 'mixed',
    '120|בניית APK אחידה עם שער חתימה': 'src',
    '181|מודול ה-service worker': 'src',
    '61|ניסיון חוזר בתור הסנכרון': 'src',
    '62|מנגנון משיכה אחיד': 'src',
    '65|נעילת חוסר-פעילות': 'src',
    '66|מודל הסשן — בזיכרון בלבד': 'src',
    '180|בדיקת עדכון תקופתית ל-service worker': 'src',
    '85|פסק זמן אחיד לקריאות רשת': 'src',
    '86|מאזיני מצב רשת': 'src',
    '146|גריעת tombstones לפי גיל': 'src',
    '179|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '118|גשר שיתוף': 'src',
    '68|העברת מזהה ל-DOM': 'src',
    '69|מיון אחד לכל התצוגות': 'src',
    '74|ייצוא — שני דפוסים מוצהרים': 'src',
    '110|שכבת אייקונים': 'src',
    '153|שכבת קלט אחידה': 'src',
    '153|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '75|שכבת המודאל': 'src',
    '155|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [29, 40, 42, 44, 53, 82, 47, 151, 94, 134, 54, 22, 59, 25];
...
const FLOOR = { shared: 153, app: 10, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי ושער מצב הרשת' };
/* ⚠️ סוף פר-אפליקציה */
...
const MATRIX = [
  ...
  { row: 127, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 141, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 87, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 88, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 167, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 97, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 143, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 140, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 113, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 159, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 131, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 132, name: '`migrations/` — כמות',
  ...
  { row: 148, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 149, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 150, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 163, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 138, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 161, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 161, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 124, name: 'עריכת נתונים אופליין',
  ...
  { row: 147, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 162, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 117, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 165, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 158, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 152, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 118, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 103, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 106, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 122, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 128, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 129, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 55, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 55, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 125, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 80, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 98, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  ...
  { row: 155, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 181, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 142, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 161, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 103, name: 'מעטפת WebView חתומה',
  ...
  { row: 161, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 120, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 181, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 180, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 146, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 179, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 110, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 153, name: 'שכבת קלט אחידה',
  ...
  { row: 153, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  133: { claim: 'בעלות הסכימה המשותפת' },
  145: { claim: '`kv_rishon`' },
  174: { claim: 'ז. עמודה בלי קורא' },
  ...
  108: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  111: { claim: 'margin' },
  ...
  96: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  67: { claim: 'CANON' },
  82: { claim: 'storage' },
  81: { claim: 'ג · פעולה שדורשת רשת' },
  92: { claim: 'עברית' },
  94: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  95: { claim: 'BANNER_W' },
  170: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  93: { claim: 'מפנה לקובץ' },
  107: { claim: 'android/app/src/main' },
  114: { claim: 'fgDriftMax' },
  115: { claim: 'CANON_MANIFEST' },
  116: { claim: 'שדות זהים' },
  119: { claim: 'BUILD_SHA' },
  123: { claim: 'SHARED_SHA' },
  121: { claim: 'versionCode' },
  104: { claim: 'WebView' },
  105: { claim: 'android:theme' },
  109: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  126: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  130: { claim: 'migrations' },
  137: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  139: { claim: 'א. חותמת בכל רשומה' },
  54: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  134: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  135: { claim: 'ו. טבלה מקבילה' },
  136: { claim: 'ב. חתימת סכימה' },
  154: { claim: 'type=password' },
  79: { claim: 'aria-label' },
  ...
  160: { claim: 'pass_salt' },
  164: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  166: { claim: '⏳' },
  49: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  168: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  175: { claim: 'ט. ערך נגזר שהתיישן' },
  177: { claim: 'ח. הגדרה שהוחלפה' },
  176: { claim: 'ד. רשימת-היתר' },
  178: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  91: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  99: { claim: 'CLEANUP_SHA' },
  100: { claim: 'בלוק ה-APP' },
  151: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
