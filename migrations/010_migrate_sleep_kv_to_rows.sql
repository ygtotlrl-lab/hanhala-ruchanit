-- ============================================================================
-- 010_migrate_sleep_kv_to_rows.sql — העברת סדרי השינה לשכבת השורות
-- ============================================================================
--
...
-- ⛔ זו הנקודה שאסור להפוך ל-`do update`:
...
-- `updatedAt` משלו ב-`kv`. ⛔ אין להמציא כאן `now()` — חותמת
...
--: הדוח פר-תלמיד קורא את טבלת הבן **בלי join לאב**, ולכן סימון
...
-- בטענה ש«השדה תמיד קיים»: הוא אינו.
-- 3. ⭐ **`note` מועבר, ולא מושמט** — זה ההבדל היחיד מהנוכחות.
...
-- מיושם כאן **מלכתחילה**: מזהה תלמיד הוא uuid, וסינון `^\d+$`
...
-- -- נבדק ב-`kv`: 21 סדרים · 988 סימונים.
