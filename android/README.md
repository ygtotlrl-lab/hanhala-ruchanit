...
| **versionCode** | 1 — המעטפת הראשונה בריפו הזה, טוענת מהרשת. (ה-APK הישן `yeshiva-manager.apk` נבנה מחוץ לריפו במפתח זמני — הוא איננו אב של המעטפת הזו, ר' CLAUDE.md) |
...
ה-APK **החתום** יורד כ-artifact בשם `hanhala-ruchanit-signed-apk`.
...
```bash
../signing/sign-apk.sh app/build/outputs/apk/release/app-release-unsigned.apk hanhala.apk
```
