...
const APP = {
  ...
  appGates: {
      'attend': 'מודד את המפתח הנטוש של הנוכחות ואת המנגנון החי ששרד — ⛔ ולשלוש האחרות אין מסלול נוכחות שהוחלף',
      'dupsession': 'מודד שכלל מניעת כפילות הסדרים מוגדר פעם אחת ונבדק בשני אתרי היצירה, ובאימוץ שאינו מוחק סימונים — ⛔ ולשלוש האחרות אין סדר שנפתח ונסגר לאותו יום ואין שתי נקודות יצירה לאותו דבר',
      ...
      'import': 'מודד את מסלול ייבוא התלמידים ואת הכתיבה המקומית שנכשלת ברעש — ⛔ ולשלוש האחרות אין קליטה המונית מקובץ',
      'mirror_split': 'מודד את פיצול רשומת הסדר לשורות ואת ארבעת מקרי המיזוג שבה — ⛔ ולשלוש האחרות אין רשומה שנושאת מפה של סימונים',
      ...
      'roles': 'מודד את מודל ההרשאות ואת שלוש דרגות התפקיד — ⛔ וביומן אין כניסה ואין תפקיד שיוכרע',
      'offline_login': 'מודד את הכניסה האופליין מול הטביעה ואת ארבע התשובות שלה — ⛔ וביומן אין כניסה ואין משתמש שיאומת',
      'read': 'מודד את היעדר הנפילה-חזרה למפתח-ערך ואת משפך הקריאה האחד — ⛔ ולשכר ולגיוס לא הייתה מעולם שכבת מפתח-ערך שאפשר ליפול אליה',
      'sleep_rows': 'מודד את שכבת השורות של סדרי השינה ואת מצבת המחיקה שבה — ⛔ ולשלוש האחרות אין מעקב שינה',
      'tables': 'מודד את מבנה שכבת השורות של הנוכחות והשינה — ⛔ ולשלוש האחרות אין סדר שנפתח ונסגר עם סימונים',
      'window': 'מודד את חלון החודש העברי ואת הפעלת החלון החם עליו — ⛔ ולשלוש האחרות אין מסך שמוצג לפי חודש עברי',
  ...
  only: {
    ...
    'setup-db.html': 'כלי עזר חד-פעמי שנכתב בסבב 92 ומושך את migrations/000 — ⛔ ואינו עותק של האפליקציה (כלל קריטי 6)',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [17, 20, 21, 117, 181];
