...
const APP = {
  ...
  scanKind: {
    ...
    'test_attend': 'raw — מריץ את מסלול הנוכחות בארגז חול, ומחרוזות הן הנתונים',
    ...
    'test_import': 'raw — מריץ את מסלול הייבוא בארגז חול',
    ...
    'test_dupsession': 'raw — מריץ את כלל הכפילות בארגז חול, ושמות הסדרים הם מחרוזות',
    ...
    'test_sleep_rows': 'raw — מודד שמות טבלאות שינה — כולם מחרוזות',
    'test_sources': 'raw — מודד את הצהרת המקורות — שמות טבלאות במחרוזות',
    'test_stage_a': 'raw — מריץ את שלב א בארגז חול',
    'test_tables': 'raw — מודד שמות טבלאות ומסלולי כתיבה שחיים כמחרוזות',
    'test_window': 'raw — מודד קריאות עם חלון — הארגומנטים הם מחרוזות',
  ...
  gapRows: [226, 140, 144, 168, 67, 153, 108],
  ...
  tableProbe: {
    ...
    187: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '99|חותמת רעננות נכשלת סגור': 'src',
    '88|`Enter` שומר בכל שדה עריכה': 'src',
    '89|מאזין אחד, בהאצלה מ-`document`': 'src',
    '92|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '187|מבנה טופס הכניסה': 'src',
    '153|שם נגזר אינו נשמר ברשומה': 'src',
    '209|קבוע בלי קורא': 'mixed',
    '212|מפתח אחסון בלי קורא': 'src',
    '39|שער מריץ את כל טענותיו': 'tools',
    '98|שכבת המראה': 'src',
    '213|הגירה מקומית שהושלמה': 'mixed',
    '189|מראת המשתמשים': 'src',
    '188|מודל ההרשאות': 'src',
    '26|שער סורק קוד מולבן': 'tools',
    '26|ההלבנה שומרת על מספרי השורות': 'mixed',
    '169|הקשר נלכד בכניסה לפונקציה': 'src',
    '102|פריסה במסכי טלפון וטאבלט': 'src',
    '103|ערכת נושא — בהיר וכהה': 'src',
    '105|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '106|אין סגנון מוטבע': 'src',
    '108|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '203|הערה שמתארת מצב שחלף': 'mixed',
    '214|שאילתת `@media` מתה': 'src',
    '126|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '127|ייצוא והנפקה — דרך מוצהרת': 'src',
    '109|המסמך מתפרסר נקי': 'src',
    '110|לוג מסלול — תג ורמה': 'src',
    '83|מיכל המודאל ילד ישיר של `body`': 'src',
    '122|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '171|הגיבוי שלם ומעומד': 'src',
    '79|פעולה מגיבה מיד': 'src',
    '80|משיכה מסוננת בשרת': 'src',
    '139|דגימת-יתר במחולל': 'tools',
    '191|`PBKDF2` — פרמטרים': 'src',
    '157|`migrations/` — תוכן': 'src',
    '158|`migrations/` — כמות': 'src',
    '179|מיזוג מכל — מפה ומערך': 'src',
    '180|ברירת מחדל אינה ממוזגת': 'src',
    '181|רשימה אינה נושאת פריט כפול': 'src',
    '172|`sh_backup` — טבלת הגיבוי': 'src',
    '24|שער אינו מקליד מספר שורה': 'tools',
    '207|פונקציה בלי קוראים': 'mixed',
    '30|שער ממתין לתנאי ולא לשעון': 'tools',
    '195|סודות אינם עוזבים את המכשיר': 'src',
    '166|דפוס `upsert` — onConflict מוכרז': 'src',
    '193|טביעה חסרה מושלמת אוטומטית': 'src',
    '193|כניסה אופליין': 'src',
    '150|עריכת נתונים אופליין': 'src',
    '176|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '194|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '77|אובייקט יקר נבנה פעם אחת': 'src',
    '78|מסך מציג מיד ומרענן ברקע': 'src',
    '143|האפליקציה ניתנת להתקנה מכרום': 'src',
    '200|דגלי מעבר — אין דגל דלוק': 'src',
    '85|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '190|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '97|פינוי אוטומטי': 'src',
    '97|אופק הפינוי נוקה מהסנכרון': 'src',
    '183|אימות פינוי מול הענן': 'src',
    '144|שיתוף קבצים': 'src',
    '128|מעטפת APK (WebView)': 'src',
    '131|אין נכסים מוטבעים': 'src',
    '148|מפתח חתימה קבוע': 'src',
    '154|מקור אמת יחיד לסכימה': 'src',
    '155|קובץ התקנה מלא': 'src',
    '63|גיבוי יומי אוטומטי': 'src',
    '63|יומן פעולות': 'src',
    '151|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '86|דפוס הודעת שגיאה יחיד': 'src',
    '93|`toast` — חתימה, גוף ומחלקות': 'src',
    '93|סיווג ההודעה נגזר מהמסלול': 'src',
    '225|מיכל באנר העדכון במקור': 'src',
    '225|הרענון מ-controllerchange בלבד': 'src',
    '225|סימן דחיית הבאנר מתמיד': 'src',
    '123|רישום כשלי כתיבה': 'src',
    '64|חלון חם במכשיר': 'src',
    '187|מסך שינוי סיסמה עצמי': 'src',
    '227|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '65|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '193|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '66|מודול מזהי רשומות': 'src',
    '66|מזהה אינו מומר למספר': 'src',
    '68|מזהה מכשיר': 'src',
    '65|ליבת מיזוג משותפת': 'src',
    '128|מעטפת WebView חתומה': 'tools',
    '193|אימות מול טביעה בענן': 'mixed',
    '146|בניית APK אחידה עם שער חתימה': 'src',
    '227|מודול ה-service worker': 'src',
    '69|ניסיון חוזר בתור הסנכרון': 'src',
    '70|מנגנון משיכה אחיד': 'src',
    '73|נעילת חוסר-פעילות': 'src',
    '74|מודל הסשן — בזיכרון בלבד': 'src',
    '226|בדיקת עדכון תקופתית ל-service worker': 'src',
    '100|פסק זמן אחיד לקריאות רשת': 'src',
    '101|מאזיני מצב רשת': 'src',
    '175|גריעת tombstones לפי גיל': 'src',
    '225|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '144|גשר שיתוף': 'src',
    '75|העברת מזהה ל-DOM': 'src',
    '76|מיון אחד לכל התצוגות': 'src',
    '81|ייצוא — שני דפוסים מוצהרים': 'src',
    '136|שכבת אייקונים': 'src',
    '184|שכבת קלט אחידה': 'src',
    '184|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '82|שכבת המודאל': 'src',
    '187|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [32, 47, 48, 50, 59, 96, 57, 182, 115, 160, 62, 25, 67, 28, 37, 41, 198];
...
const CAPS = {
  ...
  rowswin: {
    ...
    block: { sha: '20d09e44161006e5', lines: 41,
...
const FLOOR = { shared: 87, app: 9, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי' };
/* ⚠️ סוף פר-אפליקציה */
...
const MATRIX = [
  ...
  { row: 99, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 92, name: 'כפתור שכותב מושבת בזמן הכתיבה',
  ...
  { row: 153, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 26, name: 'שער סורק קוד מולבן',
    probe: () => scanKindGaps().length === 0 },
  { row: 26, name: 'ההלבנה שומרת על מספרי השורות',
  ...
  { row: 169, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 102, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 105, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 108, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 103, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 203, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 110, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 122, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 171, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 168, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 139, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 191, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 157, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 158, name: '`migrations/` — כמות',
  ...
  { row: 179, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 180, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 181, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 172, name: '`sh_backup` — טבלת הגיבוי',
    probe: () => (/BK_TABLE\s*=\s*'([a-z_]+)'/.exec(srcRefs) || [])[1] === 'sh_backup' },
  { row: 24, name: 'שער אינו מקליד מספר שורה',
    probe: () => typedRowSites().length === 0 },
  { row: 207, name: 'פונקציה בלי קוראים',
  ...
  { row: 30, name: 'שער ממתין לתנאי ולא לשעון',
  ...
  { row: 195, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 166, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 193, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 193, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 150, name: 'עריכת נתונים אופליין',
  ...
  { row: 176, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 194, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 143, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 200, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 190, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 97, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 183, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 144, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 128, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 131, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 148, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 154, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 155, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 63, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 63, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 151, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 93, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 93, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 123, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 64, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 187, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 187, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 227, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 170, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 193, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 128, name: 'מעטפת WebView חתומה',
  ...
  { row: 193, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 146, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 227, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 226, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 39, name: 'שער מריץ את כל טענותיו',
    probe: () => gateSealGaps().length === 0 },
  { row: 209, name: 'קבוע בלי קורא',
    probe: () => constNoReaderGaps().length === 0 },
  { row: 212, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 98, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 213, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 100, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 175, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 225, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 225, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 225, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 225, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 136, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 184, name: 'שכבת קלט אחידה',
  ...
  { row: 184, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  104: { claims: { test_visual: ['[semantic-role]'] } },
  ...
  208: { claims: { test_codescan: ['[call-arity]', '[call-arity-registry]'] } },
  /*  ⭐ סבב 150 — ⛔ השוואה לאפס דורשת שהיעד קיים: ⚠️ אפס מתוך אפס
   *  אינו השלמה, ⭐ והשומר על היעד ולא על ההפרש. */
  185: { claims: { test_codescan: ['[zero-guard]', '[zero-guard-registry]'] } },
  ...
  118: { claims: { test_failsurface: ['[fail-surface]', '[fail-literal]', '[queued-msg]', '[msg-shared]'] } },
  /*  ⭐ סבב 148 — ⛔ אסימון עיצוב בדפוס אחד: ⚠️ המספר שנושא מקף,
   *  ⭐ האסימון שחי כאן בלבד ומוצהר, ⛔ והחריץ שעומד במבחן המכני. */
  91: { claims: { test_names: ['[token-pattern]', '[token-app]', '[token-slot]'] } },
  ...
  198: { claims: { test_docfacts: ['[doc-name]', '[doc-round]'],
  ...
  229: { claims: { test_behavior: ['fresh'] } },
  ...
  114: { claims: { test_sistername: ['[sister-comment]', '[sister-ambig]'] } },
  ...
  51: { claims: { test_rowscan: ['ז · כל מרשם מוכרז', 'ח · מפקד נגזר ואינו מוקלד'] } },
  /*  ⭐ סבב 145 — ⛔ אב-ובן: ⚠️ הבן יורש את המחיקה ואת החותמת,
   *  ⭐ והאב נדחף לפניו. */
  177: { claims: { test_parentchild: ['[pc-inherit]', '[pc-order-live]'] } },
  /*  ⭐ סבב 144 — ⛔ הגדרת פונקציה נקראת מהמסד: ⚠️ `rename` אינו נוגע
   *  בגוף `plpgsql`, ⭐ ומיגרציה שרצה היא היסטוריה. */
  164: { claim: 'ל1 · ההגדרה החיה נקראה מהמסד' },
  ...
  196: { claim: 'אין `service_role` בקובץ' },
  34: { claim: 'מסלול «' },
  58: { claim: 'א · אין שתי שורות על אותו probe' },
  38: { claim: 'א · כל דפוס מוכרז נושא מוטציה' },
  33: { claim: 'א · כל מפתח נקרא' },
  ...
  199: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  72: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  159: { claim: 'בעלות הסכימה המשותפת' },
  173: { claim: '`ya_settings_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  174: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  215: { claim: 'ז. עמודה בלי קורא' },
  94: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  134: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  137: { claim: 'margin' },
  42: { manual: 'השוואת זמנים בין הריפו אינה בהישג ידו של שער שרץ בריפו אחד — ⛔ נאכפת בתוצאתה בלבד' },
  57: { claim: 'אין פרק פערים נפרד' },
  /*  ⭐ סבב 148 — ⛔ שם פונקציה שחוזר ביותר מריפו אחד בתוכן שונה:
   *  ⚠️ מוצהר ב-`APP.productFns` עם היכולת שמצדיקה, ⭐ ונמדד בטביעת הגוף. */
  121: { claims: { test_codescan: ['[fn-product]'] },
  ...
  206: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
   ...
   *  מוטציה שאין בו. */
  35: { claim: 'לכל אחד שינוי, תווית' },
  36: { claim: 'שם הטענה שתיפול' },
  32: { claim: 'כיסוי הטבלה' },
  46: { claim: 'ועמודת התקן שלה אוסרת במפורש' },
  47: { claim: ['מספור רציף ובלי כפילויות', 'nameSignGaps', 'catOrderGaps'] },
  48: { claim: 'הערה ריקה' },
  50: { claim: 'COUNT_NOTE' },
  54: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  228: { claim: 'CACHE_NAME' },
  44: { manual: 'מספר בדיווח הוא התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו את הדיווח, ⚠️ ונאכף בתוצאתו בלבד' },
  25: { claims: { test_readonly: 'drift', 'check-capabilities': 'writeGateGaps' } },
  ...
  37: { claims: { 'check-comments': 'תווית מוטציה מודפסת',
                  'check-capabilities': ['regexBoundGaps', 'anchorGaps'],
                  'test_anchors': 'מ1' } },
  21: { claim: 'תקן הבאנר' },
  16: { claim: 'דפוס הבודקים' },
  107: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  /*  ⭐ סבב 148 — ⛔ רכיב משותף מתנהג אותו דבר: ⚠️ ההצלבה בין הריפו,
   *  ⭐ וההצהרה ב-`APP.appClasses` שנמדדת משני צדדיה. */
  90: { claims: { test_visual: ['מחלקה משותפת שכלליה נבדלים', 'הצהרת מחלקה שאין לה סטייה',
                               'הצהרת מחלקה בלי נימוק תפקידי',
                               'מחלקה בשימוש כאן שכלל שלה חי באחות'] } },
  43: { claim: 'מספרי הבאנר' },
  ...
  28: { claims: { 'check-comments': 'ריצת check-js', 'check-capabilities': 'gateKindGaps' } },
  ...
  41: { claims: { 'check-js': 'תקציב זמן לסט',
                  'check-capabilities': 'gateSizeGaps' } },
  29: { claim: 'ברירת המחדל אינה המהירה' },
  ...
  96: { claim: 'storage' },
  95: { claim: 'ג · פעולה שדורשת רשת' },
  112: { claim: 'עברית' },
  115: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  116: { claim: 'BANNER_W' },
  210: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
                 'אין מחלקה שנוספת ל-DOM בלי כלל CSS'] },
  49: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  113: { claim: 'מפנה לקובץ' },
  133: { claim: 'android/app/src/main' },
  140: { claim: 'fgDriftMax' },
  141: { claim: 'CANON_MANIFEST' },
  142: { claim: 'שדות זהים' },
  145: { claim: 'BUILD_SHA' },
  149: { claim: 'SHARED_SHA' },
  147: { claim: 'versionCode' },
  129: { claim: 'WebView' },
  130: { claim: 'android:theme' },
  135: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  152: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  156: { claim: 'migrations' },
  165: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  167: { claim: 'א. חותמת בכל רשומה' },
  62: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  160: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  161: { claim: 'ו. טבלה מקבילה' },
  162: { claim: 'ב. חתימת סכימה' },
  163: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  186: { claim: 'type=password' },
  87: { claim: 'aria-label' },
  ...
  197: { claim: '[sec-keyfile]' },
  201: { claim: '⏳' },
  205: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  218: { claim: 'ט. ערך נגזר שהתיישן' },
  220: { claim: 'ח. הגדרה שהוחלפה' },
  219: { claim: 'ד. רשימת-היתר' },
  222: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  111: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  124: { claim: 'CLEANUP_SHA' },
  125: { claim: 'בלוק ה-APP' },
  182: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
