...
const APP = {
  ...
  gapRows: [44, 121, 125, 148, 59, 134, 95],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    168: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    167: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    164: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '164|מבנה טופס הכניסה': 'src',
    '134|שם נגזר אינו נשמר ברשומה': 'src',
    '181|מפתח אחסון בלי קורא': 'src',
    '36|שער מריץ את כל טענותיו': 'tools',
    '87|שכבת המראה': 'src',
    '182|הגירה מקומית שהושלמה': 'mixed',
    '166|מראת המשתמשים': 'src',
    '165|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'tools',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '149|הקשר נלכד בכניסה לפונקציה': 'src',
    ...
    '176|הערה שמתארת מצב שחלף': 'mixed',
    '183|שאילתת `@media` מתה': 'src',
    ...
    '151|הגיבוי שלם ומעומד': 'src',
    '71|פעולה מגיבה מיד': 'src',
    '72|משיכה מסוננת בשרת': 'src',
    '120|דגימת-יתר במחולל': 'tools',
    '168|`PBKDF2` — פרמטרים': 'src',
    '138|`migrations/` — תוכן': 'src',
    '139|`migrations/` — כמות': 'src',
    '157|מיזוג מכל — מפה ומערך': 'src',
    '158|ברירת מחדל אינה ממוזגת': 'src',
    '159|רשימה אינה נושאת פריט כפול': 'src',
    '152|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'tools',
    '179|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'tools',
    '172|סודות אינם עוזבים את המכשיר': 'src',
    '146|דפוס `upsert` — onConflict מוכרז': 'src',
    '170|טביעה חסרה מושלמת אוטומטית': 'src',
    '170|כניסה אופליין': 'src',
    '131|עריכת נתונים אופליין': 'src',
    '156|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '171|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '69|אובייקט יקר נבנה פעם אחת': 'src',
    '70|מסך מציג מיד ומרענן ברקע': 'src',
    '124|האפליקציה ניתנת להתקנה מכרום': 'src',
    '174|דגלי מעבר — אין דגל דלוק': 'src',
    '76|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '167|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '44|ארבעת מימדי הסריקה': 'src',
    '86|פינוי אוטומטי': 'src',
    '86|אופק הפינוי נוקה מהסנכרון': 'src',
    '161|אימות פינוי מול הענן': 'src',
    '125|שיתוף קבצים': 'src',
...
export const ROWS = [29, 43, 45, 47, 52, 85, 50, 160, 101, 141, 54, 22, 59, 25, 34, 38];
...
const MATRIX = [
  ...
  { row: 149, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 176, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 151, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 148, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 168, name: '`PBKDF2` — פרמטרים', app: true },
  ...
  { row: 157, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 158, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 159, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 172, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 146, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 170, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 156, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 171, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 174, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 167, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 161, name: 'אימות פינוי מול הענן',
  ...
  { row: 164, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 164, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 191, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 150, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 170, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 170, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 191, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 190, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 155, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 189, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 189, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 189, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 189, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 162, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  154: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  184: { claim: 'ז. עמודה בלי קורא' },
  ...
  178: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  102: { claim: 'BANNER_W' },
  180: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  145: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  147: { claim: 'א. חותמת בכל רשומה' },
  ...
  169: { claim: 'pass_salt' },
  173: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  175: { claim: '⏳' },
  177: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  185: { claim: 'ט. ערך נגזר שהתיישן' },
  187: { claim: 'ח. הגדרה שהוחלפה' },
  186: { claim: 'ד. רשימת-היתר' },
  188: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  160: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
