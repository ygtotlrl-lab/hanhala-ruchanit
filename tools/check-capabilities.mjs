...
const APP = {
  ...
  gapRows: [220, 137, 141, 165, 67, 150, 106],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    187: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    186: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    148: (c) => !/ysCfgSet\('(?:hr_students|hr_sessions|hr_sleep_sessions)'/.test(c.src)
    ...
    183: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '183|מבנה טופס הכניסה': 'src',
    '150|שם נגזר אינו נשמר ברשומה': 'src',
    '204|קבוע בלי קורא': 'mixed',
    '206|מפתח אחסון בלי קורא': 'src',
    '39|שער מריץ את כל טענותיו': 'tools',
    '97|שכבת המראה': 'src',
    '207|הגירה מקומית שהושלמה': 'mixed',
    '185|מראת המשתמשים': 'src',
    '184|מודל ההרשאות': 'src',
    '26|שער סורק קוד מולבן': 'tools',
    '26|ההלבנה שומרת על מספרי השורות': 'mixed',
    '166|הקשר נלכד בכניסה לפונקציה': 'src',
    ...
    '199|הערה שמתארת מצב שחלף': 'mixed',
    '208|שאילתת `@media` מתה': 'src',
    '123|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '124|ייצוא והנפקה — דרך מוצהרת': 'src',
    '107|המסמך מתפרסר נקי': 'src',
    '108|לוג מסלול — תג ורמה': 'src',
    '83|מיכל המודאל ילד ישיר של `body`': 'src',
    '119|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '168|הגיבוי שלם ומעומד': 'src',
    '79|פעולה מגיבה מיד': 'src',
    '80|משיכה מסוננת בשרת': 'src',
    '136|דגימת-יתר במחולל': 'tools',
    '187|`PBKDF2` — פרמטרים': 'src',
    '154|`migrations/` — תוכן': 'src',
    '155|`migrations/` — כמות': 'src',
    '176|מיזוג מכל — מפה ומערך': 'src',
    '177|ברירת מחדל אינה ממוזגת': 'src',
    '178|רשימה אינה נושאת פריט כפול': 'src',
    '169|`sh_backup` — טבלת הגיבוי': 'src',
    '24|שער אינו מקליד מספר שורה': 'tools',
    '203|פונקציה בלי קוראים': 'mixed',
    '30|שער ממתין לתנאי ולא לשעון': 'tools',
    '191|סודות אינם עוזבים את המכשיר': 'src',
    '163|דפוס `upsert` — onConflict מוכרז': 'src',
    '189|טביעה חסרה מושלמת אוטומטית': 'src',
    '189|כניסה אופליין': 'src',
    '147|עריכת נתונים אופליין': 'src',
    '173|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '190|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '77|אובייקט יקר נבנה פעם אחת': 'src',
    '78|מסך מציג מיד ומרענן ברקע': 'src',
    '140|האפליקציה ניתנת להתקנה מכרום': 'src',
    '196|דגלי מעבר — אין דגל דלוק': 'src',
    '84|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '186|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '96|פינוי אוטומטי': 'src',
    '96|אופק הפינוי נוקה מהסנכרון': 'src',
    '180|אימות פינוי מול הענן': 'src',
    '141|שיתוף קבצים': 'src',
    '125|מעטפת APK (WebView)': 'src',
    '128|אין נכסים מוטבעים': 'src',
    '145|מפתח חתימה קבוע': 'src',
    '151|מקור אמת יחיד לסכימה': 'src',
    '152|קובץ התקנה מלא': 'src',
    '63|גיבוי יומי אוטומטי': 'src',
    '63|יומן פעולות': 'src',
    '148|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '85|דפוס הודעת שגיאה יחיד': 'src',
    '92|`toast` — חתימה, גוף ומחלקות': 'src',
    '92|סיווג ההודעה נגזר מהמסלול': 'src',
    '219|מיכל באנר העדכון במקור': 'src',
    '219|הרענון מ-controllerchange בלבד': 'src',
    '219|סימן דחיית הבאנר מתמיד': 'src',
    '120|רישום כשלי כתיבה': 'src',
    '64|חלון חם במכשיר': 'src',
    '183|מסך שינוי סיסמה עצמי': 'src',
    '221|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '65|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '189|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '66|מודול מזהי רשומות': 'src',
    '66|מזהה אינו מומר למספר': 'src',
    '68|מזהה מכשיר': 'src',
    '65|ליבת מיזוג משותפת': 'src',
    '125|מעטפת WebView חתומה': 'tools',
    '189|אימות מול טביעה בענן': 'mixed',
    '143|בניית APK אחידה עם שער חתימה': 'src',
    '221|מודול ה-service worker': 'src',
    '69|ניסיון חוזר בתור הסנכרון': 'src',
    '70|מנגנון משיכה אחיד': 'src',
    '73|נעילת חוסר-פעילות': 'src',
    '74|מודל הסשן — בזיכרון בלבד': 'src',
    '220|בדיקת עדכון תקופתית ל-service worker': 'src',
    '99|פסק זמן אחיד לקריאות רשת': 'src',
    '100|מאזיני מצב רשת': 'src',
    '172|גריעת tombstones לפי גיל': 'src',
    '219|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '141|גשר שיתוף': 'src',
    '75|העברת מזהה ל-DOM': 'src',
    '76|מיון אחד לכל התצוגות': 'src',
    '81|ייצוא — שני דפוסים מוצהרים': 'src',
    '133|שכבת אייקונים': 'src',
    '181|שכבת קלט אחידה': 'src',
    '181|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '82|שכבת המודאל': 'src',
    '183|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [32, 47, 48, 50, 59, 95, 57, 179, 113, 157, 62, 25, 67, 28, 37, 41, 194];
...
const CAPS = {
  ...
  msgs: {
    ...
    block: { sha: '3bd12c2aa7936318', lines: 35,
  ...
  kvval: {
    ...
    block: { sha: '3ceab995439e7980', lines: 23,
...
const MATRIX = [
  ...
  { row: 150, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 166, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 199, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 119, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 168, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 165, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 136, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 187, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 154, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 155, name: '`migrations/` — כמות',
  ...
  { row: 176, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 177, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 178, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 191, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 163, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 189, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 189, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 147, name: 'עריכת נתונים אופליין',
  ...
  { row: 173, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 190, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 140, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 196, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 186, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 180, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 141, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 125, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 128, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 145, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 151, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 152, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 63, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 63, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 148, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 183, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 183, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 221, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 167, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 189, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 125, name: 'מעטפת WebView חתומה',
  ...
  { row: 189, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 143, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 221, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 220, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 172, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 219, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 219, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 219, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 219, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 133, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 181, name: 'שכבת קלט אחידה',
  ...
  { row: 181, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  116: { claims: { test_names: ['[name-policy]', '[name-policy-why]', '[fn-sister-prefix]',
  ...
  215: { claims: { test_mignames: ['[mig-pattern]', '[mig-seq]', '[mig-file-no-entry]',
  ...
  194: { claims: { test_docfacts: ['[doc-name]', '[doc-round]'],
  ...
  198: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  192: { claim: 'אין `service_role` בקובץ' },
  34: { claim: 'מסלול «' },
  ...
  195: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  72: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  156: { claim: 'בעלות הסכימה המשותפת' },
  170: { claim: '`ya_settings_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  171: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  209: { claim: 'ז. עמודה בלי קורא' },
  ...
  131: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  134: { claim: 'margin' },
  ...
  118: { claims: { test_codescan: ['[fn-product]'] },
  ...
  202: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  114: { claim: 'BANNER_W' },
  205: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  130: { claim: 'android/app/src/main' },
  137: { claim: 'fgDriftMax' },
  138: { claim: 'CANON_MANIFEST' },
  139: { claim: 'שדות זהים' },
  142: { claim: 'BUILD_SHA' },
  146: { claim: 'SHARED_SHA' },
  144: { claim: 'versionCode' },
  126: { claim: 'WebView' },
  127: { claim: 'android:theme' },
  132: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  149: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  153: { claim: 'migrations' },
  162: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  164: { claim: 'א. חותמת בכל רשומה' },
  62: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  157: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  158: { claim: 'ו. טבלה מקבילה' },
  159: { claim: 'ב. חתימת סכימה' },
  160: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  182: { claim: 'type=password' },
  86: { claim: 'aria-label' },
  ...
  193: { claim: '[sec-keyfile]' },
  197: { claim: '⏳' },
  201: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  212: { claim: 'ט. ערך נגזר שהתיישן' },
  214: { claim: 'ח. הגדרה שהוחלפה' },
  213: { claim: 'ד. רשימת-היתר' },
  216: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  121: { claim: 'CLEANUP_SHA' },
  122: { claim: 'בלוק ה-APP' },
  179: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
