 ...
 *  מעטפת חייבת לממש, נאכפים זהה בכולן; (ב) **חתימה על הלוגיקה**: hash על
 ...
 *
 *  **הנימוק המדוד:** כל העותקים של אותה מעטפת — שניים כמעט זהים
 ...
 *  ביטחון. ⭐ והחתימה פר-ריפו ⛔ ולא אחת לכולן: המעטפת של יומן נושאת גשר
...
const APP = {
  /* ⭐ הליבה המשותפת — זהה בית-לבית בכל הריפו פרט לשורת
...
/*  ⚠️ טוקניזציה ולא ביטוי רגולרי — בדיוק הלקח של
...
function normalize(text) {
  ...
  /*  ⭐ שם החבילה מנורמל גם הוא — הוא מציין-זהות ולא לוגיקה,
...
/*  ⭐ החתימה הקנונית של מעטפת **בלי גשר** — נמדדה ונמצאה זהה
...
/*  ⭐⭐ החתימה הקנונית של **הליבה**, וזה החידוש: עד אז ליומן
 *  הייתה חתימה משלה, מפני שגשר השיתוף הכפיל אצלה את המעטפת ולא ניתן היה
 *  להשוות. אחרי החילוץ הליבה זהה בית-לבית ב**כולן** — כולל יומן — והגשר
 *  יושב מחוצה לה. ⛔ סחיפה של שורה אחת בליבה נתפסת בכל הריפו בנפרד. */
...
const CONTRACT = [
  ...
  /*  ⭐ ההקשחה שנמדדה חסרה ב-gius והושלמה שם: המעטפת אינה טוענת
...
/*  ⭐ חוזה ההתנהגות נבדק ב**ליבה** — שם הוא יושב מאז החילוץ,
 *  ובאותה צורה בכולן. ⛔ מעטפת פר-אפליקציה שתממש אחד מהם בעצמה היא
 ...
 *  הרחבת-הישג בלי צורך.                                                */
/*  ⛔ הליבה המשותפת אינה נושאת גשר לעולם — גשר בליבה היה מגיע
 *  לכולן בבת אחת, כלומר בדיוק ההפך ממה שהחריגה המדודה של יומן אומרת. */
...
if (APP.shareBridge && !hasBridge) {
  ...
  fail('⛔ נמצא גשר מקורי במעטפת שלא הוכרזה כנושאת גשר — ' +
       'דף שנטען מהרשת אינו מקבל גשר בלי צורך מדוד');
} else {
...
if (coreSha === APP.coreSha) {
  ...
  fail(`חתימת הליבה המשותפת השתנתה: ${APP.coreSha} → ${coreSha}\n` +
       '   ⛔ שינוי בליבה הוא שינוי בכל האפליקציות — מעדכנים את הבלוק ואת החתימה בכולן, באותו סבב.');
}
...
if (APP.coreSha !== CORE_SHA) {
  ...
  pass(`⭐ חתימת הליבה היא הקנונית בכולן: ${CORE_SHA}`);
}
...
/*  ⛔ היציאה עברה לסוף — המוטציות רצות אחרי הטענות. */
/* ───────────────────────────────────────────────────────────────────────────
   ⛔ מוטציה ומוטציית-נגד
   ───────────────────────────────────────────────────────────────────────────
...
if (!process.env.RD67_MUT) {
  ...
  const _mut = (label, file, edit, expectFail) => {
    /*  ⛔ עותק לכל מוטציה, ⛔ ובכוונה — ⚠️ נמדדו **שניים** בהרצה
