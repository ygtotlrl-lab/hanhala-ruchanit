...
const APP = {
  ...
  absenceClaims: {
    ...
    '198|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי':
      'לאיסור אין מיקום נכון, שהוא טענת היעדר — ' +
      'ולצידו נמדד ששם פונקציית הכתיבה נושא קורא חי, והיא נקודת המעבר האחת',
    '191|מבנה טופס הכניסה':
  ...
  gapRows: [227, 144, 148, 172, 66, 157, 110],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    195: (c) => c.hasCode(/HR_PASS_ITER\s*=\s*100000\b/)
    ...
    194: (c) => !!c.fnBody('hrUsersCacheSave')
    ...
    155: (c) => !/hrCfgSet\('(?:hr_students|hr_sessions|hr_sleep_sessions)'/.test(c.src)
    ...
    191: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '91|המשוב בלחיצה נקבע ולא נירש': 'src',
    '100|חותמת רעננות נכשלת סגור': 'src',
    '87|`Enter` שומר בכל שדה עריכה': 'src',
    '88|מאזין אחד, בהאצלה מ-`document`': 'src',
    '93|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '191|מבנה טופס הכניסה': 'src',
    '157|שם נגזר אינו נשמר ברשומה': 'src',
    '40|שער מריץ את כל טענותיו': 'tools',
    '99|שכבת המראה': 'src',
    '213|הגירה מקומית שהושלמה': 'mixed',
    '193|מראת המשתמשים': 'src',
    '192|מודל ההרשאות': 'src',
    '28|שער סורק קוד מולבן': 'tools',
    '28|ההלבנה שומרת על מספרי השורות': 'mixed',
    '173|הקשר נלכד בכניסה לפונקציה': 'src',
    '103|פריסה במסכי טלפון וטאבלט': 'src',
    '104|ערכת נושא — בהיר וכהה': 'src',
    '106|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '107|אין סגנון מוטבע': 'src',
    '110|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '207|הערה שמתארת מצב שחלף': 'mixed',
    '214|שאילתת `@media` מתה': 'src',
    '129|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '130|ייצוא והנפקה — דרך מוצהרת': 'src',
    '111|המסמך מתפרסר נקי': 'src',
    '112|לוג מסלול — תג ורמה': 'src',
    '82|מיכל המודאל ילד ישיר של `body`': 'src',
    '125|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '175|הגיבוי שלם ומעומד': 'src',
    '78|פעולה מגיבה מיד': 'src',
    '79|משיכה מסוננת בשרת': 'src',
    '143|דגימת-יתר במחולל': 'tools',
    '195|`PBKDF2` — פרמטרים': 'src',
    '161|`migrations/` — תוכן': 'src',
    '162|`migrations/` — כמות': 'src',
    '183|מיזוג מכל — מפה ומערך': 'src',
    '184|ברירת מחדל אינה ממוזגת': 'src',
    '185|רשימה אינה נושאת פריט כפול': 'src',
    '176|`sh_backup` — טבלת הגיבוי': 'src',
    '26|שער אינו מקליד מספר שורה': 'tools',
    '32|שער ממתין לתנאי ולא לשעון': 'tools',
    '199|סודות אינם עוזבים את המכשיר': 'src',
    '170|דפוס `upsert` — onConflict מוכרז': 'src',
    '197|טביעה חסרה מושלמת אוטומטית': 'src',
    '197|כניסה אופליין': 'src',
    '154|עריכת נתונים אופליין': 'src',
    '180|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '198|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '76|אובייקט יקר נבנה פעם אחת': 'src',
    '77|מסך מציג מיד ומרענן ברקע': 'src',
    '147|האפליקציה ניתנת להתקנה מכרום': 'src',
    '204|דגלי מעבר — אין דגל דלוק': 'src',
    '84|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '194|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '98|פינוי אוטומטי': 'src',
    '98|אופק הפינוי נוקה מהסנכרון': 'src',
    '187|אימות פינוי מול הענן': 'src',
    '148|שיתוף קבצים': 'src',
    '131|מעטפת APK (WebView)': 'src',
    '134|אין נכסים מוטבעים': 'src',
    '152|מפתח חתימה קבוע': 'src',
    '158|מקור אמת יחיד לסכימה': 'src',
    '159|קובץ התקנה מלא': 'src',
    '62|גיבוי יומי אוטומטי': 'src',
    '62|יומן פעולות': 'src',
    '155|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '85|דפוס הודעת שגיאה יחיד': 'src',
    '94|`toast` — חתימה, גוף ומחלקות': 'src',
    '94|סיווג ההודעה נגזר מהמסלול': 'src',
    '226|מיכל באנר העדכון במקור': 'src',
    '226|הרענון מ-controllerchange בלבד': 'src',
    '226|סימן דחיית הבאנר מתמיד': 'src',
    '126|רישום כשלי כתיבה': 'src',
    '63|חלון חם במכשיר': 'src',
    '191|מסך שינוי סיסמה עצמי': 'src',
    '228|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '64|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '197|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '65|מודול מזהי רשומות': 'src',
    '65|מזהה אינו מומר למספר': 'src',
    '67|מזהה מכשיר': 'src',
    '64|ליבת מיזוג משותפת': 'src',
    '131|מעטפת WebView חתומה': 'tools',
    '197|אימות מול טביעה בענן': 'mixed',
    '150|בניית APK אחידה עם שער חתימה': 'src',
    '228|מודול ה-service worker': 'src',
    '68|ניסיון חוזר בתור הסנכרון': 'src',
    '69|מנגנון משיכה אחיד': 'src',
    '72|נעילת חוסר-פעילות': 'src',
    '73|מודל הסשן — בזיכרון בלבד': 'src',
    '227|בדיקת עדכון תקופתית ל-service worker': 'src',
    '101|פסק זמן אחיד לקריאות רשת': 'src',
    '102|מאזיני מצב רשת': 'src',
    '179|גריעת tombstones לפי גיל': 'src',
    '226|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '148|גשר שיתוף': 'src',
    '74|העברת מזהה ל-DOM': 'src',
    '75|מיון אחד לכל התצוגות': 'src',
    '80|ייצוא — שני דפוסים מוצהרים': 'src',
    '140|שכבת אייקונים': 'src',
    '188|שכבת קלט אחידה': 'src',
    '188|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '81|שכבת המודאל': 'src',
    '191|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [36, 34, 48, 49, 57, 97, 55, 186, 117, 164, 61, 27, 66, 30, 38, 42, 202, 209];
...
const MATRIX = [
  { row: 91, name: 'המשוב בלחיצה נקבע ולא נירש',
  ...
  { row: 100, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 93, name: 'כפתור שכותב מושבת בזמן הכתיבה',
    probe: () => busyGuardGaps().length === 0 },
  { row: 88, name: 'מאזין אחד, בהאצלה מ-`document`',
  ...
  { row: 87, name: '`Enter` שומר בכל שדה עריכה',
  ...
  { row: 157, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 173, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 103, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 106, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 110, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 104, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 207, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 112, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 82, name: 'מיכל המודאל ילד ישיר של `body`',
  ...
  { row: 125, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 175, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 172, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 78, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 79, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 143, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 195, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 161, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 162, name: '`migrations/` — כמות',
  ...
  { row: 183, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 184, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 185, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 176, name: '`sh_backup` — טבלת הגיבוי',
  ...
  { row: 199, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 170, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 197, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 197, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 154, name: 'עריכת נתונים אופליין',
  ...
  { row: 180, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 198, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 76, name: 'אובייקט יקר נבנה פעם אחת',
  ...
  { row: 77, name: 'מסך מציג מיד ומרענן ברקע',
  ...
  { row: 147, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 204, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 84, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 194, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 98, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 187, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 148, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 131, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 134, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 152, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 158, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 159, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 62, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 62, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 155, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 85, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 94, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 94, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 63, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 191, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 191, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 228, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 62, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 174, name: 'פינוי גיבויים אוטומטי במסד',
   ...
   *  ⛔ ולא `NaN`. */
  { row: 65, name: 'מזהה אינו מומר למספר',
  ...
  { row: 131, name: 'מעטפת WebView חתומה',
  ...
  { row: 197, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 150, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 228, name: 'מודול ה-service worker',
  ...
  { row: 68, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 73, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 227, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 99, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 213, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 101, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 179, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 226, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 226, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 226, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 226, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 75, name: 'מיון אחד לכל התצוגות',
  ...
  { row: 80, name: 'ייצוא — שני דפוסים מוצהרים',
    probe: () => exportMarkupSites().length === 0 },
  { row: 140, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 188, name: 'שכבת קלט אחידה',
  ...
  { row: 188, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const CLAIM_NO_MUT = {
  ...
  '52|ט · probe מודד מימוש':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '53|יא · נימוק ⭕ מונה את מה שאינו נאכף':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '54|תוכן הקבצים הנלווים':
    'המוטציה מריצה את הבודק על עותק ברתמה — ⛔ והתווית נוקבת בשם הבודק שנפל, ⚠️ ולא בשם הטענה',
  ...
  '71|כתיבת משתמש':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '90|מחלקה משותפת שמצביה נבדלים':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '95|ליטרל עברי באתר הודעה':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '96|ג · פעולה שדורשת רשת':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '108|ליטרל צבע בגיליון':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '108|ערך שאינו דרגה בסולם':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '108|ערך מספרי שאינו נגזר':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '116|[sister-ambig]':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '117|RULE_W':
  ...
  '133|android:theme':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '136|android/app/src/main':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '144|fgDriftMax':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '145|CANON_MANIFEST':
    'המוטציה מריצה את הבודק על עותק ברתמה — ⛔ והתווית נוקבת בשם הבודק שנפל, ⚠️ ולא בשם הטענה',
  '156|ג. כל מפתח שהקוד מבקש':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '160|migrations':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '164|ה. דפוס עמודות אחיד':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '165|ו. טבלה מקבילה':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '166|ב. חתימת סכימה':
  ...
  '169|יג. תוספת-בלבד':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '171|א. חותמת בכל רשומה':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '177|`ya_settings_rishon`':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '178|י. ערך במפתח-ערך':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '178|יא. ערך פגום נבדל':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '186|ג. כל מפתח שהקוד מבקש':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '189|[zero-guard-registry]':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '190|type=password':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '196|pass_salt':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '200|אין `service_role` בקובץ':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '203|[synclog-push]':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '206|[core-allow]':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '209|כל קובץ בעץ מוזכר במקום אחר':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '210|מזהה שנמחק ונשאר לו קורא':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '211|[call-arity-registry]':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '215|ז. עמודה בלי קורא':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '216|[sync-cols-why]':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '218|ט. ערך נגזר שהתיישן':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '219|ד. רשימת-היתר':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '220|ח. הגדרה שהוחלפה':
...
const GATES = {
  ...
  122: { claims: { test_names: ['[name-policy]', '[name-policy-why]', '[fn-sister-prefix]',
  ...
  221: { claims: { test_mignames: ['[mig-pattern]', '[mig-seq]', '[mig-file-no-entry]',
  ...
  53: { claims: { test_rowscan: ['יא · נימוק ⭕ מונה את מה שאינו נאכף'] } },
  ...
  202: { claims: { test_docfacts: ['[doc-name]', '[doc-round]'],
  ...
  206: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  119: { claims: { test_period: ['[verb-registry]', '[verb-old-name]'],
  ...
  230: { manual: 'המסך הראשון מול מסד ריק אינו נגזר מהטקסט — ⛔ הוא נמדד במבחן הקבלה, ⚠️ ושער אינו מריץ דפדפן' },
  /*  ⭐ סבב 147 — ⛔ שם אפליקציה אחות בקוד: ⚠️ אפס אזכור לשם של ריפו
   *  אחר בקבצי המוצר והמעטפת, ⭐ ואפס נכס אייקון שזהה בית-לבית לאחות. */
  223: { claims: { test_sistername: ['[sister-name]', '[sister-asset]', '[sister-orphan]'] } },
  224: { claims: { test_toolsid: ['[scan-derived]'] } },
  225: { claims: { test_toolsid: ['[scan-enforcer]'] } },
  /*  ⭐ סבב 148 — ⛔ הערה אינה מפנה לאפליקציה אחרת: ⚠️ בשם הריפו או
   *  בשמה העברי, ⭐ וההכרזה על שם דו-משמעי נמדדת משני צדדיה. */
  116: { claims: { test_sistername: ['[sister-comment]', '[sister-ambig]'] } },
  ...
  58: { claims: { test_coremod: ['[block-kind]', '[block-core]'] } },
  59: { claims: { test_toolsid: ['[tool-uncovered]', '[gate-run]', '[gate-count]'] } },
  ...
  200: { claim: 'אין `service_role` בקובץ' },
  56: { claim: 'א · אין שתי שורות על אותו probe' },
  ...
  70: { claim: 'שכבת הדחיפה' },
  /*  ⛔ שלוש טענות באותו שער (סבב 131) — ⚠️ המשפך, הודעת החסימה שהוא
   *  מחזיר, והקבוע שאתרי האימות מפנים אליו: ⭐ שלושתן על אותו מסלול. */
  203: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  71: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  163: { claim: 'בעלות הסכימה המשותפת' },
  177: { claim: '`ya_settings_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  178: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  215: { claim: 'ז. עמודה בלי קורא' },
  95: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  124: { claims: { test_codescan: ['[fn-product]'] },
  ...
  210: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  229: { claim: 'CACHE_NAME' },
  45: { manual: 'הדיווח אינו בעץ — ⛔ אין קובץ שאפשר למדוד בו מספר שהוקלד, ⚠️ ונאכף בתוצאתו בלבד' },
  ...
  108: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  109: { claims: { test_visual: ['[util-ratio]', '[util-chain]'] } },
  ...
  90: { claim: 'מחלקה משותפת שמצביה נבדלים' },
  ...
  114: { claim: 'עברית' },
  117: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  118: { claim: 'BANNER_W' },
  115: { claim: 'מפנה לקובץ' },
  136: { claim: 'android/app/src/main' },
  144: { claim: 'fgDriftMax' },
  145: { claim: 'CANON_MANIFEST' },
  146: { claim: 'שדות זהים' },
  149: { claim: 'BUILD_SHA' },
  153: { claim: 'SHARED_SHA' },
  151: { claim: 'versionCode' },
  132: { claim: 'WebView' },
  133: { claim: 'android:theme' },
  139: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  156: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה', test_read: 'אין ראיה' } },
  160: { claim: 'migrations' },
  169: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  171: { claim: 'א. חותמת בכל רשומה' },
  61: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת', test_pendflush: 'pendClear' } },
  164: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  165: { claim: 'ו. טבלה מקבילה' },
  166: { claim: 'ב. חתימת סכימה' },
  167: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  190: { claim: 'type=password' },
  86: { claim: 'aria-label' },
  ...
  66: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps', 'hebFormGaps'], test_date: 'hebDate' } },
  196: { claim: 'pass_salt' },
  /*  ⭐ סבב 148 — ⛔ סריקה הפוכה: ⚠️ כל אתר חשוד מפיל אלא אם הוא
   *  מוכרז עם נימוקו, ⭐ ורשימת דפוסים הייתה מוצאת את מה שכבר תוקן. */
  201: { claim: '[sec-keyfile]' },
  205: { claims: { test_filesets: ['[oneoff-gate]', '[oneoff-round]', '[oneoff-stale]'] } },
  209: { claims: { test_orphans: 'כל קובץ בעץ מוזכר במקום אחר',
  ...
  218: { claim: 'ט. ערך נגזר שהתיישן' },
  220: { claim: 'ח. הגדרה שהוחלפה' },
  219: { claim: 'ד. רשימת-היתר' },
  ...
  113: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  127: { claim: 'CLEANUP_SHA' },
  128: { claim: 'בלוק ה-APP' },
  186: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
  ...
  62: { claims: { test_stage_a: 'bkMaybeDaily' } },
  63: { claims: { test_hotwin: 'hwSweep' } },
  64: { claims: { test_merge_pending: 'pending' } },
  65: { claims: { test_ids: 'newClientId' } },
  67: { claims: { test_devid: 'deviceKey' } },
  68: { claims: { test_pendflush: 'RTY_CFG' } },
  69: { claims: { test_pull: 'PL_CFG' } },
  72: { claims: { test_lock: 'LOCK_MS' } },
  73: { claims: { test_session: '_sessUser' } },
  74: { claims: { test_idarg: 'badWraps' } },
  100: { claims: { test_kvmeta: 'kvMeta' } },
  131: { claims: { test_shell: 'ShellActivity' } },
  174: { claims: { test_backup_policy: 'p_keep' } },
  192: { claims: { test_roles: 'admin' } },
  194: { claims: { test_users_patch: 'partialLeak' } },
  197: { claims: { test_offline_login: 'pass_fp' } },
  228: { claims: { test_swcore: 'respondWith' } },
};
