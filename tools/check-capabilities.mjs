...
const APP = {
  ...
  gapRows: [46, 125, 129, 153, 62, 138, 99],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    174: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    173: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    136: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    170: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    '91|חותמת רעננות נכשלת סגור': 'src',
    '82|`Enter` שומר בכל שדה עריכה': 'src',
    '83|מאזין אחד, בהאצלה מ-`document`': 'src',
    '84|כפתור שכותב מושבת בזמן הכתיבה': 'src',
    '170|מבנה טופס הכניסה': 'src',
    '138|שם נגזר אינו נשמר ברשומה': 'src',
    '189|מפתח אחסון בלי קורא': 'src',
    '38|שער מריץ את כל טענותיו': 'tools',
    '90|שכבת המראה': 'src',
    '190|הגירה מקומית שהושלמה': 'mixed',
    '172|מראת המשתמשים': 'src',
    '171|מודל ההרשאות': 'src',
    '25|שער סורק קוד מולבן': 'tools',
    '25|ההלבנה שומרת על מספרי השורות': 'mixed',
    '154|הקשר נלכד בכניסה לפונקציה': 'src',
    '94|פריסה במסכי טלפון וטאבלט': 'src',
    '95|ערכת נושא — בהיר וכהה': 'src',
    '96|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '97|אין סגנון מוטבע': 'src',
    '99|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '184|הערה שמתארת מצב שחלף': 'mixed',
    '191|שאילתת `@media` מתה': 'src',
    '112|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '113|ייצוא והנפקה — דרך מוצהרת': 'src',
    '100|המסמך מתפרסר נקי': 'src',
    '101|לוג מסלול — תג ורמה': 'src',
    '78|מיכל המודאל ילד ישיר של `body`': 'src',
    '108|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '156|הגיבוי שלם ומעומד': 'src',
    '74|פעולה מגיבה מיד': 'src',
    '75|משיכה מסוננת בשרת': 'src',
    '124|דגימת-יתר במחולל': 'tools',
    '174|`PBKDF2` — פרמטרים': 'src',
    '142|`migrations/` — תוכן': 'src',
    '143|`migrations/` — כמות': 'src',
    '163|מיזוג מכל — מפה ומערך': 'src',
    '164|ברירת מחדל אינה ממוזגת': 'src',
    '165|רשימה אינה נושאת פריט כפול': 'src',
    '157|`sh_backup` — טבלת הגיבוי': 'src',
    '23|שער אינו מקליד מספר שורה': 'tools',
    '187|פונקציה בלי קוראים': 'mixed',
    '29|שער ממתין לתנאי ולא לשעון': 'tools',
    '178|סודות אינם עוזבים את המכשיר': 'src',
    '151|דפוס `upsert` — onConflict מוכרז': 'src',
    '176|טביעה חסרה מושלמת אוטומטית': 'src',
    '176|כניסה אופליין': 'src',
    '135|עריכת נתונים אופליין': 'src',
    '161|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '177|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '72|אובייקט יקר נבנה פעם אחת': 'src',
    '73|מסך מציג מיד ומרענן ברקע': 'src',
    '128|האפליקציה ניתנת להתקנה מכרום': 'src',
    '182|דגלי מעבר — אין דגל דלוק': 'src',
    '79|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '173|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '46|ארבעת מימדי הסריקה': 'src',
    '89|פינוי אוטומטי': 'src',
    '89|אופק הפינוי נוקה מהסנכרון': 'src',
    '167|אימות פינוי מול הענן': 'src',
    '129|שיתוף קבצים': 'src',
    '114|מעטפת APK (WebView)': 'src',
    '117|אין נכסים מוטבעים': 'src',
    '133|מפתח חתימה קבוע בריפו': 'src',
    '139|מקור אמת יחיד לסכימה': 'src',
    '140|קובץ התקנה מלא': 'src',
    '58|גיבוי יומי אוטומטי': 'src',
    '58|יומן פעולות': 'src',
    '136|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '80|דפוס הודעת שגיאה יחיד': 'src',
    '85|`toast` — חתימה, גוף ומחלקות': 'src',
    '85|סיווג ההודעה נגזר מהמסלול': 'src',
    '198|מיכל באנר העדכון במקור': 'src',
    '198|הרענון מ-controllerchange בלבד': 'src',
    '198|סימן דחיית הבאנר מתמיד': 'src',
    '109|רישום כשלי כתיבה': 'src',
    '59|חלון חם במכשיר': 'src',
    '170|מסך שינוי סיסמה עצמי': 'src',
    '200|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '60|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '176|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '61|מודול מזהי רשומות': 'src',
    '61|מזהה אינו מומר למספר': 'src',
    '63|מזהה מכשיר': 'src',
    '60|ליבת מיזוג משותפת': 'src',
    '114|מעטפת WebView חתומה': 'tools',
    '176|אימות מול טביעה בענן': 'mixed',
    '131|בניית APK אחידה עם שער חתימה': 'src',
    '200|מודול ה-service worker': 'src',
    '64|ניסיון חוזר בתור הסנכרון': 'src',
    '65|מנגנון משיכה אחיד': 'src',
    '68|נעילת חוסר-פעילות': 'src',
    '69|מודל הסשן — בזיכרון בלבד': 'src',
    '199|בדיקת עדכון תקופתית ל-service worker': 'src',
    '92|פסק זמן אחיד לקריאות רשת': 'src',
    '93|מאזיני מצב רשת': 'src',
    '160|גריעת tombstones לפי גיל': 'src',
    '198|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '129|גשר שיתוף': 'src',
    '70|העברת מזהה ל-DOM': 'src',
    '71|מיון אחד לכל התצוגות': 'src',
    '76|ייצוא — שני דפוסים מוצהרים': 'src',
    '121|שכבת אייקונים': 'src',
    '168|שכבת קלט אחידה': 'src',
    '168|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '77|שכבת המודאל': 'src',
    '170|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [31, 45, 47, 49, 55, 88, 53, 166, 105, 145, 57, 24, 62, 27, 36, 40];
...
const MATRIX = [
  ...
  { row: 91, name: 'חותמת רעננות נכשלת סגור', app: true },
  ...
  { row: 84, name: 'כפתור שכותב מושבת בזמן הכתיבה',
    probe: () => busyGuardGaps().length === 0 },
  { row: 83, name: 'מאזין אחד, בהאצלה מ-`document`',
  ...
  { row: 82, name: '`Enter` שומר בכל שדה עריכה',
  ...
  { row: 138, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 154, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 94, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 96, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 99, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 95, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 184, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 101, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 78, name: 'מיכל המודאל ילד ישיר של `body`',
  ...
  { row: 108, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 156, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 153, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 74, name: 'פעולה מגיבה מיד',
    probe: () => actNoFeedback().length === 0 },
  { row: 75, name: 'משיכה מסוננת בשרת', app: true },
  ...
  { row: 124, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 174, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 142, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 143, name: '`migrations/` — כמות',
  ...
  { row: 163, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 164, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 165, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 178, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 151, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 176, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 176, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 135, name: 'עריכת נתונים אופליין',
  ...
  { row: 161, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 177, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 72, name: 'אובייקט יקר נבנה פעם אחת',
  ...
  { row: 73, name: 'מסך מציג מיד ומרענן ברקע',
  ...
  { row: 128, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 182, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 79, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 173, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 46, name: 'ארבעת מימדי הסריקה',
    probe: () => {
  ...
  { row: 89, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 167, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 129, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 114, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 117, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 133, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 139, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 140, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 58, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 58, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 136, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 80, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  ...
  { row: 85, name: 'סיווג ההודעה נגזר מהמסלול',
    probe: () => toastKindGaps().length === 0 },
  { row: 85, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 109, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 59, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 170, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 170, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 200, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 58, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 155, name: 'פינוי גיבויים אוטומטי במסד',
   ...
   *  ⛔ ולא `NaN`. */
  { row: 61, name: 'מזהה אינו מומר למספר',
  ...
  { row: 114, name: 'מעטפת WebView חתומה',
  ...
  { row: 176, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 131, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 200, name: 'מודול ה-service worker',
  ...
  { row: 64, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 69, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 199, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 189, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 90, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 190, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 92, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 160, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 198, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 198, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 198, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 198, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 71, name: 'מיון אחד לכל התצוגות',
  ...
  { row: 76, name: 'ייצוא — שני דפוסים מוצהרים',
    probe: () => exportMarkupSites().length === 0 },
  { row: 121, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 168, name: 'שכבת קלט אחידה',
  ...
  { row: 168, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  197: { claims: { test_sistername: ['[sister-name]', '[sister-asset]'] } },
  ...
  179: { claim: 'אין `service_role` בקובץ' },
  33: { claim: 'מסלול «' },
  54: { claim: 'א · אין שתי שורות על אותו probe' },
  ...
  66:  { claim: 'שכבת הדחיפה' },
  /*  ⛔ שלוש טענות באותו שער (סבב 131) — ⚠️ המשפך, הודעת החסימה שהוא
   *  מחזיר, והקבוע שאתרי האימות מפנים אליו: ⭐ שלושתן על אותו מסלול. */
  181: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  67: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  144: { claim: 'בעלות הסכימה המשותפת' },
  158: { claim: '`tb_kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  159: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  192: { claim: 'ז. עמודה בלי קורא' },
  86: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  13: { claim: 'CANON_MD' },
  52: { claim: 'תוכן הקבצים הנלווים' },
  119: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  122: { claim: 'margin' },
  41: { manual: 'השוואת זמנים בין הריפו אינה בהישג ידו של שער שרץ בריפו אחד — ⛔ נאכפת בתוצאתה בלבד' },
  53: { claim: 'אין פרק פערים נפרד' },
  107: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  186: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  44: { claim: 'ועמודת התקן שלה אוסרת במפורש' },
  45: { claim: ['מספור רציף ובלי כפילויות', 'nameSignGaps', 'catOrderGaps'] },
  47: { claim: 'הערה ריקה' },
  49: { claim: 'COUNT_NOTE' },
  51: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  201: { claim: 'CACHE_NAME' },
  ...
  98: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  ...
  55: { claims: { 'check-capabilities': ['sha:', 'orderGaps', 'blockRowGaps'],
                  test_sharedsync: ['block-drift', 'canon-drift'],
                  test_signedshared: 'unsignedTwins',
                  test_toolsid: ['pure-tools', 'pure-undeclared'] } },
  88: { claim: 'storage' },
  87: { claim: 'ג · פעולה שדורשת רשת' },
  103: { claim: 'עברית' },
  105: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  106: { claim: 'BANNER_W' },
  188: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
                 'אין מחלקה שנוספת ל-DOM בלי כלל CSS'] },
  48: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  104: { claim: 'מפנה לקובץ' },
  118: { claim: 'android/app/src/main' },
  125: { claim: 'fgDriftMax' },
  126: { claim: 'CANON_MANIFEST' },
  127: { claim: 'שדות זהים' },
  130: { claim: 'BUILD_SHA' },
  134: { claim: 'SHARED_SHA' },
  132: { claim: 'versionCode' },
  115: { claim: 'WebView' },
  116: { claim: 'android:theme' },
  120: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  137: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  141: { claim: 'migrations' },
  150: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  152: { claim: 'א. חותמת בכל רשומה' },
  57: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  145: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  146: { claim: 'ו. טבלה מקבילה' },
  147: { claim: 'ב. חתימת סכימה' },
  148: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  169: { claim: 'type=password' },
  81: { claim: 'aria-label' },
  ...
  62: { claims: { 'check-capabilities': ['hebdate', 'monthFormGaps'] } },
  175: { claim: 'pass_salt' },
  180: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  183: { claim: '⏳' },
  185: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  193: { claim: 'ט. ערך נגזר שהתיישן' },
  195: { claim: 'ח. הגדרה שהוחלפה' },
  194: { claim: 'ד. רשימת-היתר' },
  196: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  102: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  110: { claim: 'CLEANUP_SHA' },
  111: { claim: 'בלוק ה-APP' },
  166: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
