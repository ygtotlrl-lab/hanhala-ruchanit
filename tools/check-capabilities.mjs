...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    103: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    85: (c) => c.hasCode(/YS_KV_LEGACY_WRITE\s*=\s*false/),
...
export const ROWS = [37, 39, 40, 42, 43, 45, 60];
...
const MATRIX = [
  { row: 103, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 91, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  { row: 105, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 84, name: 'עריכת נתונים אופליין',
  ...
  { row: 94, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 108, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 59, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 87, name: 'מקור אמת יחיד לסכימה', probe: () => hasPath(APP.schemaFile) },
  { row: 88, name: 'קובץ התקנה מלא',       probe: () => hasPath(APP.schemaFile) },
  { row: 47, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 47, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 85, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 22, name: 'בודקים — משימה מוצהרת', probe: () => checkerMissions() },
  { row: 61, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  { row: 48, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 48, name: 'שחזור מקומי מהענן',
  ...
  { row: 47, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 92, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 49, name: 'מנוע מיזוג עם הגנת ⏳',
    probe: () => /isPend \|\| tsOf\(loc\) > tsOf\(rem\)/.test(fnBody('_mergePick')) },
  { row: 105, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 72, name: 'מעטפת WebView חתומה',
  ...
  { row: 105, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 52, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 55, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 46, name: 'רענון תקופתי של מונה הממתינים',
    probe: () => hasCode(/setInterval\(\s*pendRender\s*,\s*60000\s*\)/) },
  { row: 63, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  בשקט».                                                        */
  { row: 93, name: 'גריעת tombstones לפי גיל',
  ...
  { row: 74, name: 'שכבת אייקונים',
    probe: () => iconAudit('.').length === 0 },
  { row: 96, name: 'שכבת קלט אחידה',
...
const GATES = {
  ...
  38: { claim: 'עמודת «מה זה» היא הוראה מחייבת' },
  39: { claim: 'מספור רציף ובלי כפילויות' },
  40: { claim: 'הערה ריקה' },
  31: { claim: 'drift' },
  34: { claim: 'measure-gap' },
  35: { manual: 'חפיפה בין שערים נסרקת ידנית — ⛔ «אותה טענה» היא קריאת משמעות' },
  30: { claim: 'תווית מוטציה מודפסת' },
  27: { claim: 'תקן הבאנר' },
  23: { claim: 'דפוס הבודקים' },
  36: { claim: 'מספרי הבאנר' },
  33: { claim: 'ריצת check-js' },
  42: { claim: 'כל כלל מיוצג בטבלה' },
  43: { claim: '🔲 אינו קבוע' },
  44: { manual: 'התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו קריאה חסכונית' },
  45: { claim: 'sha:' },
  56: { claim: 'CANON' },
  60: { claim: 'storage' },
  62: { claim: 'ג · פעולה שדורשת רשת' },
  65: { claim: 'עברית' },
  66: { claim: 'RULE_W' },
  67: { claim: 'BANNER_W' },
  68: { claim: 'מספר סבב' },
  69: { claim: 'CSS מתות' },
  70: { manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ והמונה נמדד ידנית' },
  71: { claim: 'מפנה לקובץ' },
  ...
  82: { claim: 'WebView' },
  86: { manual: 'נפילה-חזרה ל-`kv` נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  89: { claim: 'migrations' },
  90: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  97: { claim: 'type=password' },
  98: { claim: 'aria-label' },
  101: { manual: 'שני מנועי תאריך — ⛔ טרם הוכרע איזה, ואין מה לאכוף' },
  102: { manual: '«`catch` ריק סביב כתיבה» טרם נוסח כשער — נסרק ידנית בכל סבב שנוגע' },
  104: { claim: 'pass_salt' },
  106: { manual: 'מצב העמודה במסד אינו נראה מהריפו' },
  107: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  109: { claim: '⏳' },
  110: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  111: { manual: 'קיום טבלה או מפתח במסד אינו נראה מהריפו' },
  112: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  113: { manual: '«קובץ בלי קורא» דורש סריקה על ארבעת הריפו — נעשה ידנית' },
  114: { manual: 'קיום מפתח במסד אינו נראה מהריפו' },
  115: { manual: 'רשימת-היתר הגיבויים יושבת במיגרציה שכבר רצה' },
  116: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  118: { claim: 'CLEANUP_SHA' },
  119: { claim: 'בלוק ה-APP' },
};
...
const RULE_ROWS = {
  '⛔ git': 118,
  '⛔ הסבב מעדכן את הטבלה': 40,
  '⛔ מדידה לפני שינוי': 113,
  '⛔ סריקת האזור שנגעו בו': 110,
  '⛔ משמעת קריאה וחיסכון טוקנים': 44,
  '⛔ הכרעה שאינה של הסשן': 44,
  '⛔⛔ רכיב משותף = קוד זהה בית-לבית + מיקום זהה + חיווט חי': 45,
  '⛔ מתי כותבים הערה': 70,
  '⛔ הערה אינה מפנה לקובץ': 71,
  '⛔ צורת ההערה': 65,
  '⛔ דפוסי כתיבת ממשק': 57,
  '⛔ מבחן שקודו מנוטרל נושא נימוק בבאנר': 25,
  '⛔ הבדל מכוון מנומק בכתב, במקום שבו רואים אותו': 25,
  '⛔ ערך שקיים בקוד או במסד אינו מוצהר בתיעוד': 36,
  '⛔ קובץ נשפט לפי תפקידו': 5,
  '⛔ קובץ תיעוד אינו מסביר כלל שהטבלה אוכפת': 41,
  '⛔ תקרה היא מספר עגול שנקבע בהחלטה': 6,
  '⛔ תקציב הקבצים': 8,
  '⛔ שרשרת היכולת החדשה': 37,
  '⛔⛔ עמודת «מה זה» היא הוראה מחייבת': 38,
  '⛔ כל כלל בא לידי ביטוי בטבלה': 42,
  '⛔ מספור רציף ומבני, וסריקת כפילויות': 39,
  '⛔ תקן עמודת ההערות': 40,
  '⛔ `probe` מאמת **ערך** — ⛔ ולא קיום': 37,
  '⛔ תקן תוכן המבחן': 26,
  '⛔ תקן תוכן המוטציה': 29,
  '⛔ שער אינו מריץ את `check-js` במלואו': 33,
  '⛔ שני שערים על אותה טענה — אחד מהם מיותר': 35,
  '⛔ בדיקה שסופרת מילה אינה בדיקה': 30,
  '⛔ מבחן מצהיר אילו שורות בטבלה הוא אוכף': 37,
  '⛔ מוטציה נוקבת בשם הטענה שאמורה ליפול': 29,
  '⛔ תקן הבאנר — ארבע פסקאות בסדר קבוע': 27,
  '⛔ דפוס הבודקים והודעת הכשל': 23,
  '⛔ מספר בבאנר נגזר מהקבוע שבקוד': 36,
  '⛔ מה שאינו ניתן לאכיפה מכנית — נרשם ככזה': 40,
  '⛔ שער קורא בלבד': 31,
  '⛔ שער חדש נכנס ל-`APP.gates`': 37,
  '⛔ שינוי מכוון ברכיב משותף': 45,
};
