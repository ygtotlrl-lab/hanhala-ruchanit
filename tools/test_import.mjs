 ...
 *  ל-schar ול-gius אין ייבוא כלל — ובדיקה שנכתבת על מה שאינו קיים היא
 *  הצהרה ולא מדידה (כלל ברזל 14). `check-structure.mjs` מתיר `test_round*`
 *  ב-`tools/`, וזה המקום שהשער קורא.
 *
 *  שלושת הממצאים שהסבב סגר, וכל אחד מהם עם מוטציה:
 *    1. המזהה היה `maxId++` — שני מכשירים שמייבאים במקביל הקצו את אותם
 *       מזהים לתלמידים שונים, והמיזוג ראה בהם רשומה אחת.
 *    2. הייבוא לא סימן ⏳ — לא נספר במונה, לא נחשב «לדחיפה», ושער הפינוי
 *       היה רשאי לפנות תלמיד שטרם עלה.
 *    3. תוצאת הכתיבה המקומית לא נבדקה — «N תלמידים נוספו בהצלחה» הוצג גם
 *       כשהכתיבה נכשלה (כלל ברזל 2).
 *
 *  הבדיקה מריצה את **הפונקציה האמיתית** (נחתכת מ-`index.html` בהתאמת
 *  סוגריים) ברתמת `vm`, עם CSV אמיתי ו-FileReader מדומה.
 */
