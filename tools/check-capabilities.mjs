...
/*  ⭐ שורה 63 נמדדת ע"י `audit` של שער סבב 66 — ⛔ ולא ע"י probe משלה
...
/*  ⭐ ושורה 84 נמדדת ע"י `audit` של שער סבב 67 — ⛔ אותו נימוק בדיוק:
...
const APP = {
  ...
  tableProbe: {
    ...
    88:  (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    73: (c) => c.hasCode(/YS_KV_LEGACY_WRITE\s*=\s*false/),
    ...
    87: (c) => !!c.fnBody('changeMyPassword')
...
const CAPS = {
   ...
   *  שנאכף כאן הוא **הליבה**, ומה שנאכף בטבלה (שורה 40) הוא ששתי
   ...
   *  **בתוך** הליבה ולכן נאכפים בחתימה עצמה. מה שנאכף בטבלה (שורה 110)
   ...
   *  (שורה 43) כ-⭕ מנומק. ⛔ «קיים רק באחת, בשקט» אינו מצב חוקי
   ...
   *  בטבלה (שורה 44) כ-⭕ מנומק, בדיוק כמו `lock`.                    */
  sess: {
    name: 'מודל הסשן',
...
const MATRIX = [
  ...
  { row: 83, name: 'אימות פינוי מול הענן',
    probe: () => /\bverify\s*:/.test(policyBlock()) },
  { row: 66, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 61, name: 'מעטפת APK (WebView)',
    probe: () => hasPath('android/app/src/main/AndroidManifest.xml') },
  { row: 70, name: 'מפתח חתימה קבוע בריפו',
    probe: () => hasPath('signing') &&
                 fs.readdirSync('signing').some((f) => f.endsWith('.keystore')) },
  { row: 75, name: 'מקור אמת יחיד לסכימה', probe: () => hasPath(APP.schemaFile) },
  { row: 76, name: 'קובץ התקנה מלא',       probe: () => hasPath(APP.schemaFile) },
  { row: 36, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 36, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 73, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 37, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 37, name: 'שחזור מקומי מהענן',
    probe: () => callSites('hwRestoreMount').length > 0 },
  { row: 87, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 110, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 36, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 80, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 38, name: 'מנוע מיזוג עם הגנת ⏳',
    probe: () => /isPend \|\| tsOf\(loc\) > tsOf\(rem\)/.test(fnBody('_mergePick')) },
  { row: 93, name: 'חסימת משתמש מושבת בכניסה אופליין',
   ...
   *  הרשומות שלו עדיין חותמות זמן. הנימוק יושב בשורה 39 שבטבלה. */
  { row: 39, name: 'מודול מזהי רשומות',
  ...
  { row: 61, name: 'מעטפת WebView חתומה',
  ...
  { row: 93, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 68, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 110, name: 'מודול ה-service worker',
   ...
   *  אותו כלל בדיוק כמו בשורה 110.                                   */
  { row: 41, name: 'ניסיון חוזר בתור הסנכרון',
   ...
   *  בדיוק כמו בשורות 110 ו-41.                                       */
  { row: 42, name: 'מנגנון משיכה אחיד',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
   ...
   *  מעליה. ⛔ ליבה בלי פרמטרים אינה מודול אלא קוד שהועתק — אותו כלל
   *  בדיוק כמו בשורות 110, 41 ו-42.                                   */
  { row: 43, name: 'נעילת חוסר-פעילות',
  ...
  { row: 44, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 109, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 35, name: 'רענון תקופתי של מונה הממתינים',
    probe: () => hasCode(/setInterval\(\s*pendRender\s*,\s*60000\s*\)/) },
  { row: 52, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  בשקט».                                                        */
  { row: 81, name: 'גריעת tombstones לפי גיל',
    probe: () => hasCode(/TOMBSTONE_TTL_MS/) },
  { row: 108, name: 'אוטו-אפדייט מ-raw.githubusercontent',
  ...
  { row: 63, name: 'שכבת אייקונים',
    probe: () => iconAudit('.').length === 0 },
  { row: 84, name: 'שכבת קלט אחידה',
...
const GATES = {
  ...
  57: 'check-comments',
  58: 'test_docrules',
  59: { manual: '«האם ההערה עדיין נכונה» הוא התאמה למציאות ולא צורה — ⛔ שער מודד צורה בלבד' },
  60: 'check-comments',
  62: 'test_filesets',
  64: 'test_iconlayer',
  65: 'test_iconlayer',
  ...
  69: 'test_gradle',
  71: 'test_android',
  74: { manual: 'נפילה-חזרה ל-`kv` נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  77: 'test_sources',
  ...
  90: { manual: '«`catch` ריק סביב כתיבה» טרם נוסח כשער — נסרק ידנית בכל סבב שנוגע' },
  91: 'test_passwords',
  92: 'test_passwords',
  ...
  100: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  101: { manual: '«קובץ בלי קורא» דורש סריקה על ארבעת הריפו — נעשה ידנית' },
  102: { manual: 'קיום מפתח במסד אינו נראה מהריפו' },
  103: { manual: 'רשימת-היתר הגיבויים יושבת במיגרציה שכבר רצה' },
  104: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  106: 'test_workflows',
  107: 'test_filesets',
};
