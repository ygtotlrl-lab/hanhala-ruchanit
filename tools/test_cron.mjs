...
/* ⭐ המיגרציה של הפרויקט המשותף יושבת כאן, מפני ש-`sh_backup` מוגדרת כאן
...
const APP = {
  ...
  sweepMigration: 'migrations/036_rename_shared_tables_to_sh.sql',
  ...
  /*  ⛔ משפחת סכימה משותפת שנייה — ⚠️ `ya_settings_rishon`/`ya_settings_ramataviv`
...
/*  ⛔ השורה שהקובץ הזה אוכף — ⚠️ בעלות הסכימה המשותפת: ⭐ עותק
 *  אחד, בריפו אחד, ⛔ והנמדד הוא היעדר העותק השני. */
export const ROWS = [166, 171, 180, 218];
...
const FLOOR = { shared: 25, app: 33, appWhy: 'מספר הטבלאות והמשימות — הבעלות על מיגרציית הגיבוי כאן' };
/* ⚠️ סוף פר-אפליקציה */
...
/*  שם משתנה רשימת-ההיתר — ⛔ נגזר מההצהרה ⛔ ואינו מוקלד:
...
/* ────── מפתחות הגיבוי שהאפליקציה באמת כותבת — נגזרים מ-`BK_CFG.sources()` ──
   ⚠️ נגזרים ולא מוצהרים: רשימה מוצהרת בקובץ הבדיקה היא
...
function bkKeysFromSrc(src) {
  ...
  let body = m[1]
    ...
    .replace(/cols:\s*'[^']*'/g, '')
    .replace(/order:\s*'[^']*'/g, '')
    /*  ⛔ `ts` הוא שם עמודת החותמת — ⚠️ הוא הוסף לכל מקור-טבלה
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
/*  ⛔ מונה ולא נוכחות — ⚠️ `test(SRC)` עובר גם על הצהרה כפולה
...
function t1() {
  once(/function _bkRetention\(c, keys\)/,
    '1א · `_bkRetention` קיימת במודול המשותף (⛔ אין להסיר)');
  once(/\.in\('key', keys\)/,
...
function t2(sql) {
  ...
  /* ⚠️ 03:00 ולא 03:17 — הקבצים תיארו 03:17 בעוד שהמשימה שנרשמה
  ...
  /* ⭐ שקילות דו-כיוונית: כל מפתח שהאפליקציה כותבת נמצא
...
mutStage();
if (!RUN_MUT) {
  console.log('\n⏭ test_cron: המוטציות רצות ברמה המלאה (--full) — ⛔ ואינן נמדדות כאן');
  process.exit(failed ? 1 : 0);
}
/* ══════════════════════════════════════════════════════════════════════════
   4 · מוטציות — שתי הדרישות
   ══════════════════════════════════════════════════════════════════════════ */
function t4(sql) {
  const daily = contribution(SRC)[0];
...
/* ── הרצה ──────────────────────────────────────────────────────────────── */
console.log('· ' + FACTS.slug + ' — פינוי גיבויים אוטומטי במסד');
t1();
...
if (APP.migration) {
  /* ⭐ «המצב האפקטיבי» ולא «הקובץ הראשון»: מיגרציה שכבר רצה אינה
...
/*  ⛔ משפחה שנייה — שתי טבלאות ההגדרות של היומן. ⚠️ ההבדל
...
/*  ⛔ מוטציה: שם טבלה שהוסב ונשאר בגוף החי — ⚠️ הפונקציה
