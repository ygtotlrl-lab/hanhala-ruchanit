 ...
 *  אינן נכתבות לעץ.
 *
 *  **הנימוק המדוד:** תפוגת סשן לא הייתה קיימת באף אחת מהן — ⛔ ההבדל היה
...
const APP = {
  ...
  /*  ⚠️ מפתחות הסשן שהאפליקציה שמרה בהם בעבר — ⛔ וכל מפתח כאן
   *  חייב להתנקות בעלייה. */
  legacy: [],
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
const START = '/* ═══ מודל הסשן — מודול משותף';
...
const FLOOR = { shared: 3, app: 18, appWhy: 'שכבת הכניסה — הסשן מחווט למשתמש המחובר בשלוש, וביומן אין כניסה' };
/* ⚠️ סוף פר-אפליקציה */
...
if (!APP.present) {
  ...
  console.log(failures ? `\n✗ מודל הסשן — ${failures} נכשלו`
                       : '\n✓ מודל הסשן — 3 טענות עברו, 0 נכשלו');
...
console.log(failures ? `\n✗ מודל הסשן — ${failures} נכשלו`
                     : `\n✓ מודל הסשן — ${mn} טענות עברו, 0 נכשלו`);
process.exit(failures ? 1 : 0);
