-- ============================================================================
-- 005_structured_tables.sql — הנהלה רוחנית עוברת לטבלאות מובנות (סבב 36, שלב א)
-- ============================================================================
-- ⛔⛔ **הקובץ הזה נכתב ולא הורץ (סבב 36) — «נכתב» אינו «רץ».** הרצתו היא
--     החלטת המנהל, והיא נדרשת *לפני* שהמתג של שלב ב נדלק. הקוד ב-`index.html`
--     בטוח לפריסה גם בלי הרצה: `ysRowsPush` נכשלת סגור ומדלגת בשקט, וה-`kv`
--     ממשיך להיות המאסטר.
--
-- ── מה זה פותר ─────────────────────────────────────────────────────────────
-- hanhala-ruchanit היא האחרונה בארגון שנתוניה אינם בטבלאות אלא כמפתחות `kv`,
-- כשכל מפתח הוא **ערך יחיד** (מטריצת היכולות, שורה 15). נמדד מול המסד החי
-- ב-2026-08-18, ב-`SELECT` בלבד:
...
-- סבב 35 יושב כאן רדום (`HW_CFG.enabled=false`).
--
...
--    בכל שאילתת חלון.
--
-- ⛔ **בלי מפתח זר פיזי מ-`ys_marks` לאב (סבב 36)** — המחיקה בארגון היא רכה
...
-- הנתונים החיים (2026-08-18) הראתה **5 התנגשויות** — שני סדרים חיים לאותו
-- שם-סדר ולאותו יום:
--   נגלה צהריים · 2026-06-23   1782220054450_30m2p + 1782222468527_mjq7g
--   נגלה בוקר   · 2026-05-25   1779695986325_8xboy + 1779708492319_j0qm3
--   נגלה צהריים · 2026-05-19   1779199478427_xfwvm + 1779195927855_r462p
--   חסידות בוקר · 2026-06-16   1781614067051_vsbck + 1781584237473_9u1la
--   מנחה        · 2026-07-08   1783509468290_5z9b6 + 1783498625625_y992a (השני מחוק)
--
...
--      `r.id` מאז סבב 2, ו-`client_id` נגזר ממנו. אילוץ ייחודיות על צמד
...
--   • `ys_sleep_sessions` (38KB) — אותו מבנה בדיוק, ויעבור באותו דפוס
--     בסבב ייעודי. שלב א מכסה את הנתיב שנמדד כבעייתי (360KB).
...
--    (`jsonb`) ו⛔ **אינו נגזר מחדש מ-`date_iso`** (סבב 36) — שחזור תאריך
--    עברי הוא שכבת תצוגה בלבד, וזה הלקח שנרשם ב-yoman סבב 31 (`snapHDate`
...
--    שהמכשיר כתב (`2026-05-17T05:38:35.425Z`), ו-`timestamptz` היה מחזיר
...
-- ⛔ `SELECT, INSERT, UPDATE` ותו לא (כלל ברזל 10 סעיף 9, סבב 29) — מחיקה כאן
...
--    רשומת התלמיד היא בעלת צורה **משתנה**: נמדדו 13 שדות שונים, מהם 9
