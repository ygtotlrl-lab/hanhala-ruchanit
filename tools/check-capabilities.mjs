...
const APP = {
  ...
  gapRows: [228, 146, 150, 174, 68, 159, 112],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    197: (c) => c.hasCode(/HR_PASS_ITER\s*=\s*100000\b/)
    ...
    196: (c) => !!c.fnBody('hrUsersCacheSave')
    ...
    157: (c) => !/hrCfgSet\('(?:hr_students|hr_sessions|hr_sleep_sessions)'/.test(c.src)
    ...
    193: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '193|מבנה טופס הכניסה': 'src',
    '159|שם נגזר אינו נשמר ברשומה': 'src',
    '41|שער מריץ את כל טענותיו': 'tools',
    '101|שכבת המראה': 'src',
    '215|הגירה מקומית שהושלמה': 'mixed',
    '195|מראת המשתמשים': 'src',
    '194|מודל ההרשאות': 'src',
    '28|שער סורק קוד מולבן': 'tools',
    '28|ההלבנה שומרת על מספרי השורות': 'mixed',
    '175|הקשר נלכד בכניסה לפונקציה': 'src',
    ...
    '209|הערה שמתארת מצב שחלף': 'mixed',
    '216|שאילתת `@media` מתה': 'src',
    ...
    '177|הגיבוי שלם ומעומד': 'src',
    '80|פעולה מגיבה מיד': 'src',
    '81|משיכה מסוננת בשרת': 'src',
    '145|דגימת-יתר במחולל': 'tools',
    '197|`PBKDF2` — פרמטרים': 'src',
    '163|`migrations/` — תוכן': 'src',
    '164|`migrations/` — כמות': 'src',
    '185|מיזוג מכל — מפה ומערך': 'src',
    '186|ברירת מחדל אינה ממוזגת': 'src',
    '187|רשימה אינה נושאת פריט כפול': 'src',
    '178|`sh_backup` — טבלת הגיבוי': 'src',
    '26|שער אינו מקליד מספר שורה': 'tools',
    '32|שער ממתין לתנאי ולא לשעון': 'tools',
    '201|סודות אינם עוזבים את המכשיר': 'src',
    '172|דפוס `upsert` — onConflict מוכרז': 'src',
    '199|טביעה חסרה מושלמת אוטומטית': 'src',
    '199|כניסה אופליין': 'src',
    '156|עריכת נתונים אופליין': 'src',
    '182|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '200|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '78|אובייקט יקר נבנה פעם אחת': 'src',
    '79|מסך מציג מיד ומרענן ברקע': 'src',
    '149|האפליקציה ניתנת להתקנה מכרום': 'src',
    '206|דגלי מעבר — אין דגל דלוק': 'src',
    '86|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '196|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '100|פינוי אוטומטי': 'src',
    '100|אופק הפינוי נוקה מהסנכרון': 'src',
    '189|אימות פינוי מול הענן': 'src',
    '150|שיתוף קבצים': 'src',
    '133|מעטפת APK (WebView)': 'src',
    '136|אין נכסים מוטבעים': 'src',
    '154|מפתח חתימה קבוע': 'src',
    '160|מקור אמת יחיד לסכימה': 'src',
    '161|קובץ התקנה מלא': 'src',
    '64|גיבוי יומי אוטומטי': 'src',
    '64|יומן פעולות': 'src',
    '157|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    ...
    '181|גריעת tombstones לפי גיל': 'src',
    ...
    '190|שכבת קלט אחידה': 'src',
    '190|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '83|שכבת המודאל': 'src',
    '193|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [34, 49, 50, 59, 99, 57, 188, 119, 166, 63, 27, 68, 30, 39, 43, 204, 211];
...
const CAPS = {
  ...
  scales: {
    ...
    block: { sha: '6f80cd74da28c254', lines: 39,
             css: true,
...
const MATRIX = [
  ...
  { row: 159, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 175, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 209, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 177, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 174, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 197, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 163, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 164, name: '`migrations/` — כמות',
  ...
  { row: 185, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 186, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 187, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 178, name: '`sh_backup` — טבלת הגיבוי',
  ...
  { row: 201, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 172, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 199, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 199, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 156, name: 'עריכת נתונים אופליין',
  ...
  { row: 182, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 200, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 206, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 196, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 189, name: 'אימות פינוי מול הענן',
  ...
  { row: 154, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 160, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 161, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 64, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 64, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 157, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 193, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 193, name: 'מבנה טופס הכניסה',
  ...
  { row: 176, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 199, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 199, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 152, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 215, name: 'הגירה מקומית שהושלמה',
   ...
   *  ולא רק מוגדרים. */
  { row: 181, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 190, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  /*  ⭐ סבב 157 — ⛔ שכבת השירות אינה מערכת שנייה: ⚠️ כלל `u-*` בלי
   *  שימוש · יחס שירות שעובר אחד · ⭐ ושרשרת שחוזרת יותר מפעמיים. */
  111: { claims: { test_rulesdocs: ['[util-orphan]', '[util-ratio]', '[util-chain]'] } },
  ...
  191: { claims: { test_codescan: ['[zero-guard]', '[zero-guard-registry]'] } },
  ...
  223: { claims: { test_mignames: ['[mig-pattern]', '[mig-seq]', '[mig-file-no-entry]',
  ...
  184: { claims: { test_parentchild: ['[pc-rowkey]'] } },
  ...
  204: { claims: { test_docfacts: ['[doc-name]', '[doc-round]'],
  ...
  208: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  202: { claim: 'אין `service_role` בקובץ' },
  36: { claim: 'מסלול «' },
  ...
  205: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  73: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  165: { claim: 'בעלות הסכימה המשותפת' },
  179: { claim: '`ya_settings_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  180: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  217: { claim: 'ז. עמודה בלי קורא' },
  ...
  140: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  143: { claim: 'margin' },
  ...
  120: { claim: 'BANNER_W' },
  117: { claim: 'מפנה לקובץ' },
  138: { claim: 'android/app/src/main' },
  146: { claim: 'fgDriftMax' },
  147: { claim: 'CANON_MANIFEST' },
  148: { claim: 'שדות זהים' },
  151: { claim: 'BUILD_SHA' },
  155: { claim: 'SHARED_SHA' },
  153: { claim: 'versionCode' },
  134: { claim: 'WebView' },
  135: { claim: 'android:theme' },
  141: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  158: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  162: { claim: 'migrations' },
  171: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  173: { claim: 'א. חותמת בכל רשומה' },
  63: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  166: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  167: { claim: 'ו. טבלה מקבילה' },
  168: { claim: 'ב. חתימת סכימה' },
  169: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  192: { claim: 'type=password' },
  88: { claim: 'aria-label' },
  ...
  203: { claim: '[sec-keyfile]' },
  207: { claims: { test_filesets: ['[oneoff-gate]', '[oneoff-round]', '[oneoff-stale]'] } },
  ...
  220: { claim: 'ט. ערך נגזר שהתיישן' },
  222: { claim: 'ח. הגדרה שהוחלפה' },
  221: { claim: 'ד. רשימת-היתר' },
  224: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  188: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
