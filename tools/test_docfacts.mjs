/* ───────────────────────────────────────────────────────────────────────────
   ...
   **מה אינו נאכף כאן:** ⛔ **ערכי המפתח** — ⚠️ טביעה · מועד יצירה · תוקף:
   ⭐ מקורם `keytool` ⛔ ולא העץ, ⚠️ והשורות שנושאות אותם מוכרזות
   ב-`APP.keyLines` ונכתבות מחדש בסבב שמחליף את המפתח · ⛔ **ופרוזה
   חופשית** — ⚠️ המדידה על שם בגרשיים אחוריים בלבד, ⭐ שפסקה אינה ניתנת
...
const APP = {
  ...
  retired: {},
  /*  ⛔ שורות שערכן מקורו ב-`keytool` — ⚠️ **מה נכנס**: הביטוי שמאתר את
   *  השורה; ⛔ **ומה מפיל**: הכרזה שאין לה שורה חיה. ⭐ **ולמה המבנה
   *  קיים**: מועד היצירה והתוקף נגזרים מהמפתח ⛔ ולא מהעץ, ⚠️ והם נכתבים
   *  מחדש בסבב שמחליף אותו: ⭐ מדידת פורמט עליהם כאן הייתה דורשת לכתוב
   *  ערך שאין לו מקור. */
  keyLines: ['\\|\\s*\\*\\*תוקף\\*\\*\\s*\\|'],
  /*  ⛔ נימוק להיעדר שורות כאלה — ⚠️ **מה נכנס**: למה אין באפליקציה הזו
   *  שורת ערך מפתח; ⛔ **ומה מפיל**: רשימה ריקה בלי נימוק. ⭐ **ולמה ריק**:
   *  יש שורות, והנימוק נמדד ברשימה עצמה. */
  keyLinesWhy: '',
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
/*  ⛔ השורות בטבלת התשתית שהקובץ הזה אוכף — ⚠️ המיפוי נגזר מכאן ⛔ ואינו
 *  רשימה שנייה בבודק. */
export const ROWS = [126, 191];
 ...
 *  שהיא, ⭐ ומה שאינו `YYYY-MM-DD` מפיל: ⛔ ושורת ערך מפתח מוכרזת
 *  ב-`APP.keyLines` — ⚠️ מקורה `keytool`, ⭐ והיא נכתבת מחדש בסבב
 *  שמחליף את המפתח. */
...
export function dateGaps(lines, keyLines) {
  const out = [];
  lines.forEach((l, i) => {
    if (keyLines.some((re) => re.test(l))) return;
    for (const m of noTicks(l).matchAll(DATE_ANY))
      if (!DATE_ISO.test(m[1])) out.push('שורה ' + (i + 1) + ': «' + m[1] + '»');
  });
  return out;
}
/*  ⛔ והכרזת שורת מפתח נמדדת משני צדדיה — ⚠️ הכרזה שאין לה שורה חיה
 *  היא היתר שלא נסגר, ⭐ והיא מתארת מצב שחלף. */
export function keyLineGaps(lines, keyLines) {
  return keyLines.filter((re) => !lines.some((l) => re.test(l)))
                 .map((re) => String(re));
}
...
export function roundGaps(lines, born, keyLines) {
  const out = [];
  lines.forEach((l, i) => {
    if (keyLines.some((re) => re.test(l))) return;
...
const KEY_RE = (APP.keyLines || []).map((s) => new RegExp(s));
...
/* ── ג · פורמט אחד לתאריך ──────────────────────────────────────────────── */
{
  const g = dateGaps(ALL, KEY_RE);
  t(n++, g.length === 0,
    `[doc-date] תאריך שאינו \`YYYY-MM-DD\` — נמדדו ${g.length} והצפוי אפס` +
    `${g.length ? ' (' + g.slice(0, 6).join(' · ') + ')' : ''}. ` +
    'כותבים את התאריך בפורמט אחד, או מכריזים את שורת ערך המפתח ב-`APP.keyLines`');
}
{
  /*  ⛔ רשימה ריקה נושאת נימוק — ⚠️ **היעדר מוצהר**: ⭐ שדה חסר נקרא «לא
   *  נשאל», ⛔ וריק בלי נימוק אינו נבדל מרשימה שנשמטה. */
  const g = keyLineGaps(ALL, KEY_RE);
  const why = String(APP.keyLinesWhy || '').trim();
  const bare = KEY_RE.length === 0 && why.length < 20;
  t(n++, g.length === 0 && !bare,
    `[doc-date] הכרזת שורת ערך מפתח — נמדדו ${g.length} מתוך ${KEY_RE.length} הכרזות ` +
    `בלי שורה חיה, ונימוק באורך ${why.length}; והצפוי אפס, ונימוק בן 20 תווים לפחות ` +
    `כשהרשימה ריקה${g.length ? ' (' + g.join(' · ') + ')' : ''}. ` +
    'מסירים מ-`APP.keyLines` הכרזה שאין לה שורה, או כותבים ב-`APP.keyLinesWhy` למה אין');
}
...
/* ── ד · אזכור סבב שקדם ללידה ──────────────────────────────────────────── */
{
  ...
  const g = born ? roundGaps(ALL, born, KEY_RE) : [];
  t(n++, g.length === 0,
    born
...
if (RUN_MUT) {
  ...
  {
    const got = dateGaps(['התאריך 10.08.2026 נמדד בסבב 41'], KEY_RE);
    t(n++, got.length === 1,
  ...
  {
    const got = roundGaps(['⭐ ובסבב 39 הוחלף גם ה-keystore'], 145, KEY_RE);
    t(n++, got.length === 1,
  ...
  {
    const got = keyLineGaps(['שורה שאינה תואמת'], [/zzNoSuchKeyLine/]);
    t(n++, got.length === 1,
      'מ6 · ⛔ מוטציה: הכרזת שורת מפתח בלי שורה חיה מפילה את «[doc-date]» — ' +
      `נמדדו ${got.length} והצפוי 1`);
  }
  {
  ...
  {
    const got = dateGaps(['נמדד ב-2026-09-15 והתוצאה נשמרה'], KEY_RE);
    t(n++, got.length === 0,
  ...
  {
    const got = roundGaps(['⭐ סבב 148 — הסחף נסגר'], 145, KEY_RE);
    t(n++, got.length === 0,
