...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    198: (c) => c.hasCode(/HR_PASS_ITER\s*=\s*100000\b/)
    ...
    197: (c) => !!c.fnBody('hrUsersCacheSave')
    ...
    158: (c) => !/hrCfgSet\('(?:hr_students|hr_sessions|hr_sleep_sessions)'/.test(c.src)
    ...
    194: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '101|שכבת המראה': 'src',
    '216|הגירה מקומית שהושלמה': 'mixed',
    ...
    '210|הערה שמתארת מצב שחלף': 'mixed',
    '217|שאילתת `@media` מתה': 'src',
    ...
    '197|נתיב עדכון חלקי למראת המשתמשים': 'src',
    ...
    '229|מיכל באנר העדכון במקור': 'src',
    '229|הרענון מ-controllerchange בלבד': 'src',
    '229|סימן דחיית הבאנר מתמיד': 'src',
    '128|רישום כשלי כתיבה': 'src',
    '65|חלון חם במכשיר': 'src',
    '194|מסך שינוי סיסמה עצמי': 'src',
    '231|מטמון-CDN מראש עם ריפוי עצמי': 'src',
...
export const ROWS = [37, 34, 50, 51, 59, 99, 57, 189, 119, 167, 63, 27, 68, 30, 39, 43, 205, 212];
...
const MATRIX = [
  ...
  { row: 210, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 231, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 231, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 230, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 216, name: 'הגירה מקומית שהושלמה',
  ...
  { row: 229, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 229, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 229, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 229, name: 'עדכון אוטומטי — בדיקה מחזורית',
...
const CLAIM_NO_MUT = {
  ...
  '54|ט · probe מודד מימוש':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '55|יא · נימוק ⭕ מונה את מה שאינו נאכף':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  ...
  '209|[core-allow]':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '212|כל קובץ בעץ מוזכר במקום אחר':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '213|מזהה שנמחק ונשאר לו קורא':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '214|[call-arity-registry]':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '218|ז. עמודה בלי קורא':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '219|[sync-cols-why]':
    'המוטציה שבשער נוקבת במנגנון שהיא שוברת — ⛔ ולא בתווית שמאתרת את הטענה בפלט',
  '221|ט. ערך נגזר שהתיישן':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '222|ד. רשימת-היתר':
    'הטענה נמדדת מול המסד החי — ⛔ ומוטציה הייתה כותבת לייצור',
  '223|ח. הגדרה שהוחלפה':
...
const GATES = {
  ...
  219: { claims: { test_dbscan: ['[sync-cols]', '[sync-cols-why]'] } },
  ...
  224: { claims: { test_mignames: ['[mig-pattern]', '[mig-seq]', '[mig-file-no-entry]',
  ...
  211: { claims: { test_signeddead: ['[signed-dead]', '[skip-why]'],
  ...
  220: { claims: { test_cron: ['ל2 · כל שם טבלה שבגוף החי'] } },
  ...
  233: { manual: 'המסך הראשון מול מסד ריק אינו נגזר מהטקסט — ⛔ הוא נמדד במבחן הקבלה, ⚠️ ושער אינו מריץ דפדפן' },
  ...
  227: { claims: { test_toolsid: ['[scan-derived]'] } },
  228: { claims: { test_toolsid: ['[scan-enforcer]'] } },
  ...
  213: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  49: { claim: 'ועמודת התקן שלה אוסרת במפורש' },
  ...
  212: { claims: { test_orphans: 'כל קובץ בעץ מוזכר במקום אחר',
  ...
  221: { claim: 'ט. ערך נגזר שהתיישן' },
  223: { claim: 'ח. הגדרה שהוחלפה' },
  222: { claim: 'ד. רשימת-היתר' },
  ...
  231: { claims: { test_swcore: 'respondWith' } },
};
