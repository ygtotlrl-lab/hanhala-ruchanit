...
const APP = {
  ...
  scanKind: {
    ...
    'test_coremod': 'raw — סמן הבלוק החתום הוא הערה, ⛔ וההלבנה מוחקת בדיוק את מה שהוא מודד',
  ...
  gapRows: [225, 143, 147, 171, 67, 156, 109],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    194: (c) => c.hasCode(/HR_PASS_ITER\s*=\s*100000\b/)
    ...
    193: (c) => !!c.fnBody('hrUsersCacheSave')
    ...
    154: (c) => !/hrCfgSet\('(?:hr_students|hr_sessions|hr_sleep_sessions)'/.test(c.src)
    ...
    190: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '190|מבנה טופס הכניסה': 'src',
    '156|שם נגזר אינו נשמר ברשומה': 'src',
    '40|שער מריץ את כל טענותיו': 'tools',
    '99|שכבת המראה': 'src',
    '212|הגירה מקומית שהושלמה': 'mixed',
    '192|מראת המשתמשים': 'src',
    '191|מודל ההרשאות': 'src',
    '27|שער סורק קוד מולבן': 'tools',
    '27|ההלבנה שומרת על מספרי השורות': 'mixed',
    '172|הקשר נלכד בכניסה לפונקציה': 'src',
    '103|פריסה במסכי טלפון וטאבלט': 'src',
    '104|ערכת נושא — בהיר וכהה': 'src',
    '106|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '107|אין סגנון מוטבע': 'src',
    '109|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '206|הערה שמתארת מצב שחלף': 'mixed',
    '213|שאילתת `@media` מתה': 'src',
    '128|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '129|ייצוא והנפקה — דרך מוצהרת': 'src',
    '110|המסמך מתפרסר נקי': 'src',
    '111|לוג מסלול — תג ורמה': 'src',
    '83|מיכל המודאל ילד ישיר של `body`': 'src',
    '124|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '174|הגיבוי שלם ומעומד': 'src',
    '79|פעולה מגיבה מיד': 'src',
    '80|משיכה מסוננת בשרת': 'src',
    '142|דגימת-יתר במחולל': 'tools',
    '194|`PBKDF2` — פרמטרים': 'src',
    '160|`migrations/` — תוכן': 'src',
    '161|`migrations/` — כמות': 'src',
    '182|מיזוג מכל — מפה ומערך': 'src',
    '183|ברירת מחדל אינה ממוזגת': 'src',
    '184|רשימה אינה נושאת פריט כפול': 'src',
    '175|`sh_backup` — טבלת הגיבוי': 'src',
    '25|שער אינו מקליד מספר שורה': 'tools',
    '31|שער ממתין לתנאי ולא לשעון': 'tools',
    '198|סודות אינם עוזבים את המכשיר': 'src',
    '169|דפוס `upsert` — onConflict מוכרז': 'src',
    '196|טביעה חסרה מושלמת אוטומטית': 'src',
    '196|כניסה אופליין': 'src',
    '153|עריכת נתונים אופליין': 'src',
    '179|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '197|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '77|אובייקט יקר נבנה פעם אחת': 'src',
    '78|מסך מציג מיד ומרענן ברקע': 'src',
    '146|האפליקציה ניתנת להתקנה מכרום': 'src',
    '203|דגלי מעבר — אין דגל דלוק': 'src',
    '85|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '193|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '98|פינוי אוטומטי': 'src',
    '98|אופק הפינוי נוקה מהסנכרון': 'src',
    '186|אימות פינוי מול הענן': 'src',
    '147|שיתוף קבצים': 'src',
    '130|מעטפת APK (WebView)': 'src',
    '133|אין נכסים מוטבעים': 'src',
    '151|מפתח חתימה קבוע': 'src',
    '157|מקור אמת יחיד לסכימה': 'src',
    '158|קובץ התקנה מלא': 'src',
    '63|גיבוי יומי אוטומטי': 'src',
    '63|יומן פעולות': 'src',
    '154|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '86|דפוס הודעת שגיאה יחיד': 'src',
    '94|`toast` — חתימה, גוף ומחלקות': 'src',
    '94|סיווג ההודעה נגזר מהמסלול': 'src',
    '224|מיכל באנר העדכון במקור': 'src',
    '224|הרענון מ-controllerchange בלבד': 'src',
    '224|סימן דחיית הבאנר מתמיד': 'src',
    '125|רישום כשלי כתיבה': 'src',
    '64|חלון חם במכשיר': 'src',
    '190|מסך שינוי סיסמה עצמי': 'src',
    '226|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '65|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '196|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '66|מודול מזהי רשומות': 'src',
    '66|מזהה אינו מומר למספר': 'src',
    '68|מזהה מכשיר': 'src',
    '65|ליבת מיזוג משותפת': 'src',
    '130|מעטפת WebView חתומה': 'tools',
    '196|אימות מול טביעה בענן': 'mixed',
    '149|בניית APK אחידה עם שער חתימה': 'src',
    '226|מודול ה-service worker': 'src',
    '69|ניסיון חוזר בתור הסנכרון': 'src',
    '70|מנגנון משיכה אחיד': 'src',
    '73|נעילת חוסר-פעילות': 'src',
    '74|מודל הסשן — בזיכרון בלבד': 'src',
    '225|בדיקת עדכון תקופתית ל-service worker': 'src',
    '101|פסק זמן אחיד לקריאות רשת': 'src',
    '102|מאזיני מצב רשת': 'src',
    '178|גריעת tombstones לפי גיל': 'src',
    '224|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '147|גשר שיתוף': 'src',
    '75|העברת מזהה ל-DOM': 'src',
    '76|מיון אחד לכל התצוגות': 'src',
    '81|ייצוא — שני דפוסים מוצהרים': 'src',
    '139|שכבת אייקונים': 'src',
    '187|שכבת קלט אחידה': 'src',
    '187|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '82|שכבת המודאל': 'src',
    '190|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [33, 48, 49, 58, 97, 56, 185, 116, 163, 62, 26, 67, 29, 38, 42, 201, 208];
...
const CAPS = {
  ...
  scales: {
    ...
    block: { sha: '21a6ee5bd2d5e98e', lines: 47,
             css: true,
...
const FLOOR = { shared: 90, app: 9, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי' };
/* ⚠️ סוף פר-אפליקציה */
...
const MATRIX = [
  ...
  { row: 156, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 27, name: 'שער סורק קוד מולבן',
    probe: () => scanKindGaps().length === 0 },
  { row: 27, name: 'ההלבנה שומרת על מספרי השורות',
  ...
  { row: 172, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 109, name: 'גרף נבנה מ-CSS ולא מספרייה',
  ...
  { row: 206, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 111, name: 'לוג מסלול — תג ורמה',
  ...
  { row: 124, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 174, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 171, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 142, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 194, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 160, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 161, name: '`migrations/` — כמות',
  ...
  { row: 182, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 183, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 184, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 175, name: '`sh_backup` — טבלת הגיבוי',
    probe: () => (/BK_TABLE\s*=\s*'([a-z_]+)'/.exec(srcRefs) || [])[1] === 'sh_backup' },
  { row: 25, name: 'שער אינו מקליד מספר שורה',
  ...
  { row: 31, name: 'שער ממתין לתנאי ולא לשעון',
  ...
  { row: 198, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 169, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 196, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 196, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 153, name: 'עריכת נתונים אופליין',
  ...
  { row: 179, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 197, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 146, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 203, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 193, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 186, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 147, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 130, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 133, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 151, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 157, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 158, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 63, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 63, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 154, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 125, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 64, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 190, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 190, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 226, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 173, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 196, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 130, name: 'מעטפת WebView חתומה',
  ...
  { row: 196, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 149, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 226, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 225, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 212, name: 'הגירה מקומית שהושלמה',
   ...
   *  ולא רק מוגדרים. */
  { row: 178, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 224, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 224, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 224, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 224, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 139, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 187, name: 'שכבת קלט אחידה',
  ...
  { row: 187, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  120: { claims: { test_failsurface: ['[fail-surface]', '[fail-literal]', '[queued-msg]', '[msg-shared]'] } },
  ...
  215: { claims: { test_dbscan: ['[sync-cols]', '[sync-cols-why]'] } },
  ...
  121: { claims: { test_names: ['[name-policy]', '[name-policy-why]', '[fn-sister-prefix]',
  ...
  220: { claims: { test_mignames: ['[mig-pattern]', '[mig-seq]', '[mig-file-no-entry]',
  ...
  201: { claims: { test_docfacts: ['[doc-name]', '[doc-round]'],
  ...
  205: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  118: { claims: { test_period: ['[verb-registry]', '[verb-old-name]'],
  ...
  119: { claims: { test_period: ['[table-role]', '[table-const]'] } },
  /*  ⭐ סבב 148 — ⛔ תחילית הטבלאות והאחסון נגזרת משם הריפו: ⚠️ ראשי
   *  התיבות מול הסכימה המוצהרת, ⭐ וההגירה המקומית שנוקבת בתחילית שירדה. */
  122: { claims: { test_period: ['[prefix-derived]', '[prefix-legacy]'] } },
  /*  ⭐ סבב 148 — ⛔ שם טבלה שחי בגוף פונקציה שבמסד: ⚠️ ההגדרה נקראת
   *  ב-`pg_get_functiondef` ומוצלבת ל-`DB_SCHEMA`, ⭐ ו-`rename` אינו נוגע בגוף. */
  216: { claims: { test_cron: ['ל2 · כל שם טבלה שבגוף החי'] } },
  /*  ⭐ סבב 148 — ⛔ התקנה טרייה עובדת: ⚠️ המסך הראשון נמדד בדפדפן אמיתי
   *  מול לקוח שמחזיר אפס שורות, ⭐ ואחסון שנוקה. */
  228: { claims: { test_behavior: ['fresh'] } },
  ...
  115: { claims: { test_sistername: ['[sister-comment]', '[sister-ambig]'] } },
  ...
  202: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  72: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  162: { claim: 'בעלות הסכימה המשותפת' },
  176: { claim: '`ya_settings_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  177: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  214: { claim: 'ז. עמודה בלי קורא' },
  ...
  136: { claims: { test_coremod: ['[asset-file]'] } },
  137: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  140: { claim: 'margin' },
  ...
  123: { claims: { test_codescan: ['[fn-product]'] },
  ...
  209: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  19: { claim: '[no-deps]' },
  22: { claims: { test_filesets: 'appGates', 'check-comments': 'מכריז היעדר',
                  test_toolsid: 'tools-drift' } },
  20: { claim: 'טענות על התנהגות' },
   ...
   *  מוטציה שאין בו. */
  36: { claim: 'לכל אחד שינוי, תווית' },
  37: { claim: 'שם הטענה שתיפול' },
  33: { claim: ['כיסוי הטבלה', 'הפניות GATES'] },
  ...
  29: { claims: { 'check-comments': 'ריצת check-js', 'check-capabilities': 'gateKindGaps' } },
  ...
  30: { claim: 'ברירת המחדל אינה המהירה' },
  ...
  113: { claim: 'עברית' },
  116: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  117: { claim: 'BANNER_W' },
  114: { claim: 'מפנה לקובץ' },
  135: { claim: 'android/app/src/main' },
  143: { claim: 'fgDriftMax' },
  144: { claim: 'CANON_MANIFEST' },
  145: { claim: 'שדות זהים' },
  148: { claim: 'BUILD_SHA' },
  152: { claim: 'SHARED_SHA' },
  150: { claim: 'versionCode' },
  131: { claim: 'WebView' },
  132: { claim: 'android:theme' },
  138: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  155: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  159: { claim: 'migrations' },
  168: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  170: { claim: 'א. חותמת בכל רשומה' },
  62: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  163: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  164: { claim: 'ו. טבלה מקבילה' },
  165: { claim: 'ב. חתימת סכימה' },
  166: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  189: { claim: 'type=password' },
  87: { claim: 'aria-label' },
  ...
  200: { claim: '[sec-keyfile]' },
  204: { claims: { test_filesets: ['[oneoff-gate]', '[oneoff-round]', '[oneoff-stale]'] } },
  208: { claims: { test_orphans: 'כל קובץ בעץ מוזכר במקום אחר',
  ...
  217: { claim: 'ט. ערך נגזר שהתיישן' },
  219: { claim: 'ח. הגדרה שהוחלפה' },
  218: { claim: 'ד. רשימת-היתר' },
  221: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  112: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  126: { claim: 'CLEANUP_SHA' },
  127: { claim: 'בלוק ה-APP' },
  185: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
