 ...
 *
 *  **מה נאכף:** ⛔ **היעדר** הנפילה-חזרה למפתח-ערך · משפך קריאה אחד ·
 *  ⛔ וכשל שמחזיר «אין ראיה» ⛔ ולא מערך ריק. ⛔ ולצידם רתמת `vm` שמריצה
 *  את הבלוק האמיתי מול מסד מזויף, ⛔ ומוטציות: כל התנהגות שהופכה חייבת
 *  להפיל טענה.
 *
 *  **הנימוק המדוד:** השקילות אומתה מול המסד — 345 סדרים · 988 סימונים ·
 *  71 תלמידים · 21 סדרי שינה, ⛔ ואפס רשומות שקיימות בערך השלם בלבד.
 *
 *  **מה יישבר בלעדיו:** ⛔ נתיב שיוחזר «ליתר ביטחון» הוא מקור אמת שני,
 *  ⚠️ והוא נכנס בשקט; ⛔ ומיזוג מול מערך ריק מוחק את מה שלא הספיק לעלות.
 *
...
const APP = {
  /*  ⭐ המפתחות שנקראים מטבלאות — ⛔ **אינם נגזרים**: שמות טבלאות בקוד, ⚠️ והסכימה נמדדת בשער אחר · המפתחות שעברו לטבלאות, ואתרי הקריאה שחייבים לעבור דרך המשפך. */
  keys: ['hr_sessions', 'hr_students_rows', 'hr_sleep_sessions'],
  /*  ⭐ נקודת הקריאה האחת — ⛔ **אינה נגזרת**: שם פונקציה ב-`index.html`, ⚠️ ואין קובץ שמצהיר עליה */
  funnel: 'hrCloudGet',
  /*  ⭐ הקריאה הגולמית שאסור לקרוא מחוץ לנקודה — ⛔ **אינה נגזרת**: שם פונקציה ב-`index.html`, ⚠️ ואין קובץ שמצהיר עליה */
  rawGet: 'hrCfgGet',
  /*  ⭐ ריצפת אתרי הקריאה — ⛔ **אינה נגזרת**: מספר מסלולים במוצר, ⚠️ והשער סופר מולה */
  minCallSites: 6,
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
 ...
 *  טענה משותפת שאבדה. */
const FLOOR = { shared: 0, app: 19, appWhy: 'מקור הקריאה מהטבלאות — מספר הטבלאות שהמסך מושך נבדל' };
...
const START = 'שלב ב — מקור הקריאה עובר לטבלאות';
const END = '/* ═══ סוף שכבת השורות';
/*  ⛔ העימוד עבר למודול המשותף — ⚠️ הוא יושב מחוץ לבלוק שנחתך
...
/*  ⛔ ההרכבה משותפת למראה ולענן — ⚠️ היא יושבת מעל הבלוק
...
/*  ⛔ הנפילה-חזרה הוסרה — ⚠️ השקילות אומתה מול המסד, ⭐ ומאותו
...
/*  ⛔ מה שנאכף כאן הוא **היעדר** הנפילה-חזרה — ⚠️ שורה אחת
...
console.log(failed ? `\n✗ מקור הקריאה — ${failed} נכשלו` : '\n✓ מקור הקריאה — כל הטענות עברו');
process.exit(failed ? 1 : 0);
