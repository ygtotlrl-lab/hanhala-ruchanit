...
const APP = {
  ...
  gapRows: [41, 118, 122, 144, 59, 131, 92],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    164: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    163: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    129: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    160: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '160|מבנה טופס הכניסה': 'src',
    '131|שם נגזר אינו נשמר ברשומה': 'src',
    '176|מפתח אחסון בלי קורא': 'src',
    '33|שער מריץ את כל טענותיו': 'tools',
    '86|שכבת המראה': 'src',
    '177|הגירה מקומית שהושלמה': 'mixed',
    '162|מראת המשתמשים': 'src',
    '161|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'tools',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '145|הקשר נלכד בכניסה לפונקציה': 'src',
    '89|פריסה במסכי טלפון וטאבלט': 'src',
    '90|ערכת נושא — בהיר וכהה': 'src',
    '91|אין סגנון מוטבע': 'src',
    '92|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '172|הערה שמתארת מצב שחלף': 'mixed',
    '178|שאילתת `@media` מתה': 'src',
    '105|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '106|ייצוא והנפקה — דרך מוצהרת': 'src',
    '93|המסמך מתפרסר נקי': 'src',
    '94|לוג מסלול — תג ורמה': 'src',
    '75|מיכל המודאל ילד ישיר של `body`': 'src',
    '101|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '147|הגיבוי שלם ומעומד': 'src',
    '71|פעולה מגיבה מיד': 'src',
    '72|משיכה מסוננת בשרת': 'src',
    '117|דגימת-יתר במחולל': 'tools',
    '164|`PBKDF2` — פרמטרים': 'src',
    '135|`migrations/` — תוכן': 'src',
    '136|`migrations/` — כמות': 'src',
    '153|מיזוג מכל — מפה ומערך': 'src',
    '154|ברירת מחדל אינה ממוזגת': 'src',
    '155|רשימה אינה נושאת פריט כפול': 'src',
    '148|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'tools',
    '174|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'tools',
    '168|סודות אינם עוזבים את המכשיר': 'src',
    '142|דפוס `upsert` — onConflict מוכרז': 'src',
    '166|טביעה חסרה מושלמת אוטומטית': 'src',
    '166|כניסה אופליין': 'src',
    '128|עריכת נתונים אופליין': 'src',
    '152|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '167|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '69|אובייקט יקר נבנה פעם אחת': 'src',
    '70|מסך מציג מיד ומרענן ברקע': 'src',
    '121|האפליקציה ניתנת להתקנה מכרום': 'src',
    '170|דגלי מעבר — אין דגל דלוק': 'src',
    '76|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '163|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '41|ארבעת מימדי הסריקה': 'src',
    '85|פינוי אוטומטי': 'src',
    '85|אופק הפינוי נוקה מהסנכרון': 'src',
    '157|אימות פינוי מול הענן': 'src',
    '122|שיתוף קבצים': 'src',
    '107|מעטפת APK (WebView)': 'src',
    '110|אין נכסים מוטבעים': 'src',
    '126|מפתח חתימה קבוע בריפו': 'src',
    '132|מקור אמת יחיד לסכימה': 'src',
    '133|קובץ התקנה מלא': 'src',
    '55|גיבוי יומי אוטומטי': 'src',
    '55|יומן פעולות': 'src',
    '129|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '77|דפוס הודעת שגיאה יחיד': 'src',
    '82|`toast` — חתימה, גוף ומחלקות': 'src',
    '82|סיווג ההודעה נגזר מהמסלול': 'src',
    '184|מיכל באנר העדכון במקור': 'src',
    '184|הרענון מ-controllerchange בלבד': 'src',
    '184|סימן דחיית הבאנר מתמיד': 'src',
    '102|רישום כשלי כתיבה': 'src',
    '56|חלון חם במכשיר': 'src',
    '56|שחזור מקומי מהענן': 'src',
    '160|מסך שינוי סיסמה עצמי': 'src',
    '186|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '57|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '166|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '58|מודול מזהי רשומות': 'src',
    '58|מזהה אינו מומר למספר': 'src',
    '60|מזהה מכשיר': 'src',
    '57|ליבת מיזוג משותפת': 'src',
    '107|מעטפת WebView חתומה': 'tools',
    '166|אימות מול טביעה בענן': 'mixed',
    '124|בניית APK אחידה עם שער חתימה': 'src',
    '186|מודול ה-service worker': 'src',
    '61|ניסיון חוזר בתור הסנכרון': 'src',
    '62|מנגנון משיכה אחיד': 'src',
    '65|נעילת חוסר-פעילות': 'src',
    '66|מודל הסשן — בזיכרון בלבד': 'src',
    '185|בדיקת עדכון תקופתית ל-service worker': 'src',
    '87|פסק זמן אחיד לקריאות רשת': 'src',
    '88|מאזיני מצב רשת': 'src',
    '151|גריעת tombstones לפי גיל': 'src',
    '184|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '122|גשר שיתוף': 'src',
    '67|העברת מזהה ל-DOM': 'src',
    '68|מיון אחד לכל התצוגות': 'src',
    '73|ייצוא — שני דפוסים מוצהרים': 'src',
    '114|שכבת אייקונים': 'src',
    '158|שכבת קלט אחידה': 'src',
    '158|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '74|שכבת המודאל': 'src',
    '160|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [29, 40, 42, 44, 53, 84, 47, 156, 98, 138, 54, 22, 59, 25, 32, 35];
...
const CAPS = {
  ...
  toastcss: {
    ...
    block: { sha: 'dbbde8473c2551ba', lines: 14,
             css: true,
  ...
  bchartcss: {
    ...
    block: { sha: 'f41843f09b261d27', lines: 18,
             css: true,
...
const BLOCK_ORDER = ['toastcss', 'bchartcss', 'bp', 'neterr', 'rowswin', 'guardonline', 'storage', 'schemastale', 'busyguard', 'savepipe', 'swreg', 'backup',
...
const FLOOR = { shared: 81, app: 10, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי ושער מצב הרשת' };
/* ⚠️ סוף פר-אפליקציה */
...
function inlineStyleSites() {
  return [...src.matchAll(STYLE_ATTR)].map((m) => ({ at: m.index, val: m[1] }));
}
function inlineColorGaps() {
  const out = [];
  const allow = APP.inlineColorAllow || {};
  const zones = [];
  for (const n of Object.keys(allow)) {
    const r = fnRange(n);
    if (!r) { out.push(`אזור מוכרז שאין לו פונקציה: ${n} ⛔ — מסירים אותו מ-APP.inlineColorAllow`); continue; }
    if (String(allow[n]).trim().split(/\s+/).length < 4)
      out.push(`אזור מוכרז בלי נימוק: ${n} ⛔ — מוסיפים את הסיבה שהצבעים שם קבועים`);
    zones.push([n, r]);
  }
  const seen = new Set();
  ...
  for (const [n] of zones)
    if (!seen.has(n)) out.push(`אזור מוכרז שאין בו ליטרל: ${n} ⛔ — מסירים אותו מ-APP.inlineColorAllow`);
  return out;
}
...
const MATRIX = [
  ...
  { row: 131, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 145, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 172, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 101, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 147, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 144, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 117, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 164, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 135, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 136, name: '`migrations/` — כמות',
  ...
  { row: 153, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 154, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 155, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 168, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 142, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 166, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 166, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 128, name: 'עריכת נתונים אופליין',
  ...
  { row: 152, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 167, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 121, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 170, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 163, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 157, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 122, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 107, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 110, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 126, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 132, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 133, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 55, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 55, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 129, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 102, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 56, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 56, name: 'שחזור מקומי מהענן',
    probe: () => horizonReleaseWired() },
  { row: 160, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 160, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 186, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 146, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 166, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 107, name: 'מעטפת WebView חתומה',
  ...
  { row: 166, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 124, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 186, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 185, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 151, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 184, name: 'מיכל באנר העדכון במקור',
    probe: () => /<div id="updater">/.test(src) &&
                 !/\.id\s*=\s*['"]updater['"]/.test(code) &&
                 !/createElement\([^)]*\)[^;]{0,80}updater/.test(code) },
  { row: 184, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 184, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 184, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 114, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 158, name: 'שכבת קלט אחידה',
  ...
  { row: 158, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  137: { claim: 'בעלות הסכימה המשותפת' },
  149: { claim: '`kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  150: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  179: { claim: 'ז. עמודה בלי קורא' },
  ...
  112: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  115: { claim: 'margin' },
  ...
  100: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  96: { claim: 'עברית' },
  98: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  99: { claim: 'BANNER_W' },
  175: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  97: { claim: 'מפנה לקובץ' },
  111: { claim: 'android/app/src/main' },
  118: { claim: 'fgDriftMax' },
  119: { claim: 'CANON_MANIFEST' },
  120: { claim: 'שדות זהים' },
  123: { claim: 'BUILD_SHA' },
  127: { claim: 'SHARED_SHA' },
  125: { claim: 'versionCode' },
  108: { claim: 'WebView' },
  109: { claim: 'android:theme' },
  113: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  130: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  134: { claim: 'migrations' },
  141: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  143: { claim: 'א. חותמת בכל רשומה' },
  54: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  138: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  139: { claim: 'ו. טבלה מקבילה' },
  140: { claim: 'ב. חתימת סכימה' },
  159: { claim: 'type=password' },
  78: { claim: 'aria-label' },
  ...
  165: { claim: 'pass_salt' },
  169: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  171: { claim: '⏳' },
  49: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  173: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  180: { claim: 'ט. ערך נגזר שהתיישן' },
  182: { claim: 'ח. הגדרה שהוחלפה' },
  181: { claim: 'ד. רשימת-היתר' },
  183: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  95: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  103: { claim: 'CLEANUP_SHA' },
  104: { claim: 'בלוק ה-APP' },
  156: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
