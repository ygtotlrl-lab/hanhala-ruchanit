...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    110: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    109:  (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    92: (c) => c.hasCode(/YS_KV_LEGACY_WRITE\s*=\s*false/),
    ...
    108: (c) => !!c.fnBody('changeMyPassword')
...
const MATRIX = [
  ...
  { row: 114, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 112, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 116, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 128, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 128, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 127, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 126, name: 'אוטו-אפדייט מ-raw.githubusercontent',
...
const GATES = {
  ...
  115: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  117: { claim: '⏳' },
  118: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  124: { manual: 'קיום טבלה או מפתח במסד אינו נראה מהריפו' },
  122: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  119: { manual: '«קובץ בלי קורא» דורש סריקה על ארבעת הריפו — נעשה ידנית' },
  125: { manual: 'קיום מפתח במסד אינו נראה מהריפו' },
  121: { manual: 'רשימת-היתר הגיבויים יושבת במיגרציה שכבר רצה' },
  123: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
