#!/usr/bin/env node
/*  test_pull.mjs — מנגנון המשיכה המאוחד.
 *
 *  **מה נאכף:** הפולינג קורא שורת חותמת אחת ומושך את הנתונים **רק כשהיא
 *  התקדמה**. ⛔ ואין פולינג שני שמסנכרן — ⚠️ `setInterval` מחוץ למודול
 *  שקורא לפונקציית הסנכרון הוא בדיוק הדחיפה התקופתית העיוורת שהוסרה.
 *
 *  **הנימוק המדוד:** שתי אפליקציות עבדו כך, ⛔ אחת משכה ארבע טבלאות
 *  ב-`select('*')` **בכל תקתוק**, ⛔ ואחת משכה משיכה מלאה כל עשרים שניות —
 *  ⚠️ אסימטריה שאיש לא החליט עליה.
 *
 *  **מה יישבר בלעדיו:** ⛔ משיכה מלאה בכל תקתוק היא עומס שגדל עם הנתונים
 *  ⚠️ ואינו נראה עד שהוא כבר גדול, ⭐ ומסלול שני שמכריע מתי למשוך הוא
 *  מקור אמת שני.
 *
 *  **מה אינו נאכף כאן:** ⛔ תוכן המשיכה עצמה — ⚠️ מה נמשך ולאן הוא ממוזג
 *  נאכף בשערי המיזוג, ⭐ וכאן נמדד **מתי** בלבד.
 *
 *  ⚠️ הרתמה מריצה את הליבה האמיתית ב-`vm` עם קונפיג ושעון מזויפים —
 *  ⛔ ואינה בודקת ביטויים רגולריים על הטקסט. ⛔ המוטציות אינן נכתבות לעץ.
...
const START = '/* ═══ מנגנון המשיכה — מודול משותף';
...
/*  ⛔ כל מסלול כתיבה לענן מקדם את החותמת — אחרת השינוי נשאר
...
mutStage();
if (!RUN_MUT) {
  console.log('\n⏭ test_pull: המוטציות רצות ברמה המלאה (--full) — ⛔ ואינן נמדדות כאן');
