...
const APP = {
  ...
  calendar: { kind: 'hebrew',
    why: 'הנוכחות והשינה נרשמות ליום עברי — ⚠️ ושנת הלימודים מתגלגלת באלול, ⭐ מפני שהבוגרים עוזבים בסופו' },
  ...
  kvTable: { names: ['hr_settings'],
    why: 'הטבלה שההגדרות נקראות ונכתבות אליה — ⭐ מוסד אחד, ⛔ ולכן שם אחד' },
  ...
  coreVerbs: {
    monthKeyOf: { name: null,
      why: 'מפתח החודש של רשומה — ⛔ וכאן הרשומה היא יום: ⚠️ הנוכחות והשינה נרשמות לתאריך מלא, ⭐ ואין מפתח חודש שנתונים מקובצים לפיו' },
    monthLabel: { name: null,
      why: 'תווית החודש לתצוגה — ⛔ ואין כאן מסך שמוצג לחודש: ⚠️ המסכים הם יום, תלמיד ושיעור' },
    monthsWithData: { name: null,
      why: 'החודשים שיש בהם רשומות — ⛔ ואין כאן ניווט בין חודשים: ⚠️ הבחירה היא תאריך בשדה, ⭐ והשאילתה מסננת טווח' },
    yearOf: { name: null,
      why: 'השנה של רשומה — ⛔ וכאן היא שנת לימודים ולא שנת לוח: ⚠️ `hrSchoolYear` מתגלגלת באלול, ⭐ ושם שיאמר «השנה» היה מטעה' },
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
 ...
 *  רשימה שנייה בבודק. */
export const ROWS = [46, 117, 118, 121];
