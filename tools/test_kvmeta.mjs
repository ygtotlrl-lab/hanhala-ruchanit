...
const APP = {
  ...
  kvMeta: { table: 'hr_settings_meta', sites: 8 },
  ...
  mig: {
    ...
    grant: 'kv',
    /*  ⛔ השם שהמיגרציה רושמת כמקרה שאינו שלה — ⚠️ והוא בשמו שבשעת
     *  הריצה: ⭐ מיגרציה שכבר רצה אינה נערכת, ⛔ והשם החי היום הוא `ya_`. */
    note: 'tb_subs_meta',
    why: '',
  },
};
...
/*  ⛔ מונה ולא נוכחות — ⚠️ בדיקת נוכחות עוברת גם על הצהרה כפולה
...
/*  ⛔ נמדד על המקור הגולמי — ⚠️ שם הטבלה חי כליטרל מחרוזת,
...
const KEY = 'k';
/*  ⛔ שני מסלולי חותמת, ושניהם מוצהרים — ⚠️ **`fetch-key`**:
...
if (APP.mig.file) {
  ...
  /*  ⛔ הלקח — ⚠️ פונקציה חדשה נולדת נגישה כ-RPC לכל מי שמחזיק
...
/* ── מוטציות ───────────────────────────────────────────────────────────── */
console.log('  — מוטציות —');
/*  ⛔ המוטציות אינן נכתבות לעץ — ⚠️ מוטציה שנכתבת לעץ
