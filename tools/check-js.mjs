#!/usr/bin/env node
/*  שער תקינות JS — חובה לפני כל דחיפה (סבב 33: אוחד לכל הריפו).
 *
 ...
 *  כך — הפער שהוליד את השער הזה. מסבב 33 הוא זהה בית-לבית בכל
 *  הריפו פרט לבלוק APP.
 *
...
const BUDGET_EXEMPT = {
  ...
  /*  ⛔ שער שכל גופו מוטציות (סבב 139) — ⚠️ 18 הזזות ומוטציית-נגד, ⭐ וכל
  ...
  'test_crossgate.mjs': 'משווה שערים זה מול זה, ⛔ וכל מוטציה מריצה ' +
                        ...
                        'נמדד ומודפס בכל הרצה',
  /*  ⛔ החריגה הזו אינה אחידה בכולם — ⚠️ הזמן תלוי במספר שורות
...
/*  ⛔⛔ המספר ירד מההערה (סבב 109) — ⚠️ הוא נגזר מ-`BUDGET_EXEMPT` ממש
 ...
 *  צריך, ⛔ והמספר היה רק מונה שנסחף. */
{
  ...
  /*  ⛔ וכל נימוק מבחין בין הרמות (סבב 92) — ⚠️ כולם חורגים ברמה המלאה
...
function runGate(gate) {
  return new Promise((resolve) => {
    ...
    /*  ⛔ טווח הריצפה נמסר לשער המוכרז בלבד (סבב 118) — ⚠️ שער שמספר
...
/*  ⛔ ההצלבה מול הזמנים שנמדדו (סבב 140) — ⚠️ **מה נכנס**: `BUDGET_EXEMPT`
