...
const APP = {
  ...
  appFns: {
    ...
    _lkActive:
      'בודקת אם שעון חוסר-הפעילות פעיל בהקשר הנוכחי — וביומן אין כניסה ואין סשן שיש מה לנעול',
    _lkHide:
      'מסתירה את אזהרת הנעילה כשהמשתמש חזר לפעול — וביומן אין כניסה ואין אזהרה שתרד',
    _lkShow:
      'מציגה את אזהרת הנעילה כשנותרה דקה של חוסר-פעילות — וביומן אין כניסה ואין מה לנעול',
    _lkStyle:
      'מזריקה פעם אחת את גיליון הסגנון של שכבת האזהרה שלפני הנעילה — וביומן אין כניסה ואין שכבה שתוצג',
    _lkWarnEl:
      'בונה ומחזירה את מיכל אזהרת הנעילה ב-DOM — וביומן אין כניסה ואין אזהרה שתיבנה',
    _sessLegacy:
      'מחזירה את מפתחות הסשן הישנים שיש לפנות מהדיסק — וביומן אין כניסה ואין מפתח סשן שנכתב אי פעם',
    _sessPurge:
      'מוחקת את מפתחות הסשן הישנים ורושמת ביומן כמה נמחקו — וביומן אין כניסה ואין מפתח שיימחק',
    _writeUserSend:
      'שולחת את גוף המשתמש לענן ב-upsert או ב-update לפי קיום המזהה — וביומן אין טבלת משתמשים ואין שורה שתישלח',
    _hebBadDate:
    ...
    doLogin:
      'מאמתת שם וסיסמה מול הענן ומכניסה לאפליקציה — וביומן אין כניסה ואין משתמש שיאומת',
    doLogout:
    ...
    isAdminOf:
      'מכריעה אם שורת משתמש שנמסרה נושאת את דרגת המנהל — וביומן אין טבלת משתמשים ואין שורה שתיבדק',
    lkBoot:
      'מחווטת את מדידת חוסר-הפעילות ומתחילה את השעון בעלייה — וביומן אין כניסה ואין שעון שיתחיל',
    lkReset:
      'מאפסת את שעון חוסר-הפעילות בכל מגע של המשתמש — וביומן אין כניסה ואין שעון שיאופס',
    lkShown:
      'מחזירה אם אזהרת הנעילה מוצגת כרגע — וביומן אין כניסה ואין אזהרה שתיבדק',
    lkStop:
      'עוצרת את שעון חוסר-הפעילות בניתוק — וביומן אין כניסה ואין ניתוק שיעצור אותו',
    mirrorBoot:
      'נקודת ההפעלה האחת של שכבת המראה בעליית האפליקציה — וביומן אין שכבת מראה, והקריאה המקומית שם היא מפתח-ערך פר-מפתח',
    mirrorKey:
      'גוזרת את מפתח האחסון של טבלה בשכבת המראה משם הטבלה — וביומן אין שכבת מראה, והקריאה המקומית שם היא מפתח-ערך פר-מפתח',
    mirrorLoad:
      'טוענת את כל טבלאות המראה מהדיסק בעלייה — וביומן אין שכבת מראה, והקריאה המקומית שם היא מפתח-ערך פר-מפתח',
    mirrorLoadOne:
      'טוענת טבלה אחת מהדיסק אל שכבת המראה — וביומן אין שכבת מראה, והקריאה המקומית שם היא מפתח-ערך פר-מפתח',
    mirrorSave:
      'כותבת טבלה מהזיכרון לדיסק דרך שער החלון החם — וביומן אין שכבת מראה, והקריאה המקומית שם היא מפתח-ערך פר-מפתח',
    mirrorTables:
      'מחזירה את רשימת הטבלאות שיש להן מפתח בשכבת המראה — וביומן אין שכבת מראה, והקריאה המקומית שם היא מפתח-ערך פר-מפתח',
    mirrorWrite:
      'כותבת שורות שהתקבלו ישירות לדיסק בלי לגעת בזיכרון — וביומן אין שכבת מראה, והקריאה המקומית שם היא מפתח-ערך פר-מפתח',
    openAddStudent:
      'פותחת את טופס הוספת התלמיד — וביומן ובגיוס אין תלמידים, אלא רשומות יום ותורמים',
    renderSettings:
      'חושפת את ניהול המשתמשים ואת ההרשאות במסך ההגדרות לפי הדרגה — ובשלוש האחרות אין תת-מסך הגדרות שנחשף לפי דרגה',
    saveUser:
      'שומרת שורת משתמש ממסך ניהול המשתמשים וגוזרת לה טביעה — ובשכר אין מסך ניהול משתמשים, ומשתמש נוצר במסד בידי המנהל',
    sessActive:
      'מחזירה אם יש משתמש מחובר כרגע — וביומן אין כניסה ואין סשן שייבדק',
    sessBoot:
      'מריצה פעם אחת בעלייה את פינוי שרידי הסשן מהדיסק — וביומן אין כניסה ואין שריד סשן שיפונה',
    sessClear:
      'מנקה את המשתמש המחובר מהזיכרון בניתוק — וביומן אין כניסה ואין מה לנקות',
    sessGet:
      'מחזירה את המשתמש המחובר מהזיכרון — וביומן אין כניסה ואין משתמש שיוחזר',
    sessSet:
      'מציבה את המשתמש המחובר בזיכרון בלבד — וביומן אין כניסה ואין משתמש מחובר',
    sessState:
      'מחזירה את מצב מודול הסשן לאבחון, בלי אובייקט המשתמש עצמו — וביומן אין כניסה ואין מודול שמצבו יידווח',
    writeUser:
  ...
  productFns: {
    ...
    saveUser:
      'שומרת שורת משתמש דרך הכותב האחד — ⚠️ והשדות שנשמרים נבדלים: תפקיד והרשאות פר-עמוד בהנהלה, ותפקיד ודגל פעיל בגיוס',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
 ...
 *  רשימה שנייה בבודק. */
export const ROWS = [62, 126, 192, 213];
