/* ══════════════════════════════════════════════════════════════════════════
   ...
   ⚠️⚠️ **ומה שהשער הזה אינו מודד:** הוא קורא **קבצים**, ⛔ ואינו רואה את
      המסד החי. מיגרציה שנכתבה ולא הורצה עוברת אותו במלואו — וזו בדיוק
      הסיבה שהדגל כבוי (כלל ברזל 20, אותה מגבלה).
   ══════════════════════════════════════════════════════════════════════════ */
...
ok(/var\s+YS_KV_UPDATED_AT\s*=\s*false\s*;/.test(SRC),
   '1 · ⛔ `YS_KV_UPDATED_AT` מוגדר ו**כבוי** — המיגרציה טרם הורצה');
...
/* 2 — דגל דלוק בעץ. ⛔ זו הרגרסיה המסוכנת: הדלקה לפני שהמיגרציה רצה. */
ok(!/var\s+YS_KV_UPDATED_AT\s*=\s*true\s*;/.test(SRC),
   '18 · ⛔ מוטציה-נגד: הדגל אינו דלוק בעץ — הדלקה לפני הרצת המיגרציה מחזירה את הבאג');
