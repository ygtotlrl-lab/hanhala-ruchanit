...
const APP = {
  ...
  actMap: 'DOM_ACTIONS',
  gapRows: [99, 103, 119, 120, 121, 125, 126, 129, 135],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    139: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    138: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    110: (c) => !/ysCfgSet\('(?:ys_students|ys_attend_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    137: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [36, 40, 41, 42, 39, 50, 72, 45, 130];
...
const MATRIX = [
  ...
  { row: 123, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 120, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
   ...
   *  אחד. */
  { row: 121, name: 'הקשר נלכד בכניסה לפונקציה', app: true },
  ...
  { row: 98, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 139, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 115, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 116, name: '`migrations/` — כמות',
    probe: () => migLineGaps().length === 0 },
  { row: 128, name: 'דפוס עמודות אחיד',
  ...
  { row: 124, name: '`kv_backup` — טבלת הגיבוי',
  ...
  { row: 125, name: '`kv_rishon` / `kv_ramataviv` — הגדרות היומן', app: true },
  { row: 28, name: 'שער אינו מקליד מספר שורה',
    probe: () => typedRowSites().length === 0 },
  { row: 149, name: 'פונקציה בלי קוראים',
  ...
  { row: 143, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 118, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 141, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 141, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 109, name: 'עריכת נתונים אופליין',
  ...
  { row: 127, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 142, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 102, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 145, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 138, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 131, name: 'אימות פינוי מול הענן',
    probe: () => /\bverify\s*:/.test(policyBlock()) },
  { row: 103, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 88, name: 'מעטפת APK (WebView)',
  ...
  { row: 136, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  ...
  { row: 137, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 156, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 122, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 141, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 88, name: 'מעטפת WebView חתומה',
  ...
  { row: 141, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 105, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 156, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 155, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 126, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 154, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 95, name: 'שכבת אייקונים',
    probe: () => iconAudit('.').length === 0 },
  { row: 132, name: 'שכבת קלט אחידה',
...
const GATES = {
  ...
  93: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  96: { claim: 'margin' },
  ...
  84: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  81: { claim: 'BANNER_W' },
  150: { claim: 'CSS מתות' },
  82: { manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ והמונה נמדד ידנית' },
  83: { claim: 'מפנה לקובץ' },
  92: { claim: 'android/app/src/main' },
  99: { claim: 'fgDriftMax' },
  100: { claim: 'CANON_MANIFEST' },
  101: { claim: 'שדות זהים' },
  104: { claim: 'BUILD_SHA' },
  108: { claim: 'SHARED_SHA' },
  106: { claim: 'versionCode' },
  89: { claim: 'WebView' },
  90: { claim: 'android:theme' },
  94: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  111: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  114: { claim: 'migrations' },
  117: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  119: { manual: 'קיום רשומה בלי חותמת יושב במסד ואינו נראה מהריפו — ⛔ המדידה היא פעולת מנהל' },
  129: { manual: 'חתימת הסכימה החיה נגזרת מ-`information_schema` ואינה נראית מהריפו — ⛔ ההשוואה היא פעולת מנהל' },
  133: { claim: 'type=password' },
  134: { claim: 'aria-label' },
  135: { manual: 'מנוע התאריך — ⛔ נאכף ב-`test_date` ביומן ובהנהלה, ⚠️ ובשכר ובגיוס אין צרכן תאריך עברי ואין שער' },
  140: { claim: 'pass_salt' },
  144: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  146: { claim: '⏳' },
  147: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  152: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  148: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  151: { manual: 'רשימת-היתר הגיבויים יושבת במיגרציה שכבר רצה' },
  153: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  86: { claim: 'CLEANUP_SHA' },
  87: { claim: 'בלוק ה-APP' },
  130: { claim: 'cfgKeyGaps' },
};
 ...
 *  ⛔ **ומה שנשאר כאן הוא שורה שמייצגת יותר מכלל אחד** (סבב 90) — ⚠️ שבעה
 *  זוגות יושרו לשם אחד ⛔ ושישה אינם יכולים: ⭐ שלוש שורות מייצגות שני
 *  כללים כל אחת, ⛔ ושני סעיפים אינם יכולים לשאת כותרת אחת. */
