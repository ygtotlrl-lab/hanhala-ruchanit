...
const APP = {
  ...
  gapRows: [44, 121, 125, 147, 59, 134, 95],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    167: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    166: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    132: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    163: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '163|מבנה טופס הכניסה': 'src',
    '134|שם נגזר אינו נשמר ברשומה': 'src',
    '180|מפתח אחסון בלי קורא': 'src',
    '36|שער מריץ את כל טענותיו': 'tools',
    '87|שכבת המראה': 'src',
    '181|הגירה מקומית שהושלמה': 'mixed',
    '165|מראת המשתמשים': 'src',
    '164|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'tools',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '148|הקשר נלכד בכניסה לפונקציה': 'src',
    '90|פריסה במסכי טלפון וטאבלט': 'src',
    '91|ערכת נושא — בהיר וכהה': 'src',
    '92|סולם אחד לגודל, לריווח ולרדיוס': 'src',
    '93|אין סגנון מוטבע': 'src',
    '95|גרף נבנה מ-CSS ולא מספרייה': 'src',
    '175|הערה שמתארת מצב שחלף': 'mixed',
    '182|שאילתת `@media` מתה': 'src',
    '108|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '109|ייצוא והנפקה — דרך מוצהרת': 'src',
    '96|המסמך מתפרסר נקי': 'src',
    '97|לוג מסלול — תג ורמה': 'src',
    '75|מיכל המודאל ילד ישיר של `body`': 'src',
    '104|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '150|הגיבוי שלם ומעומד': 'src',
    '71|פעולה מגיבה מיד': 'src',
    '72|משיכה מסוננת בשרת': 'src',
    '120|דגימת-יתר במחולל': 'tools',
    '167|`PBKDF2` — פרמטרים': 'src',
    '138|`migrations/` — תוכן': 'src',
    '139|`migrations/` — כמות': 'src',
    '156|מיזוג מכל — מפה ומערך': 'src',
    '157|ברירת מחדל אינה ממוזגת': 'src',
    '158|רשימה אינה נושאת פריט כפול': 'src',
    '151|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'tools',
    '178|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'tools',
    '171|סודות אינם עוזבים את המכשיר': 'src',
    '145|דפוס `upsert` — onConflict מוכרז': 'src',
    '169|טביעה חסרה מושלמת אוטומטית': 'src',
    '169|כניסה אופליין': 'src',
    '131|עריכת נתונים אופליין': 'src',
    '155|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '170|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '69|אובייקט יקר נבנה פעם אחת': 'src',
    '70|מסך מציג מיד ומרענן ברקע': 'src',
    '124|האפליקציה ניתנת להתקנה מכרום': 'src',
    '173|דגלי מעבר — אין דגל דלוק': 'src',
    '76|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '166|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '44|ארבעת מימדי הסריקה': 'src',
    '86|פינוי אוטומטי': 'src',
    '86|אופק הפינוי נוקה מהסנכרון': 'src',
    '160|אימות פינוי מול הענן': 'src',
    '125|שיתוף קבצים': 'src',
    '110|מעטפת APK (WebView)': 'src',
    '113|אין נכסים מוטבעים': 'src',
    '129|מפתח חתימה קבוע בריפו': 'src',
    '135|מקור אמת יחיד לסכימה': 'src',
    '136|קובץ התקנה מלא': 'src',
    '55|גיבוי יומי אוטומטי': 'src',
    '55|יומן פעולות': 'src',
    '132|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '77|דפוס הודעת שגיאה יחיד': 'src',
    '82|`toast` — חתימה, גוף ומחלקות': 'src',
    '82|סיווג ההודעה נגזר מהמסלול': 'src',
    '188|מיכל באנר העדכון במקור': 'src',
    '188|הרענון מ-controllerchange בלבד': 'src',
    '188|סימן דחיית הבאנר מתמיד': 'src',
    '105|רישום כשלי כתיבה': 'src',
    '56|חלון חם במכשיר': 'src',
    '163|מסך שינוי סיסמה עצמי': 'src',
    '190|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '57|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '169|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '58|מודול מזהי רשומות': 'src',
    '58|מזהה אינו מומר למספר': 'src',
    '60|מזהה מכשיר': 'src',
    '57|ליבת מיזוג משותפת': 'src',
    '110|מעטפת WebView חתומה': 'tools',
    '169|אימות מול טביעה בענן': 'mixed',
    '127|בניית APK אחידה עם שער חתימה': 'src',
    '190|מודול ה-service worker': 'src',
    '61|ניסיון חוזר בתור הסנכרון': 'src',
    '62|מנגנון משיכה אחיד': 'src',
    '65|נעילת חוסר-פעילות': 'src',
    '66|מודל הסשן — בזיכרון בלבד': 'src',
    '189|בדיקת עדכון תקופתית ל-service worker': 'src',
    '88|פסק זמן אחיד לקריאות רשת': 'src',
    '89|מאזיני מצב רשת': 'src',
    '154|גריעת tombstones לפי גיל': 'src',
    '188|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '125|גשר שיתוף': 'src',
    '67|העברת מזהה ל-DOM': 'src',
    '68|מיון אחד לכל התצוגות': 'src',
    '73|ייצוא — שני דפוסים מוצהרים': 'src',
    '117|שכבת אייקונים': 'src',
    '161|שכבת קלט אחידה': 'src',
    '161|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '74|שכבת המודאל': 'src',
    '163|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [29, 43, 45, 47, 52, 85, 50, 159, 101, 141, 54, 22, 59, 25, 34, 38];
...
const CAPS = {
  ...
  toastcss: {
    ...
    block: { sha: '3b81925e449d4775', lines: 14,
             css: true,
  ...
  scales: {
    ...
    block: { sha: 'd8c760b47d30822a', lines: 47,
             css: true,
  ...
  storage: {
    ...
    block: { sha: 'ee5164516bf09598', lines: 609,
  ...
  pending: {
    ...
    block: { sha: '1acb827262024e03', lines: 285,
  ...
  lock: {
    ...
    block: { sha: '71ef636cef8143ab', lines: 109,
  ...
  uihelp: {
    ...
    block: { sha: '9173f3f90c2bc9c2', lines: 123,
...
/*  ⛔ אות קטגוריה שירדה — ⚠️ **מה נכנס**: האות, ⛔ והנימוק שמסביר למה
 *  הקטגוריה אינה בטבלה; ⛔ **ומה מפיל**: אות שירדה ועדיין יש לה כותרת,
 *  ואות מוכרזת בלי נימוק. ⭐ **ולמה המבנה קיים**: האותיות **אינן מוסטות**
 *  כשקטגוריה יורדת — ⚠️ הסטה הייתה שוברת כל הפניה לקטגוריה, ⛔ ובלי
 *  המרשם הרצף היה מדווח שגיאה על מצב מכוון. */
const CAT_GAP = {
  'ד': 'כללי העבודה אינם בטבלה — ⛔ אין להם תוצר בריפו שאפשר למדוד בו אם צייתו',
};
...
const CAT_ORDER = [
  ...
  { g: 'ה', phases: [{ p: 'הכלל', w: ['חתימות sha256', 'כל פונקציה —'] },
 ...
 *  אחת בארבעתן, ⛔ ובלי כלל משלו הוא יורש את מה שבמקרה נמצא מעליו.
...
/*  ⛔ מה נכנס לרשימה: אסימון שקיים בארבעת הריפו ⛔ ולא אסימון מוצר —
 ...
 *  פטורה מ«הוגדר ואין לו קורא»**: מושג המצב חי בארבעתן — ⚠️ הטוסט צובע
...
/*  ⛔ נימוק שנוקב בנוכחות בלבד (סבב 140) — ⚠️ «אינו בארבעתן» הוא
...
const PRESENCE_ONLY = ['ואינו בארבעתן', 'ואינה בארבעתן', 'ואינם בארבעתן',
  ...
  for (const n of here) {
    ...
    if (!why)
      out.push(`אסימון שאינו באוצר המילים ואינו מוצהר: ${n} ⛔ — מיישרים אותו ` +
               `לשם שקיים בארבעתן, או מכריזים ב-APP.appTokens עם נימוקו`);
...
const MATRIX = [
  ...
  { row: 134, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 148, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 90, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 92, name: 'סולם אחד לגודל, לריווח ולרדיוס',
  ...
  { row: 95, name: 'גרף נבנה מ-CSS ולא מספרייה',
    probe: () => chartGaps().length === 0 },
  { row: 91, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 175, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 104, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 150, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 147, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 120, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 167, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 138, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 139, name: '`migrations/` — כמות',
  ...
  { row: 156, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 157, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 158, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 171, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 145, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 169, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 169, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 131, name: 'עריכת נתונים אופליין',
  ...
  { row: 155, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 170, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 124, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 173, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 166, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 44, name: 'ארבעת מימדי הסריקה',
    probe: () => {
  ...
  { row: 86, name: 'אופק הפינוי נוקה מהסנכרון',
  ...
  { row: 160, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 125, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 110, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 113, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 129, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 135, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 136, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 55, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 55, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 132, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 105, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 56, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 163, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 163, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 190, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 149, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 169, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 110, name: 'מעטפת WebView חתומה',
  ...
  { row: 169, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 127, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 190, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 189, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 36, name: 'שער מריץ את כל טענותיו',
    probe: () => gateSealGaps().length === 0 },
  { row: 180, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 87, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 181, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 88, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 154, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 188, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 188, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 188, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 188, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 117, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 161, name: 'שכבת קלט אחידה',
  ...
  { row: 161, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  31: { claim: 'מסלול «' },
  51: { claim: 'א · אין שתי שורות על אותו probe' },
  35: { claim: 'א · כל דפוס מוכרז נושא מוטציה' },
  30: { claim: 'א · כל מפתח נקרא' },
  /*  ⭐ סבב 141 — ⛔ שם פונקציה חלקי: ⚠️ השער מודד את שני הצדדים,
   *  ⭐ ואת טיב הנימוק — ⛔ הצהרה שהיא נוכחות בלבד נופלת בו. */
  53: { claim: 'fn-undeclared' },
  ...
  140: { claim: 'בעלות הסכימה המשותפת' },
  152: { claim: '`kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  153: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  183: { claim: 'ז. עמודה בלי קורא' },
  83: { claim: 'ליטרל עברי באתר הודעה' },
  ...
  13: { claim: 'CANON_MD' },
  49: { claim: 'תוכן הקבצים הנלווים' },
  115: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  118: { claim: 'margin' },
  39: { manual: 'השוואת זמנים בין הריפו אינה בהישג ידו של שער שרץ בריפו אחד — ⛔ נאכפת בתוצאתה בלבד' },
  50: { claim: 'אין פרק פערים נפרד' },
  103: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  177: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
   ...
   *  מוטציה שאין בו. */
  32: { claim: 'לכל אחד שינוי, תווית' },
  33: { claim: 'שם הטענה שתיפול' },
  29: { claim: 'כיסוי הטבלה' },
  42: { claim: 'ועמודת התקן שלה אוסרת במפורש' },
  43: { claim: ['מספור רציף ובלי כפילויות', 'nameSignGaps', 'catOrderGaps'] },
  45: { claim: 'הערה ריקה' },
  47: { claim: 'COUNT_NOTE' },
  48: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  191: { claim: 'CACHE_NAME' },
  41: { manual: 'מספר בדיווח הוא התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו את הדיווח, ⚠️ ונאכף בתוצאתו בלבד' },
  ...
  34: { claims: { 'check-comments': 'תווית מוטציה מודפסת',
                  'check-capabilities': ['regexBoundGaps', 'anchorGaps'],
                  'test_anchors': 'מ1' } },
  19: { claim: 'תקן הבאנר' },
  16: { claim: 'דפוס הבודקים' },
  94: { claims: { test_visual: ['ליטרל צבע בגיליון', 'ערך שאינו דרגה בסולם', 'ערך מספרי שאינו נגזר'] } },
  40: { claim: 'מספרי הבאנר' },
  ...
  38: { claims: { 'check-js': 'תקציב זמן לסט',
                  'check-capabilities': 'gateSizeGaps' } },
  26: { claim: 'ברירת המחדל אינה המהירה' },
  52: { claims: { 'check-capabilities': ['sha:', 'orderGaps', 'blockRowGaps'],
                  test_sharedsync: ['block-drift', 'canon-drift'],
                  test_signedshared: 'unsignedTwins',
                  test_toolsid: ['pure-tools', 'pure-undeclared'] } },
  85: { claim: 'storage' },
  84: { claim: 'ג · פעולה שדורשת רשת' },
  99: { claim: 'עברית' },
  101: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  102: { claim: 'BANNER_W' },
  179: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
                 'אין מחלקה שנוספת ל-DOM בלי כלל CSS'] },
  46: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  100: { claim: 'מפנה לקובץ' },
  114: { claim: 'android/app/src/main' },
  121: { claim: 'fgDriftMax' },
  122: { claim: 'CANON_MANIFEST' },
  123: { claim: 'שדות זהים' },
  126: { claim: 'BUILD_SHA' },
  130: { claim: 'SHARED_SHA' },
  128: { claim: 'versionCode' },
  111: { claim: 'WebView' },
  112: { claim: 'android:theme' },
  116: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  133: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  137: { claim: 'migrations' },
  144: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  146: { claim: 'א. חותמת בכל רשומה' },
  ...
  143: { claim: ['ב. חתימת סכימה', 'יג. כל שאילתה נפתרת'] },
  162: { claim: 'type=password' },
  78: { claim: 'aria-label' },
  ...
  168: { claim: 'pass_salt' },
  172: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  174: { claim: '⏳' },
  176: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  184: { claim: 'ט. ערך נגזר שהתיישן' },
  186: { claim: 'ח. הגדרה שהוחלפה' },
  185: { claim: 'ד. רשימת-היתר' },
  187: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  98: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  106: { claim: 'CLEANUP_SHA' },
  107: { claim: 'בלוק ה-APP' },
  159: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
   ...
   ──────────────────────────────────────────────────────────────────────── */
{
  ...
  /*  ⛔ הרצף מדלג על אות שירדה ⛔ ואינו מסיט — ⚠️ המרשם הוא ההצהרה,
   *  ⭐ ושני הצדדים נמדדים: אות שירדה וחזרה, והכרזה בלי נימוק. */
  const want = [];
  for (let n = 1; want.length < letters.length && n < 40; n++)
    if (!(HEB_ORD(n) in CAT_GAP)) want.push(HEB_ORD(n));
  const badLetter = letters
    .map((g, i) => (g === want[i] ? null : `«${g}» במקום «${want[i]}»`))
    .filter(Boolean)
    .concat(Object.keys(CAT_GAP).filter((g) => letters.indexOf(g) >= 0)
      .map((g) => `«${g}» מוכרזת כמי שירדה ויש לה כותרת`))
    .concat(Object.entries(CAT_GAP).filter(([, w]) => String(w).trim().split(/\s+/).length < 4)
      .map(([g]) => `«${g}» מוכרזת בלי נימוק`));
  if (badLetter.length)
