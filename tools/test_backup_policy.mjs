...
const APP = {
  ...
  migration: 'migrations/012_backup_retention_cap.sql',
...
/* ⚠️ הערכים נקראים משורת **טבלה** ולא מפרוזה — ⛔ פרוזה אינה ניתנת
...
function t1() {
  ...
  /*  ⛔ אין להוסיף כאן טענות על «המוקדם מביניהם», `pre-`, `PRE_*`,
   *  `bk_retention_sweep` או מגבלת השער — ⚠️ אלה טענות על **פרוזה
   *  שמתארת את ה-SQL**, ⛔ בזמן שסעיפים 2–4 אוכפים את ה-SQL עצמו:
   *  ⭐ טענה על תיאור אינה טענה על המנגנון. */
  return rv;
}
...
/* ── הרצה ──────────────────────────────────────────────────────────────── */
console.log('· ' + FACTS.slug + ' — מדיניות הגיבויים');
t1();
if (APP.migration) {
