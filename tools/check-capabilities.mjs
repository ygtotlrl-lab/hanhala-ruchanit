...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    161: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    160: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    126: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    157: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '85|שכבת המראה': 'src',
    '174|הגירה מקומית שהושלמה': 'mixed',
    '159|מראת המשתמשים': 'src',
    '158|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'tools',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '142|הקשר נלכד בכניסה לפונקציה': 'src',
    '88|פריסה במסכי טלפון וטאבלט': 'src',
    '89|ערכת נושא — בהיר וכהה': 'src',
    '169|הערה שמתארת מצב שחלף': 'mixed',
    '175|שאילתת `@media` מתה': 'src',
    '102|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '103|ייצוא והנפקה — דרך מוצהרת': 'src',
    '90|המסמך מתפרסר נקי': 'src',
    '91|לוג מסלול — תג ורמה': 'src',
    '76|מיכל המודאל ילד ישיר של `body`': 'src',
    '98|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '144|הגיבוי שלם ומעומד': 'src',
    '72|פעולה מגיבה מיד': 'src',
    '73|משיכה מסוננת בשרת': 'src',
    '114|דגימת-יתר במחולל': 'tools',
    '161|`PBKDF2` — פרמטרים': 'src',
    '132|`migrations/` — תוכן': 'src',
    '133|`migrations/` — כמות': 'src',
    '150|מיזוג מכל — מפה ומערך': 'src',
    '151|ברירת מחדל אינה ממוזגת': 'src',
    '152|רשימה אינה נושאת פריט כפול': 'src',
    '145|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'tools',
    '171|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'tools',
    '165|סודות אינם עוזבים את המכשיר': 'src',
    '139|דפוס `upsert` — onConflict מוכרז': 'src',
    '163|טביעה חסרה מושלמת אוטומטית': 'src',
    '163|כניסה אופליין': 'src',
    '125|עריכת נתונים אופליין': 'src',
    '149|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '164|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '70|אובייקט יקר נבנה פעם אחת': 'src',
    '71|מסך מציג מיד ומרענן ברקע': 'src',
    '118|האפליקציה ניתנת להתקנה מכרום': 'src',
    '167|דגלי מעבר — אין דגל דלוק': 'src',
    '77|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '160|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '41|ארבעת מימדי הסריקה': 'src',
    '84|פינוי אוטומטי': 'src',
    '154|אימות פינוי מול הענן': 'src',
    '119|שיתוף קבצים': 'src',
    '104|מעטפת APK (WebView)': 'src',
    '107|אין נכסים מוטבעים': 'src',
    '123|מפתח חתימה קבוע בריפו': 'src',
    '129|מקור אמת יחיד לסכימה': 'src',
    '130|קובץ התקנה מלא': 'src',
    '55|גיבוי יומי אוטומטי': 'src',
    '55|יומן פעולות': 'src',
    '126|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '78|דפוס הודעת שגיאה יחיד': 'src',
    '81|`toast` — חתימה, גוף ומחלקות': 'src',
    '99|רישום כשלי כתיבה': 'src',
    '56|חלון חם במכשיר': 'src',
    '56|שחזור מקומי מהענן': 'src',
    '157|מסך שינוי סיסמה עצמי': 'src',
    '183|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '57|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '163|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '58|מודול מזהי רשומות': 'src',
    '58|מזהה אינו מומר למספר': 'src',
    '60|מזהה מכשיר': 'src',
    '57|ליבת מיזוג משותפת': 'src',
    '104|מעטפת WebView חתומה': 'tools',
    '163|אימות מול טביעה בענן': 'mixed',
    '121|בניית APK אחידה עם שער חתימה': 'src',
    '183|מודול ה-service worker': 'src',
    '61|ניסיון חוזר בתור הסנכרון': 'src',
    '62|מנגנון משיכה אחיד': 'src',
    '65|נעילת חוסר-פעילות': 'src',
    '66|מודל הסשן — בזיכרון בלבד': 'src',
    '182|בדיקת עדכון תקופתית ל-service worker': 'src',
    '86|פסק זמן אחיד לקריאות רשת': 'src',
    '87|מאזיני מצב רשת': 'src',
    '148|גריעת tombstones לפי גיל': 'src',
    '181|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '119|גשר שיתוף': 'src',
    '68|העברת מזהה ל-DOM': 'src',
    '69|מיון אחד לכל התצוגות': 'src',
    '74|ייצוא — שני דפוסים מוצהרים': 'src',
    '111|שכבת אייקונים': 'src',
    '155|שכבת קלט אחידה': 'src',
    '155|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '75|שכבת המודאל': 'src',
    '157|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [29, 40, 42, 44, 53, 83, 47, 153, 95, 135, 54, 22, 59, 25];
...
const CAPS = {
  ...
  uihelp: {
    ...
    block: { sha: '8b8f688a95604443', lines: 107,
  ...
  keysave: {
    ...
    block: { sha: '750ee0db1144d2c9', lines: 29,
...
const FLOOR = { shared: 158, app: 10, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי ושער מצב הרשת' };
/* ⚠️ סוף פר-אפליקציה */
...
function keySaveGaps() {
  ...
  if (!/function ksFire\([\s\S]{0,900}?closest\('\.ksave'\)[\s\S]{0,400}?data-ksave[\s\S]{0,200}?data-kesc/.test(src))
    out.push('מודול המקש אינו קורא את ההיקף, את שני הסימונים ואת מפת הפעולות');
  if (!new RegExp(APP.actMap + '\\[act\\]').test(src)) out.push('הניתוב אינו עובר במפת הפעולות');
...
const MATRIX = [
  ...
  { row: 128, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 142, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 88, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 89, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 169, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 98, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 144, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 141, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 114, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 161, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 132, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 133, name: '`migrations/` — כמות',
  ...
  { row: 150, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 151, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 152, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 165, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 139, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 163, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 163, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 125, name: 'עריכת נתונים אופליין',
  ...
  { row: 149, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 164, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 118, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 167, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 160, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 154, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 119, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 104, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 107, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 123, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 129, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 130, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 55, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 55, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 126, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 81, name: '`toast` — חתימה, גוף ומחלקות',
  ...
  { row: 99, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  ...
  { row: 143, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 163, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 104, name: 'מעטפת WebView חתומה',
  ...
  { row: 163, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 121, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 183, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 182, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 173, name: 'מפתח אחסון בלי קורא',
    probe: () => storeKeyGaps().length === 0 },
  { row: 85, name: 'שכבת המראה', probe: () => mirrorLayerGaps().length === 0 },
  { row: 174, name: 'הגירה מקומית שהושלמה',
    probe: () => localMigrationGaps().length === 0 },
  { row: 86, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  ולא רק מוגדרים. */
  { row: 148, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 181, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 111, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 155, name: 'שכבת קלט אחידה',
  ...
  { row: 155, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  134: { claim: 'בעלות הסכימה המשותפת' },
  146: { claim: '`kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  147: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  176: { claim: 'ז. עמודה בלי קורא' },
  ...
  109: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  112: { claim: 'margin' },
  ...
  97: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  67: { claim: 'CANON' },
  83: { claim: 'storage' },
  82: { claim: 'ג · פעולה שדורשת רשת' },
  93: { claim: 'עברית' },
  95: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  96: { claim: 'BANNER_W' },
  172: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  94: { claim: 'מפנה לקובץ' },
  108: { claim: 'android/app/src/main' },
  115: { claim: 'fgDriftMax' },
  116: { claim: 'CANON_MANIFEST' },
  117: { claim: 'שדות זהים' },
  120: { claim: 'BUILD_SHA' },
  124: { claim: 'SHARED_SHA' },
  122: { claim: 'versionCode' },
  105: { claim: 'WebView' },
  106: { claim: 'android:theme' },
  110: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  127: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  131: { claim: 'migrations' },
  138: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  140: { claim: 'א. חותמת בכל רשומה' },
  54: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  135: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  136: { claim: 'ו. טבלה מקבילה' },
  137: { claim: 'ב. חתימת סכימה' },
  156: { claim: 'type=password' },
  79: { claim: 'aria-label' },
  ...
  162: { claim: 'pass_salt' },
  166: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  168: { claim: '⏳' },
  49: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  170: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  177: { claim: 'ט. ערך נגזר שהתיישן' },
  179: { claim: 'ח. הגדרה שהוחלפה' },
  178: { claim: 'ד. רשימת-היתר' },
  180: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  92: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  100: { claim: 'CLEANUP_SHA' },
  101: { claim: 'בלוק ה-APP' },
  153: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
