...
export const ROWS = [7, 10, 38, 156, 86];
 ...
 *  יושבת בתוך `rules-table` ⛔ ולא איפה שהיא בקובץ — הבלוק הוא מה
...
t(commentsFails({ 'tools/test_bump.mjs': `/*  ⛔ ${TOOLCNT} */\n` + rd('tools/test_bump.mjs') }),
  'מ20 · ספירה שנמדדה בכלי חיצוני בהערת **קוד** **מפילה** את check-comments');
t(commentsFails({ 'CLAUDE.md': DOC.replace(/^(\| 29 \|(?:[^|\n]*\|){6})[^|\n]*\|/m, `$1 ⚠️ ${TOOLCNT} |`) }),
  'מ21 · ואותה ספירה בעמודת ההערות של **הטבלה** **מפילה** אף היא');
/*  ⭐ מוטציית-נגד: מדידה שהשער כן מריץ ⛔ אינה מפילה — ⚠️ «מול המסד»
 *  ו«בדפדפן» חוזרות בכל הרצה, ⭐ ואין להן שם כלי בגרשיים אחוריים. */
t(!commentsFails({ 'CLAUDE.md': DOC.replace(/^(\| 29 \|(?:[^|\n]*\|){6})[^|\n]*\|/m, '$1 ⚠️ נמדד מול המסד: 8 · 8 שורות, ובדפדפן 3 · 3 |') }),
