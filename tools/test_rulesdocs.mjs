...
const APP = {
  app: 'hanhala-ruchanit',  cachePrefix: 'hanhala-ruchanit-',
...
const FLOOR = { shared: 27, app: 22, appWhy: 'מספר השערים והבודקים שהריפו נושא — כל שער פרטי מוסיף טענת תוכן' };
/* ⚠️ סוף פר-אפליקציה */
...
/* ── ב. תוכן הקבצים לפי תפקידם ─────────────────────────────────────────── */
{
   ...
   *  ⛔ **והחצי שמדד «פרק כללים» ירד בסבב 97** — ⚠️ בלוקי הכללים
 ...
 *  ⚠️ **מה נכנס**: כל טענה ש-`GATES` מצביעה עליה, מול אזור המוטציות של
 *  השער ומול הרתמה; ⛔ **ומה מפיל**: טענה שאין לה מוטציה ואין לה שורה
 *  ב-`CLAIM_NO_MUT`, ⚠️ ושורה שם לטענה שמוטציה כבר נוקבת בשמה.
 *  ⭐ **והנימוק המדוד**: ה-probe של «מוטציות — כיסוי» מדד **שערים**,
 *  ⛔ וטענה בתוך שער אינה שער: ⚠️ והיא חמקה. */
{
  ...
  /*  ⛔ הטענה נבחרת מהמרשם ⛔ ואינה מוקלדת — ⚠️ מפתח קשיח נסחף בכל
   *  מספור מחדש, ⭐ והמוטציה מפסיקה לפגוע במה שהיא באה למדוד. */
   ...
   *  שאוכף אותה אין אזור מוטציות: ⛔ ולכן הסרתה מהרתמה מותירה אותה
   *  בלי מוטציה ובלי נימוק. */
  /*  ⛔ השם נבנה משני חלקים — ⚠️ ליטרל מלא כאן היה מופע שני ברתמה,
   *  ⭐ והמוטציה הייתה מדלגת על עצמה בשתיקה. */
  ...
  if (harn.split(NAMED).length !== 2) {
    ...
    /*  ⭐ מוטציית-נגד: אותה טענה בשם שנקצר בעקביות ⛔ אינה מפילה —
     *  ⚠️ הנמדד הוא **שיש לה מוטציה** ⛔ ולא הניסוח שהיא נושאת. */
 ...
 *  שהשורה באה לסלק. */
{
  ...
  /*  ⛔⛔ מ75 · נ47 — אוצר המילים אינו רשימה סגורה (סבב 139): ⚠️ **מה
   ...
   *  מפיל — ⚠️ הנמדד הוא **שהאסימון מוצהר**, ⛔ ולא השם שהוא נושא. */
  {
    ...
    /*  ⛔ ההיקף הוא בלוק `appTokens` בלבד (סבב 140) — ⚠️ סריקה על כל
  ...
  /*  ⛔⛔ מ76 · נ48 — פס שנבנה ביד (סבב 139): ⚠️ **מה נכנס**: גוף `bar`
  ...
  /*  ⛔⛔ מ77 · נ49 — נימוק שנוקב בנוכחות בלבד (סבב 140): ⚠️ **מה נכנס**:
  ...
  /*  ⛔⛔ מ78 · נ50 — הפניה ל-`var()` שאין לה הגדרה (סבב 140): ⚠️ **מה
  ...
  {
    /*  ⛔ הכלל נבחר מהגיליון ⛔ ואינו מוקלד — ⚠️ מחלקת שירות שיורדת
     *  משאירה מוטציה שמחליפה מחרוזת שאינה שם, ⭐ והיא עוברת בשתיקה. */
    const idx = rd('app.css');
    const hit = /\n(\.u-[a-z0-9-]+\{)([^}]*)\}/.exec(idx);
    if (!hit) {
