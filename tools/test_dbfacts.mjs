/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════
   **מה נאכף:** תשע טענות שנמדדות מול המסד עצמו במפתח ה-`anon` שכבר יושב
...
export const ROWS = [142, 137, 138, 139, 155, 178, 179, 180, 181, 149];
...
if (RUN_MUT && !SELFTEST) {
  ...
  const reply = (scen, url) => {
    /*  ⛔ ענף נפרד לשאילתת המיון — ⚠️ היא נבדלת ב-`order=`: ⭐ ענף אחד
     *  לשתיהן היה מפיל את «חתימת סכימה» על תשובה שנועדה למיון. */
    if (/[?&]order=/.test(url))
      return scen === 'sortcol'
...
else console.log(`\n✓ סבב 93 (עובדות המסד החי) — תשע הטענות נמדדו מול המסד`);
