...
const APP = {
  ...
  /*  ⛔ הטבלאות שנושאות חותמת, וטיפוסה — ⚠️ `bigint` היא חותמת שהמכשיר
   *  מייצר, ⭐ ובה אפס הוא **הישן ביותר** ולא «לא ידוע»; ⛔ `ts` היא
   *  חותמת שרת מטריגר `touch`, ⚠️ ובה הכשל היחיד האפשרי הוא ריק. */
  stamped: { ys_sessions: 'bigint', ys_marks: 'bigint', ys_students_rows: 'bigint',
             ys_sleep_sessions: 'bigint', ys_sleep_marks: 'bigint',
             ys_settings: 'ts', ys_users: 'ts' },
  ...
  /*  ⛔ המיגרציה האחרונה שמגדירה את רשימת-ההיתר מחדש — ⚠️ מיגרציה שכבר
   *  רצה אינה נערכת, ⭐ ולכן שינוי ברשימה הוא תמיד קובץ חדש. */
  allowlistDoc: 'migrations/022_backup_allowlist_restore_ys_cls_years.sql'
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
