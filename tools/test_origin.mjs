...
const APP = {
  ...
  evictCalc: {
    seeds: ['refreshDashStats', 'mirrorLoad', 'hrMirrorPutRecs', 'MIRROR'],
    /*  ⛔ הקוראים — ⚠️ פונקציה שנוקבת באחד מהם ומסכמת נמדדת, ⭐ או נכשלת. */
    readers: ['atLoadData', 'hrMirrorRecs'],
    seed: `(function () {
  var DAY = 86400000, now = Date.now(), iso = function (t) { return new Date(t).toISOString().slice(0, 10); };
  var sts = [{ id: 's1', name: 'א', cls: 'a' }, { id: 's2', name: 'ב', cls: 'a' }].map(function (s) { s.updatedAt = now - 900 * DAY; return s; });
  var recs = [{ id: 'r0b', session: 'ערב', date_iso: iso(now), updatedAt: now, marks: { s1: { s: 'e' }, s2: { s: 'p' } } }];
  for (var d = 300; d >= 0; d -= 7) recs.push({ id: 'r' + d, session: 'בוקר', date_iso: iso(now - d * DAY), updatedAt: now - d * DAY, marks: { s1: { s: 'p' }, s2: { s: d % 2 ? 'x' : 'l' } } });
  MIRROR = {};
  hrMirrorPutRecs('hr_students_rows', sts);
  hrMirrorPutRecs('hr_sessions', recs);
})();`,
    load: "mirrorLoad(); window._atData = null;",
    calls: {
      refreshDashStats: "refreshDashStats().then(function () { return ['s-tot', 's-pre', 's-abs', 's-rate'].map(function (id) { return document.getElementById(id).textContent; }); })",
    },
  },
  /*  ⛔ מוטציות פרטיות לאפליקציה — ⚠️ **מה נכנס**: `[מחט, תחליף, טענה,
   *  הודעה]`; ⛔ **ומה מפיל**: מוטציה שאינה מפילה את הטענה שהיא נוקבת בה.
   *  ⭐ **ולמה היא קיימת**: הסימונים הם הטבלה שמילאה את האחסון, ⛔ והסרתם
   *  מהפינוי חייבת ליפול. */
  mutations: [
    ['function _hrRecTs(r) {', 'function _hrRecTs(r) { return 1;',
     '[evict-calc-stable]', 'חותמת שמגיעה לכל רשומה מפנה את סדרי היום'],
    ["key: mirrorKey('hr_marks'),", "key: mirrorKey('hr_marks_off'),", '[mirror-evict]',
     'הסימונים יצאו מהפינוי'],
  ],
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
