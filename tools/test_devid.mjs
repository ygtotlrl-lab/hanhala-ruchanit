...
const APP = {
  /* ⛔ המפתח ההיסטורי — אינו משתנה לעולם. */
...
const START = '/* ═══ מזהה מכשיר — מודול משותף';
...
/*  ⛔ הערך נחלץ ומושווה — ⚠️ בדיקת נוכחות עוברת גם כשההצהרה
 ...
 *  «מודד התנהגות» ל«סופר טקסט»: שינוי רווחים והצהרה חדשה
 *  וחיה אינם משנים דבר במה שהליבה עושה, ⛔ ולכן הרתמה חייבת להמשיך
 *  לעבור. ⛔ ואינה הערה — הערה אינה קוד, ואינה מוכיחה דבר. */
...
console.log(bad ? `\n❌ ${FACTS.slug}: ${n} טענות, ${bad} נכשלו`
                : `\n✓ מזהה מכשיר — ${n} טענות עברו, 0 נכשלו`);
