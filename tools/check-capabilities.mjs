...
const APP = {
  ...
  actMap: 'DOM_ACTIONS',
  gapRows: [38, 107, 111, 129, 130, 134, 147],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    151: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    150: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    118: (c) => !/ysCfgSet\('(?:ys_students|ys_attend_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    149: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [27, 37, 39, 41, 50, 74, 44, 142, 87, 139, 51];
...
const CAPS = {
  ...
  push: {
    ...
    block: { sha: '42f73c12168f0e93', lines: 106,
...
const BLOCK_ORDER = ['bp', 'neterr', 'rowswin', 'guardonline', 'storage', 'stale', 'techinfo', 'status', 'backup',
                     ...
                     'devid', 'mergecore', 'tomb', 'writeUser', 'uihelp', 'uniq'];
...
const MATRIX = [
  ...
  { row: 81, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 82, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 159, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 91, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 132, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 129, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
   ...
   *  אחד. */
  { row: 130, name: 'הקשר נלכד בכניסה לפונקציה', app: true },
  ...
  { row: 106, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 151, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 123, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 124, name: '`migrations/` — כמות',
  ...
  { row: 137, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 138, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 133, name: '`kv_backup` — טבלת הגיבוי',
  ...
  { row: 134, name: '`kv_rishon` / `kv_ramataviv` — הגדרות היומן', app: true },
  { row: 21, name: 'שער אינו מקליד מספר שורה',
    probe: () => typedRowSites().length === 0 },
  { row: 161, name: 'פונקציה בלי קוראים',
  ...
  { row: 155, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 127, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 153, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 153, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 117, name: 'עריכת נתונים אופליין',
  ...
  { row: 136, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 154, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 110, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 157, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 150, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 143, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 111, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 96, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 99, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 115, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 120, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 121, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 52, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 52, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 118, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 148, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  ...
  { row: 149, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 168, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 131, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 153, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 96, name: 'מעטפת WebView חתומה',
  ...
  { row: 153, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 113, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 168, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 167, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 135, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 166, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 103, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 144, name: 'שכבת קלט אחידה',
...
const GATES = {
  ...
  59:  { claim: 'שכבת הדחיפה' },
  60: { claim: 'כתיבת משתמש' },
  125: { claim: 'בעלות הסכימה המשותפת' },
  ...
  101: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  104: { claim: 'margin' },
  ...
  90: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  86: { claim: 'עברית' },
  87: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  88: { claim: 'BANNER_W' },
  162: { claim: 'CSS מתות' },
  40: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  89: { claim: 'מפנה לקובץ' },
  100: { claim: 'android/app/src/main' },
  107: { claim: 'fgDriftMax' },
  108: { claim: 'CANON_MANIFEST' },
  109: { claim: 'שדות זהים' },
  112: { claim: 'BUILD_SHA' },
  116: { claim: 'SHARED_SHA' },
  114: { claim: 'versionCode' },
  97: { claim: 'WebView' },
  98: { claim: 'android:theme' },
  102: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  119: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  122: { claim: 'migrations' },
  126: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  128: { claim: 'א. חותמת בכל רשומה' },
  51: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  ...
  145: { claim: 'type=password' },
  146: { claim: 'aria-label' },
  147: { manual: 'מנוע התאריך — ⛔ נאכף ב-`test_date` ביומן ובהנהלה, ⚠️ ובשכר ובגיוס אין צרכן תאריך עברי ואין שער' },
  152: { claim: 'pass_salt' },
  156: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  158: { claim: '⏳' },
  46: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  160: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  164: { claim: 'ד. רשימת-היתר' },
  165: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  85: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  92: { claim: 'CLEANUP_SHA' },
  93: { claim: 'בלוק ה-APP' },
  142: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
