...
if (!process.env.RD67_MUT) {
  ...
  console.log('\n— מוטציות (סבב 67) —');
  _mut('⛔ שינוי התנהגות ב-sw.js מפיל את קו הבסיס', 'sw.js',
       (s) => s.replace(/mode:\s*'cors'/, "mode: 'no-cors'"), true);
  _mut('⭐ מוטציית-נגד: הוספת שורת הערה ל-sw.js ⛔ אינה מפילה', 'sw.js',
       (s) => s + '\n// הערה\n', false);
}
