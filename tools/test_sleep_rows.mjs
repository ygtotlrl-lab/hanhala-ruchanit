 ...
 *  טענה משותפת שאבדה. */
const FLOOR = { shared: 0, app: 29, appWhy: 'סדרי השינה בשכבת השורות — יכולת שקיימת בהנהלה בלבד' };
...
const ok = (m, c) => { RAN++; if (c) { pass++; console.log('  ok   ' + m); } else { fail++; console.log('  FAIL ' + m); } };
