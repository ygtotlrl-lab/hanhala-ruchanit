...
const APP = {
   ...
   *  שקיים ואינו עטוף. */
  /*  ⚠️ ירד מ-40 ל-5 — שכבת המודאל נסגרה, ⛔ וכל אתרי ה-`onclick`
   *  עברו ל-`data-act` ול-`data-id`. ⭐ החמישה שנשארו אינם `onclick` אלא
   *  `oninput`/`onkeydown` על שדות הקלט של האיחור וההערה — ⛔ הם מעבירים
   *  גם את `this.value`, ⚠️ ודלגציה על `input` היא סבב אחר. */
  sites: 0,
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
const bad = (m) => { RAN++; fail++; console.log('  FAIL ' + m); };
...
function stripComments(t) {
  ...
  while (i < n) {
    ...
    /*  ⚠️ הערה מוחלפת ברווחים ו**שורות החדשה נשמרות** — בלי זה
 ...
 *  נושא שם שמסתיים ב-`id` — ⛔ ואין לו חריגה: ⭐ רשימת ההחרגה נותרה
 *  ריקה בכל הריפו, ⚠️ וירדה עם מי שקרא אותה. ⚠️ ומה שהשער **אינו** מודד: ערך שנקרא כמו מזהה ואינו כזה.
 */
...
if (wrongly.length === 0)
  ...
  bad('1ב · ⛔ ' + wrongly.length + ' עטיפות על ערך שאינו מזהה: ' +
      ...
      ' — או שהערך אינו מזהה, או שהוא מזהה ואז שמו מסתיים ב-`id`.');
...
console.log(`${fail ? '✗' : '✓'} העברת מזהה ל-DOM — ${pass} טענות עברו, ${fail} נכשלו\n`);
