/* ───────────────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────────────────
   **מה נאכף:** בלוקי `rules-*` שב-`CLAUDE.md` זהים **בית-לבית** בין
   כל הריפו, ⛔ וגם החתימות שמוצהרות להם ב-`CANON` שב-`check-docs`
   נושאות אותו ערך בכל העותקים.
   ...
   **הנימוק המדוד:** שער החתימות מודד ש**החתימה תואמת לתוכן באותו ריפו**,
   ...
   גם את החתימה מקבל חתימות תקינות על תכנים נבדלים — ⭐ ונמדד: שורת
   ...
   **מה יישבר בלעדיו:** ⛔ הכלל המרכזי — «יכולת שבטבלה קיימת בכולן» —
...
if (roots.length === PEERS.length) {
  ...
  t(n++, !live.some((x) => x.kind.startsWith('block')),
    `א. בלוקי rules-* זהים בית-לבית ב-${roots.length} הריפו ${of('block-drift')}${of('block-missing')}`);
 ...
 *  ⚠️ אחרת מסלול הכשל היה רץ רק במכונה שיש בה כולם. */
/*  ⛔ כותב על עותק — ⚠️ השער משווה בין ריפו אחיות, ⛔ ומסלול הכשל דורש אחות סינתטית. */
 ...
 *  ל«אוסרות לגעת בבלוק». */
/*  ⛔ השורה נגזרת ⛔ ואינה מוקלדת (סבב 96) — ⚠️ הנימוק נמדד: כאן ישבה
 *  המחרוזת `| 64 | טיפול באירועים |`, ⭐ והמספור מחדש הזיז את השורה
 *  ל-70: ⛔ ה-`replace` הפסיק להתאים, המוטציית-נגד לא שינתה דבר,
 *  ⚠️ והיא «עברה» מבלי שנמדד דבר. */
const ROW_RE = /^\| \d+ \| טיפול באירועים \|/m;
let padded = 0;
for (const r of [A, B]) {
  edit(r, DOC, (s) => s.replace(ROW_RE, (m) => { padded++; return m.replace(/\|$/, ' |'); }));
  edit(r, DOCGATE, (s) => s.replace(/(\['table',\s*')[0-9a-f]{16}/, '$1' + '1'.repeat(16)));
}
t(n++, padded === 2, `נ1א · מוטציית-הנגד שינתה את שני העותקים — נמדדו ${padded} והצפוי 2`);
{
  const got = audit(pair).map((x) => x.kind);
  t(n++, got.length === 0,
