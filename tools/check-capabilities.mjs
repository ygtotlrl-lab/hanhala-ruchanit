...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    112: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    111:  (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    94: (c) => c.hasCode(/YS_KV_LEGACY_WRITE\s*=\s*false/),
    ...
    110: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [34, 38, 39, 40, 37, 48, 63, 43];
...
const MATRIX = [
  ...
  { row: 85, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 112, name: '`PBKDF2` — פרמטרים', app: true },
  ...
  { row: 116, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 100, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 114, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 114, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 93, name: 'עריכת נתונים אופליין',
  ...
  { row: 103, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 118, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 111, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 110, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 130, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 101, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 114, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 77, name: 'מעטפת WebView חתומה',
  ...
  { row: 114, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 90, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 130, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 129, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 102, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 128, name: 'אוטו-אפדייט מ-raw.githubusercontent',
  ...
  { row: 82, name: 'שכבת אייקונים',
    probe: () => iconAudit('.').length === 0 },
  { row: 105, name: 'שכבת קלט אחידה',
...
const GATES = {
  ...
  81: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  83: { claim: 'margin' },
  30: { manual: 'השוואת זמנים בין הריפו אינה בהישג ידו של שער שרץ בריפו אחד — ⛔ נאכפת בתוצאתה בלבד' },
  44: { manual: 'המדידה שלפני המחיקה היא התנהגות סשן — ⛔ נאכפת רק בתוצאתה, בשער ההסרות' },
  43: { claim: 'אין פרק פערים נפרד' },
  74: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  47: { manual: 'מחזור העבודה ב-git הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  45: { claim: 'מזהה שנמחק ונשאר לו קורא' },
  84: { claim: 'tile-bg' },
  ...
  34: { claim: 'כיסוי הטבלה' },
  35: { claim: 'ועמודת התקן שלה אוסרת במפורש' },
  38: { claim: 'מספור רציף ובלי כפילויות' },
  39: { claim: 'הערה ריקה' },
  40: { claim: 'COUNT_NOTE' },
  41: { manual: 'עדכון הסימון הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  36: { manual: 'קריאת הטבלה לפני הכתיבה היא התנהגות סשן שאינה בעץ — ⛔ נאכפת רק בתוצאתה' },
  131: { claim: 'CACHE_NAME' },
  ...
  73: { claim: 'מפנה לקובץ' },
  80: { claim: 'android/app/src/main' },
  86: { claim: 'fgDriftMax' },
  87: { claim: 'CANON_MANIFEST' },
  89: { claim: 'BUILD_SHA' },
  91: { claim: 'versionCode' },
  78: { claim: 'WebView' },
  95: { manual: 'נפילה-חזרה ל-`kv` נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  98: { claim: 'migrations' },
  99: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  106: { claim: 'type=password' },
  107: { claim: 'aria-label' },
  108: { manual: 'שני מנועי תאריך — ⛔ טרם הוכרע איזה, ואין מה לאכוף' },
  109: { manual: '«`catch` ריק סביב כתיבה» טרם נוסח כשער — נסרק ידנית בכל סבב שנוגע' },
  113: { claim: 'pass_salt' },
  115: { manual: 'מצב העמודה במסד אינו נראה מהריפו' },
  117: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  119: { claim: '⏳' },
  120: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  126: { manual: 'קיום טבלה או מפתח במסד אינו נראה מהריפו' },
  124: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  121: { manual: '«קובץ בלי קורא» דורש סריקה על ארבעת הריפו — נעשה ידנית' },
  127: { manual: 'קיום מפתח במסד אינו נראה מהריפו' },
  123: { manual: 'רשימת-היתר הגיבויים יושבת במיגרציה שכבר רצה' },
  125: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  75: { claim: 'CLEANUP_SHA' },
  76: { claim: 'בלוק ה-APP' },
};
