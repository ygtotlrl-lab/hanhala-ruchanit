...
--
-- ⛔ **`sync_log` ו-`kv_backup` אינן כאן, ואין להוסיף אותן לקובץ הזה** — הן
-- **יומני ראיות** וצריכות סט צר יותר: `INSERT` + `SELECT` בלבד, **בלי
-- `UPDATE`**, שנחוץ כאן ל-`ysKvSet`. ⛔ אין «ליישר» אותן לסט שבקובץ הזה
-- בשם האחידות — הצרוּת שם היא ההגנה. ר' כלל ברזל 10 סעיף 9.
-- ⚠️ **הן מטופלות ב-`002_revoke_log_tables.sql`, וזו אינה כפילות:** בזמן
-- כתיבת 001 נמדד ש-anon כבר מחזיק בהן `INSERT, SELECT` בלבד, ומכאן ההנחה
-- שאין מה לעשות. ⛔ ההנחה הייתה חלקית — `authenticated` החזיק בהן את הסט
-- **המלא**, כולל `UPDATE` ו-`TRUNCATE`. ר' הלקח בראש 002.
--
...
--   kv_backup   → INSERT, SELECT      (לא נגע כאן — ר' 002)
--   sync_log    → INSERT, SELECT      (לא נגע כאן — ר' 002)
