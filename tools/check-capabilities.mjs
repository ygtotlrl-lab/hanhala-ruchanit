...
const APP = {
  ...
  writeActs: ['login', 'user-toggle', 'user-save', 'my-pass-save', 'perms-save', 'switch-confirm'],
  writeActsWhy: '',
  ...
  scanKind: {
    ...
    'check-status-area': 'raw — מודד את תגיות אזור המצב ואת הטקסט שבתוכן',
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    163: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    162: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    128: (c) => !/ysCfgSet\('(?:ys_students|ys_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    159: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '159|מבנה טופס הכניסה': 'src',
    '130|שם נגזר אינו נשמר ברשומה': 'src',
    '175|מפתח אחסון בלי קורא': 'src',
    '33|שער מריץ את כל טענותיו': 'tools',
    '87|שכבת המראה': 'src',
    '176|הגירה מקומית שהושלמה': 'mixed',
    '161|מראת המשתמשים': 'src',
    '160|מודל ההרשאות': 'src',
    '23|שער סורק קוד מולבן': 'tools',
    '23|ההלבנה שומרת על מספרי השורות': 'mixed',
    '144|הקשר נלכד בכניסה לפונקציה': 'src',
    '90|פריסה במסכי טלפון וטאבלט': 'src',
    '91|ערכת נושא — בהיר וכהה': 'src',
    '171|הערה שמתארת מצב שחלף': 'mixed',
    '177|שאילתת `@media` מתה': 'src',
    '104|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '105|ייצוא והנפקה — דרך מוצהרת': 'src',
    '92|המסמך מתפרסר נקי': 'src',
    '93|לוג מסלול — תג ורמה': 'src',
    '76|מיכל המודאל ילד ישיר של `body`': 'src',
    '100|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '146|הגיבוי שלם ומעומד': 'src',
    '72|פעולה מגיבה מיד': 'src',
    '73|משיכה מסוננת בשרת': 'src',
    '116|דגימת-יתר במחולל': 'tools',
    '163|`PBKDF2` — פרמטרים': 'src',
    '134|`migrations/` — תוכן': 'src',
    '135|`migrations/` — כמות': 'src',
    '152|מיזוג מכל — מפה ומערך': 'src',
    '153|ברירת מחדל אינה ממוזגת': 'src',
    '154|רשימה אינה נושאת פריט כפול': 'src',
    '147|`kv_backup` — טבלת הגיבוי': 'src',
    '21|שער אינו מקליד מספר שורה': 'tools',
    '173|פונקציה בלי קוראים': 'mixed',
    '27|שער ממתין לתנאי ולא לשעון': 'tools',
    '167|סודות אינם עוזבים את המכשיר': 'src',
    '141|דפוס `upsert` — onConflict מוכרז': 'src',
    '165|טביעה חסרה מושלמת אוטומטית': 'src',
    '165|כניסה אופליין': 'src',
    '127|עריכת נתונים אופליין': 'src',
    '151|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '166|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '70|אובייקט יקר נבנה פעם אחת': 'src',
    '71|מסך מציג מיד ומרענן ברקע': 'src',
    '120|האפליקציה ניתנת להתקנה מכרום': 'src',
    '169|דגלי מעבר — אין דגל דלוק': 'src',
    '77|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '162|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '41|ארבעת מימדי הסריקה': 'src',
    '86|פינוי אוטומטי': 'src',
    '156|אימות פינוי מול הענן': 'src',
    '121|שיתוף קבצים': 'src',
    '106|מעטפת APK (WebView)': 'src',
    '109|אין נכסים מוטבעים': 'src',
    '125|מפתח חתימה קבוע בריפו': 'src',
    '131|מקור אמת יחיד לסכימה': 'src',
    '132|קובץ התקנה מלא': 'src',
    '55|גיבוי יומי אוטומטי': 'src',
    '55|יומן פעולות': 'src',
    '128|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '78|דפוס הודעת שגיאה יחיד': 'src',
    '83|`toast` — חתימה, גוף ומחלקות': 'src',
    '83|סיווג ההודעה נגזר מהמסלול': 'src',
    '183|מיכל באנר העדכון במקור': 'src',
    '183|הרענון מ-controllerchange בלבד': 'src',
    '183|סימן דחיית הבאנר מתמיד': 'src',
    '101|רישום כשלי כתיבה': 'src',
    '56|חלון חם במכשיר': 'src',
    '56|שחזור מקומי מהענן': 'src',
    '159|מסך שינוי סיסמה עצמי': 'src',
    '185|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '57|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '165|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '58|מודול מזהי רשומות': 'src',
    '58|מזהה אינו מומר למספר': 'src',
    '60|מזהה מכשיר': 'src',
    '57|ליבת מיזוג משותפת': 'src',
    '106|מעטפת WebView חתומה': 'tools',
    '165|אימות מול טביעה בענן': 'mixed',
    '123|בניית APK אחידה עם שער חתימה': 'src',
    '185|מודול ה-service worker': 'src',
    '61|ניסיון חוזר בתור הסנכרון': 'src',
    '62|מנגנון משיכה אחיד': 'src',
    '65|נעילת חוסר-פעילות': 'src',
    '66|מודל הסשן — בזיכרון בלבד': 'src',
    '184|בדיקת עדכון תקופתית ל-service worker': 'src',
    '88|פסק זמן אחיד לקריאות רשת': 'src',
    '89|מאזיני מצב רשת': 'src',
    '150|גריעת tombstones לפי גיל': 'src',
    '183|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '121|גשר שיתוף': 'src',
    '68|העברת מזהה ל-DOM': 'src',
    '69|מיון אחד לכל התצוגות': 'src',
    '74|ייצוא — שני דפוסים מוצהרים': 'src',
    '113|שכבת אייקונים': 'src',
    '157|שכבת קלט אחידה': 'src',
    '157|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '75|שכבת המודאל': 'src',
    '159|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [29, 40, 42, 44, 53, 85, 47, 155, 97, 137, 54, 22, 59, 25, 32];
...
const CAPS = {
  ...
  storage: {
    ...
    block: { sha: '8bd8b6d3906101db', lines: 613,
  ...
  status: {
    name: 'אזור המצב',
    docRows: ['`status` — אזור מצב'],
    block: { sha: '29eda90deaa2f417', lines: 46,
  ...
  backup: {
    ...
    block: { sha: '5dcedbbea71b18a7', lines: 342,
  ...
  hotwin: {
    ...
    docRows: ['`hw` — חלון חם'],
    block: { sha: 'eacc25f9afabbf6c', lines: 192,
...
const BLOCK_ORDER = ['bp', 'neterr', 'rowswin', 'guardonline', 'storage', 'schemastale', 'busyguard', 'swreg', 'status', 'backup',
...
const FLOOR = { shared: 165, app: 10, appWhy: 'כניסה — סשן · נעילה · הרשאות · כתיבת משתמש · שכבת המראה; ומעליהם מנוע התאריך העברי ושער מצב הרשת' };
/* ⚠️ סוף פר-אפליקציה */
...
const SILENT_DOM_ALLOW = {
  ...
  syncStatusMount: 'נקודת עגינה — ⛔ מסך ההגדרות אינו מצויר בכל מסך, ⚠️ ואזור המצב חי בסופו בלבד',
  bkStatusMount: 'נקודת עגינה — ⛔ אותו נימוק, ⚠️ והיא נתלית על עוגן שחי במסך ההגדרות בלבד',
...
function writeActGaps() {
  ...
  for (const a of want) {
    ...
    if (!/\breturn\s+[A-Za-z_$]/.test(body))
...
function busyGuardGaps() {
  ...
  return out.concat(writeActGaps());
}
...
function themeGaps() {
  ...
  return out.concat(dualRoleGaps(), contrastGaps(), updaterLiteralGaps(), brandTokenGaps(), tokenVocabGaps(), pairContrastGaps());
}
...
const MATRIX = [
  ...
  { row: 130, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 144, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 171, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 100, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 146, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 143, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 116, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 163, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 134, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 135, name: '`migrations/` — כמות',
  ...
  { row: 152, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 153, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 154, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 167, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 141, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 165, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 165, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 127, name: 'עריכת נתונים אופליין',
  ...
  { row: 151, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 166, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 120, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 169, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 162, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 156, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 121, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 106, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 109, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 125, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 131, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 132, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 55, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 55, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 128, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 101, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  ...
  { row: 159, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 159, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 185, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 145, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 165, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 106, name: 'מעטפת WebView חתומה',
  ...
  { row: 165, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 123, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 185, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 184, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 150, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 183, name: 'מיכל באנר העדכון במקור',
    probe: () => /<div id="updater">/.test(src) &&
                 !/\.id\s*=\s*['"]updater['"]/.test(code) &&
                 !/createElement\([^)]*\)[^;]{0,80}updater/.test(code) },
  { row: 183, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 183, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 183, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 113, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 157, name: 'שכבת קלט אחידה',
  ...
  { row: 157, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  136: { claim: 'בעלות הסכימה המשותפת' },
  148: { claim: '`kv_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  149: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  178: { claim: 'ז. עמודה בלי קורא' },
  ...
  111: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  114: { claim: 'margin' },
  ...
  99: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  95: { claim: 'עברית' },
  97: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  98: { claim: 'BANNER_W' },
  174: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  96: { claim: 'מפנה לקובץ' },
  110: { claim: 'android/app/src/main' },
  117: { claim: 'fgDriftMax' },
  118: { claim: 'CANON_MANIFEST' },
  119: { claim: 'שדות זהים' },
  122: { claim: 'BUILD_SHA' },
  126: { claim: 'SHARED_SHA' },
  124: { claim: 'versionCode' },
  107: { claim: 'WebView' },
  108: { claim: 'android:theme' },
  112: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  129: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  133: { claim: 'migrations' },
  140: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  142: { claim: 'א. חותמת בכל רשומה' },
  54: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  137: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  138: { claim: 'ו. טבלה מקבילה' },
  139: { claim: 'ב. חתימת סכימה' },
  158: { claim: 'type=password' },
  79: { claim: 'aria-label' },
  ...
  164: { claim: 'pass_salt' },
  168: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  170: { claim: '⏳' },
  49: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  172: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  179: { claim: 'ט. ערך נגזר שהתיישן' },
  181: { claim: 'ח. הגדרה שהוחלפה' },
  180: { claim: 'ד. רשימת-היתר' },
  182: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  94: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  102: { claim: 'CLEANUP_SHA' },
  103: { claim: 'בלוק ה-APP' },
  155: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
