...
const APP = {
  ...
  cloudPutWhy: '',
  cloudWrites: ['savePerms', 'saveAbsenceReasons', 'atSaveCfg', 'atSaveTreats', '_slCleanCfg',
                'slSaveCfg', 'slSaveTreats', 'saveUser', 'toggleUserActive', 'changeMyPassword'],
  cloudWriteAllow: {
  ...
  gapRows: [217, 48, 136, 140, 164, 68, 149, 106],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    186: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    185: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    147: (c) => !/ysCfgSet\('(?:hr_students|hr_sessions|hr_sleep_sessions)'/.test(c.src)
    ...
    182: (c) => !!c.fnBody('changeMyPassword')
  ...
  probeInput: {
    ...
    '182|מבנה טופס הכניסה': 'src',
    '149|שם נגזר אינו נשמר ברשומה': 'src',
    '203|קבוע בלי קורא': 'mixed',
    '205|מפתח אחסון בלי קורא': 'src',
    '39|שער מריץ את כל טענותיו': 'tools',
    '97|שכבת המראה': 'src',
    '206|הגירה מקומית שהושלמה': 'mixed',
    '184|מראת המשתמשים': 'src',
    '183|מודל ההרשאות': 'src',
    '26|שער סורק קוד מולבן': 'tools',
    '26|ההלבנה שומרת על מספרי השורות': 'mixed',
    '165|הקשר נלכד בכניסה לפונקציה': 'src',
    ...
    '198|הערה שמתארת מצב שחלף': 'mixed',
    '207|שאילתת `@media` מתה': 'src',
    '122|ספרייה חיצונית — גרסה מוצהרת': 'src',
    '123|ייצוא והנפקה — דרך מוצהרת': 'src',
    '107|המסמך מתפרסר נקי': 'src',
    '108|לוג מסלול — תג ורמה': 'src',
    '84|מיכל המודאל ילד ישיר של `body`': 'src',
    '118|פונקציה משותפת אינה יוצאת בשקט': 'src',
    '167|הגיבוי שלם ומעומד': 'src',
    '80|פעולה מגיבה מיד': 'src',
    '81|משיכה מסוננת בשרת': 'src',
    '135|דגימת-יתר במחולל': 'tools',
    '186|`PBKDF2` — פרמטרים': 'src',
    '153|`migrations/` — תוכן': 'src',
    '154|`migrations/` — כמות': 'src',
    '175|מיזוג מכל — מפה ומערך': 'src',
    '176|ברירת מחדל אינה ממוזגת': 'src',
    '177|רשימה אינה נושאת פריט כפול': 'src',
    '168|`sh_backup` — טבלת הגיבוי': 'src',
    '24|שער אינו מקליד מספר שורה': 'tools',
    '202|פונקציה בלי קוראים': 'mixed',
    '30|שער ממתין לתנאי ולא לשעון': 'tools',
    '190|סודות אינם עוזבים את המכשיר': 'src',
    '162|דפוס `upsert` — onConflict מוכרז': 'src',
    '188|טביעה חסרה מושלמת אוטומטית': 'src',
    '188|כניסה אופליין': 'src',
    '146|עריכת נתונים אופליין': 'src',
    '172|מחיקה רכה בלבד — אין `DELETE` פיזי': 'src',
    '189|סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי': 'src',
    '78|אובייקט יקר נבנה פעם אחת': 'src',
    '79|מסך מציג מיד ומרענן ברקע': 'src',
    '139|האפליקציה ניתנת להתקנה מכרום': 'src',
    '195|דגלי מעבר — אין דגל דלוק': 'src',
    '85|טיפול באירועים — דלגציה ממאזין אחד': 'src',
    '185|נתיב עדכון חלקי למראת המשתמשים': 'src',
    '48|ארבעת מימדי הסריקה': 'src',
    '96|פינוי אוטומטי': 'src',
    '96|אופק הפינוי נוקה מהסנכרון': 'src',
    '179|אימות פינוי מול הענן': 'src',
    '140|שיתוף קבצים': 'src',
    '124|מעטפת APK (WebView)': 'src',
    '127|אין נכסים מוטבעים': 'src',
    '144|מפתח חתימה קבוע': 'src',
    '150|מקור אמת יחיד לסכימה': 'src',
    '151|קובץ התקנה מלא': 'src',
    '64|גיבוי יומי אוטומטי': 'src',
    '64|יומן פעולות': 'src',
    '147|נתונים בטבלאות מובנות': 'src',
    '14|בודקים — קיום': 'src',
    '15|בודקים — משימה מוצהרת': 'tools',
    '86|דפוס הודעת שגיאה יחיד': 'src',
    '92|`toast` — חתימה, גוף ומחלקות': 'src',
    '92|סיווג ההודעה נגזר מהמסלול': 'src',
    '216|מיכל באנר העדכון במקור': 'src',
    '216|הרענון מ-controllerchange בלבד': 'src',
    '216|סימן דחיית הבאנר מתמיד': 'src',
    '119|רישום כשלי כתיבה': 'src',
    '65|חלון חם במכשיר': 'src',
    '182|מסך שינוי סיסמה עצמי': 'src',
    '218|מטמון-CDN מראש עם ריפוי עצמי': 'src',
    '66|מנוע מיזוג — ⏳ שובר שוויון': 'src',
    '188|חסימת משתמש מושבת בכניסה אופליין': 'src',
    '67|מודול מזהי רשומות': 'src',
    '67|מזהה אינו מומר למספר': 'src',
    '69|מזהה מכשיר': 'src',
    '66|ליבת מיזוג משותפת': 'src',
    '124|מעטפת WebView חתומה': 'tools',
    '188|אימות מול טביעה בענן': 'mixed',
    '142|בניית APK אחידה עם שער חתימה': 'src',
    '218|מודול ה-service worker': 'src',
    '70|ניסיון חוזר בתור הסנכרון': 'src',
    '71|מנגנון משיכה אחיד': 'src',
    '74|נעילת חוסר-פעילות': 'src',
    '75|מודל הסשן — בזיכרון בלבד': 'src',
    '217|בדיקת עדכון תקופתית ל-service worker': 'src',
    '99|פסק זמן אחיד לקריאות רשת': 'src',
    '100|מאזיני מצב רשת': 'src',
    '171|גריעת tombstones לפי גיל': 'src',
    '216|עדכון אוטומטי — בדיקה מחזורית': 'src',
    '140|גשר שיתוף': 'src',
    '76|העברת מזהה ל-DOM': 'src',
    '77|מיון אחד לכל התצוגות': 'src',
    '82|ייצוא — שני דפוסים מוצהרים': 'src',
    '132|שכבת אייקונים': 'src',
    '180|שכבת קלט אחידה': 'src',
    '180|קריאת ערך מספרי עוברת ב-readNum': 'src',
    '83|שכבת המודאל': 'src',
    '182|שכבת כניסה מלאה': 'src',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
...
export const ROWS = [32, 47, 49, 51, 60, 95, 58, 178, 113, 156, 63, 25, 68, 28, 37, 41];
...
const MATRIX = [
  ...
  { row: 149, name: 'שם נגזר אינו נשמר ברשומה',
  ...
  { row: 165, name: 'הקשר נלכד בכניסה לפונקציה',
  ...
  { row: 198, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 118, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 167, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 164, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
  ...
  { row: 135, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 186, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 153, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 154, name: '`migrations/` — כמות',
  ...
  { row: 175, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 176, name: 'ברירת מחדל אינה ממוזגת',
  ...
  { row: 177, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 190, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 162, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 188, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 188, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 146, name: 'עריכת נתונים אופליין',
  ...
  { row: 172, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 189, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 139, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 195, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 185, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 179, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 140, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 124, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 127, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 144, name: 'מפתח חתימה קבוע',
    probe: () => keystoreFixed() },
  { row: 150, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 151, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 64, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 64, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 147, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 119, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  { row: 65, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 182, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 182, name: 'מבנה טופס הכניסה',
    probe: () => src.indexOf('<form') < 0 },
  { row: 218, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 166, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 188, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 124, name: 'מעטפת WebView חתומה',
  ...
  { row: 188, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 142, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 218, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 217, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 171, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 216, name: 'מיכל באנר העדכון במקור',
  ...
  { row: 216, name: 'הרענון מ-controllerchange בלבד',
    probe: () => swReloadGaps().length === 0 },
  { row: 216, name: 'סימן דחיית הבאנר מתמיד',
    probe: () => swDismissGaps().length === 0 },
  { row: 216, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 132, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 180, name: 'שכבת קלט אחידה',
  ...
  { row: 180, name: 'קריאת ערך מספרי עוברת ב-readNum',
...
const GATES = {
  ...
  213: { claims: { test_mignames: ['[mig-pattern]', '[mig-seq]', '[mig-file-no-entry]',
  ...
  197: { claims: { test_coremod: ['[core-prefix]', '[core-allow]'] } },
  ...
  191: { claim: 'אין `service_role` בקובץ' },
  34: { claim: 'מסלול «' },
  ...
  194: { claims: { test_push: ['[synclog-decl]', '[synclog-site]', '[synclog-push]'] } },
  73: { claims: { test_push: ['כתיבת משתמש', 'הודעת החסימה', 'הודעת שש הספרות'] } },
  155: { claim: 'בעלות הסכימה המשותפת' },
  169: { claim: '`ya_settings_rishon`' },
  /*  ⛔ שתי הטענות באותו שער — ⚠️ הערכים במסד החי, ⭐ ונקודת הקריאה
   *  שמבחינה בין «אין ערך» ל«ערך פגום»: ⛔ שער אחד לשני הצדדים.  */
  170: { claims: { test_dbfacts: ['י. ערך במפתח-ערך', 'יא. ערך פגום נבדל'] } },
  208: { claim: 'ז. עמודה בלי קורא' },
  ...
  130: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  133: { claim: 'margin' },
  ...
  117: { claims: { test_codescan: ['[fn-product]'] },
  ...
  201: { claims: { test_removals: 'מזהה שנמחק ונשאר לו קורא',
  ...
  114: { claim: 'BANNER_W' },
  204: { claim: ['אין מחלקת CSS שאינה מוחלת לעולם', 'אין מזהה id שאין לו קורא',
  ...
  129: { claim: 'android/app/src/main' },
  136: { claim: 'fgDriftMax' },
  137: { claim: 'CANON_MANIFEST' },
  138: { claim: 'שדות זהים' },
  141: { claim: 'BUILD_SHA' },
  145: { claim: 'SHARED_SHA' },
  143: { claim: 'versionCode' },
  125: { claim: 'WebView' },
  126: { claim: 'android:theme' },
  131: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  148: { claims: { test_dbfacts: 'ג. כל מפתח שהקוד מבקש', test_dbscan: 'ה. מקור הקריאה' } },
  152: { claim: 'migrations' },
  161: { claims: { test_dbfacts: ['יב. הרשאות במסד', 'יג. תוספת-בלבד'] } },
  163: { claim: 'א. חותמת בכל רשומה' },
  63: { claims: { 'check-capabilities': 'רענון הממתינים', test_push: 'ה. כשל רשת' } },
  156: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  157: { claim: 'ו. טבלה מקבילה' },
  158: { claim: 'ב. חתימת סכימה' },
  159: { claims: { test_dbscan: ['א. כל שאילתה נפתרת', 'ב. כל שאילתה מול הסכימה המוצהרת'],
                    test_dbfacts: ['טו. מראת הסכימה'] } },
  181: { claim: 'type=password' },
  87: { claim: 'aria-label' },
  ...
  192: { claim: '[sec-keyfile]' },
  196: { claim: '⏳' },
  200: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  210: { claim: 'ט. ערך נגזר שהתיישן' },
  212: { claim: 'ח. הגדרה שהוחלפה' },
  211: { claim: 'ד. רשימת-היתר' },
  214: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  178: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
