...
const APP = {
  ...
  core:  'android/app/src/main/java/com/hanhala/ruchanit/ShellActivity.java',
  /* המעטפת הפר-אפליקציתית — זהות בלבד, ואצל יומן גם הגשר. */
  shell: 'android/app/src/main/java/com/hanhala/ruchanit/MainActivity.java',
 ...
 *  מזו בשקט — בדיוק מה שאסור. ליומן חתימה משלה, מפני שגשר
 ...
 *  שיורדת מכאן היא יכולת שנעלמה ממעטפת אחת בשקט — ⛔ בדיוק מה
 *  שאסור.                                                                */
const CONTRACT = [
...
if (!process.env.RD67_MUT) {
  ...
  console.log('\n— מוטציות (סבב 67) —');
  _mut('⛔ שינוי בית בליבת ShellActivity מזיז את החתימה ומפיל', 'android/app/src/main/java/com/hanhala/ruchanit/ShellActivity.java',
       (s) => s.replace(/\bonCreate\b/, 'onCreateRenamed'), true);
  _mut('⭐ מוטציית-נגד: הוספת שורה ריקה בסוף הקובץ ⛔ אינה מפילה', 'android/app/src/main/java/com/hanhala/ruchanit/ShellActivity.java',
       (s) => s.replace(/\s*$/, '\n'), false);
}
