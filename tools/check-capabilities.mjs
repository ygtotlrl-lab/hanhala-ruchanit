...
const APP = {
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    101: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    83: (c) => c.hasCode(/YS_KV_LEGACY_WRITE\s*=\s*false/),
    ...
    99: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [32, 34, 35, 37, 38, 43, 58, 39];
...
const MATRIX = [
  { row: 101, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 89, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  { row: 103, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 82, name: 'עריכת נתונים אופליין',
  ...
  { row: 92, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 106, name: 'דגלי מעבר — אין דגל דלוק',
    probe: () => legacyFlagsOn().length === 0 },
  { row: 57, name: 'טיפול באירועים — דלגציה ממאזין אחד',
    probe: () => {
  ...
  { row: 100, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  { row: 114, name: 'פינוי אוטומטי',
    probe: () => /tier2\s*[:=]\s*\[\s*\{/.test(policyBlock()) },
  { row: 93, name: 'אימות פינוי מול הענן',
    probe: () => /\bverify\s*:/.test(policyBlock()) },
  { row: 77, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 69, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 71, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 81, name: 'מפתח חתימה קבוע בריפו',
    probe: () => hasPath('signing') &&
                 fs.readdirSync('signing').some((f) => f.endsWith('.keystore')) },
  { row: 85, name: 'מקור אמת יחיד לסכימה', probe: () => hasPath(APP.schemaFile) },
  { row: 86, name: 'קובץ התקנה מלא',       probe: () => hasPath(APP.schemaFile) },
  { row: 45, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 45, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 83, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 59, name: 'דפוס הודעת שגיאה יחיד', probe: () => errPatternSites() === 0 },
  { row: 46, name: 'חלון חם במכשיר',
    probe: () => /\benabled\s*:\s*true\b/.test(cfgBlock('HW_CFG')) },
  { row: 46, name: 'שחזור מקומי מהענן',
    probe: () => callSites('hwRestoreMount').length > 0 },
  { row: 99, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 122, name: 'מטמון-CDN מראש עם ריפוי עצמי',
    probe: () => fileHas('sw.js', /CDN_ASSETS/) && fileHas('sw.js', /ensureCdnCached/) },
  { row: 45, name: 'גיבוי יומי מטבלאות מובנות',
  ...
  { row: 90, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 47, name: 'מנוע מיזוג עם הגנת ⏳',
    probe: () => /isPend \|\| tsOf\(loc\) > tsOf\(rem\)/.test(fnBody('_mergePick')) },
  { row: 103, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 69, name: 'מעטפת WebView חתומה',
  ...
  { row: 103, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 79, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 122, name: 'מודול ה-service worker',
  ...
  { row: 50, name: 'ניסיון חוזר בתור הסנכרון',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 53, name: 'מודל הסשן — בזיכרון בלבד',
  ...
  { row: 121, name: 'בדיקת עדכון תקופתית ל-service worker',
  ...
  { row: 44, name: 'רענון תקופתי של מונה הממתינים',
    probe: () => hasCode(/setInterval\(\s*pendRender\s*,\s*60000\s*\)/) },
  { row: 61, name: 'פסק זמן אחיד לקריאות רשת',
   ...
   *  בשקט».                                                        */
  { row: 91, name: 'גריעת tombstones לפי גיל',
    probe: () => hasCode(/TOMBSTONE_TTL_MS/) },
  { row: 120, name: 'אוטו-אפדייט מ-raw.githubusercontent',
...
const GATES = {
  ...
  27: { claim: 'gen-icons' },
  31: { claim: ['מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  40: { manual: 'המדידה שלפני המחיקה היא התנהגות סשן — ⛔ נאכפת רק בתוצאתה, בשער ההסרות' },
  39: { claim: 'אין פרק פערים נפרד' },
  68: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  115: { manual: 'מחזור העבודה ב-git הוא התנהגות סשן שאינה בעץ — ⛔ נאכף רק בתוצאתו' },
  41: { claim: 'מזהה שנמחק ונשאר לו קורא' },
  74: { claim: 'tile-bg' },
  ...
  35: { claim: 'הערה ריקה' },
  26: { claim: 'drift' },
  29: { claim: 'measure-gap' },
  30: { manual: 'חפיפה בין שערים נסרקת ידנית — ⛔ «אותה טענה» היא קריאת משמעות' },
  ...
  38: { claim: '🔲 אינו קבוע' },
  42: { manual: 'התנהגות סשן שאינה בעץ — ⛔ אין קובץ שאפשר למדוד בו קריאה חסכונית' },
  43: { claim: 'sha:' },
  54: { claim: 'CANON' },
  58: { claim: 'storage' },
  60: { claim: 'ג · פעולה שדורשת רשת' },
  63: { claim: 'עברית' },
  64: { claim: 'RULE_W' },
  65: { claim: 'BANNER_W' },
  110: { claim: 'CSS מתות' },
  66: { manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ והמונה נמדד ידנית' },
  67: { claim: 'מפנה לקובץ' },
  72: { claim: 'android/app/src/main' },
  75: { claim: 'fgDriftMax' },
  76: { claim: 'CANON_MANIFEST' },
  78: { claim: 'BUILD_SHA' },
  80: { claim: 'versionCode' },
  70: { claim: 'WebView' },
  84: { manual: 'נפילה-חזרה ל-`kv` נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  87: { claim: 'migrations' },
  88: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  95: { claim: 'type=password' },
  96: { claim: 'aria-label' },
  97: { manual: 'שני מנועי תאריך — ⛔ טרם הוכרע איזה, ואין מה לאכוף' },
  98: { manual: '«`catch` ריק סביב כתיבה» טרם נוסח כשער — נסרק ידנית בכל סבב שנוגע' },
  102: { claim: 'pass_salt' },
  104: { manual: 'מצב העמודה במסד אינו נראה מהריפו' },
  105: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  107: { claim: '⏳' },
  108: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  118: { manual: 'קיום טבלה או מפתח במסד אינו נראה מהריפו' },
  112: { manual: 'מצב ההרצה יושב ב-`schema_migrations` ואינו נראה מהריפו' },
  109: { manual: '«קובץ בלי קורא» דורש סריקה על ארבעת הריפו — נעשה ידנית' },
  119: { manual: 'קיום מפתח במסד אינו נראה מהריפו' },
  111: { manual: 'רשימת-היתר הגיבויים יושבת במיגרציה שכבר רצה' },
  113: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  116: { claim: 'CLEANUP_SHA' },
  117: { claim: 'בלוק ה-APP' },
};
...
const RULE_ROWS = {
  ...
  '⛔ מדידה לפני שינוי': 40,
  '⛔ סריקת האזור שנגעו בו': 108,
  '⛔ משמעת קריאה וחיסכון טוקנים': 42,
  '⛔ הכרעה שאינה של הסשן': 42,
  '⛔⛔ רכיב משותף = קוד זהה בית-לבית + מיקום זהה + חיווט חי': 43,
  '⛔ תקציב הקבצים, ותקרה שהיא מספר עגול': 4,
  '⛔ ההערה — מתי נכתבת, צורתה ותוכנה': 66,
  '⛔ דפוסי כתיבת ממשק': 55,
  '⛔ הבדל מכוון מנומק בכתב, במקום שבו רואים אותו': 68,
  '⛔ ערך שקיים בקוד או במסד אינו מוצהר בתיעוד': 25,
  '⛔ קובץ נשפט לפי תפקידו': 3,
  '⛔ קובץ תיעוד אינו מסביר כלל שהטבלה אוכפת': 36,
  '⛔ שרשרת היכולת החדשה': 32,
  '⛔ כל כלל בא לידי ביטוי בטבלה': 37,
  '⛔ מספור רציף ומבני, וסריקת כפילויות': 34,
  '⛔ `probe` מאמת **ערך** — ⛔ ולא קיום': 32,
  '⛔ בדיקה שסופרת מילה אינה בדיקה': 24,
  '⛔ מבחן מצהיר אילו שורות בטבלה הוא אוכף': 32,
  '⛔ מה שאינו ניתן לאכיפה מכנית — נרשם ככזה': 39,
  '⛔ שינוי מכוון ברכיב משותף': 43,
};
