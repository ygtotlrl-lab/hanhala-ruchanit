#!/usr/bin/env node
/*  test_android.mjs — שער החתימה של שכבת האנדרואיד.
 *
 *  **מה נאכף:** שלושה סוגי סטייה — (א) **חוזה**: הרשאה שנעלמה,
 *  `configChanges` שאינו הסופרסט הקנוני, או גשר שיתוף שקיים/חסר בניגוד
 *  למוצהר; (ב) **חתימת המניפסט**: החלק המשותף מול החתימה הקנונית;
 *  (ג) **חתימת קובץ הבנייה**, אותו דבר.
 *
 *  **הנימוק המדוד:** שני הקבצים שקובעים איך ה-APK נבנה ומה אנדרואיד מרשה
 *  לו לא נשאו שום חתימה — ⛔ הרשאה שנוספה, `<queries>` שנעלם או
 *  `configChanges` שנסחף עברו בשקט בכל הריפו: ⚠️ נמדד ערך אחד בשלושה
 *  וערך אחר ברביעי, ⛔ ואיש לא החליט על ההבדל.
 *
 *  **מה יישבר בלעדיו:** ⛔ «קיים רק באחת, בשקט» — ⚠️ שינוי בהרשאות אינו
 *  נראה בקוד האפליקציה כלל, ⭐ והוא מתגלה רק כשמשהו מפסיק לעבוד במכשיר.
 *
 *  **מה אינו נאכף כאן:** ⛔ מזהי החבילה, מספרי הגרסה, שם האפליקציה וכל
 *  ההערות **מנורמלים החוצה בכוונה** — ⚠️ אלה מצייני-זהות והיסטוריה ולא
 *  לוגיקת בנייה, ⛔ ובלי נרמולם כל ריפו היה נושא חתימה משלו. ⚠️ ה-`versionCode`
 *  **כן** נאכף — ⭐ במקום שבו הוא באמת שער.
 *
 *  ⭐ **וזו מדידה ולא הצהרה:** אחרי הנרמול שלושת הריפו חסרי-הגשר נושאים
 *  **חתימה זהה בדיוק**, ⛔ וההפרש כולו נמדד ונמצא **בדיוק** בלוקי גשר
 *  השיתוף ⛔ ותו לא. ⛔ הפרש שאינו הגשר מפיל, גם אם החתימה הפרטית עודכנה.
...
const APP = {
  ...
  /* ⚠️ אין כאן גשר שיתוף — אין `navigator.share` בקוד, ולכן אין
     ...
     הקנוניות של חסרי-הגשר, וזהות בשלושת הריפו. */
  shareBridge: false,
  manifestSha: '5689f6b481333147',
  gradleSha:   '9cbf35bb88e9b753',
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
 ...
 *  ⭐ מעכשיו שני בלוקי הגשר מופשטים החוצה, ו**כל הריפו** חייבים לשאת
...
const CONTRACT = [
  ...
  { name: 'supportsRtl=true',
    ...
    why: 'כל האפליקציות עבריות; בלעדיה המערכת מיישרת את המעטפת לשמאל' },
...
/*  ⛔ ערכת הנושא של המעטפת זהה בכולם (סבב 80) — ⚠️ **בלי מסך מלא**:
 ...
 *  ⚠️ נמדד: 6 · 4 · 4 · 6 אזכורים בכולן, ⛔ ובלוק ההערות ביומן מנה עד
...
const checkSha = (label, text, want, canon) => {
  ...
  if (got !== want) {
    fail(`${label}: החלק המשותף אינו תואם לחתימה — ${got} במקום ${want}. ` +
         'שינוי מכוון = עדכון בכל הריפו ובכל עותקי הבדיקה, באותו סבב');
    return;
  }
...
/* ── ב2+ג2. ⭐ החתימה חסרת-הגשר — נאכפת בכולן (סבב 46ב) ──────────────────────
...
const bare = (label, stripped, canon) => {
  ...
  if (got === canon) pass(`${label} ללא הגשר: תואם לקנונית (${got})`);
            ...
            'להיות זהה בכולן, או מוכרזת פרטית עם נימוק');
};
