...
const APP = {
  ...
  actMap: 'DOM_ACTIONS',
  gapRows: [38, 106, 110, 128, 129, 133, 145],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    150: (c) => c.hasCode(/YS_PASS_ITER\s*=\s*100000\b/)
    ...
    148: (c) => !!c.fnBody('ysUsersCacheSave')
    ...
    117: (c) => !/ysCfgSet\('(?:ys_students|ys_attend_sessions|ys_sleep_sessions)'/.test(c.src)
    ...
    147: (c) => !!c.fnBody('changeMyPassword')
...
export const ROWS = [27, 37, 39, 41, 50, 73, 44, 140, 86, 138];
...
const MATRIX = [
  ...
  { row: 80, name: 'פריסה במסכי טלפון וטאבלט',
  ...
  { row: 81, name: 'ערכת נושא — בהיר וכהה',
  ...
  { row: 158, name: 'הערה שמתארת מצב שחלף',
  ...
  { row: 90, name: 'פונקציה משותפת אינה יוצאת בשקט',
  ...
  { row: 131, name: 'הגיבוי שלם ומעומד',
    probe: () => {
  ...
  { row: 128, name: 'החלפת הקשר מאפסת את כל המצב', app: true },
   ...
   *  אחד. */
  { row: 129, name: 'הקשר נלכד בכניסה לפונקציה', app: true },
  ...
  { row: 105, name: 'דגימת-יתר במחולל',
    probe: () => Number((/^const SS = (\d+);/m.exec(genSrc()) || [])[1]) === GEN_SS },
  { row: 150, name: '`PBKDF2` — פרמטרים', app: true },
  { row: 122, name: '`migrations/` — תוכן',
    probe: () => migContentGaps().length === 0 },
  { row: 123, name: '`migrations/` — כמות',
  ...
  { row: 136, name: 'מיזוג מכל — מפה ומערך',
  ...
  { row: 137, name: 'רשימה אינה נושאת פריט כפול',
  ...
  { row: 132, name: '`kv_backup` — טבלת הגיבוי',
  ...
  { row: 133, name: '`kv_rishon` / `kv_ramataviv` — הגדרות היומן', app: true },
  { row: 21, name: 'שער אינו מקליד מספר שורה',
    probe: () => typedRowSites().length === 0 },
  { row: 160, name: 'פונקציה בלי קוראים',
  ...
  { row: 154, name: 'סודות אינם עוזבים את המכשיר',
    probe: () => {
  ...
  { row: 126, name: 'דפוס `upsert` — onConflict מוכרז', app: true },
  ...
  { row: 152, name: 'טביעה חסרה מושלמת אוטומטית',
  ...
  { row: 152, name: 'כניסה אופליין',
    probe: () => !!(APP.offlineLoginFn && fnRange(APP.offlineLoginFn)) },
  { row: 116, name: 'עריכת נתונים אופליין',
  ...
  { row: 135, name: 'מחיקה רכה בלבד — אין `DELETE` פיזי',
  ...
  { row: 153, name: 'סיסמאות בענן — אין מסלול שכותב או קורא טקסט גלוי',
    probe: () => {
  ...
  { row: 109, name: 'האפליקציה ניתנת להתקנה מכרום',
    probe: () => installGaps().length === 0 },
  { row: 156, name: 'דגלי מעבר — אין דגל דלוק',
  ...
  { row: 148, name: 'נתיב עדכון חלקי למראת המשתמשים', app: true },
  ...
  { row: 141, name: 'אימות פינוי מול הענן',
    probe: () => perKeyWitness() && witnessFed() },
  { row: 110, name: 'שיתוף קבצים',
    probe: () => hasCode(/_androidShareImage|navigator\s*\.\s*share\b/) },
  { row: 95, name: 'מעטפת APK (WebView)',
   ...
   *  שמתיישן בכל שחרור. */
  { row: 98, name: 'אין נכסים מוטבעים',
    probe: () => !hasPath('android/app/src/main/assets/index.html') &&
                 !hasPath('android/app/src/main/assets/sw.js') },
  { row: 114, name: 'מפתח חתימה קבוע בריפו',
    probe: () => keystoreFixed() },
  { row: 119, name: 'מקור אמת יחיד לסכימה', probe: () => schemaSingleSource() },
  { row: 120, name: 'קובץ התקנה מלא',       probe: () => schemaIdempotent() },
  { row: 52, name: 'גיבוי יומי אוטומטי',   probe: () => present.backup === true },
  { row: 52, name: 'יומן פעולות',          probe: () => present.log === true },
  { row: 117, name: 'נתונים בטבלאות מובנות', app: true },
  ...
  { row: 146, name: 'רישום כשלי כתיבה', probe: () => silentWriteCatches().length === 0 },
  ...
  { row: 147, name: 'מסך שינוי סיסמה עצמי', app: true },
  { row: 167, name: 'מטמון-CDN מראש עם ריפוי עצמי',
  ...
  { row: 130, name: 'פינוי גיבויים אוטומטי במסד',
  ...
  { row: 152, name: 'חסימת משתמש מושבת בכניסה אופליין',
  ...
  { row: 95, name: 'מעטפת WebView חתומה',
  ...
  { row: 152, name: 'אימות מול טביעה בענן',
    probe: () => {
  ...
  { row: 112, name: 'בניית APK אחידה עם שער חתימה',
    probe: () => {
  ...
  { row: 167, name: 'מודול ה-service worker',
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 166, name: 'בדיקת עדכון תקופתית ל-service worker',
   ...
   *  ולא רק מוגדרים. */
  { row: 134, name: 'גריעת tombstones לפי גיל',
    probe: () => {
  ...
  { row: 165, name: 'עדכון אוטומטי — בדיקה מחזורית',
  ...
  { row: 102, name: 'שכבת אייקונים',
    probe: () => memoByHash('icons', ICON_SIG(), () => iconAudit('.')).length === 0 },
  { row: 142, name: 'שכבת קלט אחידה',
...
const GATES = {
  ...
  149: { claim: 'כתיבת משתמש' },
  124: { claim: 'בעלות הסכימה המשותפת' },
  ...
  100: { claim: ['gen-icons', 'מצהיר את ', 'ריק נושא הערת נימוק במקומו'] },
  103: { claim: 'margin' },
  ...
  89: { manual: '«סטייה מדפוס» היא קריאת משמעות — ⛔ נסרקת ידנית בכל סבב שנוגע' },
  ...
  85: { claim: 'עברית' },
  86: { claims: { 'check-comments': 'RULE_W', 'check-capabilities': 'bannerGaps' } },
  87: { claim: 'BANNER_W' },
  161: { claim: 'CSS מתות' },
  40: { claim: 'פתיחת ההערה בטבלה',
        manual: '«למה ולא מה» הוא קריאת משמעות — ⛔ שער מודד צורה בלבד, ⚠️ ונסרק ידנית' },
  88: { claim: 'מפנה לקובץ' },
  99: { claim: 'android/app/src/main' },
  106: { claim: 'fgDriftMax' },
  107: { claim: 'CANON_MANIFEST' },
  108: { claim: 'שדות זהים' },
  111: { claim: 'BUILD_SHA' },
  115: { claim: 'SHARED_SHA' },
  113: { claim: 'versionCode' },
  96: { claim: 'WebView' },
  97: { claim: 'android:theme' },
  101: { claim: ['מיקום המאסטר אינו מזיז', 'שהמסכה החתוכה היא כל מקורם'] },
  118: { manual: 'הנפילה-חזרה ביומן נסרקת ידנית; ⛔ קיום המפתחות במסד אינו נראה מהריפו' },
  121: { claim: 'migrations' },
  125: { manual: 'ההרשאות יושבות במסד ואינן נראות מהריפו — אימות הוא פעולת מנהל' },
  127: { claim: 'א. חותמת בכל רשומה' },
  138: { claims: { 'check-capabilities': 'colPatternGaps', test_dbfacts: 'ה. דפוס עמודות אחיד' } },
  139: { claim: 'ב. חתימת סכימה' },
  143: { claim: 'type=password' },
  144: { claim: 'aria-label' },
  145: { manual: 'מנוע התאריך — ⛔ נאכף ב-`test_date` ביומן ובהנהלה, ⚠️ ובשכר ובגיוס אין צרכן תאריך עברי ואין שער' },
  151: { claim: 'pass_salt' },
  155: { manual: 'היעדר סוד נסרק ידנית; ⛔ שער טקסטואלי היה נכשל על כל מחרוזת' },
  157: { claim: '⏳' },
  46: { manual: 'התאמת הערה למציאות אינה ניתנת לאכיפה מכנית' },
  159: { claim: 'כל קובץ בעץ מוזכר במקום אחר' },
  163: { claim: 'ד. רשימת-היתר' },
  164: { manual: 'מצב הענפים המרוחקים אינו נראה מעותק העבודה' },
  ...
  84: { manual: '«קורא סביר יופתע» אינו נגזר מהטקסט — ⛔ נאכף בתוצאתו בלבד' },
  91: { claim: 'CLEANUP_SHA' },
  92: { claim: 'בלוק ה-APP' },
  140: { claims: { 'check-capabilities': 'cfgKeyGaps', test_dbfacts: 'ג. כל מפתח שהקוד מבקש' } },
};
